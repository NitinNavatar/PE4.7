...
public class PEFSTG extends BaseLib {
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm, ObjectFeatureName.recordTypes)) {
					...
					if (result.isEmpty()) {
						log(LogStatus.INFO, "The Descriptions of Firm record type have been verified " + description,
								YesNo.No);
						sa.assertTrue(true, "The Descriptions of Firm record type have been verified" + description);
					} else {
						log(LogStatus.ERROR, "The Descriptions of Firm record type are not verified " + result,
								YesNo.No);
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Profiles)) {
				...
				if (setup.VerifyDefaultRecordTypeForObject("System Administrator", "Company")) {
					log(LogStatus.PASS, "Default record \"company\" has been verified for System Administrator",
							YesNo.Yes);
					sa.assertTrue(true, "Default record \"company\" has been verified for System Administrator");
				} else {
					log(LogStatus.FAIL, "Default record \"company\" is not verified for System Administrator",
							YesNo.Yes);
			...
			if (parentID != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, tab)) {
					...
					if (setup.clickOnObjectFeature(environment, mode, tab, ObjectFeatureName.pageLayouts)) {
						...
						List<String> result = setup.verifyFieldsAvailabilityAndNonAvailabilityOnPageLayout(
								sectionsInPageLayout, PageLayouts, fieldsAlreadyAddedLayoutWise,
								fieldsNotAlreadyAddedLayoutWise, 30);

						if (result.isEmpty()) {
							log(LogStatus.PASS, "All fields are verified on Page Layout of object " + tab.toString(),
									YesNo.No);
						...
						log(LogStatus.FAIL, "not able to click on page layout link of objet :" + tab.toString(),
								YesNo.No);
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm,
					...
					if (FRB.verifyField(fieldName)) {
						...
						if (result.isEmpty()) {
							log(LogStatus.PASS,
									"Picklist value of Entity type field has been verified on the firm object "
											+ picklistName,
									YesNo.Yes);
							sa.assertTrue(true,
									"Picklist value of Entity type field has been verified on the firm object"
							...
							log(LogStatus.FAIL, "Picklist value of Entity type field is not matched on the firm object",
									YesNo.Yes);
							sa.assertTrue(false,
									"Picklist value of Entity type field is not matched on the firm object");
						}
					} else {
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm, ObjectFeatureName.recordTypes)) {
					...
					if (result.isEmpty()) {
						log(LogStatus.PASS,
								"Entity type picklist value has been verified on different Firm Record type",
								YesNo.Yes);
						sa.assertTrue(true,
								"Entity type picklist value has been verified on different Firm Record type");
					}
						...
						log(LogStatus.FAIL,
								"Entity type picklist values are not verified on different Firm Record type " + result,
								YesNo.Yes);
						sa.assertTrue(false,
								"Entity type picklist values are not verified on different Firm Record type " + result);
					}
	...
	public void PEFSTGTc013_VerifyPagLayoutAssignmentForFirmRecordTypes(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm, ObjectFeatureName.pageLayouts)) {

					ArrayList<String> result = setup.verifyPageLayoutAssignment(recordType, userProfile, 50);
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Record Pages assignment for Firm Record Types has been verified",
								YesNo.Yes);
						sa.assertTrue(true, "Record Pages assignment for Firm Record Types has been verified");
					} else {
						log(LogStatus.FAIL, "Record Pages assignment for Firm Record Types is not verified " + result,
								YesNo.Yes);
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm,
					...
					result = setup.VerifyLightningRecordPagesAssignment(PEFSTG_Tc014_RecordType,
							PEFSTG_Tc014_UserProfile, "APP, RECORD TYPE, AND PROFILE", 50);
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Lightning Record Pages assignment for Firm Record Types has been verified",
								YesNo.Yes);
						sa.assertTrue(true,
								"Lightning Record Pages assignment for Firm Record Types has been verified");
					} else {
						log(LogStatus.FAIL,
								"Lightning Record Pages assignment for Firm Record Types is not verified " + result,
								YesNo.Yes);
						sa.assertTrue(false,
								"Lightning Record Pages assignment for Firm Record Types is not verified " + result);
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.searchStandardOrCustomObject(projectName, mode, object.Firm)) {
				...
				if (setup.clickOnObjectFeature(projectName, mode, object.Firm,
					...
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Picklist value of " + fieldName1
								+ " field has been verified on the firm object " + activePickListValue, YesNo.Yes);
						sa.assertTrue(true, "Picklist value of " + fieldName1
								+ " field has been verified on the firm object " + activePickListValue);
					} else {
						log(LogStatus.FAIL, "Picklist value of " + fieldName1
								+ " field is not matched on the firm object " + result, YesNo.Yes);
						sa.assertTrue(false, "Picklist value of " + fieldName1
								+ " field is not matched on the firm object " + result);
					}

					result = FRB.verifyPicklistValue(fieldName2, deactivePickListValue, Condition.deactivate);
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Picklist value of " + fieldName2
								+ " field has been verified on the firm object " + deactivePickListValue, YesNo.Yes);
						sa.assertTrue(true, "Picklist value of " + fieldName2
								+ " field has been verified on the firm object " + deactivePickListValue);
					} else {
						log(LogStatus.FAIL, "Picklist value of " + fieldName2
								+ " field is not matched on the firm object " + result, YesNo.Yes);
						sa.assertTrue(false, "Picklist value of " + fieldName2
								+ " field is not matched on the firm object " + result);
					}
				} else {
					log(LogStatus.ERROR, "Could not click on the Field and Relationship object feature name",
	...
	public void PEFSTGTc016_CreateANewAdvisorRecordAndVerifyPage(String projectName) {
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (ip.createInstitution(projectName, environment, mode, PEFSTGINS1_Institution, PEFSTGINS1_RecordType,
					null, null)) {
				log(LogStatus.PASS, "successfully Created Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType, YesNo.No);
				sa.assertTrue(true, "successfully Created Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType);

			} else {
				sa.assertTrue(false, "Not Able to Create Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType);
				log(LogStatus.SKIP, "Not Able to Create Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType, YesNo.Yes);
			}
		} else {
	...
	public void PEFSTGTc019_VerifyButtonsAndTabsOnAdvisorRecordPage(String projectName) {
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (BP.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution, TabName.InstituitonsTab, 50)) {
				log(LogStatus.INFO, "successfully open the record : " + PEFSTGINS1_Institution, YesNo.No);
				List<String> negativeResultOfButtons = BP.verifyButtonsOnAPageAndInDownArrowButton(
				...
				if (negativeResultOfTabs.isEmpty()) {
					...
					log(LogStatus.FAIL, "-----Not Verified Buttons on Page-----:" + negativeResultOfTabs, YesNo.Yes);
				}
			} else {
				log(LogStatus.ERROR, "Not able to open the record : " + PEFSTGINS1_Institution, YesNo.Yes);
				sa.assertTrue(false, "Not able to open the record : " + PEFSTGINS1_Institution);
	...
	public void PEFSTGTc0020_VerifyCustomActionsOnAdvisorRecordPage(String projectName) {
		...
		/*
		 * String[] recordPageButtonNames =
		 * " <Section>New Affiliation<Section>New Client".split("<Section>");
		 * 
		 * String[] recordPageLabelValuesAndTypesOfElements =
		 * "Salutation<Br>Mr.<Br>DropDown<Break>First Name<Br>pe_advisor_record<Br>TextBox<Break>Last Name<Br>Contact 1<Br>TextBox<Break>Primary Contact<Br>select<Br>Checkbox<Break>Title<Br>TitleAuto1231<Br>TextBox<Break>Phone<Br>3214236545<Br>TextBox<Break>Email<Br>pe_contact3243@yopmail.com<Br>TextBox"
		 * .split("<Section>");
		 * 
		 * String[][] requiredRecordPageLabelValuesAndTypesOfElements = null; int
		 * recordPageLoopCount = 0; for (String recordPageLabelValuesAndTypesOfElement :
		 * recordPageLabelValuesAndTypesOfElements) {
		 * requiredRecordPageLabelValuesAndTypesOfElements = new
		 * String[recordPageLabelValuesAndTypesOfElement .split("<Break>").length][3];
		 * int i = 0; for (String recordPageLabelValuesAndTypesOfEle :
		 * recordPageLabelValuesAndTypesOfElement.split("<Break>")) { for (int j = 0; j
		 * < 3; j++) { requiredRecordPageLabelValuesAndTypesOfElements[i][j] =
		 * recordPageLabelValuesAndTypesOfEle .split("<Br>")[j]; } i++; }
		 * 
		 * if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
		 * 
		 * if(bp.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution,
		 * TabName.InstituitonsTab, 50)) {
		 * log(LogStatus.INFO,"successfully open the record : "+PEFSTGINS1_Institution,
		 * YesNo.No);
		 * 
		 * if (bp.enterDetailsForRecord(projectName,
		 * requiredRecordPageLabelValuesAndTypesOfElements, action.SCROLLANDBOOLEAN,
		 * 30))
		 * 
		 * {
		 * 
		 * if (click(driver, bp.pageLevelCreateRecordPopupSaveOrCancelButton("Save",
		 * 30), "Save Button ", action.SCROLLANDBOOLEAN)) { log(LogStatus.INFO,
		 * "Clicked on Save Button on " + recordPageButtonNames[recordPageLoopCount] +
		 * " Form Page", YesNo.No); }
		 * 
		 * else { log(LogStatus.ERROR, "Not Able to Click on Save Button on " +
		 * recordPageButtonNames[recordPageLoopCount] + " Form Page", YesNo.Yes);
		 * 
		 * }
		 * 
		 * } else { log(LogStatus.ERROR, "Not Able to enter Values on " +
		 * recordPageButtonNames[recordPageLoopCount] +
		 * " Form Page, So Not Able to Create New Refferal Record", YesNo.Yes);
		 * 
		 ...
		 * 
		 * } else {
		 * log(LogStatus.ERROR,"Not able to open the record "+PEFSTGINS1_Institution,
		 * YesNo.No);
		 * sa.assertTrue(false,"Not able to open the record "+PEFSTGINS1_Institution); }
		 * } else { log(LogStatus.ERROR,"Not able to click on Firm tab",YesNo.No);
		 * sa.assertTrue(false,"Not able to click on Firm tab"); }
		 * 
		 * 
		 * recordPageLoopCount++; }
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (BP.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution, TabName.InstituitonsTab, 50)) {
				...
				if (CommonLib.click(driver, ele, "Contact Tab", action.SCROLLANDBOOLEAN)) {
					ArrayList<String> result = fb.VerifyContactsTabOnAdvisorRecordPage(PEFSTG_Tc022_RecordType1,
							PEFSTG_Tc022_RecordType2, 50);
					if (result.isEmpty()) {
		...
		if (BP.clickOnTab(projectName, TabName.InstituitonsTab)) {
			...
			if (BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01", TabName.InstituitonsTab, 50)) {
				...
				if (click(driver, ele, "Contacts Tab", action.SCROLLANDBOOLEAN)) {
					...
					if (result.isEmpty()) {
						log(LogStatus.PASS,
								"Sorting on All fields of Contacts and Affiliations grid on Contacts tab has been verified",
								YesNo.No);
						sa.assertTrue(true,
								"Sorting on All fields of Contacts and Affiliations grid on Contacts tab has been verified");
					} else {
						log(LogStatus.FAIL,
								"Sorting on All fields of Contacts and Affiliations grid on Contacts tab have not been verified -- "
										+ result,
								YesNo.No);
						sa.assertTrue(false,
								"Sorting on All fields of Contacts and Affiliations grid on Contacts tab have not been verified -- "
	...
	public void PEFSTGTc0024_VerifyPageRedirectionForTheClickableFieldsOnContactsAndAffiliationsGridOnContactsTab(
			String projectName) {
		...
		for (int i = 0; i < tabName.size(); i++) {
			if (BP.clickOnTab(projectName, "Firms")) {
				if (BP.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution, TabName.InstituitonsTab, 50)) {
					ArrayList<String> result = FB.VerifyPageRedirectionForTheClickableFieldsOnContactsAndAffiliations(
							PEFSTG_Tc025_RecordType1, PEFSTG_Tc025_RecordType1, tabName.get(i));
					if (result.isEmpty()) {
						log(LogStatus.PASS,
								"Page redirection for the clickable fields on Contacts and Affiliations grid on Contacts tab has been verified",
								YesNo.No);
						sa.assertTrue(true,
								"Page redirection for the clickable fields on Contacts and Affiliations grid on Contacts tab has been verified");
					} else {
						log(LogStatus.FAIL,
								"Page redirection for the clickable fields on Contacts and Affiliations grid on Contacts tab is not verified "
										+ result,
								YesNo.No);
						sa.assertTrue(false,
								"Page redirection for the clickable fields on Contacts and Affiliations grid on Contacts tab is not verified "
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01", TabName.InstituitonsTab, 50)) {
				...
				if (click(driver, ele, "Contacts Tab", action.SCROLLANDBOOLEAN)) {
					...
					if (result.isEmpty()) {
						log(LogStatus.PASS,
								"Inline editing for Contacts and Affiliations grid on Contacts tab has been verified",
								YesNo.No);
						sa.assertTrue(true,
								"Inline editing for Contacts and Affiliations grid on Contacts tab has been verified");
					} else {
						log(LogStatus.FAIL,
								"Inline editing for Contacts and Affiliations grid on Contacts tab is not verified",
								YesNo.No);
						sa.assertTrue(false,
								"Inline editing for Contacts and Affiliations grid on Contacts tab is not verified");
	...
	public void PEFSTGTc0026_01_PreconditionForClientTab(String projectName) {
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (ip.createInstitution(projectName, environment, mode, institutionName, "Institution", null, null)) {
				log(LogStatus.INFO, "successfully Created Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType, YesNo.No);
				...
				if (CPB.CreateNewContactFromTab(projectName, labelName, value, inputType)) {
					...
					if (ele != null) {
						if (CommonLib.clickUsingJavaScript(driver, ele, "Fundraising tab", action.BOOLEAN)) {
							...
							if (FRP.createFundRaising(environment, mode, "Auto test 1", "Sumo Logic - User",
									institutionName, "1st Closing", "Prospect", "5000", "Status notes", "2022", "July",
									"22")) {
								...
								if (FRP.CreateNewFundraisingContactFromTab(projectName, labelName, value, inputType)) {
									...
									if (BP.clickOnTab(projectName, projectName, TabName.InstituitonsTab)) {
										...
										if (CPB.clickOnCreatedContact(projectName, null, "pe_advisor_record01")) {
											...
											ele = CommonLib.FindElement(driver, xPath, "Client tab",
											...
											if (CommonLib.click(driver, ele, "Client", action.SCROLLANDBOOLEAN)) {
												...
												if (BP.createNewRecordThroughSDG(projectName, "Clients", "New Client",
														labelName, value, inputType, 30)) {
													log(LogStatus.PASS, "Record has been created on client SDG",
															YesNo.No);
													sa.assertTrue(true, "Record has been created on client SDG");
												} else {
													log(LogStatus.ERROR, "Record is not created on client SDG",
															YesNo.No);
													sa.assertTrue(false, "Record is not created on client SDG");
												}
											} else {
												log(LogStatus.ERROR, "Not able to click pe_advisor_record01",
														YesNo.Yes);
				...
				sa.assertTrue(false, "Not Able to Create Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType);
				log(LogStatus.FAIL, "Not Able to Create Account/Entity : " + PEFSTGINS1_Institution
						+ " of record type : " + PEFSTGINS1_RecordType, YesNo.Yes);
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on firm tab", YesNo.Yes);
			...
			if (ip.createInstitution(projectName, environment, mode, institutionName, "Institution", null, null)) {
				...
				if (CPB.CreateNewContactFromTab(projectName, labelName, value, inputType)) {
					...
					if (ele != null) {
						if (CommonLib.clickUsingJavaScript(driver, ele, "Fundraising tab", action.BOOLEAN)) {
							log(LogStatus.INFO, "Clicked on Fundraising tab", YesNo.Yes);
							if (FRP.createFundRaising(environment, mode, "Auto test 2", "Sumo Logic - User",
									institutionName, "1st Closing", "Prospect", "5000", "Status notes", "2022", "July",
									"22")) {
								log(LogStatus.PASS, "Fundraising has been created", YesNo.No);
								sa.assertTrue(true, "Fundraising has been created");
	 ...
	 * 
	 * public void
	 * PEFSTGTc0026_VerifySortingOnAllFieldsOfClientsFundraisingGridOnClientsTab(
	 * String projectName) {
	 * 
	 * LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
	 * BasePageBusinessLayer BP = new BasePageBusinessLayer(driver);
	 ...
	 * 
	 * String
	 * data="Contacts<fieldBreak>Title<f>Email<f>Phone<f>Last Touchpoint<defaultSortBreak>Name<sortOrder>ASC<break>Affiliations<fieldBreak>Name<f>Firm<f>End Date<f>Last Touchpoint<defaultSortBreak>Start Date<sortOrder>DESC"
	 * ;
	 * 
	 * if(BP.clickOnTab(projectName, TabName.InstituitonsTab)) { log(LogStatus.INFO,
	 * "Clicked On Firms Tab", YesNo.No);
	 * 
	 * if(BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01",
	 * TabName.InstituitonsTab, 50)) { log(LogStatus.PASS,
	 * "Enter the Name and clicked on it", YesNo.No);
	 * if(FB.VerifySortingOnFields(data)) { log(LogStatus.INFO,
	 * "Clicked On Firms Details Page", YesNo.No); } else { log(LogStatus.FAIL,
	 * "Not able to Clicked On Firms Details Page", YesNo.Yes); sa.assertTrue(false,
	 * "Not able to Clicked On Firms Details Page"); }
	 * 
	 * } else { log(LogStatus.FAIL, "Unable the Enter Name and not clicked on it",
	 * YesNo.No); sa.assertTrue(false,
	 * "Unable the Enter Name and not clicked on it"); } }else { log(LogStatus.FAIL,
	 * "Not able to clicked on Firms Tab", YesNo.Yes); sa.assertTrue(false,
	 * "Not able to clicked on Firms Tab"); } lp.CRMlogout(); sa.assertAll();
	 * 
	 * }
	 * 
	 */
			...
			if (BP.clickOnTab(projectName, "Firms")) {
				if (BP.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution, TabName.InstituitonsTab, 50)) {
					...
					if (click(driver, ele, "Contacts Tab", action.SCROLLANDBOOLEAN)) {
						...
						ArrayList<String> result = FB
								.verifyPageRedirectionForTheClickableFieldsOnClientsGridFundraisingGridAndReferalGridOnClientsTabAndReferralTab(
										projectName, PEFSTGINS1_Institution, gridNameAndFieldName[i]);
						if (result.isEmpty()) {
							log(LogStatus.PASS,
									"Page redirection for the clickable fields on Client and Fundraising grid on Clients tab has been verified",
									YesNo.No);
							sa.assertTrue(true,
									"Page redirection for the clickable fields on Client and Fundraising grid on Clients tab has been verified");
						} else {
							log(LogStatus.FAIL,
									"Page redirection for the clickable fields on Client and Fundraising grid on Clients tab is not verified "
											+ result,
									YesNo.No);
							sa.assertTrue(false,
									"Page redirection for the clickable fields on Client and Fundraising grid on Clients tab is not verified "
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (BP.clickOnAlreadyCreatedItem(projectName, PEFSTGINS1_Institution, TabName.InstituitonsTab, 50)) {
				...
				if (click(driver, ele, "Client Tab", action.SCROLLANDBOOLEAN)) {
					...
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Inline editing for Client grid on Clients tab has been verified",
								YesNo.No);
						sa.assertTrue(true, "Inline editing for Client grid on Clients tab tab has been verified");
					} else {
						log(LogStatus.FAIL,
								"Inline editing for Client grid on Clients tab on Contacts tab is not verified",
								YesNo.No);
						sa.assertTrue(false,
								"Inline editing for Client grid on Clients tab on Contacts tab is not verified");
					}
					ArrayList<String> result1 = BP.verifyEditOrLockIconOnSDgRecord(fundraisingSDGName, recordName,
							iconType);
					if (result1.isEmpty()) {
						log(LogStatus.PASS, "Locked icon is visible on all record of fundraising grid " + recordName,
								YesNo.No);
						sa.assertTrue(true, "Locked icon is visible on all record of fundraising grid " + recordName);
					} else {
						log(LogStatus.FAIL, "Locked icon is not visible on  records of fundraising grid " + result1,
								YesNo.No);
				...
				if (click(driver, ele, "Referrals", action.SCROLLANDBOOLEAN)) {
					...
					if (result.isEmpty()) {
						...
						if (FPB.checkAscOrder(xPath, "Name")) {
							log(LogStatus.PASS,
									"Default Ascending order has been verified on Name Record of Referral SDG",
									YesNo.No);
							sa.assertTrue(true,
									"Default Ascending order has been verified on Name Record of Referral SDG");
						}
							...
							log(LogStatus.FAIL,
									"Default Ascending order is not verified on Name Record of Referral SDG", YesNo.No);
							sa.assertTrue(false,
									"Default Ascending order is not verified on Name Record of Referral SDG");
						}
					} else {
					...
					if (FPB.verifyBtnVisibilityOnSDG("New Referral")) {
						...
						if (FPB.verifySDGPopUPOpen("New Referral", "New Firm: Company")) {
							...
							if (CommonLib.clickUsingJavaScript(driver, ele, "Close popup", action.BOOLEAN)) {
								...
								if (BP.createNewRecordThroughSDG(projectName, "Referrals", "New Referral", labels,
										values, typeOfFields, 30)) {
	...
	public void PEFSTGTc0031_VerifySortingOnAllFieldsOfReferralsGridOnReferralsTab(String projectName) {
		...
		// String
		// data="Clients<fieldBreak>Client<f>Role<defaultSortBreak>Client<sortOrder>ASC<noSortOrderField>Main
		// Contact<break>Fundraising<fieldBreak>Client Name<f>Fund<f>Stage<f>Close
		// Date<f>Potential Investment
		// (M)<f><defaultSortBreak>null<sortOrder>null<noSortOrderField>Status Notes";
		...
		if (BP.clickOnTab(projectName, TabName.InstituitonsTab)) {
			...
			if (BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01", TabName.InstituitonsTab, 50)) {
				...
				if (click(driver, ele, "Referrals Tab", action.SCROLLANDBOOLEAN)) {
					...
					ArrayList<String> result = FB.VerifySortingOnFields(data);
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Sorting on All fields of Referral grid on Referral tab has been verified",
								YesNo.No);
						sa.assertTrue(true, "Sorting on All fields of Referral grid on Referral tab has been verified");
					} else {
						log(LogStatus.FAIL,
								"Sorting on All fields of Referral grid on Referral tab are not verified -- " + result,
								YesNo.No);
						sa.assertTrue(false,
								"Sorting on All fields of Referral grid on Referral tab are not verified -- " + result);
	...
	@Parameters({ "projectName" })
	@Test
	public void PEFSTGTc0032_VerifyPageRedirectionForTheClickableFieldsOnReferralsGridOnReferralsTab(
			String projectName) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		BasePageBusinessLayer BP = new BasePageBusinessLayer(driver);
		FirmPageBusinessLayer FB = new FirmPageBusinessLayer(driver);
		...
		if (BP.clickOnTab(projectName, "Firms")) {
			if (BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01", TabName.InstituitonsTab, 50)) {
				...
				if (click(driver, ele, "Referrals Tab", action.SCROLLANDBOOLEAN)) {
					...
					ArrayList<String> result = FB.VerifyInlineEditingForContactsAndAffiliationsGrid(val);
					if (result.isEmpty()) {
						log(LogStatus.PASS, "Inline editing for Referrals grid on Referral tab has been verified",
								YesNo.No);
						sa.assertTrue(true, "Inline editing for Referrals grid on Referral tab has been verified");
					} else {
						log(LogStatus.FAIL, "Inline editing for Referrals grid on Referral tab is not verified",
								YesNo.No);
					...
					if (result.isEmpty()) {
						...
						if (FB.uploadFileAndVerify("AutomationTest.xlsx", "xlsx", "8KB")) {
							...
							if (FB.verifyViewAllButtonForAdvisorOnFileTab(ExpectedColumnName)) {
								log(LogStatus.PASS, "Column name for advisor record on Files tab has been verified",
										YesNo.No);
								sa.assertTrue(true, "Column name for advisor record on Files tab has been verified");

							} else {
								log(LogStatus.FAIL, "Column name for advisor record on Files tab are not verified",
										YesNo.No);
		...
		if (BP.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (BP.clickOnAlreadyCreatedItem(projectName, "pe_advisor_record01", TabName.InstituitonsTab, 50)) {
				...
				if (BP.verifyActivityRecord(recordAndActivityTitle)) {
					log(LogStatus.PASS, "Activity Timeline has been created and record are apearing on Activity",
							YesNo.No);
					sa.assertTrue(true, "Activity Timeline has been created and record are apearing on Activity");
				} else {
					log(LogStatus.FAIL, "Activity Timeline has been created but records are not apearing on Activity",
							YesNo.No);
	...
	public void PEFSTGTcS014_VerifySortingOnAffiliationsSDG(String projectName) {
		...
		List<String> pickListColumnAndValues = new ArrayList<String>();
		lp.CRMLogin(crmUser1EmailID, adminPassword);
						...
						if (alreadyAddedComponentToHomePage != null) {
							...
							if (home.sdgGridExpandedByDefaultIfNotThenExpand(TitleOfSDG)) {
								...
								if (home.pageSizeSelect(TitleOfSDG, pageSize)) {
									...
									home.verifyColumnAscendingDescendingOrder(TitleOfSDG, columnInSDG, dateColumnInSDG,
											amountColumnInSDG, pickListColumnAndValues, "Yes");
