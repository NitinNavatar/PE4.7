...
// pipeline related list on company page (Pipeline Name<>Stage<>Log In Date<>Investment Size<>Source Firm<>Deal Type<>Our Role)	
	...
	public void PESmokeTc000_1_addField(String environment, String mode) {
		...
		if (home.clickOnSetUpLink(environment, Mode.Classic.toString())) {
			...
			abc = setup.DragNDrop(environment, Mode.Classic.toString(), object.Account, ObjectFeatureName.pageLayouts,
		...
		for (int i = 0; i < 3; i++) {
			try {
				if (home.clickOnSetUpLink(environment, mode)) {
					...
					if (sp.createPEUser(crmUser1FirstName, UserLastName, emailId, crmUserLience, crmUserProfile,
							null)) {
						log(LogStatus.INFO,
	...
	public void PESmokeTc001_2_AddRemoveTabAndActivateLighting(String environment, String mode) {
		...
		for (String[] userPass : userAndPassword) {
			...
			String addRemoveTabName = "Pipelines" + "," + "Partnerships" + "," + "Commitments" + ","
					+ "Fundraising Contacts" + "," + "Marketing Initiatives" + "," + "Marketing Prospects" + ","
					+ "Reports" + "," + "Office Locations" + "," + "Navatar Setup" + "," + "Fund Distributions" + ","
					+ "Fund Drawdowns" + "," + "Investor Distributions" + "," + "Capital Calls";
			...
			if (mode.equalsIgnoreCase(Mode.Classic.toString())) {
				...
				for (String tab : addRemoveTabs) {
					ele = FindElement(driver, "//a[contains(@title,'" + tab + "')]", "Tab : " + tab,
	...
	public void PESmokeTc001_3_createCustomEmailAndTemplate(String environment, String mode) {
		...
		if (report.createCustomReportOrDashboardFolder(environment, SmokeReportFolderName,
			...
			ExcelUtils.writeData(smokeFilePath, SmokeReportFolderName, "Report", excelLabel.Variable_Name,
		...
		if (home.clickOnSetUpLink(environment, Mode.Classic.toString())) {
			...
			if (emailtemplate.createCustomEmailFolder(environment, Mode.Classic.toString(), EmailTemplate1_FolderName,
				...
				ExcelUtils.writeData(smokeFilePath, EmailTemplate1_FolderName, "CustomEmailFolder",
				...
				if (emailtemplate.createCustomEmailTemplate(environment, Mode.Classic.toString(),
					...
					ExcelUtils.writeData(smokeFilePath, EmailTemplate1_TemplateName, "CustomEmailFolder",
	...
	public void PESmokeTc001_4_createPreCondition(String environment, String mode) {
			...
			if (lp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
					...
					if (ins.createInstitution(environment, mode, SmokeINS2, SmokeINS2_RecordType, FieldLabels,
						...
						if (lp.clickOnTab(environment, mode, TabName.ContactTab)) {
							...
							if (contact.createContact(mode, SmokeC5_FName, SmokeC5_LName, SmokeINS2, emailId,
								...
								sa.assertTrue(false,
										"Not able to create Contact: " + SmokeC5_FName + " " + SmokeC5_LName);
								log(LogStatus.ERROR,
										"Not able to create Contact: " + SmokeC5_FName + " " + SmokeC5_LName,
										YesNo.Yes);
							}
				...
				if (j == 3) {
					...
					if (ins.createInstitution(environment, mode, SmokeINS4, SmokeINS4_RecordType, FieldLabels,
						...
						String emailId = contact.generateRandomEmailId();
						String contact6FieldLabels = excelLabel.Other_Street.toString() + ","
								+ excelLabel.Other_City.toString() + "," + excelLabel.Other_State.toString() + ","
								+ excelLabel.Other_Zip.toString() + "," + excelLabel.Other_Country.toString();
						String contact6FieldValues = SmokeC6_Other_Street + "," + SmokeC6_Other_City + ","
								+ SmokeC6_Other_State + "," + SmokeC6_Other_Zip + "," + SmokeC6_Other_Country;

						if (lp.clickOnTab(environment, mode, TabName.ContactTab)) {
							appLog.info("click on contact tab");
							if (contact.createContact(mode, SmokeC6_FName, SmokeC6_LName, SmokeINS4, emailId,
									SmokeC4_RecordType, contact6FieldLabels, contact6FieldValues,
									CreationPage.InstitutionPage, null, null)) {
								...
								sa.assertTrue(false,
										"Not able to create Contact: " + SmokeC6_FName + " " + SmokeC6_LName);
								log(LogStatus.ERROR,
										"Not able to create Contact: " + SmokeC6_FName + " " + SmokeC6_LName,
										YesNo.Yes);
							}
		...
		for (int i = 0; i < 4; i++) {
			if (lp.clickOnTab(environment, mode, TabName.ContactTab)) {
				String emailId = contact.generateRandomEmailId();
				if (i == 0) {
					if (contact.createContact(mode, SmokeC1_FName, SmokeC1_LName, SmokeINS1, emailId,
							SmokeC1_RecordType, null, null, CreationPage.ContactPage, null, null)) {
				...
				if (i == 1) {
					if (contact.createContact(mode, SmokeC2_FName, SmokeC2_LName, SmokeINS1, emailId,
							SmokeC2_RecordType, null, null, CreationPage.ContactPage, null, null)) {
				...
				if (i == 2) {
					if (contact.createContact(mode, SmokeC3_FName, SmokeC3_LName, SmokeINDINV1, emailId,
							SmokeC3_RecordType, null, null, CreationPage.ContactPage, null, null)) {
				...
				if (i == 3) {
					if (contact.createContact(mode, SmokeC4_FName, SmokeC4_LName, SmokeINDINV1, "", SmokeC4_RecordType,
		...
		for (int i = 0; i < 3; i++) {
			if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
				if (i == 0) {
					if (fund.createFund(mode, Smoke_Fund1, SmokeFund1_Type, SmokeFund1_InvestmentCategory, FieldLabels,
							Values[i])) {
				...
				if (i == 1) {
					if (fund.createFund(mode, Smoke_Fund2, SmokeFund2_Type, SmokeFund2_InvestmentCategory, FieldLabels,
							Values[i])) {
				...
				if (i == 2) {
					if (fund.createFund(mode, Smoke_Fund3, SmokeFund3_Type, SmokeFund3_InvestmentCategory, FieldLabels,
							Values[i])) {
		...
		if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
			if (lp.clickOnSettings_Lightning(environment, mode)) {
				if (click(driver, lp.getDisplayAndLayout_Lightning(30), "display and layout",
					...
					if (click(driver, lp.getRecordPageSettings(30), "record page settings", action.SCROLLANDBOOLEAN)) {
						ThreadSleep(3000);
						if (click(driver, lp.getRelatedListRecordPageSetting(30), "related list setting",
								action.BOOLEAN)) {
						...
						if (click(driver, lp.getActivityTimelineRecordPageSetting(30), "activity timeline",
								action.BOOLEAN)) {
							log(LogStatus.INFO, "clicked on activity setting", YesNo.No);
							if (click(driver, lp.getRecordPageSettingSave(30), "save button",
									action.SCROLLANDBOOLEAN)) {
	...
	public void PESmokeTc002_verifyAddProspectAndAddContacts(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI1)) {
				...
				if (market.actionDropdownElement(environment, PageName.Marketing_Initiative,
						ShowMoreActionDropDownList.Add_Prospect, 10) != null) {
				...
				if (market.actionDropdownElement(environment, PageName.Marketing_Initiative,
						ShowMoreActionDropDownList.Email_Prospects, 10) != null) {
				...
				if (click(driver,
						market.actionDropdownElement(environment, PageName.Marketing_Initiative,
								ShowMoreActionDropDownList.Add_Prospect, 10),
						" add prospects button", action.BOOLEAN)) {
					ThreadSleep(2000);
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
	...
	public void PESmokeTc003_verifyAddContactsFromPastMarketingInitiativeTab(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI2)) {
				...
				ThreadSleep(2000);
				ele = market.actionDropdownElement(mode, PageName.MarketingInitiatives,
						ShowMoreActionDropDownList.Add_Prospect, 10);
			...
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI3)) {
				...
				if (click(driver, ele, " add prospects button", action.BOOLEAN)) {
					...
					if (click(driver, market.getReportTab(60), "reports tab", action.SCROLLANDBOOLEAN)) {
						...
						if (sendKeys(driver, market.getSelectReportSearchBox(10), "Test11 Navatar11 Report11",
								"Invalid Value on Search Text Box", action.BOOLEAN)) {
								...
								if (ele != null) {
									...
									if (aa.contains(MarketingInitiativesPageErrorMsg.reportDoestNotExistErrorMsg)) {
										...
										appLog.error(MarketingInitiativesPageErrorMsg.reportDoestNotExistErrorMsg
												+ " error messgae is not verified. Actual :" + aa);
										sa.assertTrue(false,
												MarketingInitiativesPageErrorMsg.reportDoestNotExistErrorMsg
														+ " error messgae is not verified. Actual :" + aa);
										log(LogStatus.ERROR,
												MarketingInitiativesPageErrorMsg.reportDoestNotExistErrorMsg
														+ " error messgae is not verified. Actual :" + aa,
							...
							if (click(driver, market.getAddProspectsHideSearchPopUp(10),
									"Add Prospects Hide Search PopUp", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "clicked on Add Prospect Hide Image", YesNo.Yes);

								if (sendKeys(driver, market.getSelectReportSearchBox(10), "", "Clear Search Text Box",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Cleared Value on Search Text Box", YesNo.Yes);

									if (click(driver, market.getSelectAReportLookUpIcon(20),
											"select a report look up icon", action.SCROLLANDBOOLEAN)) {
										appLog.info("clicked on select a report look up icon");
										ThreadSleep(2000);

										if (click(driver, market.getSelectAReportSearchLookUpIcon(20),
												"select a report search look up icon", action.SCROLLANDBOOLEAN)) {
											...
											if (ele != null) {
												...
												if (aa.contains(MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg)) {
													...
													appLog.error(MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg
															+ " error messgae is not verified. Actual :" + aa);
													sa.assertTrue(false,
															MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg
																	+ " error messgae is not verified. Actual :" + aa);
													log(LogStatus.ERROR,
															MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg
																	+ " error messgae is not verified. Actual :" + aa,
												...
												if (click(driver, market.getPleaseEnterAValueOKBtnOKBtn(20),
													...
													sa.assertTrue(false,
															"Not Able to Click on Please Enter a value ok button");
													log(LogStatus.ERROR,
															"Not Able to Click on Please Enter a value ok button",
															YesNo.Yes);
												}
												...
												appLog.error(MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg
														+ " is not visible so cannot verify error message");
												sa.assertTrue(false,
														MarketingInitiativesPageErrorMsg.enterAVAlueErrorMsg
																+ " is not visible so cannot verify error message");
												log(LogStatus.ERROR,
											...
											sa.assertTrue(false,
													"Not able to click Select Report SearchBox Look Up Icon");
											log(LogStatus.ERROR,
													"Not able to click Select Report SearchBox Look Up Icon",
													YesNo.Yes);
										}
										...
										if (sendKeys(driver, market.getSelectAReportSearchTextBox(30), SmokeReportName,
												SmokeReportName, action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, " pass value in select a report search text box : "
													+ SmokeReportName, YesNo.Yes);
											ThreadSleep(2000);

											ele = market.getSelectAReportPopUpFileName(SmokeReportFolderName,
													SmokeReportName, 20);
											if (ele != null) {
												appLog.info(SmokeReportName + " is visible in " + SmokeReportFolderName
														+ " folder select a reports look up pop up");

												if (click(driver, market.getCrossImgSelectAReportPopUp(10), "Cross Img",
														action.BOOLEAN)) {
													...
													if (ele != null) {
														...
														if (msg.isEmpty() || msg.equalsIgnoreCase("")) {
															appLog.info(
																	"Search Box is Blank after Click on Clear icon");
														} else {
															sa.assertTrue(false,
																	"Search Box Sould Blank after Click on Clear icon but it has a value : "
																			+ msg);
															log(LogStatus.ERROR,
																	"Search Box Sould Blank after Click on Clear icon but it has a value : "
																			+ msg,
														...
														log(LogStatus.ERROR, "Search Box Element is not present",
																YesNo.Yes);
												...
												appLog.error(
														"searched report is not visible in select a report pop up : "
																+ SmokeReportName + " so cannot select it");
												sa.assertTrue(false,
														"searched report is not visible in select a report pop up : "
																+ SmokeReportName + " so cannot select it");
												log(LogStatus.ERROR,
											...
											sa.assertTrue(false,
													"Not able to pass value in select a report search text box : "
															+ SmokeReportName);
											log(LogStatus.ERROR,
													"Not able to pass value in select a report search text box : "
															+ SmokeReportName,
									...
									if (clickUsingJavaScript(driver, market.getSelectAReportPopUpCrossIcon(10),
											"Select A rEPORT Cross Icon", action.SCROLLANDBOOLEAN)) {
										...
										log(LogStatus.ERROR, "Not Able to Click on Select A RePORT pOPuP cROSS Icon",
												YesNo.Yes);
	...
	public void PESmokeTc005_1_addContactsOnReportsTabInAddProspects(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI3)) {
				...
				WebElement ele = market.actionDropdownElement(mode, PageName.MarketingInitiatives,
						ShowMoreActionDropDownList.Add_Prospect, 10);
				if (click(driver, ele, " add prospects button", action.BOOLEAN)) {
					ThreadSleep(2000);
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						...
						for (int i = 0; i < 2; i++) {
							...
							if (market.selectReportFromReportsTab(reportFolderName, reportName)) {
								...
								if (i == 0) {
									...
									if (sendKeys(driver, market.getSearchForAContactTextBoxOnReportTab(10),
											SmokeC1_LName, "Search Box", action.BOOLEAN)) {
										appLog.info("Values Entered on Serach Box : " + SmokeC1_LName);
										ThreadSleep(2000);
										if (click(driver, market.getSearchIconForAContactOnReportTab(10),
												"search Icon ForA Contact On ReportTab", action.SCROLLANDBOOLEAN)) {
											appLog.info("clicked on search Icon ForA Contact On Report Tab");
											ThreadSleep(2000);

											if (market.ScrollAndClickOnContactNameCheckBoxInAddProspect(
													AddProspectsTab.Report, SmokeC1_FName + " " + SmokeC1_LName,
													SmokeINS1, 10)) {
												log(LogStatus.INFO, "Conatct Found After Search : " + SmokeC1_FName
												...
												if (market.ScrollAndClickOnContactNameCheckBoxInAddProspect(
													...
													sa.assertTrue(false, "Conatct Not Found After Search : "
															+ SmokeC1_FName + " " + SmokeC1_LName);
													log(LogStatus.ERROR, "Conatct Not Found After Search : "
															+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
												}

											} else {
												sa.assertTrue(false, "Conatct Not Found After Search : " + SmokeC1_FName
														+ " " + SmokeC1_LName);
												log(LogStatus.ERROR, "Conatct Not Found After Search : " + SmokeC1_FName
														+ " " + SmokeC1_LName, YesNo.Yes);
											}

											if (click(driver, market.getClearIconForAContactOnReportTab(15),
													"Clear Img Icon", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.INFO, "clicked on Clear Img Icon", YesNo.Yes);

											} else {
												sa.assertTrue(false, "Not able to clicked on Clear Img Icon");
												log(LogStatus.ERROR, "Not able to clicked on Clear Img Icon",
														YesNo.Yes);
											}

										} else {
											sa.assertTrue(false,
													"Not able to clicked on search Icon ForA Contac on Report Tab");
											log(LogStatus.ERROR,
													"Not able to clicked on search Icon ForA Contac on Report Tab",
													YesNo.Yes);
										}
										...
										sa.assertTrue(false,
												"Not Able to Entered Values Entered on Serach Box : " + SmokeC1_LName);
										log(LogStatus.ERROR,
												"Not Able to Entered Values Entered on Serach Box : " + SmokeC1_LName,
												YesNo.Yes);
									}
								...
								if (click(driver, market.getExcludeAnyAccountsIncludedInThisMIRemoveBtn(30),
										"Remove nutton", action.SCROLLANDBOOLEAN)) {
									String text = market.getSelectMIErrorMessage(30).getText().trim();
									if (text.equalsIgnoreCase(
											MarketingInitiativesPageErrorMsg.SelectMarketingInitiativeErrorMessage)) {
										log(LogStatus.INFO, "Select MI error message is verified.", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Select MI error message is not verified. Expected: "
												+ MarketingInitiativesPageErrorMsg.SelectMarketingInitiativeErrorMessage
												+ "\tActual: " + text, YesNo.Yes);
										sa.assertTrue(false, "Select MI error message is not verified. Expected: "
												+ MarketingInitiativesPageErrorMsg.SelectMarketingInitiativeErrorMessage
												+ "\tActual: " + text);
									}
									if (click(driver, market.getSelectMIErrorMessageOKButton(30), "Ok Button",
											action.BOOLEAN))
										;
								} else {
									log(LogStatus.ERROR,
											"NOt able to click on remove button so cannot check the error message.",
											YesNo.Yes);
									sa.assertTrue(false,
											"NOt able to click on remove button so cannot check the error message.");
								}
								ThreadSleep(2000);
								if (market.selectProspectsContactAndVerifyReviewProspectList(environment, mode,
										AddProspectsTab.Report, ContactAndAccountName, false).isEmpty()) {
									...
									if (i == 0) {
										if (click(driver, market.getExcludeAnyAccountsIncludedInThisMILookUpIcon(20),
											...
											if (market.selectValueFromLookUpWindow(Smoke_MI1)) {
												...
												if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
													switchToFrame(driver, 20,
															market.getMarketInitiativeFrame_Lightning(20));
												}
												if (click(driver,
													...
													ThreadSleep(2000);
													ele = market.getReomveContactConfirmationPopUpErrorMsg(30);
													if (ele != null) {
														String aa = ele.getText().trim();
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI3)) {
				...
				if (click(driver, ele, " add prospects button", action.BOOLEAN)) {
					...
					if (click(driver, market.getSearchBtnOnAddProspect(10), "Search Button", action.SCROLLANDBOOLEAN)) {
						...
						if (ele != null) {
							...
							if (msg.contains(MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage)) {
								appLog.info(MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage
										+ " error messgae is verified.");
							} else {
								sa.assertTrue(false, MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage
										+ " error messgae is not verified. Actual :" + msg);
								log(LogStatus.ERROR, MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage
										+ " error messgae is not verified. Actual :" + msg, YesNo.Yes);
							}
							if (click(driver,
									market.getPleaseSelectASearchCriteriaPopUpOKBtn(PageName.MarketingInitiatives, 10),
									"ok button", action.SCROLLANDBOOLEAN)) {
								...
								if (ele == null) {
									log(LogStatus.INFO,
											"Select a search Criteria error is not visible after Click on OK Button",
											YesNo.No);
								} else {
									sa.assertTrue(false,
											"Select a search Criteria error is visible after Click on OK Button");
									log(LogStatus.ERROR,
											"Select a search Criteria error is visible after Click on OK Button",
											YesNo.Yes);
								...
								sa.assertTrue(false,
										"Select a search Criteria error message ok button so cannot close error message pop up");
								log(LogStatus.ERROR,
										"Select a search Criteria error message ok button so cannot close error message pop up",
										YesNo.Yes);
							}
							...
							sa.assertTrue(false,
									"Select a search Criteria error message is not visible so cannot verify error message");
							log(LogStatus.ERROR,
									"Select a search Criteria error message is not visible so cannot verify error message",
									YesNo.Yes);
						}
						...
						log(LogStatus.ERROR, "NOt able to click on Search button so cannot check the error message.",
								YesNo.Yes);
					...
					if (click(driver, market.getAddRowLink(AddProspectsTab.AccountAndContacts),
							"account and contact tab", action.BOOLEAN)) {
						...
						if (click(driver, market.getRowRemoveIcon().get(0), "remove icon", action.BOOLEAN)) {
							...
							log(LogStatus.FAIL,
									"Not able to click on remove row link so cannot check remove functionality",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on remove row link so cannot check remove functionality");
						}
						...
						log(LogStatus.FAIL, "Not able to click on Add row link so cannot check remove functionality",
								YesNo.Yes);
					...
					ele = isDisplayed(driver, FindElement(driver, "//a[@id='" + Xpath + "']", "Add filter logic link",
							action.BOOLEAN, 10), "Visibility", 10, "Add filter logic link");
					if (ele != null) {
						...
						if (click(driver, ele, "add Filter Logic Button", action.BOOLEAN)) {
							...
							ele = isDisplayed(
									driver, FindElement(driver, "//input[@id='" + Xpath + "']",
							...
							for (int j = 0; j < 2; j++) {
								Xpath = "AddRow2";
								ele = isDisplayed(driver, FindElement(driver, "//a[@id='" + Xpath + "']",
										"Add row button", action.BOOLEAN, 10), "Visibility", 10, "Add row button");
							...
							if (click(driver, market.getAddProspectClearBtn(10), "Clear Btn",
									action.SCROLLANDBOOLEAN)) {
					...
					if (market.SearchforProspects(AddProspectsTab.AccountAndContacts,
							"Account:Legal Name,Contact:Email,Contact:Legal Name", "equals,equals,equals",
							SmokeINS1 + "," + SmokeC1_EmailID + "," + SmokeINS1, "(1 AND 2) OR 3")) {
						appLog.info("Filter logic applied successfully ");
						ThreadSleep(5000);
						if (market
								.selectProspectsContactAndVerifyReviewProspectList(environment, mode,
										AddProspectsTab.AccountAndContacts, ContactAndAccountName, false, false)
								.isEmpty()) {
						...
						if (click(driver, market.getSelectProspectsWrenchIcon(PageName.MarketingInitiatives, mode, 60),
								"wrench icon", action.SCROLLANDBOOLEAN)) {
							ThreadSleep(1000);

							if (selectVisibleTextFromDropDown(driver,
									market.getColumnToDisplayViewDropDownList(PageName.MarketingInitiatives, 30),
									"view drop down list", ViewFieldsName.Contact_Fields)) {
								log(LogStatus.INFO,
										"Select value from view drop down list : " + ViewFieldsName.Contact_Fields,
										YesNo.Yes);

							} else {
								log(LogStatus.FAIL, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Contact_Fields, YesNo.Yes);
								sa.assertTrue(false, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Contact_Fields);
							}
							ThreadSleep(1000);

							if (selectVisibleTextFromDropDown(driver,
									market.getColumnToDisplayViewDropDownList(PageName.MarketingInitiatives, 30),
									"view drop down list", ViewFieldsName.Account_Fields)) {
								log(LogStatus.INFO,
										"Select value from view drop down list : " + ViewFieldsName.Account_Fields,
										YesNo.Yes);

							} else {
								log(LogStatus.FAIL, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Account_Fields, YesNo.Yes);
								sa.assertTrue(false, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Account_Fields);
							}
							ThreadSleep(1000);
						...
						if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.MarketingInitiatives, ss, null,
								null, true).isEmpty()) {
							log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
							ThreadSleep(2000);
							if (compareMultipleList(driver, "Birthdate" + "," + "Firm Phone",
									market.getSelectProspectsHeaderTextList(PageName.MarketingInitiatives)).isEmpty()) {
							...
							if (market.columnToDisplayRevertToDefaultsSettings(PageName.MarketingInitiatives, mode)) {
								log(LogStatus.PASS, "column to display settings is revert to default successfully",
										YesNo.No);
							} else {
						...
						appLog.error(
								"Not able to apply filter login so cannot verify contact name and add columns in select prospect grid");
						log(LogStatus.FAIL,
								"Not able to apply filter login so cannot verify contact name and add columns in select prospect grid",
								YesNo.Yes);
						sa.assertTrue(false,
	...
	public void PESmokeTc006_verifyEmailProspectAndSendEmail(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI1)) {
				...
				WebElement ele = market.actionDropdownElement(mode, PageName.MarketingInitiatives,
						ShowMoreActionDropDownList.Email_Prospects, 10);
				if (click(driver, ele, " Email prospects button", action.BOOLEAN)) {
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						switchToFrame(driver, 20, market.getMarketInitiativeFrame_Lightning(20));
					}
					ThreadSleep(5000);
					ele = isDisplayed(driver, FindElement(driver, XpathelementTOSearch, "", action.BOOLEAN, 20),
							"visibility", 20, SmokeC4_FName + " " + SmokeC4_LName + " Contact Name input box");
					if (ele == null) {
						log(LogStatus.INFO, SmokeC4_FName + " " + SmokeC4_LName
								+ " is not available in email prospect contact grid", YesNo.No);
					} else {
						log(LogStatus.ERROR,
								SmokeC4_FName + " " + SmokeC4_LName + " is available in email prospect contact grid",
								YesNo.Yes);
						sa.assertTrue(false,
								SmokeC4_FName + " " + SmokeC4_LName + " is available in email prospect contact grid");
					}
					if (market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(PageName.emailProspects,
							ContactAndAccountName, searchContactInEmailProspectGrid.Yes).isEmpty()) {
						log(LogStatus.INFO,
						...
						if (click(driver, market.getEmailProspectStep1NextBtn(20), "step 1 next button",
							...
							String expectedResult = "All,Capital Call Notice,Investor Distribution Notice,"
									+ EmailTemplate1_FolderName;
								...
								if (ele != null) {
									if (click(driver, ele, "Capital Call Notice preview link",
										...
										if (click(driver, market.getEmailProspectStep2NextBtn(30), "step 2 next button",
											...
											ele = market.getEmailProspectsSelectedRecipientErrorMsg(
													PageName.emailProspects, 20);
											if (ele != null) {
												String aa = ele.getText().trim();
												if (aa.contains(MarketingInitiativesPageErrorMsg
											...
											if (click(driver,
												...
												if (click(driver,
														market.getEmailProspectSendBtn(TopOrBottom.BOTTOM, 30),
														"send button", action.SCROLLANDBOOLEAN)) {
													...
													if (ele != null) {
														...
														if (aa.contains(
																...
																&& aa.contains(MarketingInitiativesPageErrorMsg
																		.generatedEmailedRecipientErrorMsg("1",
																				"Email"))) {
															log(LogStatus.INFO,
			...
			if (cp.clickOnCreatedContact(mode, SmokeC1_FName, SmokeC1_LName)) {
				...
				if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Activity_History, null)) {
					...
					if (cp.verifyCreatedActivityHistory(environment, mode, "Capital Call Notice  of Commitment")) {
						...
						log(LogStatus.SKIP, "Activity not verified: " + "Capital Call Notice  of Commitment",
								YesNo.Yes);
					}
				...
				if (click(driver, ele, " Email prospects button", action.BOOLEAN)) {
					...
					if (!lst.isEmpty()) {
						...
						if (market.verifyDate(ActualDate, null, "Last Mass Email")) {
							...
							sa.assertTrue(false, "Last Mass Email Date is not verifed of contact" + SmokeC1_FName + " "
									+ SmokeC1_LName);
							log(LogStatus.ERROR, "Last Mass Email Date is not verifed of contact" + SmokeC1_FName + " "
									+ SmokeC1_LName, YesNo.Yes);
						}
					} else {
						sa.assertTrue(false,
	...
	public void PESmokeTc008_verifyEmailProspectPageAndSendMail(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI2)) {
				...
				ThreadSleep(3000);
				ele = market.actionDropdownElement(environment, PageName.MarketingInitiatives,
						ShowMoreActionDropDownList.Email_Prospects, 10);

				if (click(driver, ele, " Email prospects button", action.BOOLEAN)) {
					ThreadSleep(5000);
					if (market.EmailProspects(environment, mode, PageName.emailProspects, "Contact:Last Name", "equals",
							SmokeC2_LName, ContactAndAccountName, searchContactInEmailProspectGrid.No, emailFolderName,
	...
	public void PESmokeTc009_1_verifyEmailAndCreateActivityforContact2(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI2)) {
				...
				WebElement ele = market.actionDropdownElement(environment, PageName.MarketingInitiatives,
						ShowMoreActionDropDownList.Email_Prospects, 10);
				if (click(driver, ele, " Email prospects button", action.BOOLEAN)) {
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						switchToFrame(driver, 20, market.getMarketInitiativeFrame_Lightning(20));
	...
	@Parameters({ "environment", "mode" })
	@Test
	public void PESmokeTc009_2_addFilterLogicAndAddColumnFromWrenchIconInEMailProspect(String environment,
			String mode) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		MarketingInitiativesPageBusinesslayer market = new MarketingInitiativesPageBusinesslayer(driver);
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		...
		if (market.clickOnTab(environment, mode, TabName.MarketingInitiatives)) {
			if (market.clickOnCreatedMarketInitiatives(environment, mode, Smoke_MI3)) {
				...
				if (click(driver, ele, " email prospects button", action.BOOLEAN)) {
					ThreadSleep(2000);
					log(LogStatus.INFO, "clicked on Email Prospect Btn", YesNo.No);
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						switchToFrame(driver, 20, market.getMarketInitiativeFrame_Lightning(20));
						...
						if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
							if (market.clickOnShowMoreDropdownOnly(environment, mode, PageName.MarketingInitiatives)) {
								...
								if (click(driver, ele, " email prospects button", action.BOOLEAN)) {
									...
									if (click(driver, market.getEmailProspectStep1CancelBottomBtn(10), "Cancel Button",
											action.SCROLLANDBOOLEAN)) {
								...
								appLog.error("Not able to click on show 4 more action button in lighting ");
								sa.assertTrue(false, "Not able to click on show 4 more action button in lighting ");
								log(LogStatus.ERROR, "Not able to click on show 4 more action button in lighting ",
										YesNo.Yes);
							}
				...
				if (click(driver, ele, " email prospects button", action.BOOLEAN)) {
					...
					// Azhar Start
					if (click(driver, market.getAddFilterLogicLinkOnEmailProspect(10), "Add Filter Logic Link",
							action.BOOLEAN)) {
						...
						ele = isDisplayed(driver, FindElement(driver, "//input[@id='" + Xpath + "']",
								"Add filter logic text box", action.BOOLEAN, 10), "Visibility", 10,
								"Add filter logic text box");
						if (ele != null) {
							log(LogStatus.INFO, "find add filter logic text box", YesNo.No);
						} else {
							log(LogStatus.FAIL, "Not able find add filter logic text box", YesNo.Yes);
						...
						for (int j = 0; j < 2; j++) {
							Xpath = "AddRow2";
							ele = isDisplayed(driver, FindElement(driver, "//a[@id='" + Xpath + "']", "Add row button",
									action.BOOLEAN, 10), "Visibility", 10, "Add row button");
					...
					if (market.SearchforEmailProspects(environment, mode, PageName.emailProspects,
							"Account:Legal Name,Contact:Email,Contact:Legal Name", "equals,equals,equals",
							SmokeINS1 + "," + SmokeC1_EmailID + "," + SmokeINS1, "(1 AND 2) OR 3")) {
						appLog.info("Filter logic applied successfully ");
						ThreadSleep(5000);
						if (market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(
								PageName.emailProspects, ContactAndAccountName, searchContactInEmailProspectGrid.No)
								.isEmpty()) {
							log(LogStatus.PASS, "Search Contacts is visible in Select Propects Grid ", YesNo.No);
						} else {
							appLog.error("Search Contacts is not visible in Select Propects Grid ");
						...
						if (click(driver, market.getSelectProspectsWrenchIcon(PageName.emailProspects, mode, 60),
								"wrench icon", action.SCROLLANDBOOLEAN)) {
							ThreadSleep(1000);

							if (selectVisibleTextFromDropDown(driver,
									market.getColumnToDisplayViewDropDownList(PageName.emailProspects, 30),
									"view drop down list", ViewFieldsName.Contact_Fields)) {
								log(LogStatus.INFO,
										"Select value from view drop down list : " + ViewFieldsName.Contact_Fields,
										YesNo.Yes);
							...
							if (selectVisibleTextFromDropDown(driver,
									market.getColumnToDisplayViewDropDownList(PageName.emailProspects, 30),
									"view drop down list", ViewFieldsName.Marketing_Prospect_Fields)) {
								log(LogStatus.INFO, "Select value from view drop down list : "
										+ ViewFieldsName.Marketing_Prospect_Fields, YesNo.No);

							} else {
								log(LogStatus.FAIL, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Marketing_Prospect_Fields, YesNo.Yes);
								sa.assertTrue(false, "Not able to select value from view drop down list : "
										+ ViewFieldsName.Marketing_Prospect_Fields);
							}
							ThreadSleep(1000);
						...
						if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.emailProspects, ss, null, null,
								true).isEmpty()) {
							log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
							ThreadSleep(2000);
							if (compareMultipleList(driver, "Birthdate" + "," + "Marketing Initiative",
									market.getSelectProspectsHeaderTextList(PageName.emailProspects)).isEmpty()) {
								log(LogStatus.PASS, "Selected Prospects Header Text is verified ", YesNo.No);
							} else {
			...
			sa.assertTrue(false, "Not able to clicked on MI Tab so cannot verify Email prospects filter");
			log(LogStatus.SKIP, "Not able to clicked on MI Tab so cannot verify Email prospects filter", YesNo.Yes);
		}
		switchToDefaultContent(driver);
		lp.CRMlogout(environment, mode);
		sa.assertAll();
	...
	public void PESmokeTc010_verifyCreateFundRaisingButton(String environment, String mode) {
			...
			if (market.clickOnTab(environment, mode, TabName.FundsTab)) {
				...
				if (i == 1) {
					if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund2)) {
						...
						if (click(driver, fund.getCreateFundRaisingBtn(environment, mode, PageName.FundsPage, 20),
							...
							if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
									PopUpName.selectFundPopUp, Smoke_Fund2, SmokeCOM1)) {
								log(LogStatus.INFO, "select fund Name and company name", YesNo.No);
								if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
									switchToFrame(driver, 30, home.getCreateFundraisingsFrame_Lighting(20));
								}
								if (click(driver, home.getSelectFundNamePopUpCancelBtn(10), "cancel button",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "clicked on cancel button", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not able to click on cancel button", YesNo.Yes);
		...
		if (market.clickOnTab(environment, mode, TabName.HomeTab)) {
			...
			if (home.clickOnNavatarEdgeLinkHomePage(navigationMenuName, action.BOOLEAN, 30)) {
				...
				log(LogStatus.ERROR,
						"Not Able to Click on " + navigationMenuName + " so cannot verify list : " + filesName,
						YesNo.Yes);
				sa.assertTrue(false,
						"Not Able to Click on " + navigationMenuName + " so cannot verify list : " + filesName);
			}
	...
	public void PESmokeTc011_verifyCreateFundraisingsFromHomePage(String environment, String mode) {
		...
		actionsList.add(
				SmokeC1_FName + " " + SmokeC1_LName + "<break>" + fundraisingContactActions.PrimaryContact.toString());
		actionsList.add(SmokeC1_FName + " " + SmokeC1_LName + "<break>"
				+ fundraisingContactActions.Role_DecisionMaker.toString());
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		...
		for (int j = 0; j <= 2; j++) {
			...
			if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
				...
				if (j == 1) {
					if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
							PopUpName.selectFundPopUp, Smoke_Fund1, null)) {
						log(LogStatus.INFO, "Select Fund : " + Smoke_Fund1, YesNo.No);
						if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
							switchToFrame(driver, 30, home.getCreateFundraisingsFrame_Lighting(20));
						}
						if (click(driver, home.getSelectFundPopUpCancelBtn(10), "cancel button",
								action.SCROLLANDBOOLEAN)) {
				...
				if (j == 2) {
					if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
						...
						if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
								action.SCROLLANDBOOLEAN)) {
							ThreadSleep(3000);
							if (click(driver, home.getSearchBasedOnAccountsAndContactsTab(30),
									"Search Based On Accounts And Contacts Tab", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "click on Search Based On Accounts And Contacts Tab", YesNo.No);
								ThreadSleep(3000);
								if (home.applyFilterOnSearchBasedOnAccountsandContacts(
										FundraisingContactPageTab.SearchBasedOnAccountsAndContacts,
										SearchBasedOnExistingFundsOptions.AllContacts, environment, mode, null,
										"Account:Legal Name", "not equal to", "", null)) {
									log(LogStatus.INFO, "apply filter logic", YesNo.No);

									if (home.checkContactOrAccountOrFundraisingPage(environment, mode,
											SmokeC1_FName + " " + SmokeC1_LName, PageName.CreateFundraisingPage,
											columnName.contactName,
											market.getEmailProspectSelectProspectsGridScrollBox(10))) {
									...
									click(driver, home.getSearchBasedOnAccountsAndContactsSearchBtn(30),
											"search button", action.SCROLLANDBOOLEAN);
									if (home.checkContactOrAccountOrFundraisingPage(environment, mode, SmokeINS1,
											PageName.CreateFundraisingPage, columnName.AccountName,
											market.getEmailProspectSelectProspectsGridScrollBox(10))) {
									...
									click(driver, home.getSearchBasedOnAccountsAndContactsSearchBtn(30),
											"search button", action.SCROLLANDBOOLEAN);
									if (home.selectInvestorsContactFromCreateFundRaising(contactNamelist, accountlist)
											.isEmpty()) {
										log(LogStatus.INFO, "contact name is selected successfully", YesNo.No);
										if (click(driver, home.getAddToFundraisingListBtn(30),
												"Add To Fundraising List Button", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, "click on Add To Fundraising List", YesNo.No);
											if (home.verifyContactNameInReviewInvestorList(contactNamelist, accountlist)
													.isEmpty()) {
												log(LogStatus.INFO, "Contact Name is verified in review investor list",
														YesNo.No);
												ThreadSleep(2000);
												if (home.clickOnReviewInvestorListFundraisingContactLink(
														accountlist.get(0))) {
													ThreadSleep(2000);
													if (home.verifyFundRaisingContactAndSetRolePrimaryContact(
															Instituon1ContactName, Institution1List, actionsList)
															.isEmpty()) {
														log(LogStatus.INFO,
																"Fundraising contact is verified and select role,primary contact successfully",
																YesNo.No);
														if (click(driver, home.getFundraisingContactPopUpApplyBtn(20),
																"fundraising contact apply button",
																action.SCROLLANDBOOLEAN)) {
															log(LogStatus.INFO, "clicked on apply button", YesNo.No);
															List<String> fieldAndValue = new ArrayList<String>();
															fieldAndValue.add(
																	"Fundraising:Investment Likely Amount (mn)<break>100000000");
															if (home.selectDefaultFundraisingValue(fieldAndValue)) {
																log(LogStatus.INFO,
																		"select  Default Fundraising Values", YesNo.No);
																if (click(driver,
																		home.getCreateFundraisingBtn(
																				PageName.CreateFundraisingPage, 30),
																		"create fundraising button",
																		action.SCROLLANDBOOLEAN)) {
																	log(LogStatus.INFO,
																			"clicked on create fundraising button",
																			YesNo.No);
																	if (click(driver,
																			home.getCreateFundraisingConfirmationOkBtn(
																					30),
																			"ok button", action.SCROLLANDBOOLEAN)) {
																		log(LogStatus.INFO, "clicked on OK button",
																				YesNo.No);
																		flag = true;
																	} else {
																		log(LogStatus.ERROR,
																				"Not able to click on OK button so cannot get contact id and verify contact details on  created fundraising",
																				YesNo.Yes);
																		sa.assertTrue(false,
																				"Not able to click on OK button so cannot get contact id and verify contact details on  created fundraising");
																	}
																	...
																	log(LogStatus.ERROR,
																			"Not able to click on create fundraising button so cannot create fundraisings",
																			YesNo.Yes);
																	sa.assertTrue(false,
																			"Not able to click on create fundraising button so cannot create fundraisings");
																}
																...
																log(LogStatus.ERROR,
																		"Not able to select  Default Fundraising Values so cannot create fundraising",
																		YesNo.Yes);
																sa.assertTrue(false,
																		"Not able to select  Default Fundraising Values so cannot create fundraising");
															}
															...
															log(LogStatus.ERROR,
																	"Not able to click on apply button so cannot create fundraising",
																	YesNo.Yes);
															sa.assertTrue(false,
																	"Not able to click on apply button so cannot create fundraising");
														}
														...
														log(LogStatus.ERROR,
																"Fundraising contact is verified and select role,primary contact",
																YesNo.Yes);
														sa.assertTrue(false,
																"Fundraising contact is verified and select role,primary contact");
													}
												} else {
													log(LogStatus.ERROR, "Not able to click on " + accountlist.get(0)
															+ " fundraising contact link so cannot verify conatatcs and create fundraising",
															YesNo.Yes);
													sa.assertTrue(false, "Not able to click on " + accountlist.get(0)
															+ " fundraising contact link so cannot verify conatatcs and create fundraising");
												}

											} else {
												log(LogStatus.ERROR,
														"Contact Name is not verified in review investor list so cannot create fundraising",
														YesNo.Yes);
												sa.assertTrue(false,
														"Contact Name is not verified in review investor list so cannot create fundraising");
											}
											...
											log(LogStatus.ERROR,
													"Not able to click on Add To Fundraising List Button so cannot create fundraising",
													YesNo.Yes);
											sa.assertTrue(false,
													"Not able to click on Add To Fundraising List Button so cannot create fundraising");
										}
										...
										log(LogStatus.ERROR,
												" Not able to select Contact Name from select investor grid so cannot create fundraising",
												YesNo.Yes);
										sa.assertTrue(false,
												" Not able to select Contact Name from select investor grid so cannot create fundraising");
									}
									...
									log(LogStatus.ERROR,
											"Not able to apply filter logic so cannot verify create fundraising page",
											YesNo.Yes);
									sa.assertTrue(false,
											"Not able to apply filter logic so cannot verify create fundraising page");
								}
								...
								log(LogStatus.ERROR,
										"Not able to click on Search Based On Accounts And Contacts Tab so cannot verify create fundraising page",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to click on Search Based On Accounts And Contacts Tab so cannot verify create fundraising page");
							}
							...
							log(LogStatus.ERROR,
									"Not able to click on select fund continue button so cannot create fundraising",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on select fund continue button so cannot create fundraising");
						}
						...
						log(LogStatus.ERROR,
								"Not able to select fund from select fund pop up so cannot verify create fundraising page",
								YesNo.Yes);
						sa.assertTrue(false,
								"Not able to select fund from select fund pop up so cannot verify create fundraising page");
					}
				...
				log(LogStatus.ERROR,
						"Not able to click on create fundraising link so cannot verify create fundraising page : " + j,
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on create fundraising link so cannot verify create fundraising page : " + j);
			}
			...
			for (int i = 0; i < ss.length; i++) {
				if (home.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
					if (frpg.clickOnCreatedFundRaising(environment, mode, ss[i])) {
						if (frpg.clickOnRelatedList(environment, mode, RecordType.Fundraising,
							...
							if (i == 0) {
								if (frpg.verifyfundraisingContacts("", SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1,
									...
									log(LogStatus.INFO,
											SmokeC1_FName + " " + SmokeC1_LName
													+ " contact details are verified on Fundraising " + ss[i],
											YesNo.No);
								} else {
									log(LogStatus.INFO,
											SmokeC1_FName + " " + SmokeC1_LName
													+ " contact details are not verified on Fundraising " + ss[i],
											YesNo.No);
									sa.assertTrue(false, SmokeC1_FName + " " + SmokeC1_LName
											+ " contact details are not verified on Fundraising " + ss[i]);
								}
							}
							if (i == 1) {
								if (frpg.verifyfundraisingContacts("", SmokeC3_FName + " " + SmokeC3_LName,
										SmokeINDINV1, SmokeFRC3_Role, SmokeFRC3_Email)) {
									log(LogStatus.INFO,
											SmokeC3_FName + " " + SmokeC3_LName + " and " + SmokeC4_FName + " "
													+ SmokeC4_LName + "  contact details are verified on Fundraising "
													+ ss[i],
											YesNo.No);
								} else {
									log(LogStatus.INFO,
								...
								if (frpg.verifyfundraisingContacts("", SmokeC4_FName + " " + SmokeC4_LName,
										SmokeINDINV1, SmokeFRC4_Role, SmokeFRC4_Email)) {
									log(LogStatus.INFO,
											SmokeC4_FName + " " + SmokeC4_LName
													+ "  contact details are verified on Fundraising " + ss[i],
											YesNo.No);
								} else {
									log(LogStatus.INFO,
							...
							log(LogStatus.ERROR,
									"Not able to click on related list tab in lightning mode so cannot get contact id "
											+ SmokeC3_FName + " " + SmokeC3_LName + "," + SmokeC4_FName + " "
											+ SmokeC4_LName,
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on related list tab in lightning mode so cannot get contact id "
						...
						log(LogStatus.ERROR,
								"Not able to click on create fundraising " + Smoke_FR1 + " so cannot get contact id "
										+ SmokeC3_FName + " " + SmokeC3_LName + "," + SmokeC4_FName + " "
										+ SmokeC4_LName,
								YesNo.Yes);
						sa.assertTrue(false,
					...
					log(LogStatus.ERROR,
							"Not able to click on fundraising tab so cannot get contact id from " + Smoke_FR2,
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on fundraising tab so cannot get contact id from " + Smoke_FR2);
				}
	...
	public void PESmokeTc012_1_verifyCreateFundraisingsForCoInvestment(String environment, String mode) {
		...
		actionsList.add(SmokeC2_FName + " " + SmokeC2_LName + "<break>" + fundraisingContactActions.Remove.toString());
		actionsList.add(SmokeC1_FName + " " + SmokeC1_LName + "<break>"
				+ fundraisingContactActions.AddNewContactInFundraisingContact.toString());
		actionsList.add(SmokeC1_FName + " " + SmokeC1_LName + "<break>" + fundraisingContactActions.Remove.toString());
		String date = getDateAccToTimeZone(BasePageErrorMessage.AmericaLosAngelesTimeZone, "M/d/yyyy");
		boolean flag = false;
		lp.CRMLogin(crmUser1EmailID, adminPassword);
			...
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund2)) {
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fundraising, RelatedList.Fundraisings,
					...
					if (click(driver, fund.getCreateFundRaisingBtn(environment, mode, PageName.FundsPage, 30),
						...
						if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
								PopUpName.selectFundPopUp, Smoke_Fund2, null)) {
							...
							if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
								...
								ThreadSleep(3000);
								if (home.applyFilterOnSearchBasedOnAccountsandContacts(
										FundraisingContactPageTab.SearchBasedOnExistingFunds,
										SearchBasedOnExistingFundsOptions.AllContacts, environment, mode, Smoke_Fund1,
										null, null, null, null)) {
									log(LogStatus.INFO, "apply filter logic", YesNo.No);
									windowScrollYAxis(driver, 0, 500);
									ThreadSleep(3000);
									if (home.clickOnInfoIcon(SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1)) {
										log(LogStatus.INFO, "clicked on contact " + SmokeC1_FName + " " + SmokeC1_LName
												+ " info icon", YesNo.No);
										ThreadSleep(2000);
										SoftAssert result = home.VerifyPastFundraisingData(Smoke_FR1, "",
												SmokeFR1_Investment_Likely_Amount, SmokeFRC1_Role, SmokeFRC1_Primary,
												date);
										sa.combineAssertions(result);
										ThreadSleep(2000);
										if (home.checkContactOrAccountOrFundraisingPage(environment, mode, Smoke_FR1,
												PageName.pastFundraisingContactPopUp, columnName.Fundraising_Name,
												home.getPastFundraisingsPopUpScrollBox(
														PageName.pastFundraisingContactPopUp, 5))) {
											log(LogStatus.INFO, "Fundraising page is verified " + Smoke_FR1, YesNo.No);

										} else {
											log(LogStatus.ERROR, "Fundraising page is not verified" + Smoke_FR1,
													YesNo.Yes);
										...
										for (int i = 0; i < 2; i++) {
											if (click(driver,
													market.getSelectProspectsWrenchIcon(
															PageName.pastFundraisingContactPopUp, mode, 60),
													"wrench icon", action.SCROLLANDBOOLEAN)) {
												ThreadSleep(1000);
												if (i == 0) {
													if (click(driver,
															home.getPastFundraisingwrenchIconCancelBtn(
																	PageName.pastFundraisingContactPopUp, 10),
															"Cancel Btn", action.BOOLEAN)) {
														log(LogStatus.INFO, "click on Wrench Icon Cancel Btn",
																YesNo.No);
													} else {
														log(LogStatus.FAIL,
																"Not able to click on Wrench Icon Cancel Btn",
																YesNo.Yes);
														sa.assertTrue(false,
																"Not able to click on Wrench Icon Cancel Btn");
													}
												...
												if (i == 1) {
													if (click(driver,
															home.getPastFundraisingwrenchIconCrossIcon(
																	PageName.pastFundraisingContactPopUp, 10),
															"cross Btn", action.BOOLEAN)) {
														log(LogStatus.INFO, "click on Wrench Icon cross Btn", YesNo.No);
													} else {
														log(LogStatus.FAIL,
																"Not able to click on Wrench Icon cross Btn",
																YesNo.Yes);
														sa.assertTrue(false,
																"Not able to click on Wrench Icon cross Btn");
													}
										...
										if (market
												.addAndRemoveCloumnInSelectProspectGrid(mode,
														PageName.pastFundraisingContactPopUp, ss, null, null, true)
												.isEmpty()) {
											log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
											ThreadSleep(2000);
											if (compareMultipleList(driver, "Created By ID",
													market.getSelectProspectsHeaderTextList(
															PageName.pastFundraisingContactPopUp))
													.isEmpty()) {
												log(LogStatus.PASS, "Selected Prospects Header Text is verified ",
														YesNo.No);
											} else {
												appLog.error("Selected Prospects Header Text is not verified");
												log(LogStatus.FAIL, "Selected Prospects Header Text is not verified",
														YesNo.Yes);
												sa.assertTrue(false, "Selected Prospects Header Text is not verified");
											}
											ThreadSleep(2000);
											if (market.columnToDisplayRevertToDefaultsSettings(
													PageName.pastFundraisingContactPopUp, mode)) {
												log(LogStatus.PASS,
														"column to display settings is revert to default successfully",
														YesNo.No);
											} else {
												appLog.error("column to display settings is not revert to default");
												log(LogStatus.FAIL,
														"column to display settings is not revert to default",
														YesNo.Yes);
												sa.assertTrue(false,
														"column to display settings is not revert to default");
											}
											if (!compareMultipleList(driver, "Created By ID",
													market.getSelectProspectsHeaderTextList(
															PageName.pastFundraisingContactPopUp))
													.isEmpty()) {
												log(LogStatus.PASS, "Select Prospects Header Text is removed",
														YesNo.No);
											} else {
												appLog.error("Select Prospects Header Text is not removed");
												log(LogStatus.FAIL, "Select Prospects Header Text is not removed",
														YesNo.Yes);
												sa.assertTrue(false, "Select Prospects Header Text is not removed");
											}
										} else {
											appLog.error("Not able to add column form column to display popup");
											log(LogStatus.FAIL, "Not able to add column form column to display popup",
													YesNo.Yes);
											sa.assertTrue(false, "Not able to add column form column to display popup");
										}

										if (click(driver,
												home.getPastFundraisingPopCrossIcon(
														PageName.pastFundraisingContactPopUp, 10),
												"past fundraising cross icon", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, "clicked on past fundraising cross icon", YesNo.No);
										} else {
											log(LogStatus.ERROR,
													"Not able to click on past fundraising cross icon so cannot close pop up",
													YesNo.Yes);
											sa.assertTrue(false,
													"Not able to click on past fundraising cross icon so cannot close pop up");
										}
										...
										/*
										 * if(click(driver, home.getPastFundraisingwrenchIconCrossIcon(PageName.
										 * pastFundraisingContactPopUp, 10), "past fundraising wrench cross icon",
										 * action.SCROLLANDBOOLEAN)) { log(LogStatus.INFO,
										 * "clicked on past fundraising Wrech cross icon", YesNo.No); }else {
										 * log(LogStatus.ERROR,
										 * "Not able to click on past fundraising Wrech cross icon so cannot close pop up"
										...
										log(LogStatus.ERROR,
												"Not able to click on contact " + SmokeC1_FName + " " + SmokeC1_LName
														+ " info icon so cannot verify past fundraising data",
												YesNo.Yes);
										sa.assertTrue(false, "Not able to click on contact " + SmokeC1_FName + " "
												+ SmokeC1_LName + " info icon so cannot verify past fundraising data");
									}
									System.err.println("><><><><>< ");
									ThreadSleep(5000);
									WebElement ele = FindElement(driver,
											"//a[contains(text(),'" + SmokeC1_FName + "')][contains(text(),'"
													+ SmokeC1_LName + "')]/../../following-sibling::span//a[text()='"
													+ SmokeINS1 + "']",
											"account name link", action.SCROLLANDBOOLEAN, 10);
									if (ele != null) {
										if (mouseOverOperation(driver, ele)) {
											...
											ele = FindElement(driver,
													"//a[contains(text(),'" + SmokeC1_FName + "')][contains(text(),'"
															...
															+ SmokeINS1 + "']/following-sibling::a//img",
													"", action.BOOLEAN, 10);
											if (ele != null) {
												if (clickUsingJavaScript(driver, ele, "account name info icon",
														action.SCROLLANDBOOLEAN)) {
													log(LogStatus.INFO, "clicked on account name " + SmokeINS1,
															YesNo.No);
													ThreadSleep(2000);
													if (home.checkContactOrAccountOrFundraisingPage(environment, mode,
															Smoke_FR1, PageName.pastFundraisingAccountPopUp,
															columnName.Fundraising_Name,
															home.getPastFundraisingsPopUpScrollBox(
																	PageName.pastFundraisingAccountPopUp, 5))) {
														log(LogStatus.INFO, "Fundraising page is verified " + Smoke_FR1,
																YesNo.No);
														if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
															switchToFrame(driver, 30,
																	home.getCreateFundraisingsFrame_Lighting(20));
														}

													} else {
														log(LogStatus.ERROR,
																"Fundraising page is not verified" + Smoke_FR1,
																YesNo.Yes);
														sa.assertTrue(false,
																"Fundraising page is not verified" + Smoke_FR1);
													}
													...
													for (int i = 0; i < 2; i++) {
														if (click(driver,
																market.getSelectProspectsWrenchIcon(
																		PageName.pastFundraisingAccountPopUp, mode, 60),
																"wrench icon", action.SCROLLANDBOOLEAN)) {
															ThreadSleep(1000);
															if (i == 0) {
																if (click(driver,
																		home.getPastFundraisingwrenchIconCancelBtn(
																				PageName.pastFundraisingAccountPopUp,
																				10),
																		"Cancel Btn", action.BOOLEAN)) {
																	log(LogStatus.INFO,
																			"click on Wrench Icon Cancel Btn",
																			YesNo.No);
																} else {
																	log(LogStatus.FAIL,
																			"Not able to click on Wrench Icon Cancel Btn",
															...
															if (i == 1) {
																if (click(driver,
																		...
																		"Cancel Btn", action.BOOLEAN)) {
																	log(LogStatus.INFO,
																			"click on Wrench Icon cross Btn", YesNo.No);
																} else {
																	log(LogStatus.FAIL,
																			"Not able to click on Wrench Icon cross Btn",
																			YesNo.Yes);
															...
															log(LogStatus.FAIL, "Not able to click on Wrench Icon",
																	YesNo.Yes);
															sa.assertTrue(false, "Not able to click on Wrench Icon");
														}
													...
													String[] ss = { "Created By ID" };
													if (market.addAndRemoveCloumnInSelectProspectGrid(mode,
															PageName.pastFundraisingAccountPopUp, ss, null, null, true)
															.isEmpty()) {
														log(LogStatus.PASS, "column added form column to display popup",
																YesNo.No);
														ThreadSleep(2000);
														if (compareMultipleList(driver, "Created By ID",
																market.getSelectProspectsHeaderTextList(
																...
																.isEmpty()) {
															log(LogStatus.PASS,
																	"Selected Prospects Header Text is verified ",
																	YesNo.No);
														} else {
															appLog.error(
																	"Selected Prospects Header Text is not verified");
															log(LogStatus.FAIL,
																	"Selected Prospects Header Text is not verified",
																	YesNo.Yes);
															sa.assertTrue(false,
														...
														ThreadSleep(2000);
														if (market.columnToDisplayRevertToDefaultsSettings(
																PageName.pastFundraisingAccountPopUp, mode)) {
															log(LogStatus.PASS,
																	"column to display settings is revert to default successfully",
																	YesNo.No);
														} else {
														...
														if (!compareMultipleList(driver, "Created By ID",
																...
																.isEmpty()) {
															log(LogStatus.PASS,
																	"Select Prospects Header Text is removed",
																	YesNo.No);
														} else {
															appLog.error("Select Prospects Header Text is not removed");
															log(LogStatus.FAIL,
																	"Select Prospects Header Text is not removed",
																	YesNo.Yes);
															sa.assertTrue(false,
																	"Select Prospects Header Text is not removed");
														}
														...
														appLog.error(
																"Not able to add column form column to display popup");
														log(LogStatus.FAIL,
																"Not able to add column form column to display popup",
																YesNo.Yes);
														sa.assertTrue(false,
													...
													if (click(driver,
															home.getPastFundraisingPopCrossIcon(
																	PageName.pastFundraisingAccountPopUp, 10),
															"past fundraising cross icon", action.SCROLLANDBOOLEAN)) {
														log(LogStatus.INFO, "clicked on past fundraising cross icon",
																YesNo.No);
													} else {
														log(LogStatus.ERROR,
																"Not able to click on past fundraising cross icon so cannot close pop up",
													...
													/*
													 * if(click(driver,
													 * home.getPastFundraisingwrenchIconCrossIcon(PageName.
													 * pastFundraisingAccountPopUp, 10),
													 * "past fundraising wrench cross icon", action.SCROLLANDBOOLEAN)) {
													 * log(LogStatus.INFO,
													 * "clicked on past fundraising wrench cross icon", YesNo.No); }else
													 * { log(LogStatus.ERROR,
													 * "Not able to click on past fundraising wrench cross icon so cannot close pop up"
													 * , YesNo.Yes); sa.assertTrue(false,
													 * "Not able to click on past fundraising wrench cross icon so cannot close pop up"
													 * ); }
													 */
													...
													log(LogStatus.ERROR, "Not able to click on account name "
															+ SmokeINS1 + "  info icon", YesNo.Yes);
													sa.assertTrue(false, "Not able to click on account name "
															+ SmokeINS1 + "  info icon");
												}
											} else {
												log(LogStatus.ERROR,
														"Not able to click on account " + SmokeINS1
																+ " info icon so cannot verify past fundraising data",
														YesNo.Yes);
												sa.assertTrue(false, "Not able to click on account " + SmokeINS1
														+ " info icon so cannot verify past fundraising data");
											}

										} else {
											log(LogStatus.ERROR, "Not able to mouse over on account name " + SmokeINS1
													+ " so cannot click on contact Name info icon", YesNo.Yes);
											sa.assertTrue(false, "Not able to mouse over on account name " + SmokeINS1
													+ " so cannot click on contact Name info icon");
										}

									} else {
										log(LogStatus.ERROR,
												"Not able to click on account name " + SmokeINS1
														+ " info icon so cannot verify past fundraising data",
												YesNo.Yes);
										sa.assertTrue(false, "Not able to click on account name " + SmokeINS1
												+ " info icon so cannot verify past fundraising data");
									}
									...
									if (home.selectInvestorsContactFromCreateFundRaising(contactNamelist, accountlist)
										...
										if (click(driver, home.getAddToFundraisingListBtn(30),
											...
											if (home.verifyContactNameInReviewInvestorList(contactNamelist, accountlist)
												...
												if (home.clickOnReviewInvestorListFundraisingContactLink(
													...
													if (home.verifyFundRaisingContactAndSetRolePrimaryContact(
															Instituon1ContactName, Institution1List, actionsList)
															.isEmpty()) {
														log(LogStatus.INFO, "contatcts are removed successfully",
																YesNo.No);
														String error = HomePageErrorMessage.fundraisingContactNoDataToDisplay;
														if (home.getFundraisingContactErrorMsg(30) != null) {
															String aa = home.getFundraisingContactErrorMsg(10).getText()
																	.trim();
															if (aa.contains(error)) {
																log(LogStatus.INFO,
																		error + " Error Message is verified.",
																		YesNo.No);
															} else {
																log(LogStatus.ERROR,
																		error + " Error Message is not verified.",
																		YesNo.Yes);
																sa.assertTrue(false,
																		error + " Error Message is not verified.");
															}
															...
															log(LogStatus.ERROR, error
																	+ " is not visible so cannot verify error message",
																	YesNo.Yes);
															sa.assertTrue(false, error
																	+ " is not visible so cannot verify error message");
														}
														if (home.getFundraisingContactRecordCount(20) != null)
															if (home.getFundraisingContactRecordCount(10).getText()
																	.contains("0")) {
																log(LogStatus.INFO,
																		"fundraising contact record contact is matched successfully",
																		YesNo.No);
															} else {
																log(LogStatus.ERROR,
																		"fundraising contact record contact is not matched",
																		YesNo.Yes);
																sa.assertTrue(false,
																		"fundraising contact record contact is not matched");
															}
														...
														if (click(driver, home.getFundraisingContactPopUpApplyBtn(20),
															...
															log(LogStatus.INFO, "clicked on apply button", YesNo.No);
															ThreadSleep(5000);
															if (click(driver, home
																	.getCreateFundraisingDefaultFundraisingValuesAddRowsLink(
																			10),
																	"add row buton", action.SCROLLANDBOOLEAN)) {
																log(LogStatus.INFO,
																		"Clicked on Add row link in Default Fundraising Values",
																		YesNo.No);
															} else {
																log(LogStatus.INFO,
																		"Not able to Add row link in Default Fundraising Values",
																		YesNo.Yes);
																sa.assertTrue(false,
																		"Not able to Add row link in Default Fundraising Values");
															}
															...
															if (home.selectDefaultFundraisingValue(fieldAndValue)) {
																log(LogStatus.INFO,
																		"select  Default Fundraising Values", YesNo.No);
																SoftAssert saaa = home
																		.deleteRowsFromCreateFundraisingDefaultFundraisingValues(
																				environment, mode);
																...
																for (int k = 0; k < 3; k++) {
																	if (click(driver,
																		...
																		if (k == 0) {
																			if (click(driver, home
																					.getCreateFundraisingPopUpCancelBtn(
																							5),
																					"cancel button",
																					action.SCROLLANDBOOLEAN)) {
																				log(LogStatus.INFO,
																				...
																				log(LogStatus.INFO,
																						"Not able to click on create fundraising cancel button",
																						YesNo.Yes);
																				sa.assertTrue(false,
																						"Not able to click on create fundraising cancel button");
																			}
																		...
																		if (k == 1) {
																			if (click(driver, home
																					.getCreateFundraisingPopUpCrossIcon(
																							5),
																					"cross button",
																					action.SCROLLANDBOOLEAN)) {
																				log(LogStatus.INFO,
																				...
																				log(LogStatus.INFO,
																						"Not able to click on create fundraising cross button",
																						YesNo.Yes);
																				sa.assertTrue(false,
																						"Not able to click on create fundraising cross button");
																			}
																		...
																		if (k == 2) {
																			if (home.selectFundNameOrCompanyNameOnCreateFundraisings(
																					environment, mode,
																					PopUpName.WarningPopUp, null,
																					SmokeCOM1)) {
																				log(LogStatus.INFO,
																						"select company name "
																								+ SmokeCOM1,
																				...
																				if (mode.equalsIgnoreCase(
																						Mode.Lightning.toString())) {
																					switchToFrame(driver, 30, home
																							.getCreateFundraisingsFrame_Lighting(
																									20));
																				}
																				if (click(driver,
																						home.getCreateFundraisingBtn(
																								PageName.WarningPopUp,
																								30),
																						"create fundraising button",
																						action.SCROLLANDBOOLEAN)) {
																					log(LogStatus.INFO,
																							...
																							YesNo.No);
																					if (click(driver, home
																							.getCreateFundraisingConfirmationOkBtn(
																									30),
																							"ok button",
																							action.SCROLLANDBOOLEAN)) {
																						log(LogStatus.INFO,
																						...
																						log(LogStatus.ERROR,
																								"Not able to click on Ok button so cannot nevigate on fundraising page",
																								YesNo.Yes);
																						sa.assertTrue(false,
																								"Not able to click on Ok button so cannot nevigate on fundraising page");
																					}
																					...
																					log(LogStatus.ERROR,
																							"Not able to click on create fundraising button so warniing pop up",
																							YesNo.Yes);
																					sa.assertTrue(false,
																							"Not able to click on create fundraising button so warniing pop up");
																				}
																				...
																				log(LogStatus.ERROR,
																						"Not able to select company name "
																								+ SmokeCOM1,
																						YesNo.Yes);
																				sa.assertTrue(false,
																						"Not able to select company name "
																		...
																		log(LogStatus.ERROR,
																				" Not able to click on create fundraising button so cannot create fundraising",
																				YesNo.Yes);
																		sa.assertTrue(false,
																				" Not able to click on create fundraising button so cannot create fundraising");
																	}
																...
																log(LogStatus.ERROR,
																		"Not able to select Fundraising:Investment Likely Amount field and pass value 100000000 ",
																		YesNo.Yes);
																sa.assertTrue(false,
																		"Not able to select Fundraising:Investment Likely Amount field and pass value 100000000 ");
															}
															...
															log(LogStatus.ERROR,
																	"Not able to click on apply button so cannot close fundraising contact pop up",
																	YesNo.Yes);
															sa.assertTrue(false,
																	"Not able to click on apply button so cannot close fundraising contact pop up");
														}
														...
														log(LogStatus.ERROR,
																"Not able to remove contacts form fundraising contact grid",
																YesNo.Yes);
														sa.assertTrue(false,
																"Not able to remove contacts form fundraising contact grid");
													}
													...
													log(LogStatus.ERROR, "Not able to click on " + accountlist.get(0)
															+ " list so cannot remove contacts", YesNo.Yes);
													sa.assertTrue(false, "Not able to click on " + accountlist.get(0)
															+ " list so cannot remove contacts");
												}
											} else {
												log(LogStatus.ERROR,
														"Not able to verify contact name in review investor list",
														YesNo.Yes);
												sa.assertTrue(false,
														"Not able to verify contact name in review investor list");
											}
											...
											log(LogStatus.ERROR,
													"Not able to click on add fundraising button so cannot create fundraising",
													YesNo.No);
											sa.assertTrue(false,
													"Not able to click on add fundraising button so cannot create fundraising");
										}
										...
										log(LogStatus.ERROR,
												"Not able to add contacts in review investor list so cannot create fundraising",
												YesNo.Yes);
										sa.assertTrue(false,
												"Not able to add contacts in review investor list so cannot create fundraising");
									}
								} else {
									log(LogStatus.ERROR, "Not able to select existing fund " + Smoke_Fund1
											+ " from Search Based On Existing Funds", YesNo.Yes);
									sa.assertTrue(false, "Not able to select existing fund " + Smoke_Fund1
											+ " from Search Based On Existing Funds");
								}
							} else {
								log(LogStatus.ERROR,
										"Not able to click on select fund continue button so cannot create fundraising",
							...
							log(LogStatus.ERROR, "Not able to select fund " + Smoke_Fund1
									+ " from select fund pop up so cannot create fundraising", YesNo.Yes);
							sa.assertTrue(false, "Not able to select fund " + Smoke_Fund1
									+ " from select fund pop up so cannot create fundraising");
						}

					} else {
						log(LogStatus.ERROR,
								"Not able to click on cerate fundraising button so cannot create fundraising",
								YesNo.Yes);
						sa.assertTrue(false,
								"Not able to click on cerate fundraising button so cannot create fundraising");
					}
					...
					log(LogStatus.ERROR,
							"Not able to click on related list link so cannot click on create fundraising button",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on related list link so cannot click on create fundraising button");
				}
		...
		if (flag) {
			...
			for (int i = 0; i < FR.length; i++)
				if (home.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
					if (frpg.clickOnCreatedFundRaising(environment, mode, FR[i])) {
						if (frpg.clickOnRelatedList(environment, mode, RecordType.Fundraising,
							...
							if (i == 0) {
								try {
									if (home.verifyGridErrorMessage1(environment, mode,
											RelatedList.Fundraising_Contacts,
											HomePageErrorMessage.NoRecordToDisplayOnFR, 10)) {
										log(LogStatus.INFO, HomePageErrorMessage.NoRecordToDisplayOnFR
												+ " Verify Error Message on " + FR[i], YesNo.No);
									} else {
										log(LogStatus.ERROR, HomePageErrorMessage.NoRecordToDisplayOnFR
												+ " Error Message is not verified on " + FR[i], YesNo.Yes);
										sa.assertTrue(false, HomePageErrorMessage.NoRecordToDisplayOnFR
												+ " Error Message is not verified on " + FR[i]);
									}
								} catch (Exception e) {
							...
							if (i == 1) {
								...
								if (frpg.verifyfundraisingContacts("", SmokeC3_FName + " " + SmokeC3_LName,
										SmokeINDINV1, SmokeFRC5_Role, SmokeFRC5_Email)) {
									log(LogStatus.INFO,
											SmokeC3_FName + " " + SmokeC3_LName
													+ "  contact details are verified on Fundraising " + FR[i],
											YesNo.No);
								} else {
									log(LogStatus.INFO,
											SmokeC3_FName + " " + SmokeC3_LName
													+ " contact details are not verified on Fundraising " + FR[i],
											YesNo.No);
									sa.assertTrue(false, SmokeC3_FName + " " + SmokeC3_LName
											+ " contact details are not verified on Fundraising " + FR[i]);
								}

								if (frpg.verifyfundraisingContacts("", SmokeC4_FName + " " + SmokeC4_LName,
										SmokeINDINV1, SmokeFRC6_Role, SmokeFRC6_Email)) {
									log(LogStatus.INFO,
											SmokeC4_FName + " " + SmokeC4_LName
													+ "  contact details are verified on Fundraising " + FR[i],
											YesNo.No);
						...
						log(LogStatus.ERROR,
								"Not able to click on create fundraising " + Smoke_FR4 + " so cannot get contact id "
										+ SmokeC3_FName + " " + SmokeC3_LName + "," + SmokeC4_FName + " "
										+ SmokeC4_LName,
								YesNo.Yes);
						sa.assertTrue(false,
					...
					log(LogStatus.ERROR,
							"Not able to click on fundraising tab so cannot get contact id from " + Smoke_FR4,
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on fundraising tab so cannot get contact id from " + Smoke_FR4);
				}
	...
	public void PESmokeTc012_2_addFilterLogicInCreateFundraising(String environment, String mode) {
		...
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
			...
			if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode, PopUpName.selectFundPopUp,
					Smoke_Fund1, SmokeCOM1)) {
				log(LogStatus.INFO, "Select Fund : " + Smoke_Fund1, YesNo.No);
				if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					switchToFrame(driver, 30, home.getCreateFundraisingsFrame_Lighting(20));
				}
				if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
					...
					for (int j = 0; j < 2; j++) {
						if (j == 0) {
							...
							if (mouseOverOperation(driver, home.getSearchBasedOnExistingFundsDownArrow(20))) {
								...
								if (clickUsingJavaScript(driver,
										home.getOnlyFundraisingContactOptionOnSearchBasedOnExistingFunds(20),
								...
								log(LogStatus.ERROR, "Not able to click on Search Based On Existing Funds Down Arrow",
										YesNo.Yes);
						...
						if (click(driver, market.getAddRowLink(AddProspectsTab.AccountAndContacts),
							...
							log(LogStatus.FAIL, "Clicked on Add row link", YesNo.No);
							ThreadSleep(2000);
							if (click(driver, home.getCreateFundraisingFilterRemoveBtn().get(0), "remove icon",
									action.BOOLEAN)) {
								log(LogStatus.FAIL, "clicked on remove row link", YesNo.No);
							} else {
								appLog.error(
							...
							appLog.error("Not able to click on Add row link so cannot check remove functionality");
							log(LogStatus.FAIL,
									"Not able to click on Add row link so cannot check remove functionality",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on Add row link so cannot check remove functionality");
						}
						if (click(driver, home.getAddFilterLogicLinkOnCreateFundraising(10), "Add Filter Logic Link",
								action.BOOLEAN)) {
							log(LogStatus.INFO, "clicked on Add Filter Logic Link", YesNo.No);
							ThreadSleep(1000);
							...
							if (click(driver, home.getCreateFundraisingClearBtn(10), "Clear Btn",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "click on Clear Btn", YesNo.No);
							} else {
								log(LogStatus.FAIL, "Not able to click on Clear Btn", YesNo.Yes);
						...
						if (j == 0) {
							if (home.applyFilterOnSearchBasedOnAccountsandContacts(
									FundraisingContactPageTab.SearchBasedOnAccountsAndContacts,
									SearchBasedOnExistingFundsOptions.AllContacts, environment, mode, null,
									"Account:Legal Name<break>Contact:Email<break>Contact:Legal Name",
									"equals<break>equals<break>equals",
									SmokeINS1 + "<break>" + SmokeC1_EmailID + "<break>" + SmokeINS1,
									"(1 AND 2) OR 3")) {
								appLog.info("Filter logic applied successfully ");
								ThreadSleep(5000);
								...
								appLog.error(
										"Not able to apply filter logic in SearchBasedOnAccountsAndContacts  so cannot verify check filter logic");
								log(LogStatus.FAIL,
										"Not able to apply filter logic in SearchBasedOnAccountsAndContacts  so cannot verify check filter logic",
										YesNo.Yes);
								sa.assertTrue(false,
							...
							if (home.applyFilterOnSearchBasedOnAccountsandContacts(
								...
								appLog.error(
										"Not able to apply filter logic in OnlyFundraisingContacts  so cannot verify check filter logic");
								log(LogStatus.FAIL,
										"Not able to apply filter logic in OnlyFundraisingContacts  so cannot verify check filter logic",
										YesNo.Yes);
								sa.assertTrue(false,
						...
						if (j == 0) {
							if (click(driver,
									market.getSelectProspectsWrenchIcon(PageName.CreateFundraisingPage, mode, 60),
									"wrench icon", action.SCROLLANDBOOLEAN)) {
								ThreadSleep(1000);

								if (selectVisibleTextFromDropDown(driver,
										market.getColumnToDisplayViewDropDownList(PageName.CreateFundraisingPage, 30),
										"view drop down list", ViewFieldsName.Contact_Fields)) {
									log(LogStatus.INFO,
											"Select value from view drop down list : " + ViewFieldsName.Contact_Fields,
											YesNo.Yes);
								...
								if (click(driver, home.getCreateFundraisingwrenchIconCancelBtn(10), "Cancel Btn",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "click on Wrench Icon Cancel Btn", YesNo.No);
								} else {
									log(LogStatus.FAIL, "Not able to click on Wrench Icon Cancel Btn", YesNo.Yes);
							...
							String[] ss = { "Birthdate", "Firm Phone" };
							if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.CreateFundraisingPage, ss,
									null, null, true).isEmpty()) {
								log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
								ThreadSleep(2000);
								if (compareMultipleList(driver, "Birthdate" + "," + "Firm Phone",
										market.getSelectProspectsHeaderTextList(PageName.CreateFundraisingPage))
										.isEmpty()) {
									log(LogStatus.PASS, "Selected Prospects Header Text is verified ", YesNo.No);
								} else {
									appLog.error("Selected Prospects Header Text is not verified");
								...
								ThreadSleep(2000);
								if (market.columnToDisplayRevertToDefaultsSettings(PageName.CreateFundraisingPage,
										mode)) {
									log(LogStatus.PASS, "column to display settings is revert to default successfully",
											YesNo.No);
								} else {
									appLog.error("column to display settings is not revert to default");
									log(LogStatus.FAIL, "column to display settings is not revert to default",
											YesNo.Yes);
									sa.assertTrue(false, "column to display settings is not revert to default");
								}
								if (!compareMultipleList(driver, "Birthdate" + "," + "Firm Phone",
					...
					log(LogStatus.ERROR,
							"Not able to click on select fund continue button so cannot create fundraising", YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on select fund continue button so cannot create fundraising");
				}
			} else {
	...
	public void PESmokeTc014_verifyCoInvestmentSetting(String environment, String mode) {
		...
		if (nsp.clickOnTab(environment, mode, TabName.NavatarSetup)) {
			if (nsp.clickOnNavatarSetupSideMenusTab(environment, NavatarSetupSideMenuTab.CoInvestmentSettings)) {
				ThreadSleep(2000);
				if (click(driver,
						nsp.getEditButtonforNavatarSetUpSideMenuTab(environment,
								NavatarSetupSideMenuTab.CoInvestmentSettings, 30),
						"edit button", action.SCROLLANDBOOLEAN)) {
					ThreadSleep(2000);
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					...
					if (click(driver,
							nsp.getEnableCheckBoxforClickNavatarSetUpSideMenuTab(environment,
									NavatarSetupSideMenuTab.CoInvestmentSettings, EditViewMode.Edit, 30),
							"Automatically link any Co-investment Fundraisings or Commitments to this Fund check box",
							action.SCROLLANDBOOLEAN)) {
						log(LogStatus.INFO,
								"clicked on Automatically link any Co-investment Fundraisings or Commitments to this Fund check box",
								YesNo.No);
						if (click(driver, nsp.getCoInvestmentFundLookUpIcon(20), "co investment look up icon",
								action.SCROLLANDBOOLEAN)) {
							if (nsp.selectValueFromLookUpWindow(Smoke_Fund1)) {
								...
								if (click(driver,
										nsp.getSaveButtonforNavatarSetUpSideMenuTab(environment,
												NavatarSetupSideMenuTab.CoInvestmentSettings, 30, TopOrBottom.TOP),
									...
									if (aa.equalsIgnoreCase(NavatarSetUpPageErrorMessage.selectCoInvestmentErrorMsg)) {
										log(LogStatus.INFO, NavatarSetUpPageErrorMessage.selectCoInvestmentErrorMsg
												+ " error message is verified", YesNo.No);
									} else {
										log(LogStatus.INFO, NavatarSetUpPageErrorMessage.selectCoInvestmentErrorMsg
												+ " error message is not verified." + " \t Actual Result: " + aa,
												YesNo.No);
										sa.assertTrue(false, NavatarSetUpPageErrorMessage.selectCoInvestmentErrorMsg
												+ " error message is not verified." + " \t Actual Result: " + aa);
									}
									if (click(driver,
											nsp.getCancelButtonforNavatarSetUpSideMenuTab(environment, mode,
													NavatarSetupSideMenuTab.CoInvestmentSettings, 20),
											"cancel button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.INFO, "click on cancel button", YesNo.No);
									} else {
										log(LogStatus.ERROR,
												"Not able to click on cancel button so cannot close co investment pop up",
												YesNo.Yes);
										sa.assertTrue(false,
												"Not able to click on cancel button so cannot close co investment pop up");
									}
									...
									log(LogStatus.ERROR,
											"Not able to Click on save button so cannot check error message",
											YesNo.Yes);
									sa.assertTrue(false,
											"Not able to Click on save button so cannot check error message");
								}
								...
								if (coinsp.activateCoInvestmentSetting(environment, mode, Smoke_Fund3)) {
									log(LogStatus.INFO, "co investment settings is activated for fund " + Smoke_Fund3,
											YesNo.No);
								} else {
									log(LogStatus.ERROR,
											"co investment settings is not activated for fund " + Smoke_Fund3,
											YesNo.Yes);
									sa.assertTrue(false,
											"co investment settings is not activated for fund " + Smoke_Fund3);
								}
								...
								log(LogStatus.ERROR, "Not able to select fund " + Smoke_Fund1
										+ " from look up PopUp so cannot check error message", YesNo.Yes);
								sa.assertTrue(false, "Not able to select fund " + Smoke_Fund1
										+ " from look up PopUp so cannot check error message");
							}
						} else {
							log(LogStatus.ERROR,
									"Not able to click on co investment look up icon so cannot check error message",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on co investment look up icon so cannot check error message");
						}
						...
						log(LogStatus.ERROR,
								"Not ale to click on Automatically link any Co-investment Fundraisings or Commitments to this Fund check box so cannot check error message",
								YesNo.Yes);
						sa.assertTrue(false,
								"Not ale to click on Automatically link any Co-investment Fundraisings or Commitments to this Fund check box so cannot check error message");
					}
				} else {
					log(LogStatus.ERROR, "Not able to click on co-investment edit button so cannot check error message",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on co-investment edit button so cannot check error message");
				}

			} else {
				log(LogStatus.ERROR, "Not able to click on co investment tab so cannot verify co investment setting",
						YesNo.Yes);
				sa.assertTrue(false, "Not able to click on co investment tab so cannot verify co investment setting");
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on Navatar Setup Tab so cannot verify co investment setting",
					YesNo.Yes);
			...
			if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode, PopUpName.selectFundPopUp,
				...
				if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
					...
					if (aa.equalsIgnoreCase(HomePageErrorMessage.selectCoInvestmentRelatedFundErrorMsg)) {
						log(LogStatus.INFO, HomePageErrorMessage.selectCoInvestmentRelatedFundErrorMsg
								+ " is verified successfully", YesNo.No);
					} else {
						log(LogStatus.ERROR, HomePageErrorMessage.selectCoInvestmentRelatedFundErrorMsg
								+ " : error message is not verified ", YesNo.Yes);
						sa.assertTrue(false, HomePageErrorMessage.selectCoInvestmentRelatedFundErrorMsg
								+ " : error message is not verified ");
					}
					if (click(driver, home.getSelectFundNamePopUpCancelBtn(20), "cancel button",
							action.SCROLLANDBOOLEAN)) {
						log(LogStatus.INFO, "clicked on Cancel buton", YesNo.No);
					}
				} else {
					log(LogStatus.ERROR,
							"Not able to click on select fund continue button so cannot check error message",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on select fund continue button so cannot check error message");
				}
			} else {
				log(LogStatus.ERROR, "Not able to select fund from select fund pop up so cannot check error message",
						YesNo.Yes);
				sa.assertTrue(false, "Not able to select fund from select fund pop up so cannot check error message");
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on create fundraising link so cannot check error message",
					YesNo.Yes);
	...
	public void PESmokeTc016_1_createFundraisingFromCompanyPage(String environment, String mode) {
		...
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
				...
				if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
					...
					if (click(driver, home.getSearchBasedOnAccountsAndContactsTab(30),
						...
						if (home.applyFilterOnSearchBasedOnAccountsandContacts(
							...
							click(driver, home.getSearchBasedOnAccountsAndContactsSearchBtn(30), "search button",
									action.SCROLLANDBOOLEAN);
							if (home.selectInvestorsContactFromCreateFundRaising(contactNamelist, accountlist)
									.isEmpty()) {
								log(LogStatus.INFO, "contact name is selected successfully", YesNo.No);
								if (click(driver, home.getAddToFundraisingListBtn(30), "Add To Fundraising List Button",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "click on Add To Fundraising List", YesNo.No);
									if (home.verifyContactNameInReviewInvestorList(contactNamelist, accountlist)
											.isEmpty()) {
										...
										ThreadSleep(2000);
										if (click(driver,
												home.getCreateFundraisingBtn(PageName.CreateFundraisingPage, 30),
												"create fundraising button", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, "clicked on create fundraising button", YesNo.No);

											///////////////////////////
											if (home.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
													PopUpName.WarningPopUp, null, SmokeCOM2)) {
												...
												if (click(driver,
														home.getCreateFundraisingBtn(PageName.WarningPopUp, 30),
														"create fundraising button", action.SCROLLANDBOOLEAN)) {
													log(LogStatus.INFO, "clicked on create fundraising button",
															YesNo.No);
													if (click(driver, home.getCreateFundraisingConfirmationOkBtn(30),
															"ok button", action.SCROLLANDBOOLEAN)) {
														log(LogStatus.INFO, "clicked on OK button", YesNo.No);
														log(LogStatus.INFO, "clicked on OK button", YesNo.No);

														ExcelUtils.writeData(smokeFilePath,
																SmokeINS1 + " - " + Smoke_Fund3, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR5",
																excelLabel.FundRaising_Name);
														ExcelUtils.writeData(smokeFilePath, Smoke_Fund3, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR5",
																excelLabel.Fund_Name);
														ExcelUtils.writeData(smokeFilePath, SmokeINS1, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR5",
																excelLabel.Legal_Name);
														ExcelUtils.writeData(smokeFilePath, "1", "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR5",
																excelLabel.Total_Fundraising_Contacts);
														ExcelUtils.writeData(smokeFilePath, SmokeCOM2, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR5",
																excelLabel.Company_Name);
														ExcelUtils.writeData(smokeFilePath,
																SmokeINDINV1 + " - " + Smoke_Fund3, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR6",
																excelLabel.FundRaising_Name);
														ExcelUtils.writeData(smokeFilePath, Smoke_Fund3, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR6",
																excelLabel.Fund_Name);
														ExcelUtils.writeData(smokeFilePath, SmokeINDINV1,
																"Fundraisings", excelLabel.Variable_Name, "SmokeFR6",
																excelLabel.Legal_Name);
														ExcelUtils.writeData(smokeFilePath, "2", "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR6",
																excelLabel.Total_Fundraising_Contacts);
														ExcelUtils.writeData(smokeFilePath, SmokeCOM2, "Fundraisings",
																excelLabel.Variable_Name, "SmokeFR6",
												...
												log(LogStatus.ERROR, "Not able to select company name " + SmokeCOM2,
														YesNo.Yes);
			...
			log(LogStatus.ERROR,
					"Not able to click on create fundraising link so cannot verify create fundraising page : ",
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on create fundraising link so cannot verify create fundraising page : ");
		}
	...
	public void PESmokeTc016_2_createFundraisingFromCompanyPage(String environment, String mode) {
		...
		for (int i = 0; i < FR.length; i++) {
			if (home.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
				if (frpg.clickOnCreatedFundRaising(environment, mode, FR[i])) {
					if (frpg.clickOnRelatedList(environment, mode, RecordType.Fundraising,
						...
						if (i == 0) {
							...
							if (frpg.verifyfundraisingContacts("", SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1,
									SmokeFRC7_Role, SmokeFRC7_Email)) {
								log(LogStatus.INFO, SmokeC1_FName + " " + SmokeC1_LName
										+ "  contact details are verified on Fundraising " + FR[i], YesNo.No);
							} else {
								log(LogStatus.INFO,
										SmokeC1_FName + " " + SmokeC1_LName
												+ " contact details are not verified on Fundraising " + FR[i],
						...
						if (i == 1) {
							...
							if (frpg.verifyfundraisingContacts("", SmokeC3_FName + " " + SmokeC3_LName, SmokeINDINV1,
									SmokeFRC8_Role, SmokeFRC8_Email)) {
								log(LogStatus.INFO, SmokeC3_FName + " " + SmokeC3_LName
										+ "  contact details are verified on Fundraising " + FR[i], YesNo.No);
							} else {
								log(LogStatus.INFO,
										SmokeC3_FName + " " + SmokeC3_LName
							...
							if (frpg.verifyfundraisingContacts("", SmokeC4_FName + " " + SmokeC4_LName, SmokeINDINV1,
									SmokeFRC9_Role, SmokeFRC9_Email)) {
								log(LogStatus.INFO, SmokeC4_FName + " " + SmokeC4_LName
										+ "  contact details are verified on Fundraising " + FR[i], YesNo.No);
							} else {
								log(LogStatus.INFO,
					...
					log(LogStatus.ERROR,
							"Not able to click on create fundraising " + Smoke_FR4 + " so cannot get contact id "
									+ SmokeC3_FName + " " + SmokeC3_LName + "," + SmokeC4_FName + " " + SmokeC4_LName,
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on create fundraising " + Smoke_FR4 + " so cannot get contact id "
									+ SmokeC3_FName + " " + SmokeC3_LName + "," + SmokeC4_FName + " " + SmokeC4_LName);
				...
				log(LogStatus.ERROR, "Not able to click on fundraising tab so cannot get contact id from " + Smoke_FR4,
						YesNo.Yes);
				...
				if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
					...
					if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraising_Contacts,
						...
						if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
							...
							List<String> result = market
									.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(
											PageName.emailFundraisingContact, ContactAndAccountName,
											searchContactInEmailProspectGrid.No);
							if (j == 0) {
								if (result.isEmpty()) {
									log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName,
											YesNo.No);
									for (int i = 0; i <= 3; i++) {
										if (i == 0) {
											if (click(driver,
													market.getStep1NextBtn(PageName.emailFundraisingContact,
															TopOrBottom.TOP, 10),
													"Top Next button", action.SCROLLANDBOOLEAN)) {
										...
										if (i == 1) {
											if (click(driver,
													market.getStep2PreviousBtn(PageName.emailFundraisingContact,
															TopOrBottom.TOP, 10),
													"Top previous button", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on top previous button", YesNo.No);
											} else {
												log(LogStatus.ERROR, "Not able to click on top previous button",
														YesNo.Yes);
										...
										if (i == 2) {
											if (click(driver,
													market.getStep1NextBtn(PageName.emailFundraisingContact,
															TopOrBottom.BOTTOM, 10),
													"bottom Next button", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
											} else {
												log(LogStatus.ERROR, "Not able to click on BOTTOM next button",
														YesNo.Yes);
										...
										if (i == 3) {
											if (click(driver,
													market.getStep2PreviousBtn(PageName.emailFundraisingContact,
															TopOrBottom.BOTTOM, 10),
													"bottom previous button", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on BOTTOM previous button", YesNo.No);
											} else {
												log(LogStatus.ERROR, "Not able to click on BOTTOM previous button",
														YesNo.Yes);
										...
										if (i == 0 && i == 2) {
											...
											if (ele != null) {
												...
												if (HomePageErrorMessage.Step2_SelectAnEmailTemplate.equals(msg)) {
													...
													sa.assertTrue(false,
															"Step2 Page Verified Not Verified Actual : " + msg
																	+ " \t Expected :"
																	+ HomePageErrorMessage.Step2_SelectAnEmailTemplate);
													log(LogStatus.FAIL,
															"Step2 Page Verified Not Verified Actual : " + msg
																	+ " \t Expected :"
																	+ HomePageErrorMessage.Step2_SelectAnEmailTemplate,
									...
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check next and previous button", YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check next and previous button");
								}
							}
							if (j == 1) {
								if (result.isEmpty()) {
									log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName,
											YesNo.No);
									if (click(
											driver, market.getStep1CancelBtn(PageName.emailFundraisingContact,
													TopOrBottom.TOP, 10),
											"Top cancel button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top cancel button step 1", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on top cancel button step 1",
												YesNo.Yes);
										sa.assertTrue(false, "Not able to click on top cancel button step 1");
									}
								} else {
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot Top Cancel bottom on step 1", YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot Top Cancel bottom on step 1");
								}
							}
							if (j == 2) {
								if (result.isEmpty()) {
									...
									if (click(
											driver, market.getStep1CancelBtn(PageName.emailFundraisingContact,
													TopOrBottom.TOP, 10),
											"bottom cancel button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on bottom cancel button step 1", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on bottom cancel button step 1",
												YesNo.Yes);
										sa.assertTrue(false, "Not able to click on bottom cancel button step 1");
									}
								} else {
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot bottom Cancel bottom on step 1", YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot bottom Cancel bottom on step 1");
								}
							}
							if (j == 3) {
								if (result.isEmpty()) {
									log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName,
											YesNo.No);
									if (click(
											driver, market.getStep1NextBtn(PageName.emailFundraisingContact,
													TopOrBottom.BOTTOM, 10),
											"bottom Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
										ThreadSleep(2000);
										if (click(driver,
												market.getStep2CancelBtn(PageName.emailFundraisingContact,
														TopOrBottom.TOP, 10),
												"top cancel button", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top cancel button step 2", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on top cancel button step 2",
													YesNo.Yes);
										...
										log(LogStatus.ERROR,
												"Not able to click on BOTTOM next button so cannot check Top Cancel button on Step2",
												YesNo.Yes);
										sa.assertTrue(false,
												"Not able to click on BOTTOM next button so cannot check Top Cancel button on Step2");
									}
									...
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check Top Cancel button on Step2", YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check Top Cancel button on Step2");
								}
							}
							if (j == 4) {
								if (result.isEmpty()) {
									...
									if (click(
										...
										if (click(driver,
											...
											log(LogStatus.ERROR, "Not able to click on bottom cancel button step 2",
													YesNo.Yes);
											sa.assertTrue(false, "Not able to click on bottom cancel button step 2");
										}
									} else {
										log(LogStatus.ERROR,
												"Not able to click on BOTTOM next button so cannot check bottom Cancel button on Step2",
												YesNo.Yes);
										sa.assertTrue(false,
												"Not able to click on BOTTOM next button so cannot check bottom Cancel button on Step2");
									}
									...
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check bottom Cancel button on Step2",
											YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check bottom Cancel button on Step2");
								}
							...
							if (j == 5) {
								if (result.isEmpty()) {
									log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName,
											YesNo.No);
									if (click(driver, market.getStep1NextBtn(PageName.emailFundraisingContact,
											TopOrBottom.TOP, 10), "Top Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button step 1", YesNo.No);
										ThreadSleep(2000);
										for (int i = 0; i <= 3; i++) {
											if (i == 0) {
												if (market.selectEmailTemplateFromEmailProspect(null,
														"Capital Call Notice")) {
													log(LogStatus.ERROR, "Select Capital Call Notice template",
															YesNo.No);
													if (click(driver,
															market.getStep2NextBtn(PageName.emailFundraisingContact,
																	TopOrBottom.TOP, 10),
															"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
														log(LogStatus.ERROR, "clicked on top next button step 2",
																YesNo.No);
													} else {
														log(LogStatus.ERROR,
																"Not able to click on top next button step 2",
																YesNo.Yes);
														sa.assertTrue(false,
																"Not able to click on top next button step 2");
													}
													...
													log(LogStatus.ERROR,
															"Not able to click select email template from step 2 so cannot check previous botton of step 3",
															YesNo.Yes);
													sa.assertTrue(false,
															"Not able to click select email template from step 2 so cannot check previous botton of step 3");
													break;
											...
											if (i == 1) {
												if (click(driver,
														market.getStep3PreviousBtn(PageName.emailFundraisingContact,
																TopOrBottom.TOP, 10),
														"Top previous button step 3", action.SCROLLANDBOOLEAN)) {
													log(LogStatus.ERROR, "clicked on top previous button step 3",
															YesNo.No);
												} else {
													log(LogStatus.ERROR,
															"Not able to click on top previous button step 3",
															YesNo.Yes);
													sa.assertTrue(false,
															"Not able to click on top previous button step 3");
												}
											...
											if (i == 2) {
												if (click(driver,
													...
													log(LogStatus.ERROR, "Not able to click on top next button step 2",
															YesNo.Yes);
											...
											if (i == 3) {
												if (click(driver,
														market.getStep3PreviousBtn(PageName.emailFundraisingContact,
																TopOrBottom.BOTTOM, 10),
														"bottom previous button step 3", action.SCROLLANDBOOLEAN)) {
													log(LogStatus.ERROR, "clicked on bottom previous button step 3",
															YesNo.No);
												} else {
													log(LogStatus.ERROR,
															"Not able to click on bottom previous button step 3",
															YesNo.Yes);
													sa.assertTrue(false,
															"Not able to click on bottom previous button step 3");
												}
											...
											if (i == 0 && i == 2) {
												...
												if (ele != null) {
													...
													if (HomePageErrorMessage.step3_ReviewAndConfirm.equals(msg)) {
														...
														sa.assertTrue(false,
																"Step3 Page Verified Not Verified Actual : " + msg
																		+ " \t Expected :"
																		+ HomePageErrorMessage.step3_ReviewAndConfirm);
														log(LogStatus.FAIL,
																"Step3 Page Verified Not Verified Actual : " + msg
																		+ " \t Expected :"
																		+ HomePageErrorMessage.step3_ReviewAndConfirm,
										...
										log(LogStatus.ERROR,
												"Not able to click on top next button step 1 so cannot check previous botton of step3",
												YesNo.Yes);
										sa.assertTrue(false,
												"Not able to click on top next button step 1 so cannot check previous botton of step3");
									}
							...
							if (j == 6) {
								if (result.isEmpty()) {
									...
									if (click(
											driver, market.getStep1NextBtn(PageName.emailFundraisingContact,
													TopOrBottom.TOP, 10),
											"Top Next button step 1", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button step 1", YesNo.No);
										if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
											...
											if (click(driver,
												...
												ThreadSleep(2000);
												if (click(driver,
														market.getStep3CancelBtn(PageName.emailFundraisingContact,
																TopOrBottom.TOP, 10),
														"top cancel button step 3", action.SCROLLANDBOOLEAN)) {
													log(LogStatus.ERROR, "clicked on top cancel button step 3",
															YesNo.No);
												} else {
													log(LogStatus.ERROR,
															"Not able to click on top cancel button step 3", YesNo.Yes);
													sa.assertTrue(false,
															"Not able to click on top cancel button step 3");
												}
											} else {
												log(LogStatus.ERROR, "Not able to click on top next button step 2",
														YesNo.Yes);
									...
									log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check step 3 cancel button", YesNo.Yes);
									sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " "
											+ SmokeC1_LName + " so cannot check step 3 cancel button");
								}
							}
							if (j == 7) {
								if (result.isEmpty()) {
										...
										if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
											...
											if (click(driver,
												...
												if (click(driver,
														market.getStep3CancelBtn(PageName.emailFundraisingContact,
																TopOrBottom.BOTTOM, 10),
														"buttom cancel button step 3", action.SCROLLANDBOOLEAN)) {
													log(LogStatus.ERROR, "clicked on buttom cancel button step 3",
															YesNo.No);
												} else {
													log(LogStatus.ERROR,
															"Not able to click on buttom cancel button step 3",
															YesNo.Yes);
													sa.assertTrue(false,
															"Not able to click on buttom cancel button step 3");
												}
							...
							log(LogStatus.ERROR,
									"Not able to click on fundraising contact button so cannot send email to contact "
											+ SmokeC1_FName + " " + SmokeC1_LName,
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on fundraising contact button so cannot send email to contact "
						...
						log(LogStatus.ERROR, "Not able to click on related list so cannot send email to contact "
								+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
						sa.assertTrue(false, "Not able to click on related list so cannot send email to contact "
								+ SmokeC1_FName + " " + SmokeC1_LName);
					}

				} else {
					log(LogStatus.ERROR, "Not able to click on fund " + Smoke_Fund1
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
					sa.assertTrue(false, "Not able to click on fund " + Smoke_Fund1
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName);
				}
			} else {
				log(LogStatus.ERROR, j + " : Not able to click on fund tab so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
				sa.assertTrue(false, j + " : Not able to click on fund tab so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName);
			}
		}
		lp.CRMlogout(environment, mode);
	...
	public void PESmokeTc017_2_EmailFundraisingContactsAndSendEMail(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraisings,
					...
					if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
						...
						if (market.EmailProspects(environment, mode, PageName.emailFundraisingContact, null, null, null,
							...
							log(LogStatus.INFO, "Email has been send to contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " successfully", YesNo.No);
						} else {
							log(LogStatus.ERROR,
									"Not able to send email to contact " + SmokeC1_FName + " " + SmokeC1_LName,
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to send email to contact " + SmokeC1_FName + " " + SmokeC1_LName);
						}
				...
				log(LogStatus.ERROR, "Not able to click on fund " + Smoke_Fund1 + " so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
				sa.assertTrue(false, "Not able to click on fund " + Smoke_Fund1 + " so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName);
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on fund tab so cannot send email to contact " + SmokeC1_FName + " "
					+ SmokeC1_LName, YesNo.Yes);
			sa.assertTrue(false, "Not able to click on fund tab so cannot send email to contact " + SmokeC1_FName + " "
					+ SmokeC1_LName);
		}
		lp.CRMlogout(environment, mode);
		sa.assertAll();
	...
	public void PESmokeTc018_verifyRecievedEmailAndCreatedActivity(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraising_Contacts,
					...
					if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
						...
						if (!lst.isEmpty()) {
							...
							if (market.verifyDate(ActualDate, null, "Last Mass Email")) {
								log(LogStatus.INFO,
										"Email Sent Date is verifed of contact" + SmokeC1_FName + " " + SmokeC1_LName,
										YesNo.No);
							} else {
								sa.assertTrue(false,
										"Email Sent Date is verifed of contact" + SmokeC1_FName + " " + SmokeC1_LName);
							...
							sa.assertTrue(false,
									"Email Sent Column is not visible in fundraising content grid so cannot verify "
											+ SmokeC1_FName + " " + SmokeC1_LName + "Email Sent");
							log(LogStatus.ERROR,
									"Email Sent Column is not visible in fundraising content grid so cannot verify "
											+ SmokeC1_FName + " " + SmokeC1_LName + "Email Sent",
									YesNo.Yes);
						...
						log(LogStatus.ERROR,
								"Not able to click on fundraising contact button so cannot check activity of contact "
										+ SmokeC1_FName + " " + SmokeC1_LName,
								YesNo.Yes);
						sa.assertTrue(false,
								"Not able to click on fundraising contact button so cannot check activity of contact "
					...
					log(LogStatus.ERROR, "Not able to click on related list so cannot check activity of contact "
							+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
					sa.assertTrue(false, "Not able to click on related list so cannot check activity of contact "
							+ SmokeC1_FName + " " + SmokeC1_LName);
				}
			} else {
				log(LogStatus.SKIP,
						"Not able to click on fund " + Smoke_Fund1
								+ " so cannot check email fundraising activities of contact " + SmokeC1_FName + " "
								+ SmokeC1_LName,
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on fund " + Smoke_Fund1
			...
			log(LogStatus.SKIP, "Not able to click on fund tab so cannot check email fundraising activities of contact "
					+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on fund tab so cannot check email fundraising activities of contact "
							+ SmokeC1_FName + " " + SmokeC1_LName);
		}
		switchToDefaultContent(driver);
		lp.CRMlogout(environment, mode);
	...
	public void PESmokeTc019_SendCustomEMailFromEmailFundraisingContact(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraisings,
					...
					if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
						...
						if (market.EmailProspects(environment, mode, PageName.emailFundraisingContact,
								"Contact:Last Name", "equals", SmokeC3_LName, ContactAndAccountName,
								searchContactInEmailProspectGrid.No, EmailTemplate1_FolderName,
								EmailTemplate1_TemplateName, null, false)) {
							log(LogStatus.INFO, "Email has been send to contact " + SmokeC3_FName + " " + SmokeC3_LName
									+ " successfully", YesNo.No);
						} else {
							log(LogStatus.ERROR,
									"Not able to send email to contact " + SmokeC3_FName + " " + SmokeC3_LName,
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to send email to contact " + SmokeC3_FName + " " + SmokeC3_LName);
						}
					...
					log(LogStatus.ERROR, "Not able to click on related list so cannot send email to contact "
							+ SmokeC3_FName + " " + SmokeC3_LName, YesNo.Yes);
					sa.assertTrue(false, "Not able to click on related list so cannot send email to contact "
							+ SmokeC3_FName + " " + SmokeC3_LName);
				}
			} else {
			...
			log(LogStatus.ERROR, "Not able to click on fund tab so cannot send email to contact " + SmokeC3_FName + " "
					+ SmokeC3_LName, YesNo.Yes);
			sa.assertTrue(false, "Not able to click on fund tab so cannot send email to contact " + SmokeC3_FName + " "
					+ SmokeC3_LName);
		}
		lp.CRMlogout(environment, mode);
		sa.assertAll();
	...
	public void PESmokeTc020_1_verifyRecievedCustomEmailAndCreatedActivityforContact3(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund3)) {
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraising_Contacts,
					...
					if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
						...
						if (!lst.isEmpty()) {
							...
							if (market.verifyDate(ActualDate, null, "Last Mass Email")) {
								log(LogStatus.INFO,
										"Email Sent Date is verifed of contact" + SmokeC3_FName + " " + SmokeC3_LName,
										YesNo.No);
							} else {
								sa.assertTrue(false,
										"Email Sent Date is verifed of contact" + SmokeC3_FName + " " + SmokeC3_LName);
							...
							sa.assertTrue(false,
									"Email Sent Column is not visible in fundraising content grid so cannot verify "
											+ SmokeC3_FName + " " + SmokeC3_LName + "Email Sent");
							log(LogStatus.ERROR,
									"Email Sent Column is not visible in fundraising content grid so cannot verify "
											+ SmokeC3_FName + " " + SmokeC3_LName + "Email Sent",
									YesNo.Yes);
				...
				log(LogStatus.SKIP,
						"Not able to click on fund " + Smoke_Fund3
								+ " so cannot check email fundraising activities of contact " + SmokeC3_FName + " "
								+ SmokeC3_LName,
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on fund " + Smoke_Fund3
			...
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Fundraisings,
					...
					if (click(driver, fund.getEmailFundraisingContactsBtn(environment, mode, 30),
						...
						if (click(driver, market.getEmailProspectApplyBtn(30), "search button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "clicked on Search button successfully", YesNo.No);
							...
							if (ele != null) {
								...
								if (click(driver,
										market.getPleaseSelectASearchCriteriaPopUpOKBtn(
												PageName.emailFundraisingContact, 10),
										"ok button", action.SCROLLANDBOOLEAN)) {
									appLog.info("Select a criteria error message ok button");
									ThreadSleep(2000);
							...
							log(LogStatus.ERROR, "Not able to click on Search button so cannot check error message "
									+ MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage, YesNo.Yes);
							sa.assertTrue(false, "Not able to click on Search button so cannot check error message "
									+ MarketingInitiativesPageErrorMsg.SelectASearchCriteriaMessage);
						}
						...
						if (click(driver, market.getAddFilterLogicLinkOnEmailProspect(10), "Add Filter Logic Link",
							...
							if (click(driver, market.getEmailProspectClearBtn(10), "Clear Btn",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "click on Clear Btn", YesNo.No);
							} else {
								log(LogStatus.FAIL, "Not able to click on Clear Btn", YesNo.Yes);
						...
						if (market.SearchforEmailProspects(environment, mode, PageName.emailFundraisingContact,
								"Account:Legal Name,Contact:Email,Contact:Legal Name", "equals,equals,equals",
								SmokeINS1 + "," + SmokeC1_EmailID + "," + SmokeINS1, "(1 AND 2) OR 3")) {
							appLog.info("Filter logic applied successfully ");
							ThreadSleep(5000);
							if (market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(
									PageName.emailFundraisingContact, ContactAndAccountName,
									searchContactInEmailProspectGrid.No).isEmpty()) {
								log(LogStatus.PASS, "Search Contacts is visible in Select Propects Grid ", YesNo.No);
							} else {
								appLog.error("Search Contacts is not visible in Select Propects Grid ");
								log(LogStatus.FAIL, "Search Contacts is not visible in Select Propects Grid ",
										YesNo.Yes);
								sa.assertTrue(false, "Search Contacts is not visible in Select Propects Grid ");
							}
							...
							if (click(driver,
									market.getSelectProspectsWrenchIcon(PageName.emailFundraisingContact, mode, 60),
									"wrench icon", action.SCROLLANDBOOLEAN)) {
								ThreadSleep(1000);

								if (selectVisibleTextFromDropDown(driver,
										market.getColumnToDisplayViewDropDownList(PageName.emailFundraisingContact, 30),
										"view drop down list", ViewFieldsName.Fundraising_Fields)) {
									log(LogStatus.INFO, "Select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Fields, YesNo.Yes);

								} else {
									log(LogStatus.FAIL, "Not able to select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Fields, YesNo.Yes);
									sa.assertTrue(false, "Not able to select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Fields);
								}
								ThreadSleep(1000);

								if (selectVisibleTextFromDropDown(driver,
										market.getColumnToDisplayViewDropDownList(PageName.emailFundraisingContact, 30),
										"view drop down list", ViewFieldsName.Fundraising_Contact_Fields)) {
									log(LogStatus.INFO, "Select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Contact_Fields, YesNo.No);

								} else {
									log(LogStatus.FAIL, "Not able to select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Contact_Fields, YesNo.Yes);
									sa.assertTrue(false, "Not able to select value from view drop down list : "
											+ ViewFieldsName.Fundraising_Contact_Fields);
								}
								ThreadSleep(1000);
							...
							if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.emailFundraisingContact,
									ss, null, null, true).isEmpty()) {
								log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
								ThreadSleep(2000);
								if (compareMultipleList(driver, "Created date",
										market.getSelectProspectsHeaderTextList(PageName.emailFundraisingContact))
										.isEmpty()) {
									log(LogStatus.PASS, "Selected Prospects Header Text is verified ", YesNo.No);
								} else {
									appLog.error("Selected Prospects Header Text is not verified");
								...
								ThreadSleep(2000);
								if (market.columnToDisplayRevertToDefaultsSettings(PageName.emailFundraisingContact,
										mode)) {
									log(LogStatus.PASS, "column to display settings is revert to default successfully",
											YesNo.No);
								} else {
	...
	public void PESmokeTc021_verifyCommmitmentButtonOnPages(String environment, String mode) {
		...
		if (market.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
			if (frpg.clickOnCreatedFundRaising(environment, mode, Smoke_FR1)) {
				...
				ThreadSleep(5000);
				boolean flag = fund.clickOnShowMoreActionDownArrow(environment, PageName.FundraisingPage,
						ShowMoreActionDropDownList.Create_Commitments, 10);
				ThreadSleep(2000);
				if (flag) {
					...
					if (click(driver, comm.getCommitmentCancelBtn(TopOrBottom.TOP, 10),
							"Commitment Creation Cancel Top Button", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.INFO, "Clicked on Commitment Creation Cancel Top Button " + SmokeCOM1, YesNo.No);
						switchToDefaultContent(driver);
						flag = fund.clickOnShowMoreActionDownArrow(environment, PageName.FundraisingPage,
								ShowMoreActionDropDownList.Create_Commitments, 10);
						ThreadSleep(2000);
						if (flag) {
							log(LogStatus.INFO, "Clicked on Commitment Creation Button" + SmokeCOM1, YesNo.No);
							...
							if (clickUsingJavaScript(driver, comm.getCommitmentCancelBtn(TopOrBottom.BOTTOM, 10),
									"Commitment Creation Cancel BOTTOM Button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "Clicked on Commitment Creation Cancel BOTTOM Button " + SmokeCOM1,
										YesNo.No);
							} else {
								log(LogStatus.ERROR,
										"Not Able to Click on Commitment Creation Cancel BOTTOM Button" + SmokeCOM1,
										YesNo.Yes);
								sa.assertTrue(false,
										"Not Able to Click on Create Commitment Creation Cancel BOTTOM Button "
												+ SmokeCOM1);
							}

						} else {
							log(LogStatus.ERROR, "Not Able to Click on Create Commitment Button " + SmokeCOM1,
									YesNo.Yes);
							sa.assertTrue(false, "Not Able to Click on Create Commitment Button " + SmokeCOM1);
						}
					} else {
						log(LogStatus.ERROR, "Not Able to Click on Commitment Creation Cancel Top Button" + SmokeCOM1,
								YesNo.Yes);
						sa.assertTrue(false,
								"Not Able to Click on Create Commitment Creation Cancel Top Button " + SmokeCOM1);
					}
				} else {
				...
				sa.assertTrue(false,
						"Not able to clicked on created FR: " + Smoke_FR1 + "so cannot verify Commitments button.");
				log(LogStatus.SKIP,
						"Not able to clicked on created FR: " + Smoke_FR1 + "so cannot verify Commitments button.",
						YesNo.Yes);
			}
			...
			log(LogStatus.SKIP, "Not able to clicked on FundraisingsTab so cannot verify Commitments button.",
					YesNo.Yes);
		}
	...
	public void PESmokeTc022_1_createCommitmentFromHomePageAction(String environment, String mode) {
		...
		String[][] commitmentInformation = { { Smoke_LP1, SmokeCOMM1_CommitmentAmount, Smoke_P1, todayDate },
				{ Smoke_LP2, SmokeCOMM2_CommitmentAmount, Smoke_P1, OneDayeAfterDate } };
		WebElement ele = null;
		...
		for (int i = 1; i <= 2; i++) {
			...
			if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
				...
				log(LogStatus.ERROR, "Not able to click on Create New Commitment link so cannot create commitments",
						YesNo.Yes);
		...
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
			log(LogStatus.INFO, "clicked on Create New Commitment link", YesNo.No);
			if (home.selectFundraisingNameOrCommitmentType(environment, mode, Smoke_FR1, null, null, null,
					CommitmentType.fundraisingName)) {
				if (home.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation,
						SmokeCOMM1_partnerType, SmokeCOMM1_TaxForms, null)) {
					log(LogStatus.INFO, "All commitment information and additional information is passed successfully",
							YesNo.Yes);
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						switchToFrame(driver, 30, home.getCreateCommitmentFrame_Lightning(20));
					...
					if (home.writeTotalAmountInExcelSheet(smokeFilePath, "SmokeFund1", "Funds")) {
						if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
								"create commitment button", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "click on create commitment button", YesNo.No);
							ThreadSleep(2000);
							if (click(driver, home.getCreateCommitmentOkBtn(30), "OK button",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "clicked on OK button", YesNo.No);
								flag = true;
								ExcelUtils.writeData(smokeFilePath, todayDate, "Commitments", excelLabel.Variable_Name,
										"SmokeCOMM1", excelLabel.Final_Commitment_Date);
								ExcelUtils.writeData(smokeFilePath, OneDayeAfterDate, "Commitments",
										excelLabel.Variable_Name, "SmokeCOMM2", excelLabel.Final_Commitment_Date);
								// writing commitment amount in lp sheet
								ExcelUtils.writeData(smokeFilePath, SmokeCOMM1_CommitmentAmount, "Limited Partner",
										excelLabel.Variable_Name, "SmokeLP1", excelLabel.Total_Fund_Commitments);
								ExcelUtils.writeData(smokeFilePath, SmokeCOMM2_CommitmentAmount, "Limited Partner",
										excelLabel.Variable_Name, "SmokeLP2", excelLabel.Total_Fund_Commitments);
							...
							log(LogStatus.ERROR,
									"Not able to click on create commitment button so cannot create commitment",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not able to click on create commitment button so cannot create commitment");
						}
						...
						log(LogStatus.ERROR,
								"Not able to write total commitment amount in fund sheet total commitment amount column in "
										+ Smoke_Fund1,
								YesNo.Yes);
						sa.assertTrue(false,
								"Not able to write total commitment amount in fund sheet total commitment amount column in "
					...
					log(LogStatus.ERROR,
							"All commitment information and additional information is not passed so cannot create commitment",
							YesNo.Yes);
					sa.assertTrue(false,
							"All commitment information and additional information is not passed so cannot create commitment");
				}
				...
				log(LogStatus.ERROR,
						"Not able to select fundraising name from commitment creation pop up so cannot create commitment",
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to select fundraising name from commitment creation pop up so cannot create commitment");
			}
	...
	public void PESmokeTc022_2_createCommitmentFromHomePageImpact(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
						...
						if (parentId != null) {
							...
							if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships,
								...
								if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
									...
									if (!id.isEmpty()) {
										ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
												"SmokeCOMM1", excelLabel.Commitment_ID);
									} else {
										log(LogStatus.ERROR, "Not able to get commitment id of  " + Smoke_LP1,
												YesNo.Yes);
									...
									if (!id.isEmpty()) {
										ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
												"SmokeCOMM2", excelLabel.Commitment_ID);
									} else {
										log(LogStatus.ERROR, "Not able to get commitment id of  " + Smoke_LP2,
												YesNo.Yes);
										sa.assertTrue(false, "Not able to get commitment id of  " + Smoke_LP2);
									}
									settingsBeforeTests();
									if (partnership.verifyPartnerShipRelatedList(environment, mode,
											PageName.PartnershipsPage, SmokeCOMM1_ID, Smoke_LP1, null,
											SmokeCOMM1_CommitmentAmount)) {
										log(LogStatus.INFO,
												"Commitment Related details are verified on partnership page "
														+ Smoke_P1,
												YesNo.No);
									} else {
										log(LogStatus.INFO,
												"Commitment Related details are not verified on partnership page "
														+ Smoke_P1,
												YesNo.Yes);
										sa.assertTrue(false,
												"Commitment Related details are not verified on partnership page "
									...
									if (partnership.verifyPartnerShipRelatedList(environment, mode,
											PageName.PartnershipsPage, SmokeCOMM2_ID, Smoke_LP2, null,
											SmokeCOMM2_CommitmentAmount)) {
										log(LogStatus.INFO,
												"Commitment Related details are verified on partnership page "
														+ Smoke_P1,
												YesNo.No);
				...
				log(LogStatus.ERROR, "Not able to click on cerate fund " + Smoke_Fund1
						+ " so cannot click on partnership " + Smoke_P1, YesNo.No);
				sa.assertTrue(false, "Not able to click on cerate fund " + Smoke_Fund1
						+ " so cannot click on partnership " + Smoke_P1);
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on fund tab so cannot verify Partnership and Commitments",
					YesNo.Yes);
		...
		for (int i = 0; i < commID.length; i++) {
			if (comm.clickOnTab(environment, mode, TabName.CommitmentsTab)) {
				if (comm.clickOnCreatedCommitmentId(environment, mode, commID[i])) {
					...
					// need to fix...
					String labelName = excelLabel.Commitment_ID + "," + excelLabel.Limited_Partner + ","
							+ excelLabel.Commitment_Amount;
					String labelValue = "";
					if (i == 0) {
					...
					log(LogStatus.ERROR,
							"Not able to click on create commitment id " + commID[i] + " so cannot verify data",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on create commitment id " + commID[i] + " so cannot verify data");
				}
				...
				log(LogStatus.ERROR,
						"Not able to click on commitment tab so cannot verify data on commit id " + commID[i],
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on commitment tab so cannot verify data on commit id " + commID[i]);
			}
		...
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedInstitution(environment, mode, SmokeINS1)) {
				String[][] labelAndvalue = { { excelLabel.Total_CoInvestment_Commitments.toString(), "0" },
						{ excelLabel.Total_Fund_Commitments.toString(),
								String.valueOf((Integer.parseInt(SmokeCOMM1_CommitmentAmount)
										+ Integer.parseInt(SmokeCOMM2_CommitmentAmount))) } };

				for (String[] strings : labelAndvalue) {
					if (ins.fieldValueVerificationOnInstitutionPage(environment, mode, TabName.InstituitonsTab,
							strings[0], strings[1])) {
				...
				String[][] commitmentRowRecord1 = {
						{ Smoke_Fund1, SmokeCOMM1_CommitmentAmount, Smoke_LP1, todayDate, "" },
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Institution, RelatedList.Commitments,
					...
					if (ins.verifyCommitmentDetails(environment, mode, commitmentRowRecord1, Smoke_Fund1, "")
							.isEmpty()) {
						log(LogStatus.INFO, "Commitment details are verified on institution page " + SmokeINS1,
								YesNo.No);
					} else {
						log(LogStatus.INFO, "Commitment details are not verified on institution page " + SmokeINS1,
								YesNo.No);
						sa.assertTrue(false, "Commitment details are not verified on institution page " + SmokeINS1);
					}
				} else {

					log(LogStatus.INFO, "Not able to click on fundraising tab on institution page" + SmokeINS1,
							YesNo.No);
					sa.assertTrue(false, "Not able to click on fundraising tab on institution page" + SmokeINS1);
				}

				if (clickUsingJavaScript(driver,
						FindElement(driver, "//a[text()='" + Smoke_LP1 + "']", Smoke_LP1 + " Link", action.BOOLEAN, 30),
						Smoke_LP1 + " Link", action.SCROLLANDBOOLEAN)) {
					String parentWin = switchOnWindow(driver);
					String[][] labelAndvalue1 = { { excelLabel.Record_Type.toString(), "Limited Partner" },
							{ excelLabel.Total_CoInvestment_Commitments.toString(),
									SmokeLP1_Total_CoInvesment_Commitments },
							{ excelLabel.Total_Fund_Commitments.toString(), SmokeLP1_Total_Fund_Commitments } };
					ThreadSleep(5000);
					for (String[] strings : labelAndvalue1) {
						if (ins.fieldValueVerificationOnInstitutionPage(environment, mode, TabName.LimitedPartne,
								strings[0], strings[1])) {
					...
					if (partnership.clickOnRelatedList(environment, mode, RecordType.Institution,
						...
						if (ins.verifyCommitmentDetailsLP(environment, mode, Smoke_Fund1, SmokeCOMM1_CommitmentAmount,
								todayDate).isEmpty()) {
							log(LogStatus.INFO,
									"Commitment Related details are verified on Limited Partner page " + Smoke_LP1,
									YesNo.No);
						} else {
							log(LogStatus.INFO,
									"Commitment Related details are not verified on Limited Partner page " + Smoke_LP1,
									YesNo.Yes);
							sa.assertTrue(false,
									"Commitment Related details are not verified on Limited Partner page " + Smoke_LP1);
						}
						...
						if (ins.clickonDetails(environment, mode, Smoke_Fund1, SmokeCOMM1_CommitmentAmount)) {
							log(LogStatus.ERROR,
									"Click on Details against " + Smoke_Fund1 + " " + SmokeCOMM1_CommitmentAmount,
									YesNo.Yes);

							String labelName = excelLabel.Commitment_ID + "," + excelLabel.Limited_Partner + ","
									+ excelLabel.Commitment_Amount + "," + excelLabel.Partnership;
							String labelValue = "";
							labelValue = SmokeCOMM1_ID + "," + Smoke_LP1 + "," + SmokeCOMM1_CommitmentAmount + ","
									+ Smoke_P1;
							String[] labels = labelName.split(",");
							String[] values = labelValue.split(",");
							for (int j = 0; j < values.length; j++) {
								if (comm.FieldValueVerificationOnCommitmentPage(environment, mode, labels[j],
										values[j])) {
									log(LogStatus.INFO, labels[j] + " : With value verified for : " + values[j],
											YesNo.No);
								} else {
									log(LogStatus.INFO, labels[j] + " : With value not verified for : " + values[j],
											YesNo.Yes);
									sa.assertTrue(false, labels[j] + " : With value not verified for : " + values[j]);
								}
							}
							...
							log(LogStatus.ERROR, "Not able to click on Details against " + Smoke_Fund1 + " "
									+ SmokeCOMM1_CommitmentAmount, YesNo.Yes);
							sa.assertTrue(false, "Not able to click on Details against " + Smoke_Fund1 + " "
									+ SmokeCOMM1_CommitmentAmount);
						}

					} else {
						log(LogStatus.ERROR, "Not able to click on related tab so cannot verify Commitments",
								YesNo.Yes);
				...
				ThreadSleep(2000);
				if (clickUsingJavaScript(driver,
						FindElement(driver, "//a[text()='" + Smoke_LP2 + "']", Smoke_LP2 + " Link", action.BOOLEAN, 30),
						Smoke_LP2 + " Link", action.SCROLLANDBOOLEAN)) {
					String parentWin = switchOnWindow(driver);
					String[][] labelAndvalue1 = { { excelLabel.Record_Type.toString(), "Limited Partner" },
							{ excelLabel.Total_CoInvestment_Commitments.toString(),
									SmokeLP2_Total_CoInvesment_Commitments },
					...
					if (partnership.clickOnRelatedList(environment, mode, RecordType.Institution,
						...
						if (ins.verifyCommitmentDetailsLP(environment, mode, Smoke_Fund1, SmokeCOMM2_CommitmentAmount,
								OneDayeAfterDate).isEmpty()) {
							log(LogStatus.INFO,
									"Commitment Related details are verified on Limited Partner page " + Smoke_LP2,
									YesNo.No);
						} else {
							log(LogStatus.INFO,
									"Commitment Related details are not verified on Limited Partner page " + Smoke_LP2,
									YesNo.Yes);
							sa.assertTrue(false,
									"Commitment Related details are not verified on Limited Partner page " + Smoke_LP2);
						}
						...
						if (ins.clickonDetails(environment, mode, Smoke_Fund1, SmokeCOMM2_CommitmentAmount)) {
							log(LogStatus.ERROR,
									"Click on Details against " + Smoke_Fund1 + " " + SmokeCOMM2_CommitmentAmount,
									YesNo.Yes);
							...
							log(LogStatus.ERROR, "Not able to click on Details against " + Smoke_Fund1 + " "
									+ SmokeCOMM2_CommitmentAmount, YesNo.Yes);
							sa.assertTrue(false, "Not able to click on Details against " + Smoke_Fund1 + " "
									+ SmokeCOMM2_CommitmentAmount);
						}
			...
			if (nspbl.clickOnNavatarSetupSideMenusTab(environment, NavatarSetupSideMenuTab.CommitmentCreation)) {
				...
				if (click(driver,
					...
					checkBox = nspbl.getEnableCheckBoxforNavatarSetUpSideMenuTab(environment, mode,
							...
							ClickOrCheckEnableDisableCheckBox.EnableOrDisable, 10);
					...
					////////////////
					if (nspbl.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
							...
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation FundRaising Information labels are verified ",
								YesNo.No);
					} else {
						log(LogStatus.FAIL, "Commitment Creation FundRaising Information labels are not verified ",
								YesNo.Yes);
					...
					if (nspbl
							.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_GeneralInformation, labels1)
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation Genral Information labels are verified ", YesNo.No);
					} else {
						log(LogStatus.FAIL, "Commitment Creation Genral Information labels are not verified ",
								YesNo.Yes);
						sa.assertTrue(false, "Commitment Creation Genral Information labels are not verified ");
					}

					String[] labels2 = { "Limited Partner", "Commitment Amount", "Partnership",
							"Final Commitment Date" };
					if (nspbl.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
							NavatarSetupSideMenuTab.CommitmentCreation,
							NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_CommitmentInformation, labels2)
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation Commitment Information labels are verified ",
								YesNo.No);
					} else {
						log(LogStatus.FAIL, "Commitment Creation Commitment Information labels are not verified ",
								YesNo.Yes);
					...
					if (nspbl.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
							NavatarSetupSideMenuTab.CommitmentCreation,
							NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_AdditionalInformation, labels3)
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation Additional Information labels are verified ",
								YesNo.No);
					} else {
						log(LogStatus.FAIL, "Commitment Creation Additional Information labels are not verified ",
								YesNo.Yes);
					...
					if (nspbl.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
							NavatarSetupSideMenuTab.CommitmentCreation,
							NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewLimitedPartner, labels4)
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation Fields For New Limited Partner labels are verified ",
								YesNo.No);
					} else {
						log(LogStatus.FAIL,
								"Commitment Creation Fields For New Limited Partner labels are not verified ",
								YesNo.Yes);
						sa.assertTrue(false,
								"Commitment Creation Fields For New Limited Partner labels are not verified ");
					}
					...
					if (nspbl.verifyLabelInEditModeforNavatarSetUpSideMenuTab(environment, mode,
							NavatarSetupSideMenuTab.CommitmentCreation,
							NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewPartnership, labels5)
							.isEmpty()) {
						log(LogStatus.PASS, "Commitment Creation Fields For New Partnership labels are verified ",
								YesNo.No);
					} else {
						log(LogStatus.FAIL, "Commitment Creation Fields For New Partnership labels are not verified ",
								YesNo.Yes);
						sa.assertTrue(false, "Commitment Creation Fields For New Partnership labels are not verified ");
					}
					if (click(driver,
							nspbl.getCancelButtonforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation, 30),
							"commitment creation cancel button", action.SCROLLANDBOOLEAN)) {
						...
						if (click(driver,
								nspbl.getEditButtonforNavatarSetUpSideMenuTab(environment,
										NavatarSetupSideMenuTab.CommitmentCreation, 60),
								"Edit Button on Commitment Creation Tab", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "Clicked on commitment Creation Edit Button", YesNo.No);
							ThreadSleep(10000);
							if (nspbl.clickOnRequiredFieldListforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FundRaisingInformation)) {
								...
								ThreadSleep(5000);
								tcsa = nspbl.verifyingFundRaisingLayoutRequiredFieldListInformationCommitmentTab(
										environment, mode, "Fundraising Layout", null, fundRaisingRowValues);
							...
							if (nspbl.clickOnRequiredFieldListforNavatarSetUpSideMenuTab(environment, mode,
								...
								String[][] AdditionalsRowValues = {
										{ "Limited Partner", "navpeII__Limited_Partner__c", "reference" },
										{ "Partnership", "navpeII__Partnership__c", "reference" } };
								ThreadSleep(5000);
								tcsa = nspbl.verifyingAdditionalLayoutRequiredFieldListInformationCommitmentTab(
										environment, mode, "Commitment Layout", null, AdditionalsRowValues);
								sa.combineAssertions(tcsa);
							...
							if (nspbl.clickOnRequiredFieldListforNavatarSetUpSideMenuTab(environment, mode,
								...
								ThreadSleep(5000);
								List<WebElement> options = allOptionsInDropDrop(driver,
										nspbl.getNewLP_CommitmentTab_DropDownList(environment, mode, 10),
										"limited partner drop down list");
								if (compareMultipleList(driver,
										"Company,Fund Manager,Fund Manager�s Fund,Individual Investor,Institution,Limited Partner",
										options).isEmpty()) {
								...
								ThreadSleep(5000);
								tcsa = nspbl.verifyingNewLPLayoutRequiredFieldListInformationCommitmentTab(environment,
										mode, "Company", null, insRowValues);
								sa.combineAssertions(tcsa);

							} else {
								log(LogStatus.SKIP, "Not able to click on Limited Partner required field link",
										YesNo.Yes);
							...
							if (nspbl.clickOnRequiredFieldListforNavatarSetUpSideMenuTab(environment, mode,
								...
								String[][] insRowValues = { { "Partnership Legal Name", "Name", "string" },
										{ "Fund", "navpeII__Fund__c", "reference" } };
								ThreadSleep(5000);
								tcsa = nspbl.verifyingNewPartnerShipLayoutRequiredFieldListInformationCommitmentTab(
										environment, mode, "Partnership Layout", null, insRowValues);
								sa.combineAssertions(tcsa);
							...
							ThreadSleep(3000);
							// Azhar Start
							NavatarSetupSideMenuTabLayoutSection[] nvsp = {
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FundRaisingInformation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_AdditionalInformation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewLimitedPartner,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewPartnership };
							for (int i = 0; i < nvsp.length; i++) {
								if (nspbl.clickOnAddIconAndThenRemoveIconOnNavatarSetupSideMenuTabSections(environment,
										mode, NavatarSetupSideMenuTab.CommitmentCreation, nvsp[i])) {
							...
							// Azhar End
							if (nspbl.addingMoreSelectFieldAndValuesToNavatarSetupSideMenuTabSections(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FundRaisingInformation,
									"Investment Likely Amount (mn)", null)) {
								log(LogStatus.INFO,
										"Investment Likely Amount (mn) value is selected in fundraising information section ",
										YesNo.No);
							} else {
								log(LogStatus.INFO,
										"Not able to select Investment Likely Amount (mn) value in fundraising information section ",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to select Investment Likely Amount (mn) value in fundraising information section ");
							}
							...
							if (nspbl.addingMoreSelectFieldAndValuesToNavatarSetupSideMenuTabSections(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_AdditionalInformation, null,
									"Placement Fee %")) {
								log(LogStatus.INFO,
										"Placement Fee % value is selected in Additional information section ",
										YesNo.No);
							} else {
								log(LogStatus.INFO,
										"Not able to select Placement Fee % value in Additional information section ",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to select Placement Fee % value in Additional information section ");
							}
							...
							if (nspbl.addingMoreSelectFieldAndValuesToNavatarSetupSideMenuTabSections(environment, mode,
									...
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewLimitedPartner,
									"Bank Name", null)) {
								log(LogStatus.INFO, "Bank Name value is selected in FieldsForNewLimitedPartner",
										YesNo.No);
							} else {
								log(LogStatus.INFO,
										"Not able to select Bank Name value in FieldsForNewLimitedPartner section ",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to select Bank Name value in FieldsForNewLimitedPartner section ");
							}
							...
							if (nspbl.addingMoreSelectFieldAndValuesToNavatarSetupSideMenuTabSections(environment, mode,
									NavatarSetupSideMenuTab.CommitmentCreation,
									NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FieldsForNewPartnership,
									"Fund Investment Category", null)) {
								log(LogStatus.INFO,
										"Fund Investment Category value is selected in FieldsForNewPartnership",
										YesNo.No);
							} else {
								log(LogStatus.INFO,
										"Not able to select Fund Investment Category value in FieldsForNewPartnership section ",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to select Fund Investment Category value in FieldsForNewPartnership section ");
							}
							if (click(driver,
									nspbl.getSaveButtonforNavatarSetUpSideMenuTab(environment,
											NavatarSetupSideMenuTab.CommitmentCreation, 30, TopOrBottom.TOP),
									"save button", action.SCROLLANDBOOLEAN)) {
	...
	public void PESmokeTc024_1_createCommitmentFromFundraisingPageAction(String environment, String mode) {
		...
		String[][] commitmentInformation = { { Smoke_LP1, SmokeCOMM3_CommitmentAmount, Smoke_P1, ThreeDayeAfterDate }, {
				Smoke_LP3 + "<break>" + CreatedOrNot.NotCreated + "<break>" + excelLabel.Bank_Name.toString() + ":"
						+ SmokeLP3_BankName,
				SmokeCOMM4_CommitmentAmount,
				Smoke_P2 + "<break>" + CreatedOrNot.NotCreated + "<break>"
						+ excelLabel.Fund_Investment_Category.toString() + ":" + SmokeP2_Fund_Investment_Category,
				FourDayeAfterDate } };
		if (frsp.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
			if (frsp.clickOnCreatedFundRaising(environment, mode, Smoke_FR1)) {
				...
				if (frsp.clickOnShowMoreActionDownArrow(environment, PageName.FundraisingPage,
						ShowMoreActionDropDownList.Create_Commitments, 20)) {
					log(LogStatus.INFO, "clicked on create commitment button", YesNo.No);
					if (home.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation, null, null,
							SmokeCoMM3_PlacementFee)) {
						log(LogStatus.INFO,
								"All commitment information and additional information is passed successfully",
								YesNo.Yes);
						if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
							switchToFrame(driver, 30, home.getCreateCommitmentFrame_Lightning(20));
						...
						if (home.writeTotalAmountInExcelSheet(smokeFilePath, "SmokeFund1", "Funds")) {
							if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
								...
								if (click(driver, home.getCreateCommitmentOkBtn(30), "OK button",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "clicked on OK button", YesNo.No);
									ExcelUtils.writeData(smokeFilePath, ThreeDayeAfterDate, "Commitments",
											excelLabel.Variable_Name, "SmokeCOMM3", excelLabel.Final_Commitment_Date);
									ExcelUtils.writeData(smokeFilePath, FourDayeAfterDate, "Commitments",
											excelLabel.Variable_Name, "SmokeCOMM4", excelLabel.Final_Commitment_Date);
								} else {
									log(LogStatus.ERROR, "Not able to click on OK button", YesNo.Yes);
									sa.assertTrue(false, "Not able to click on OK button");
								}
					...
					log(LogStatus.ERROR,
							"Not able to click on create commitment button so cannot create commitment from fundraising page",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to click on create commitment button so cannot create commitment from fundraising page");
				}
				...
				log(LogStatus.INFO, "Not able to click on created FR " + Smoke_FR1
						+ " so cannot create commitments from fundraising page", YesNo.Yes);
				sa.assertTrue(false, "Not able to click on created FR " + Smoke_FR1
						+ " so cannot create commitments from fundraising page");
			}

		} else {
			log(LogStatus.ERROR,
					"Not able to click on fundraising tab so cannot create commitments from fundraising tab",
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on fundraising tab so cannot create commitments from fundraising tab");
		}
	...
	public void PESmokeTc024_2_createCommitmentFromFundraisingPageImpact(String environment, String mode) {
						...
						if (parentId != null) {
							...
							if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships,
								...
								if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
									...
									if (!id.isEmpty()) {
										ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
												"SmokeCOMM3", excelLabel.Commitment_ID);
									} else {
										log(LogStatus.ERROR, "Not able to get commitment id of  " + Smoke_LP1,
												YesNo.Yes);
										sa.assertTrue(false, "Not able to get commitment id of  " + Smoke_LP1);
									...
									settingsBeforeTests();
									if (partnership.verifyPartnerShipRelatedList(environment, mode,
											PageName.PartnershipsPage, SmokeCOMM3_ID, Smoke_LP1, null,
											SmokeCOMM3_CommitmentAmount)) {
										log(LogStatus.INFO,
												"Commitment Related details are verified on partnership page "
														+ Smoke_P1,
												YesNo.No);
			...
			if (partnership.clickOnCreatedPartnership(environment, mode, Smoke_P2)) {
				if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships, RelatedList.Commitments,
					...
					if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
						...
						if (!id.isEmpty()) {
							ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
									"SmokeCOMM4", excelLabel.Commitment_ID);
						} else {
						...
						settingsBeforeTests();
						if (partnership.verifyPartnerShipRelatedList(environment, mode, PageName.PartnershipsPage,
								SmokeCOMM4_ID, Smoke_LP3, null, SmokeCOMM4_CommitmentAmount)) {
							log(LogStatus.INFO,
									"Commitment Related details are verified on partnership page " + Smoke_P2,
									YesNo.No);
						} else {
							log(LogStatus.INFO,
									"Commitment Related details are not verified on partnership page " + Smoke_P2,
									YesNo.Yes);
							sa.assertTrue(false,
									"Commitment Related details are not verified on partnership page " + Smoke_P2);
						}
						...
						log(LogStatus.ERROR,
								"Not able to click on commitment view all link so cannot get commitment id of LP "
										+ Smoke_LP2,
								YesNo.Yes);
						sa.assertTrue(false,
								"Not able to click on commitment view all link so cannot get commitment id of LP "
					...
					log(LogStatus.ERROR, "Not able to click on related list " + RelatedList.Partnerships.toString(),
							YesNo.Yes);
					sa.assertTrue(false, "Not able to click on related list " + RelatedList.Partnerships.toString());
				}
			} else {
				log(LogStatus.ERROR, "Not able to click on created partnership " + Smoke_P2
						+ " so cannot get commitment id of LP " + Smoke_LP2, YesNo.Yes);
				sa.assertTrue(false, "Not able to click on created partnership " + Smoke_P2
						+ " so cannot get commitment id of LP " + Smoke_LP2);
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on partnership Tab so cannot get commitment id of LP " + Smoke_LP2,
					YesNo.Yes);
		...
		if (comm.clickOnTab(environment, mode, TabName.CommitmentsTab)) {
			if (comm.clickOnCreatedCommitmentId(environment, mode, SmokeCOMM4_ID)) {
				...
				// need to fix...
				String labelName = excelLabel.Commitment_ID + "," + excelLabel.Limited_Partner + ","
						+ excelLabel.Commitment_Amount + "," + excelLabel.Placement_Fee;

				String labelValue = SmokeCOMM4_ID + "," + Smoke_LP3 + "," + SmokeCOMM4_CommitmentAmount + ","
						+ SmokeCOMM4_PlacementFee;
				String[] labels = labelName.split(",");
				String[] values = labelValue.split(",");
				for (int j = 0; j < values.length; j++) {
					if (comm.FieldValueVerificationOnCommitmentPage(environment, mode, labels[j], values[j])) {
				...
				log(LogStatus.ERROR,
						"Not able to click on create commitment id " + SmokeCOMM4_ID + " so cannot verify data",
						YesNo.Yes);
			}
		} else {
			log(LogStatus.ERROR,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM4_ID,
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM4_ID);
		}
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedInstitution(environment, mode, SmokeINS1)) {
				String[][] commitmentRowRecord = {
						{ Smoke_Fund1, SmokeCOMM1_CommitmentAmount, Smoke_LP1, Smoke_P1, "", todayDate },
						{ Smoke_Fund1, SmokeCOMM2_CommitmentAmount, Smoke_LP2, Smoke_P1, "", todayDate },
						{ Smoke_Fund1, SmokeCOMM3_CommitmentAmount, Smoke_LP1, Smoke_P1, "", todayDate },
				...
				if (ins.clickOnRelatedList(environment, mode, RecordType.Institution, RelatedList.Commitments,
					...
					log(LogStatus.INFO, "Not abel to click on related tab fundraising on institution page" + SmokeINS1,
							YesNo.No);
					sa.assertTrue(false,
							"Not abel to click on related tab fundraising on institution page" + SmokeINS1);
				}
		...
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedLP(environment, mode, Smoke_LP3)) {
				if (ins.fieldValueVerificationOnInstitutionPage(environment, mode, TabName.InstituitonsTab,
						excelLabel.Bank_Name.toString(), SmokeLP3_BankName)) {
	...
	public void PESmokeTc025_1_createCommitmentFromHomePageUsingFundButtonAction(String environment, String mode) {
		...
		String[][] commitmentInformation = { {
				Smoke_LP4 + "<break>" + CreatedOrNot.NotCreated + "<break>" + excelLabel.Bank_Name.toString(),
				SmokeCOMM5_CommitmentAmount, Smoke_P2 + "<break>" + CreatedOrNot.AlreadyCreated, SevenDayeAfterDate } };

		String bulkActionNavigationLink = BulkActions_DefaultValues.Bulk_Commitments.toString();
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
			log(LogStatus.INFO, "clicked on Create New Commitment link", YesNo.No);
			if (home.selectFundraisingNameOrCommitmentType(environment, mode, null, Smoke_Fund1, null, SmokeINDINV1,
					CommitmentType.fund)) {
				if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					switchToFrame(driver, 30, home.getCreateCommitmentFrame_Lightning(20));
				}
				ThreadSleep(5000);
				if (getText(driver, home.getCompanyNameLabelTextOnCreateCommitment(60), "company name",
						action.SCROLLANDBOOLEAN).isEmpty()) {
				...
				if (sendKeys(driver, ele, Smoke_LP4 + "1", "LP text box : " + Smoke_LP4, action.SCROLLANDBOOLEAN)) {
					...
					if (click(driver, home.getNewLimitedPartnerCrossIconInCreateCommitment(20), "", action.BOOLEAN)) {
					...
					log(LogStatus.ERROR,
							"Not able to pass value in text box " + Smoke_LP4 + " so cannot Check Cross/Cancel",
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to pass value in text box " + Smoke_LP4 + " so cannot Check Cross/Cancel");
				}
					...
					if (home.writeTotalAmountInExcelSheet(smokeFilePath, "SmokeFund1", "Funds")) {
						if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
							...
							if (click(driver, home.getCreateCommitmentOkBtn(30), "OK button",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "clicked on OK button", YesNo.No);
								ExcelUtils.writeData(smokeFilePath, SevenDayeAfterDate, "Commitments",
										excelLabel.Variable_Name, "SmokeCOMM5", excelLabel.Final_Commitment_Date);
							} else {
								log(LogStatus.ERROR, "Not able to click on OK button", YesNo.Yes);
								sa.assertTrue(false, "Not able to click on OK button");
							}
				...
				log(LogStatus.ERROR,
						"Not able to click on commitment creation continue button so cannot create commitment",
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on commitment creation continue button so cannot create commitment");
			}
	...
	public void PESmokeTc025_1_createCommitmentFromHomePageUsingFundButtonImpact(String environment, String mode) {
			...
			if (partnership.clickOnCreatedPartnership(environment, mode, Smoke_P2)) {
				if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships, RelatedList.Commitments,
					...
					if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
						...
						if (!id.isEmpty()) {
							ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
									"SmokeCOMM5", excelLabel.Commitment_ID);
						} else {
						...
						settingsBeforeTests();
						if (partnership.verifyPartnerShipRelatedList(environment, mode, PageName.PartnershipsPage,
								SmokeCOMM5_ID, Smoke_LP4, null, SmokeCOMM5_CommitmentAmount)) {
							log(LogStatus.INFO,
									"Commitment Related details are verified on partnership page " + Smoke_P2,
									YesNo.No);
						} else {
		...
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedLP(environment, mode, Smoke_LP4)) {
				String[][] labelAndvalue = { { excelLabel.Bank_Name.toString(), "" },
						{ excelLabel.Total_CoInvestment_Commitments.toString(),
								SmokeLP4_Total_CoInvesment_Commitments },
				...
				if (partnership.clickOnRelatedList(environment, mode, RecordType.Institution, RelatedList.Commitments,
					...
					if (ins.verifyCommitmentDetailsLP(environment, mode, Smoke_Fund1, SmokeCOMM5_CommitmentAmount,
							SevenDayeAfterDate).isEmpty()) {
						log(LogStatus.INFO,
								"Commitment Related details are verified on Limited Partner page " + Smoke_LP4,
								YesNo.No);
					} else {
						log(LogStatus.INFO,
								"Commitment Related details are not verified on Limited Partner page " + Smoke_LP4,
								YesNo.Yes);
						sa.assertTrue(false,
								"Commitment Related details are not verified on Limited Partner page " + Smoke_LP4);
					}
					...
					if (ins.clickonDetails(environment, mode, Smoke_Fund1, SmokeCOMM5_CommitmentAmount)) {
						log(LogStatus.ERROR,
								"Click on Details against " + Smoke_Fund1 + " " + SmokeCOMM5_CommitmentAmount,
								YesNo.Yes);

						String labelName = excelLabel.Commitment_ID + "," + excelLabel.Limited_Partner + ","
								+ excelLabel.Commitment_Amount + "," + excelLabel.Partnership;
						String labelValue = "";
						labelValue = SmokeCOMM5_ID + "," + Smoke_LP4 + "," + SmokeCOMM5_CommitmentAmount + ","
								+ Smoke_P2;
						String[] labels = labelName.split(",");
						String[] values = labelValue.split(",");
						for (int j = 0; j < values.length; j++) {
						...
						log(LogStatus.ERROR, "Not able to click on Details against " + Smoke_Fund1 + " "
								+ SmokeCOMM5_CommitmentAmount, YesNo.Yes);
						sa.assertTrue(false, "Not able to click on Details against " + Smoke_Fund1 + " "
								+ SmokeCOMM5_CommitmentAmount);
					}
		...
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedInstitution(environment, mode, SmokeINDINV1)) {
				...
				for (String[] strings : labelAndvalue) {
					if (ins.fieldValueVerificationOnInstitutionPage(environment, mode, TabName.LimitedPartne,
						...
						log(LogStatus.INFO, strings[0] + " is not verified on Individual Investor :" + Smoke_LP3,
								YesNo.No);
			...
			log(LogStatus.ERROR, "Not able to click on " + TabName.InstituitonsTab
					+ " so cannot verify Individual Investor " + SmokeINDINV1 + "details", YesNo.No);
			sa.assertTrue(false, "Not able to click on " + TabName.InstituitonsTab
					+ " so cannot verify Individual Investor " + SmokeINDINV1 + "details");
		}
		switchToDefaultContent(driver);
		lp.CRMlogout(environment, mode);
		sa.assertAll();
	...
	public void PESmokeTc025_2_createCommitmentFromHomePageUsingFundButton(String environment, String mode) {
		...
		String OneDayeAfterDate = previousOrForwardDate(1, "M/d/yyyy");
		String SevenDayeAfterDate = previousOrForwardDate(7, "M/d/yyyy");
		String[][] commitmentInformation = { { Smoke_LP4 + "<break>" + CreatedOrNot.AlreadyCreated,
				SmokeCOMM5_CommitmentAmount, Smoke_P2 + "<break>" + CreatedOrNot.NotCreated, SevenDayeAfterDate } };
			...
			if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
				...
				if (i == 1) {
					...
					if (click(driver, home.getFundTypeCommitment(20), "fund type text", action.SCROLLANDBOOLEAN)) {
						...
						List<String> returnlist = compareMultipleList(driver, "*Fund Name,*Legal Name", eleList);
						...
						if (click(driver, home.getFundTypeCommitmentCreationCancelBtn(10), "", action.BOOLEAN)) {
							...
							log(LogStatus.ERROR, "Not Able to click on fund type commitment Creation Cancel Btn",
									YesNo.Yes);
					...
					if (click(driver, home.getCoInvesmentTypeCommitment(20), "co investment type text",
							action.SCROLLANDBOOLEAN)) {
						...
						xpath = "//label[contains(text(),'Fund Name')]/../following-sibling::td/table//div/span[text()='"
								+ Smoke_Fund3 + "']";
						...
						if (click(driver, home.getSelectCompanyNameWarningPopUpLookUpIcon(20),
						...
						boolean flag = false;
						if (click(driver, home.getLegalNameLookUpIcon(PageName.CreateCommitmentCoInvestmentType, 20),
							...
							if (home.selectValueFromLookUpWindow(SmokeINDINV1)) {
								...
								for (int j = 0; j < ele1.size(); j++) {
									...
									if (element != null) {
										if (click(driver, element, "continue button", action.SCROLLANDBOOLEAN)) {
											...
											if (j == ele1.size() - 1) {
												log(LogStatus.ERROR,
														"Not able to click on create commitment pop up continue button",
														YesNo.Yes);
												sa.assertTrue(false,
														"Not able to click on create commitment pop up continue button");
								...
								if (flag) {
									...
									xpath = "//span[@id='mypage:CommitmentCreationFormId:GeneralDivId']//label[text()='Fund Name']/../following-sibling::td/span[text()='"
											+ Smoke_Fund3 + "']";
									ele = FindElement(driver, xpath, "Fund Label Text Value", action.BOOLEAN, 10);

									if (ele != null) {
										log(LogStatus.INFO, "Fund Name Label With Value Verified: " + Smoke_Fund3,
												YesNo.Yes);
									} else {
										log(LogStatus.ERROR, "Fund Name Label With Value Not Verified : " + Smoke_Fund3,
												YesNo.Yes);
										sa.assertTrue(false,
												"Fund Name Label With Value Not Verified : " + Smoke_Fund3);
									}

									xpath = "//span[@id='mypage:CommitmentCreationFormId:GeneralDivId']//label[text()='Legal Name']/../following-sibling::td/span[text()='"
											+ SmokeINDINV1 + "']";
									ele = FindElement(driver, xpath, "Legal Label Text Value", action.BOOLEAN, 10);

									if (ele != null) {
										log(LogStatus.INFO, "Legal Name Label With Value Verified: " + SmokeINDINV1,
												YesNo.Yes);
									} else {
										log(LogStatus.ERROR,
												"Legal Name Label With Value Not Verified : " + SmokeINDINV1,
												YesNo.Yes);
										sa.assertTrue(false,
												"Legal Name Label With Value Not Verified : " + SmokeINDINV1);
									}
									...
									if (home.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation,
										...
										for (int k = 1; k <= 2; k++) {
											...
											if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
												...
												if (click(driver,
														home.getSelectCompanyNameLookUpIconOnCreateCommitmentPopUp(20),
														"company name look up icon", action.SCROLLANDBOOLEAN)) {
													if (home.selectValueFromLookUpWindow(SmokeCOM2)) {
														log(LogStatus.INFO, "Company name is selected " + SmokeCOM2,
																YesNo.No);

														if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
															switchToFrame(driver, 30,
																	home.getCreateCommitmentFrame_Lightning(20));

														}

														xpath = "(//a[contains(@onclick,'CancelCompnayPopup()')])[" + i
																+ "]";
														ele = FindElement(driver, xpath, "Cross/Cancel : " + i,
																action.BOOLEAN, 10);

														if (click(driver, ele, "Cross/Cancel : " + i, action.BOOLEAN)) {
															log(LogStatus.INFO, "clicked on Cross/Cancel : " + i,
																	YesNo.Yes);
														} else {
															log(LogStatus.ERROR,
																	"Not able to click on Cross/Cancel : " + i,
																	YesNo.Yes);
															sa.assertTrue(false,
																	"Not able to click on Cross/Cancel : " + i);
														}
														...
														log(LogStatus.ERROR,
																"Not able to select Company Name " + SmokeCOM2,
																YesNo.Yes);
														sa.assertTrue(false,
																"Not able to select Company Name " + SmokeCOM2);
													}
									...
									log(LogStatus.ERROR,
											"Not able to click on create commitment pop up continue button", YesNo.Yes);
									sa.assertTrue(false,
											"Not able to click on create commitment pop up continue button");
	...
	public void PESmokeTc026_1_createCommitmentFromFundraisingForCoInvestmentAction(String environment, String mode) {
		...
		String[][] commitmentInformation = {
				{ Smoke_LP4 + "<break>" + CreatedOrNot.AlreadyCreated, SmokeCOMM6_CommitmentAmount,
						Smoke_P3 + "<break>" + CreatedOrNot.NotCreated + "<break>"
								+ excelLabel.Fund_Investment_Category.toString(),
						TenDayAfterDate },
				{ "Test1LPRmove Link<break>" + CreatedOrNot.NotCreated + "<break>" + excelLabel.Bank_Name, "12000000",
						Smoke_P3 + "<break>" + CreatedOrNot.NotCreated + "<break>"
								+ excelLabel.Fund_Investment_Category.toString(),
						ElevnDayAfterDate } };
				...
				if (frsp.clickOnCreatedFundRaising(environment, mode, Smoke_FR6)) {
					...
					if (frsp.clickOnShowMoreActionDownArrow(environment, PageName.FundraisingPage,
						...
						if (i == 1) {
							...
							for (int j = 0; j < linkClick.length; j++) {
								...
								if (ele != null) {
									...
									if (click(driver, ele, linkClick[j], action.SCROLLANDBOOLEAN)) {
										...
										ele = FindElement(driver, xpath, "on same window : " + linkClick[j],
												action.BOOLEAN, 10);
										...
										appLog.error("Not able to click on " + linkClick[j]
												+ " so cannot verify landing page");
										sa.assertTrue(false, "Not able to click on " + linkClick[j]
												+ " so cannot verify landing page");
									}
								} else {
									appLog.error(
											"Not able to click on " + linkClick[j] + " so cannot verify landing page");
									sa.assertTrue(false,
											"Not able to click on " + linkClick[j] + " so cannot verify landing page");
								}
							...
							ThreadSleep(5000);
							String[][] labelAndValue = {
									{ CreateCommitmentPageFieldLabelText.Legal_Name.toString(), SmokeINDINV1 },
							...
							SoftAssert result = home.verifyCreateCommitmentGenralOrFundraisingInfo(environment, mode,
									labelAndValue);
							sa.combineAssertions(result);

							if (home.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation, null,
									null, null)) {
								log(LogStatus.INFO,
										"All commitment information and additional information is passed successfully",
										YesNo.Yes);
								...
								SoftAssert result1 = home
										.deleteRowsFromCreateCommitmentGenralOrFundraisingInfo(environment, mode);
								...
								if (home.writeTotalAmountInExcelSheet(smokeFilePath, "SmokeFund3", "Funds")) {
									if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
										...
										if (click(driver, home.getCreateCommitmentOkBtn(30), "OK button",
												action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, "clicked on OK button", YesNo.No);
											ExcelUtils.writeData(smokeFilePath, TenDayAfterDate, "Commitments",
													excelLabel.Variable_Name, "SmokeCOMM6",
													excelLabel.Final_Commitment_Date);
					...
					log(LogStatus.INFO, "Not able to click on created FR " + Smoke_FR6
							+ " so cannot create commitments from fundraising page", YesNo.Yes);
					sa.assertTrue(false, "Not able to click on created FR " + Smoke_FR6
							+ " so cannot create commitments from fundraising page");
				}
	...
	public void PESmokeTc026_2_createCommitmentFromFundraisingForCoInvestmentImpact(String environment, String mode) {
		...
		if (fund.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund3)) {
						...
						if (parentId != null) {
							...
							if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships,
								...
								if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
									...
									if (!id.isEmpty()) {
										ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
												"SmokeCOMM6", excelLabel.Commitment_ID);
									} else {
										log(LogStatus.ERROR, "Not able to get commitment id of  " + Smoke_LP4,
												YesNo.Yes);
										sa.assertTrue(false, "Not able to get commitment id of  " + Smoke_LP4);
									}
									settingsBeforeTests();
									if (partnership.verifyPartnerShipRelatedList(environment, mode,
											PageName.PartnershipsPage, SmokeCOMM6_ID, Smoke_LP4, SmokeCOM2,
											SmokeCOMM6_CommitmentAmount)) {
										log(LogStatus.INFO,
												"Commitment Related details are verified on partnership page "
														+ Smoke_P1,
												YesNo.No);
				...
				log(LogStatus.ERROR, "Not able to click on cerate fund " + Smoke_Fund3
						+ " so cannot click on partnership " + Smoke_P1, YesNo.No);
				sa.assertTrue(false, "Not able to click on cerate fund " + Smoke_Fund3
						+ " so cannot click on partnership " + Smoke_P1);
			}
		} else {
			log(LogStatus.ERROR, "Not able to click on fund tab so cannot verify Partnership and Commitments",
		...
		if (comm.clickOnTab(environment, mode, TabName.CommitmentsTab)) {
			if (comm.clickOnCreatedCommitmentId(environment, mode, SmokeCOMM6_ID)) {
				...
				String labelName = excelLabel.Commitment_ID + "," + excelLabel.Limited_Partner + ","
						+ excelLabel.Commitment_Amount + "," + excelLabel.Placement_Fee + "," + excelLabel.Tax_Forms;
				...
				log(LogStatus.ERROR,
						"Not able to click on create commitment id " + SmokeCOMM6_ID + " so cannot verify data",
						YesNo.Yes);
			}
		} else {
			log(LogStatus.ERROR,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM6_ID,
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM6_ID);
		}
		...
		if (ins.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ins.clickOnCreatedInstitution(environment, mode, SmokeINDINV1)) {
				...
				if (ins.clickOnRelatedList(environment, mode, RecordType.Institution, RelatedList.Commitments,
					...
					log(LogStatus.INFO, "click on related fundraising tab on insitiution page" + SmokeINDINV1,
							YesNo.No);
					String[][] commitmentRowRecord = {
							{ Smoke_Fund1, SmokeCOMM5_CommitmentAmount, Smoke_LP4, todayDate, "" } };

					if (ins.verifyCommitmentDetails(environment, mode, commitmentRowRecord, Smoke_Fund1,
							SmokeCOMM5_CommitmentAmount).isEmpty()) {
						log(LogStatus.INFO,
								"Commitment details are verified on Individual Investor page " + SmokeINDINV1,
								YesNo.No);
					} else {
						log(LogStatus.INFO,
								"Commitment details are not verified on Individual Investor page " + SmokeINS1,
								YesNo.No);
						sa.assertTrue(false,
								"Commitment details are not verified on Individual Investor page " + SmokeINS1);
					}
					...
					String[][] commitmentRowRecord1 = {
							{ Smoke_Fund3, SmokeCOMM6_CommitmentAmount, Smoke_LP4, todayDate, "" } };

					if (ins.verifyCommitmentDetails(environment, mode, commitmentRowRecord1, Smoke_Fund3,
							SmokeCOMM6_CommitmentAmount).isEmpty()) {
						log(LogStatus.INFO,
								"Commitment details are verified on Individual Investor page " + SmokeINDINV1,
								YesNo.No);
					} else {
					...
					log(LogStatus.ERROR,
							"Not able to click on related fundraising tab on insitiution page" + SmokeINDINV1,
							YesNo.Yes);
		...
		if (frsp.clickOnTab(environment, mode, TabName.FundraisingsTab)) {
			if (frsp.clickOnCreatedFundRaising(environment, mode, Smoke_FR3)) {
				...
				if (frsp.clickOnShowMoreActionDownArrow(environment, PageName.FundraisingPage,
					...
					String errorMesg = HomePageErrorMessage.InsufficientPermissionErrorMsgOnCreateCommitmentPage1
							+ HomePageErrorMessage.InsufficientPermissionErrorMsgOnCreateCommitmentPage2;
					if (home.getInsufficientPermissionErrorMsgOnCreateCommitmentPage(60) != null) {
						...
						if (aa.contains(HomePageErrorMessage.InsufficientPermissionErrorMsgOnCreateCommitmentPage1)
								&& aa.contains(
										HomePageErrorMessage.InsufficientPermissionErrorMsgOnCreateCommitmentPage2)) {
							log(LogStatus.INFO, errorMesg + " : Error Message is verified ", YesNo.No);
						} else {
							log(LogStatus.ERROR, errorMesg + " : Error Message is not verified \t Actual: " + aa,
									YesNo.Yes);
							sa.assertTrue(false, errorMesg + " : Error Message is not verified \t Actual: " + aa);
						}
					} else {
						log(LogStatus.INFO,
								"Error Message popup is not visible so cannot verify error message " + errorMesg,
								YesNo.Yes);
						sa.assertTrue(false,
								"Error Message popup is not visible so cannot verify error message " + errorMesg);
					}
					if (click(driver, home.getInsufficientPermissionOKBtnOnCreateCommitmentPage(20), "ok button",
							action.SCROLLANDBOOLEAN)) {
				...
				log(LogStatus.INFO, "Not able to click on created FR " + Smoke_FR3
						+ " so cannot create commitments from fundraising page", YesNo.Yes);
				sa.assertTrue(false, "Not able to click on created FR " + Smoke_FR3
						+ " so cannot create commitments from fundraising page");
			}
	...
	public void PESmokeTc028_1_createCommitmentFromCompanyPageForCoInvestmentAction(String environment, String mode) {
		...
		String[][] commitmentInformation = {
				{ Smoke_LP5 + "<break>" + CreatedOrNot.NotCreated + "<break>" + excelLabel.Bank_Name,
						SmokeCOMM7_CommitmentAmount, Smoke_P4 + "<break>" + CreatedOrNot.NotCreated + "<break>"
								+ excelLabel.Fund_Investment_Category.toString(),
						FifteenDayAfterDate } };
		String bulkActionNavigationLink = BulkActions_DefaultValues.Bulk_Commitments.toString();
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
			log(LogStatus.INFO, "clicked on Create New Commitment link", YesNo.No);
			ThreadSleep(5000);
			if (home.selectFundraisingNameOrCommitmentType(environment, mode, null, null, SmokeCOM2, SmokeINDINV1,
					CommitmentType.coInvestment)) {
				log(LogStatus.INFO, "successfully selected co-investment and select legal name : " + SmokeINDINV1,
						YesNo.No);
				if (home.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation, null, null, null)) {
					log(LogStatus.INFO, "All commitment information and additional information is passed successfully",
							YesNo.Yes);
					...
					if (home.writeTotalAmountInExcelSheet(smokeFilePath, "SmokeFund3", "Funds")) {
						if (click(driver, home.getCreateCommitmentBtn(20, TopOrBottom.BOTTOM),
							...
							if (click(driver, home.getCreateCommitmentOkBtn(30), "OK button",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "clicked on OK button", YesNo.No);
								ExcelUtils.writeData(smokeFilePath, FifteenDayAfterDate, "Commitments",
										excelLabel.Variable_Name, "SmokeCOMM7", excelLabel.Final_Commitment_Date);
							} else {
								log(LogStatus.ERROR, "Not able to click on OK button", YesNo.Yes);
								sa.assertTrue(false, "Not able to click on OK button");
							}
				...
				log(LogStatus.ERROR, "Not able to click on co-investment option and select legal name " + SmokeINDINV1,
						YesNo.Yes);
	...
	public void PESmokeTc028_2_createCommitmentFromCompanyPageForCoInvestmentImpact(String environment, String mode) {
			...
			if (fund.clickOnCreatedFund(environment, mode, Smoke_Fund3)) {
				...
				if (fund.clickOnRelatedList(environment, mode, RecordType.Fund, RelatedList.Partnerships,
					...
					if (fund.clickOncreatedPartnershipFromRelatedList(environment, mode, Smoke_P4)) {
						...
						String parentId = switchOnWindow(driver);
						if (parentId != null) {
							log(LogStatus.ERROR, "New window is open after click on partnership " + Smoke_P4,
									YesNo.Yes);
							if (partnership.clickOnRelatedList(environment, mode, RecordType.Partnerships,
									RelatedList.Commitments, null)) {
								log(LogStatus.INFO, "clicked on related list " + RelatedList.Partnerships.toString(),
								...
								if (partnership.clickOnViewAllRelatedList(environment, mode, RelatedList.Commitments)) {
									...
									if (!id.isEmpty()) {
										ExcelUtils.writeData(smokeFilePath, id, "Commitments", excelLabel.Variable_Name,
												"SmokeCOMM7", excelLabel.Commitment_ID);
									} else {
										log(LogStatus.ERROR, "Not able to get commitment id of  " + Smoke_LP5,
												YesNo.Yes);
		...
		if (comm.clickOnTab(environment, mode, TabName.CommitmentsTab)) {
			if (comm.clickOnCreatedCommitmentId(environment, mode, SmokeCOMM7_ID)) {
				...
				log(LogStatus.ERROR,
						"Not able to click on create commitment id " + SmokeCOMM7_ID + " so cannot verify data",
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on create commitment id " + SmokeCOMM7_ID + " so cannot verify data");
			...
			log(LogStatus.ERROR,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM7_ID,
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on commitment tab so cannot verify data on commit id " + SmokeCOMM7_ID);
		}
					...
					for (int i = 0; i < topOrBottom.length; i++) {
						...
						if (click(driver,
								nsp.getRevertToDefaultButtonforNavatarSetUpSideMenuTab(environment, mode,
										NavatarSetupSideMenuTab.CommitmentCreation, topOrBottom[i], 10),
							...
							if (i == 0) {
								if (click(driver,
										nsp.getWarningPopUpNoButton(environment, 10,
												NavatarSetupSideMenuTab.CommitmentCreation),
										"No Button", action.BOOLEAN)) {
								...
								if (click(driver,
										nsp.getWarningPopUpYesButton(environment, 10,
												NavatarSetupSideMenuTab.CommitmentCreation),
										"Yes Button", action.BOOLEAN)) {
									...
									switchToDefaultContent(driver);
									if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
										switchToFrame(driver, 10,
												nsp.getnavatarSetUpTabFrame_Lighting(environment, 10));
									}
									List<String> result = new ArrayList<String>();
									String[] labels = { "Investment Likely Amount (mn)" };

									result = nsp.verifyLabelInViewModeforNavatarSetUpSideMenuTab(environment, mode,
											NavatarSetupSideMenuTab.CommitmentCreation,
											NavatarSetupSideMenuTabLayoutSection.CommitmentCreation_FundRaisingInformation,
											labels);
									...
									if (!result.isEmpty() || (result.contains("Investment Likely Amount (mn)"))) {
										...
										log(LogStatus.FAIL, "Label Not Verified for FundRaising Information ",
												YesNo.Yes);
									}
									...
									if (!result.isEmpty() || (result.contains("Placement Fee %"))) {
										...
										log(LogStatus.FAIL, "Label Not Verified for Additional Information ",
												YesNo.Yes);
									}
									...
									if (!result.isEmpty() || (result.contains(excelLabel.Bank_Name.toString()))) {
										log(LogStatus.INFO, "Label Verified on Fields For New Limited Partner",
												YesNo.No);
									} else {
										...
										log(LogStatus.FAIL, "Label Not Verified for Fields For New Limited Partner",
												YesNo.Yes);
									}
									...
									if (!result.isEmpty() || (result.contains("Fund Investment Category"))) {
										...
										log(LogStatus.FAIL, "Label Not Verified for Fields For New Partnership",
												YesNo.Yes);
									}
	...
	public void PESmokeTc045_1_VerifyFundDrawdownPage(String environment, String mode) {
		...
		String totalCommitments = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.Total_Commitments);
		String percent = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.Percent);
		...
		String callAmount = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.CallAmount);
		String capitalAmount = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.CapitalAmount);
		String ManagementFee = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.ManagementFee);
		String OtherFee = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.OtherFee);
		...
		if (lp.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fp.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				...
				for (int i = 1; i <= 2; i++) {
					ThreadSleep(2000);
					if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
							ShowMoreActionDropDownList.Create_Drawdown, 60)) {
						...
						log(LogStatus.ERROR,
								"could not click on show more dropdown : " + ShowMoreActionDropDownList.Create_Drawdown,
								YesNo.Yes);
				...
				if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
						ShowMoreActionDropDownList.Create_Drawdown, 30)) {
					log(LogStatus.INFO, "clicked on Create_Drawdowns", YesNo.No);
					/*
					 * if (mode.equalsIgnoreCase(Mode.Lightning.toString())){ switchToFrame(driver,
					 * 30, fd.getEmailingFrame_Lighting(30)); }
					 */
					...
					for (int j = 0; j < linkClick.length; j++) {
						...
						if (click(driver, fd.getPercentageRadioBtn(30), "percentage radio button",
								action.SCROLLANDBOOLEAN)) {
							appLog.info("Click on %age radio button");
							if (sendKeys(driver, fd.getPercentageTextboxCreateDrawdown(30), percent,
									"percentage textbox", action.BOOLEAN)) {
								appLog.info("Enter value on %age text box : " + percent);
								sendKeys(driver, fd.getCapitalAmountTextboxCreateDrawdown(PercentOrValue.Percent, 30),
										percentValues.split(",")[0], "capital amount", action.SCROLLANDBOOLEAN);
								sendKeys(driver, fd.getManagementFeeTextboxCreateDrawdown(PercentOrValue.Percent, 30),
										percentValues.split(",")[1], "management fee", action.SCROLLANDBOOLEAN);
								sendKeys(driver, fd.getOtherFeeTextBoxCreateDrawdown(PercentOrValue.Percent, 30),
										percentValues.split(",")[2], "other fee", action.SCROLLANDBOOLEAN);
								...
								if (click(driver, fd.getSetupCapitalCalls(30), "setup capital call sbutton",
										action.SCROLLANDBOOLEAN)) {
	...
	public void PESmokeTc045_2_VerifyFundDrawdownPage(String environment, String mode) {
		...
		for (int i = 0; i < 5; i++) {
			ExcelUtils.writeData(smokeFilePath, date, "CapitalCall", excelLabel.Variable_Name, "CC" + (i + 1),
					excelLabel.CallDate);
			ExcelUtils.writeData(smokeFilePath, OneDayAfterDate, "CapitalCall", excelLabel.Variable_Name,
					"CC" + (i + 1), excelLabel.DueDate);
		}
		ExcelUtils.writeData(smokeFilePath, date, "FundDrawdown", excelLabel.Variable_Name, "DD1", excelLabel.CallDate);
		ExcelUtils.writeData(smokeFilePath, OneDayAfterDate, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.DueDate);
			...
			if (fp.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				...
				if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
					...
					if (fd.getValueInCreateDrawdownsOrDistributionsPage(fields.split("<break>")[0]) != null) {
						String totalCommitments1 = bp.convertNumberAccordingToFormatWithCurrencySymbol(totalCommitments,
								"0,000.00");

						if (fd.getValueInCreateDrawdownsOrDistributionsPage(fields.split("<break>")[0])
								.equalsIgnoreCase(totalCommitments1)) {
					...
					String text = fd.getCapitalAmountTextboxCreateDrawdown(PercentOrValue.Value, 30)
							.getAttribute("value");
					if (text.contains("0.00")) {
						log(LogStatus.INFO, "successfully verified capital amount textbox value", YesNo.No);
					} else {
						log(LogStatus.ERROR,
								"could not verify capital amount textbox value. Expected: 0.00\tActual: " + text,
								YesNo.Yes);
						sa.assertTrue(false,
								"could not verify capital amount textbox value. Expected: 0.00\tActual: " + text);
					}
					if (getValueFromElementUsingJavaScript(driver,
							fd.getManagementFeeTextboxCreateDrawdown(PercentOrValue.Value, 30), "ManagementFee value")
							.contains("0.0")) {
					...
					if (getValueFromElementUsingJavaScript(driver,
							fd.getOtherFeeTextBoxCreateDrawdown(PercentOrValue.Value, 30), "OtherFee value")
							.contains("0.0")) {
					...
					if (click(driver, fd.getPercentageRadioBtn(30), "percentage radio button",
							action.SCROLLANDBOOLEAN)) {
						if (sendKeys(driver, fd.getPercentageTextboxCreateDrawdown(30), percent, "percentage textbox",
								action.BOOLEAN)) {
						...
						String a = getValueFromElementUsingJavaScript(driver,
								fd.getCapitalAmountTextboxCreateDrawdown(PercentOrValue.Value, 30),
								"value textbox capital call");
						if (a.trim().contains(capitalAmount)) {
							log(LogStatus.INFO, "successfully verified capital amount textbox value", YesNo.No);
						} else {
							log(LogStatus.ERROR,
									"could not verify capital amount textbox value" + a + " and " + capitalAmount,
									YesNo.Yes);
							sa.assertTrue(false, "could not verify capital amount textbox value");
						}
						a = getValueFromElementUsingJavaScript(driver,
								fd.getManagementFeeTextboxCreateDrawdown(PercentOrValue.Value, 30),
								"value textbox ManagementFee");
						if (a.trim().contains(ManagementFee)) {
							log(LogStatus.INFO, "successfully verified ManagementFee textbox value", YesNo.No);
						} else {
							log(LogStatus.ERROR,
									"could not verify ManagementFee textbox value" + a + " and " + ManagementFee,
									YesNo.Yes);
							sa.assertTrue(false, "could not verify ManagementFee textbox value");
						}
						a = getValueFromElementUsingJavaScript(driver,
								fd.getOtherFeeTextBoxCreateDrawdown(PercentOrValue.Value, 30),
								"value textbox OtherFee");
						if (a.trim().contains(OtherFee)) {
							log(LogStatus.INFO, "successfully verified OtherFee textbox value", YesNo.No);
						} else {
							log(LogStatus.ERROR, "could not verify OtherFee textbox value" + a + " and " + OtherFee,
									YesNo.Yes);
							sa.assertTrue(false, "could not verify OtherFee textbox value");
						}
						String callAmount1 = bp.convertNumberAccordingToFormatWithCurrencySymbol(callAmount,
								"0,000.00");

						if (getText(driver, fd.getCapitalCallValueCreateDrawdown(30), "capital call value",
								action.BOOLEAN).trim().contains(callAmount1)) {
							log(LogStatus.INFO, "call amount verified successfully", YesNo.No);
						} else {
							log(LogStatus.ERROR,
									"call amount value could not be verified"
											+ getText(driver, fd.getCapitalCallValueCreateDrawdown(30),
													"capital call value", action.BOOLEAN)
											+ " and " + callAmount,
									YesNo.Yes);
							sa.assertTrue(false, "call amount value could not be verified");
						}

						click(driver, fd.getSetupCapitalCalls(30), "setup capital call sbutton",
								action.SCROLLANDBOOLEAN);

						String[][] capitalCallGrids = {
								{ Smoke_LP1, SmokeCOMM1_ID, SmokeCOMM1_CommitmentAmount,
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM1_ID,
												SmokeCOMM1_CommitmentAmount, totalCommitments, capitalAmount),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM1_ID,
												SmokeCOMM1_CommitmentAmount, totalCommitments, ManagementFee),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM1_ID,
												SmokeCOMM1_CommitmentAmount, totalCommitments, OtherFee) },
								{ Smoke_LP2, SmokeCOMM2_ID, SmokeCOMM2_CommitmentAmount,
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM2_ID,
												SmokeCOMM2_CommitmentAmount, totalCommitments, capitalAmount),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM2_ID,
												SmokeCOMM2_CommitmentAmount, totalCommitments, ManagementFee),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM2_ID,
												SmokeCOMM2_CommitmentAmount, totalCommitments, OtherFee) },
								{ Smoke_LP1, SmokeCOMM3_ID, SmokeCOMM3_CommitmentAmount,
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM3_ID,
												SmokeCOMM3_CommitmentAmount, totalCommitments, capitalAmount),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM3_ID,
												SmokeCOMM3_CommitmentAmount, totalCommitments, ManagementFee),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM3_ID,
												SmokeCOMM3_CommitmentAmount, totalCommitments, OtherFee) },
								{ Smoke_LP3, SmokeCOMM4_ID, SmokeCOMM4_CommitmentAmount,
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM4_ID,
												SmokeCOMM4_CommitmentAmount, totalCommitments, capitalAmount),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM4_ID,
												SmokeCOMM4_CommitmentAmount, totalCommitments, ManagementFee),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM4_ID,
												SmokeCOMM4_CommitmentAmount, totalCommitments, OtherFee) },
								{ Smoke_LP4, SmokeCOMM5_ID, SmokeCOMM5_CommitmentAmount,
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM5_ID,
												SmokeCOMM5_CommitmentAmount, totalCommitments, capitalAmount),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM5_ID,
												SmokeCOMM5_CommitmentAmount, totalCommitments, ManagementFee),
										fd.CapitalAmountAndManagementFeeAndOtherFee(SmokeCOMM5_ID,
												SmokeCOMM5_CommitmentAmount, totalCommitments, OtherFee) } };
						...
						List<WebElement> managementFees = fd.getManagementFeeAmountListFromCapitalCall(environment,
								mode);
						...
						if (!commitments.isEmpty()) {
							...
							if (commitments.size() == capitalAmounts.size()) {
								...
								for (int i = 0; i < commitments.size(); i++) {
									...
									ExcelUtils.writeData(smokeFilePath, commitName, "CapitalCall",
											excelLabel.Variable_Name, "CC" + (i + 1), excelLabel.Commitment_ID);
									...
									ExcelUtils.writeData(smokeFilePath, amount, "CapitalCall", excelLabel.Variable_Name,
											"CC" + (i + 1), excelLabel.CapitalAmount);

									amount = managementFees.get(i).getAttribute("value").trim();
									amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0000");
									ExcelUtils.writeData(smokeFilePath, amount, "CapitalCall", excelLabel.Variable_Name,
											"CC" + (i + 1), excelLabel.ManagementFee);

									amount = otherFees.get(i).getAttribute("value").trim();
									amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0000");
									ExcelUtils.writeData(smokeFilePath, amount, "CapitalCall", excelLabel.Variable_Name,
											"CC" + (i + 1), excelLabel.OtherFee);

									totalAmounts = totalAmount.get(i).getText().trim();
									totalAmounts = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(totalAmounts,
											"0000");
									ExcelUtils.writeData(smokeFilePath, totalAmounts, "CapitalCall",
											excelLabel.Variable_Name, "CC" + (i + 1), excelLabel.CallAmount);

									if (commitName.equalsIgnoreCase(SmokeCOMM1_ID)) {
										ExcelUtils.writeData(smokeFilePath, totalAmounts, "CapitalCall",
												excelLabel.Variable_Name, "CC" + (i + 1),
												excelLabel.CallAmountReceived);
										ExcelUtils.writeData(smokeFilePath, previousOrForwardDate(0, "M/d/yyyy"),
												"CapitalCall", excelLabel.Variable_Name, "CC" + (i + 1),
												excelLabel.ReceivedDate);

									}

									convertValue = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(totalAmounts,
											"0,000.000");
									ExcelUtils.writeData(smokeFilePath, convertValue, "CapitalCall",
											excelLabel.Variable_Name, "CC" + (i + 1), excelLabel.AmountDue);
								}
						...
						if (click(driver, fd.getGenerateCapitalCall(30), "generate capital call",
							...
							if (FDID != null || !FDID.isEmpty() || !FDID.equalsIgnoreCase("")) {
								...
								if (clickUsingJavaScript(driver,
										fp.getFirstValueFromRelatedList(environment, mode,
												RelatedList.FundDrawdown.toString(), Smoke_Fund1),
									...
									if (s.size() > 0) {
										for (int i = 0; i < 5; i++) {
											ExcelUtils.writeData(smokeFilePath, s.get(i), "CapitalCall",
													excelLabel.Variable_Name, "CC" + (i + 1),
													excelLabel.CapitalCalllID);
										}
									} else {
										log(LogStatus.ERROR,
												"Either No Capital Call Id's present or Locator is not Correct for it, So not able to write Id's to excel",
												YesNo.Yes);
										sa.assertTrue(false,
												"Either No Capital Call Id's present or Locator is not Correct for it, So not able to write Id's to excel");
									}
	...
	public void PESmokeTc046_VerifyCreatedCapitalCalls(String environment, String mode) {
		...
		String[] labels = { CapitalCallPageFieldLabelText.Fund_Drawdown.toString(),
				...
				CapitalCallPageFieldLabelText.Call_Date.toString(), CapitalCallPageFieldLabelText.Due_Date.toString(),
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			...
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC1_ID)) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
							SmokeCC1Data[i], EditViewMode.View)) {
				...
				try {
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls,
							CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), "", EditViewMode.View)) {
						log(LogStatus.INFO, "successfully verified "
								+ CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), YesNo.No);
					} else {

						log(LogStatus.FAIL, "data could not be verified of "
								+ CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), YesNo.Yes);
						sa.assertTrue(false, "data could not be verified of "
								+ CapitalCallPageFieldLabelText.Call_Amount_Received.toString());
					}
				} catch (Exception e) {
				...
				try {
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls,
							CapitalCallPageFieldLabelText.Received_Date.toString(), "", EditViewMode.View)) {
						log(LogStatus.INFO,
								"successfully verified " + CapitalCallPageFieldLabelText.Received_Date.toString(),
								YesNo.No);
					} else {

						log(LogStatus.FAIL, "data could not be verified of "
								+ CapitalCallPageFieldLabelText.Received_Date.toString(), YesNo.Yes);
						sa.assertTrue(false, "data could not be verified of "
								+ CapitalCallPageFieldLabelText.Received_Date.toString());
					}
				} catch (Exception e) {
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC2_ID)) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
							SmokeCC2Data[i], EditViewMode.View)) {
						log(LogStatus.INFO, "successfully verified " + labels[i], YesNo.No);
					} else {
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC3_ID)) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
							SmokeCC3Data[i], EditViewMode.View)) {
						log(LogStatus.INFO, "successfully verified " + labels[i], YesNo.No);
					} else {
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC4_ID)) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
							SmokeCC4Data[i], EditViewMode.View)) {
						log(LogStatus.INFO, "successfully verified " + labels[i], YesNo.No);
					} else {
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC5_ID)) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
							SmokeCC5Data[i], EditViewMode.View)) {
						log(LogStatus.INFO, "successfully verified " + labels[i], YesNo.No);
					} else {
	...
	public void PESmokeTc047_VerifyEditingCapitalCalls(String environment, String mode) {
		...
		BasePageBusinessLayer bp = new BasePageBusinessLayer(driver);
		SmokeCC3_ID = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.CapitalCalllID);
		...
		String[] labels = { CapitalCallPageFieldLabelText.Fund_Drawdown.toString(),
				...
				CapitalCallPageFieldLabelText.Amount_Due.toString(),
				CapitalCallPageFieldLabelText.Commitment.toString() };
		SmokeCC3Data[0] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.DrawdownID);
		SmokeCC3Data[1] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.CapitalAmount);
		SmokeCC3Data[2] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.ManagementFee);
		SmokeCC3Data[3] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.OtherFee);
		SmokeCC3Data[4] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.CallAmount);
		SmokeCC3Data[5] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.CallDate);
		SmokeCC3Data[6] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.DueDate);
		SmokeCC3Data[7] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.CallAmountReceived);
		SmokeCC3Data[8] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.ReceivedDate);
		SmokeCC3Data[9] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.AmountDue);
		SmokeCC3Data[10] = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
				excelLabel.Commitment_ID);
		String date = getDateAccToTimeZone(BasePageErrorMessage.AmericaLosAngelesTimeZone, "MM/dd/YYYY");
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC3_ID)) {
				if (ccp.clickOnShowMoreActionDownArrow("PE", PageName.CapitalCall, ShowMoreActionDropDownList.Edit,
						20)) {
					for (int i = 0; i <= 3; i++) {
						if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[i],
								SmokeCC3Data[i], EditViewMode.Edit)) {
					...
					try {
						if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls,
								CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), "", EditViewMode.Edit)) {
							log(LogStatus.PASS, "successfully verified "
									+ CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), YesNo.No);
					...
					try {
						if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls,
								CapitalCallPageFieldLabelText.Received_Date.toString(), "", EditViewMode.Edit)) {
							log(LogStatus.PASS,
									"successfully verified " + CapitalCallPageFieldLabelText.Received_Date.toString(),
									YesNo.No);
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls,
							CapitalCallPageFieldLabelText.Commitment.toString(), SmokeCC3Data[10], EditViewMode.Edit)) {
						log(LogStatus.PASS,
								"successfully verified " + CapitalCallPageFieldLabelText.Commitment.toString(),
								YesNo.No);

					} else {
						log(LogStatus.FAIL,
								"data could not be verified of " + CapitalCallPageFieldLabelText.Commitment.toString(),
								YesNo.Yes);
						sa.assertTrue(false,
								"data could not be verified of " + CapitalCallPageFieldLabelText.Commitment.toString());
					...
					if (sendKeys(driver,
							ccp.getCapitalCallPageTextBoxOrRichTextBoxWebElement(environment, mode,
									CapitalCallPageFieldLabelText.Call_Amount_Received.toString(), 30),
							SmokeCC3Data[4], "call amount received", action.BOOLEAN)) {
						if (sendKeys(driver,
								ccp.getCapitalCallPageTextBoxOrRichTextBoxWebElement(environment, mode,
										CapitalCallPageFieldLabelText.Received_Date.toString(), 30),
								SmokeCC3Data[5], "received date", action.SCROLLANDBOOLEAN)) {
							if (click(driver, bp.getCustomTabSaveBtn(environment, 30), "save button",
									action.SCROLLANDBOOLEAN)) {
							} else {
					...
					if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode, TabName.CapitalCalls, labels[9],
							"$0.000", EditViewMode.View)) {
						log(LogStatus.PASS,
								"successfully verified " + CapitalCallPageFieldLabelText.Amount_Due.toString(),
								YesNo.No);
					} else {
						log(LogStatus.FAIL,
								"data could not be verified of " + CapitalCallPageFieldLabelText.Amount_Due.toString(),
								YesNo.Yes);
						sa.assertTrue(false,
								"data could not be verified of " + CapitalCallPageFieldLabelText.Amount_Due.toString());
					}
		...
		String[][] label = { { FundDrawdownPageFieldLabelText.DD_No.toString(), SmokeDD1_ID },
				...
				{ FundDrawdownPageFieldLabelText.Total_Call_Amount_Received.toString(),
						SmokeDD1_TotalCallAmountReceived },
		...
		if (bp.clickOnTab(environment, mode, TabName.FundDrawdowns)) {
			if (fdp.clickOnCreatedDrawdown(environment, mode, SmokeDD1_ID)) {
				for (int i = 0; i < label.length; i++) {
					if (fdp.fieldValueVerificationOnFundDrawdown(environment, mode, TabName.FundDrawdowns, label[i][0],
							label[i][1])) {
	...
	public void PESmokeTc048_VerifyCommitmentPage(String environment, String mode) {
		...
		CommitmentsPageBusinessLayer comm = new CommitmentsPageBusinessLayer(driver);
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		String text = ExcelUtils.readData(smokeFilePath, "Commitments", excelLabel.Variable_Name, "SmokeCOMM1",
				excelLabel.Commitment_ID);
		ExcelUtils.writeData(smokeFilePath, text, "Commitments", excelLabel.Variable_Name, "SmokeCOMM8",
				excelLabel.Commitment_ID);
		text = ExcelUtils.readData(smokeFilePath, "Commitments", excelLabel.Variable_Name, "SmokeCOMM1",
				excelLabel.Final_Commitment_Date);
		ExcelUtils.writeData(smokeFilePath, text, "Commitments", excelLabel.Variable_Name, "SmokeCOMM8",
				excelLabel.Final_Commitment_Date);
		String labelName = CommitmentPageFieldLabelText.Commitment_ID + "," + CommitmentPageFieldLabelText.Partner_Type
				+ "," + CommitmentPageFieldLabelText.Limited_Partner + ","
				+ CommitmentPageFieldLabelText.Final_Commitment_Date + "," + CommitmentPageFieldLabelText.Partnership
				+ "," + CommitmentPageFieldLabelText.Tax_Forms + "," + CommitmentPageFieldLabelText.Commitment_Amount
				+ "," + CommitmentPageFieldLabelText.Total_Amount_Called + ","
				+ CommitmentPageFieldLabelText.Total_Amount_Received + ","
				+ CommitmentPageFieldLabelText.Total_Uncalled_Amount + ","
				+ CommitmentPageFieldLabelText.Total_Commitment_Due + ","
				+ CommitmentPageFieldLabelText.Commitment_Called + "," + CommitmentPageFieldLabelText.Called_Due;

		String labelValue = SmokeCOMM8_ID + "," + SmokeCOMM8_partnerType + "," + Smoke_LP1 + ","
				+ SmokeCOMM8_FinalCommitmentDate + "," + Smoke_P1 + "," + SmokeCOMM8_TaxForms + ","
				+ SmokeCOMM8_CommitmentAmount + "," + SmokeCOMM8_TotalAmountCalled + ","
				+ SmokeCOMM8_TotalAmountReceived + "," + SmokeCOMM8_TotalUncalledAmount + ","
				...
				for (int i = 1; i <= 2; i++) {
					...
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						if (bp.clickOnShowMoreDropdownOnly(environment, mode, PageName.FundDrawdown)) {
							...
							WebElement ele = bp.actionDropdownElement(environment, PageName.FundDrawdown,
									ShowMoreActionDropDownList.Send_Capital_Call_Notices, 30);
							if (ele != null) {
								log(LogStatus.PASS, "successfully found send capital call notices button", YesNo.No);
								if (click(driver,
										bp.actionDropdownElement(environment, PageName.FundDrawdown,
												ShowMoreActionDropDownList.Send_Capital_Call_Notices, 30),
										ShowMoreActionDropDownList.Send_Capital_Call_Notices.toString(),
										action.BOOLEAN)) {
						...
						if (fd.getSendCapitalCallNotices_Classic(30) != null) {
							...
							if (click(driver, fd.getSendCapitalCallNotices_Classic(30),
									"send capital call notices button", action.SCROLLANDBOOLEAN)) {
					...
					if (i == 1) {
						...
						if (fd.getEmailing_text(30).getText().trim()
								.equalsIgnoreCase(FundDrawdownPageErrorMessage.emailingText1 + SmokeDD1_ID
										+ FundDrawdownPageErrorMessage.emailingText2 + Smoke_Fund1)) {
						...
						if (fd.getSpecifyRecepients(30).getText().trim()
								.equalsIgnoreCase(FundDrawdownPageErrorMessage.specifyRecepients)) {
						...
						if (fd.getNoDataToDisplayEmailing(30).getText().trim()
								.equalsIgnoreCase(BasePageErrorMessage.nodataDisplayMsg)) {
						...
						if (click(driver, fd.getStep1CancelButtonEmailing(TopOrBottom.TOP, 20),
								"step1 Cancel Button Emailing : Top", action.SCROLLANDBOOLEAN)) {
						...
						if (click(driver, fd.getStep1CancelButtonEmailing(TopOrBottom.BOTTOM, 20),
								"step1 Cancel Button Emailing : Bottom", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "Click on step1 Cancel Button Emailing : Bottom", YesNo.No);
						} else {
							log(LogStatus.ERROR, "Not Able to Click on step1 Cancel Button Emailing : Bottom",
									YesNo.Yes);
			...
			if (cp.clickOnCreatedContact(environment, SmokeC1_FName, SmokeC1_LName)) {
				...
				if (bp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Correspondence_Lists,
					...
					if (click(driver, crp.getNewCorrespondenceListButton(30), "New Correspondence List Button",
						...
						ThreadSleep(5000);
						if (crp.createNewCorrList("", mode, environment, SmokeC1_FName, SmokeC1_LName, SmokeCOMM8_ID,
								corrList)) {
							log(LogStatus.PASS, "successfully created new correspondence list", YesNo.No);
							if (cp.verifyPresenceOfCorrespondenceRelatedList(mode, environment,
									SmokeC1_FName + " " + SmokeC1_LName, Smoke_LP1, Smoke_P1, SmokeCOMM8_ID, 30)) {
								log(LogStatus.PASS, "successfully verified " + SmokeCOMM8_ID + " on related list",
										YesNo.No);
							} else {
						...
						log(LogStatus.ERROR, "could not click on new button on correspondence list related list",
								YesNo.Yes);
			...
			if (cp.clickOnCreatedContact(environment, SmokeC2_FName, SmokeC2_LName)) {
				...
				if (bp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Correspondence_Lists,
					...
					if (click(driver, crp.getNewCorrespondenceListButton(30), "New Correspondence List Button",
							action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "clicked on new button on correspondence list related list", YesNo.No);
						if (crp.createNewCorrList("", mode, environment, SmokeC2_FName, SmokeC2_LName, SmokeCOMM2_ID,
								corrList)) {
							log(LogStatus.PASS, "successfully created new correspondence list", YesNo.No);
							if (cp.verifyPresenceOfCorrespondenceRelatedList(mode, environment,
									SmokeC2_FName + " " + SmokeC2_LName, Smoke_LP2, Smoke_P1, SmokeCOMM2_ID, 30)) {
								log(LogStatus.PASS, "successfully verified " + SmokeCOMM2_ID + " on related list",
										YesNo.No);
							} else {
			...
			if (cp.clickOnCreatedContact(environment, SmokeC4_FName, SmokeC4_LName)) {
				...
				if (bp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Correspondence_Lists,
					...
					if (click(driver, crp.getNewCorrespondenceListButton(30), "New Correspondence List Button",
							action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "clicked on new button on correspondence list related list", YesNo.No);
						if (crp.createNewCorrList("", mode, environment, SmokeC4_FName, SmokeC4_LName, SmokeCOMM3_ID,
								corrList)) {
							log(LogStatus.PASS, "successfully created new correspondence list", YesNo.No);
							if (cp.verifyPresenceOfCorrespondenceRelatedList(mode, environment,
									SmokeC4_FName + " " + SmokeC4_LName, Smoke_LP1, Smoke_P1, SmokeCOMM3_ID, 30)) {
								log(LogStatus.PASS, "successfully verified " + SmokeCOMM3_ID + " on related list",
										YesNo.No);
							} else {
		...
		for (int j = 0; j <= 7; j++) {
			if (fund.clickOnTab(environment, mode, TabName.FundDrawdowns)) {
				if (fd.clickOnCreatedDrawdown(environment, mode, SmokeDD1_ID)) {
					...
					// List<String> result
					// =market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(PageName.emailCapitalCallNotice,ContactAndAccountName,
					// searchContactInEmailProspectGrid.No);
					if (j == 0) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
								SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, 30)) {
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							for (int i = 0; i <= 3; i++) {
								if (i == 0) {
									if (click(driver, market.getStep1NextBtn(PageName.emailCapitalCallNotice,
											TopOrBottom.TOP, 10), "Top Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on top next button", YesNo.Yes);
								...
								if (i == 1) {
									if (click(
											driver, market.getStep2PreviousBtn(PageName.emailCapitalCallNotice,
													TopOrBottom.TOP, 10),
											"Top previous button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top previous button", YesNo.No);
									} else {
								...
								if (i == 2) {
									if (click(
											driver, market.getStep1NextBtn(PageName.emailCapitalCallNotice,
													TopOrBottom.BOTTOM, 10),
											"bottom Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
									} else {
								...
								if (i == 3) {
									if (click(driver,
											market.getStep2PreviousBtn(PageName.emailCapitalCallNotice,
													TopOrBottom.BOTTOM, 10),
											"bottom previous button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM previous button", YesNo.No);
									} else {
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check next and previous button", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check next and previous button");
						}
					}
					if (j == 1) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1CancelBtn(PageName.emailCapitalCallNotice, TopOrBottom.TOP, 10),
									"Top cancel button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on top cancel button step 1", YesNo.No);
							} else {
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot Top Cancel bottom on step 1", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot Top Cancel bottom on step 1");
						}
					}
					if (j == 2) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot bottom Cancel bottom on step 1", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot bottom Cancel bottom on step 1");
						}
					}
					if (j == 3) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1NextBtn(PageName.emailCapitalCallNotice, TopOrBottom.BOTTOM, 10),
									"bottom Next button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
								ThreadSleep(2000);
								if (click(driver,
										market.getStep2CancelBtn(PageName.emailCapitalCallNotice, TopOrBottom.TOP, 10),
										"top cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on top cancel button step 2", YesNo.No);
								} else {
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check Top Cancel button on Step2", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check Top Cancel button on Step2");
						}
					}
					if (j == 4) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							if (click(driver,
								...
								ThreadSleep(2000);
								if (click(
										driver, market.getStep2CancelBtn(PageName.emailCapitalCallNotice,
												TopOrBottom.BOTTOM, 10),
										"bottom cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on bottom cancel button step 2", YesNo.No);
								} else {
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check bottom Cancel button on Step2", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check bottom Cancel button on Step2");
						}
					}
					if (j == 5) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1NextBtn(PageName.emailCapitalCallNotice, TopOrBottom.TOP, 10),
									"Top Next button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on top next button step 1", YesNo.No);
								ThreadSleep(2000);
								for (int i = 0; i <= 3; i++) {
									if (i == 0) {
										if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
											log(LogStatus.ERROR, "Select Capital Call Notice template", YesNo.No);
											if (click(driver,
													market.getStep2NextBtn(PageName.emailCapitalCallNotice,
															TopOrBottom.TOP, 10),
													"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
											} else {
									...
									if (i == 1) {
										if (click(driver,
												market.getStep3PreviousBtn(PageName.emailCapitalCallNotice,
														TopOrBottom.TOP, 10),
												"Top previous button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top previous button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on top previous button step 3",
													YesNo.Yes);
									...
									if (i == 2) {
										if (click(
												driver, market.getStep2NextBtn(PageName.emailCapitalCallNotice,
														TopOrBottom.TOP, 10),
												"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
									...
									if (i == 3) {
										if (click(driver,
											...
											log(LogStatus.ERROR, "Not able to click on bottom previous button step 3",
													YesNo.Yes);
					...
					if (j == 6) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
							...
							if (click(driver,
								...
								if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
									...
									if (click(
										...
										ThreadSleep(2000);
										if (click(driver,
												market.getStep3CancelBtn(PageName.emailCapitalCallNotice,
														TopOrBottom.TOP, 10),
												"top cancel button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top cancel button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on top cancel button step 3",
													YesNo.Yes);
							...
							log(LogStatus.ERROR, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check step 3 cancel button", YesNo.Yes);
							sa.assertTrue(false, "Not able to select contact " + SmokeC1_FName + " " + SmokeC1_LName
									+ " so cannot check step 3 cancel button");
						}
					}
					if (j == 7) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
								...
								if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
									...
									if (click(
										...
										if (click(driver,
											...
											log(LogStatus.ERROR, "Not able to click on buttom cancel button step 3",
													YesNo.Yes);
					...
					log(LogStatus.ERROR, "Not able to click on Draw Down " + SmokeDD1_ID
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
					sa.assertTrue(false, "Not able to click on Draw Down " + SmokeDD1_ID
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName);
				}
			} else {
				log(LogStatus.ERROR, "Not able to click on fund Draw Down tab so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
				sa.assertTrue(false, "Not able to click on fund Draw Down tab so cannot send email to contact "
						+ SmokeC1_FName + " " + SmokeC1_LName);
			}
			switchToDefaultContent(driver);
		}
	...
	public void PESmokeTc051_VerifyEmailSendToOneContact(String environment, String mode) {
		...
		BasePageBusinessLayer bp = new BasePageBusinessLayer(driver);
		String keywords[] = ExcelUtils
				.readData("FilePath", excelLabel.TestCases_Name, currentlyExecutingTC, excelLabel.KeyWord_For_Search)
				.split(",");
		...
		if (bp.clickOnTab(environment, mode, TabName.FundDrawdowns)) {
			if (fd.clickOnCreatedDrawdown(environment, mode, SmokeDD1_ID)) {
				if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					if (bp.clickOnShowMoreDropdownOnly(environment, mode, PageName.FundDrawdown)) {
						log(LogStatus.PASS, "successfully clicked on dropdown", YesNo.No);
						ele = bp.actionDropdownElement(environment, PageName.FundDrawdown,
								ShowMoreActionDropDownList.Send_Capital_Call_Notices, 30);
						if (ele != null) {
							log(LogStatus.PASS, "successfully found send capital call notices button", YesNo.No);
							if (click(driver,
									bp.actionDropdownElement(environment, PageName.FundDrawdown,
											ShowMoreActionDropDownList.Send_Capital_Call_Notices, 30),
									ShowMoreActionDropDownList.Send_Capital_Call_Notices.toString(), action.BOOLEAN)) {
								log(LogStatus.PASS, "successfully click on send button", YesNo.No);
							} else {
					...
					if (fd.getSendCapitalCallNotices_Classic(30) != null) {
						log(LogStatus.PASS, "send capital call notices button is successfully present", YesNo.No);
						if (click(driver, fd.getSendCapitalCallNotices_Classic(30), "send capital call notices button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "successfully click on send capital call button", YesNo.No);
						} else {
							log(LogStatus.ERROR, "could not click on send button", YesNo.Yes);
				...
				// verify mailing grid
				SmokeCC1_ID = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM8_ID,
						excelLabel.CapitalCalllID);
				saa = fd.verifyEmailingContentGrid(driver, PageName.FundDrawdown, SmokeC1_FName + " " + SmokeC1_LName,
						SmokeINS1, SmokeC1_EmailID, SmokeCC1_ID, SmokeCOMM8_ID, Smoke_LP1, Smoke_P1, false);
				sa.combineAssertions(saa);
				SmokeCC2_ID = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM2_ID,
						excelLabel.CapitalCalllID);
				saa = fd.verifyEmailingContentGrid(driver, PageName.FundDrawdown, SmokeC2_FName + " " + SmokeC2_LName,
						SmokeINS1, SmokeC2_EmailID, SmokeCC2_ID, SmokeCOMM2_ID, Smoke_LP2, Smoke_P1, false);
				sa.combineAssertions(saa);
				...
				if (flag) {
					log(LogStatus.ERROR,
							SmokeC4_FName + " " + SmokeC4_LName + " is found in emailing grid but it should not be",
							YesNo.Yes);
					sa.assertTrue(false,
							SmokeC4_FName + " " + SmokeC4_LName + " is found in emailing grid but it should not be");
				} else {
					log(LogStatus.INFO, "could not find " + SmokeC4_FName + " " + SmokeC4_LName + " as expected",
							YesNo.No);
				}
				...
				if (sendKeys(driver, fd.getSearchTextboxEmailingGrid(30), SmokeC1_LName, "search box",
						action.SCROLLANDBOOLEAN)) {
					if (click(driver, fd.getSearchIconEmailingGrid(30), "search icon", action.BOOLEAN)) {
						...
						saa = fd.verifyEmailingContentGrid(driver, PageName.FundDrawdown,
								SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, SmokeC1_EmailID, SmokeCC1_ID,
								SmokeCOMM8_ID, Smoke_LP1, Smoke_P1, false);
						sa.combineAssertions(saa);
				...
				if (click(driver, fd.getStep1HeaderCheckbox(30), "header checkbox", action.SCROLLANDBOOLEAN)) {
					if (click(driver, fd.getStep1NextButtonEmailing(30), "next button", action.SCROLLANDBOOLEAN)) {
						...
						String expectedResult = "All,Capital Call Notice,Investor Distribution Notice,"
								+ EmailTemplate1_FolderName;
						List<WebElement> lst = allOptionsInDropDrop(driver, fd.getFundDrawdownFolderDropDownList(20),
						...
						selectVisibleTextFromDropDown(driver, fd.getFundDrawdownFolderDropDownList(20),
								"folder dropdown", expectedResult.split(",")[1]);
						...
						if (fd.selectEmailTemplateFromFundDrawdown(expectedResult.split(",")[1],
								expectedResult.split(",")[1])) {
						...
						if (ele != null) {
							...
							if (aa.contains(MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1"))) {
								log(LogStatus.INFO, MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
										+ " error message is verified", YesNo.No);
							} else {
								sa.assertTrue(false,
										"error message is not verified Expected: "
												+ MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
												+ " \t Actual : " + aa);
								log(LogStatus.ERROR,
										"error message is not verified Expected: "
												+ MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
							...
							sa.assertTrue(false, MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
									+ " error message is not visible");
							log(LogStatus.ERROR, MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
									+ " error message is not visible", YesNo.Yes);
						}

						List<WebElement> labellist = fd.getFundDrawdownEmailingProcessingOptionsLableTextList();
						List<WebElement> checkBoxList = fd.getFundDrawdownEmailingProcessingOptionsCheckBoxList1();
						String[] expctRsult = { "BCC me on one message", "Use my signature",
								"Store an Activity for Each Message" };
						for (int i = 0; i < expctRsult.length; i++) {
							for (int j = 0; j < labellist.size(); j++) {
								if (labellist.get(j).getText().trim().contains(expctRsult[i])) {
									if (j != 2) {
										...
										if (checkBoxList.get(j).getAttribute("checked").contains("true")) {
											...
											log(LogStatus.ERROR, expctRsult[i] + "check box is not selected",
													YesNo.Yes);
										}
									}
									break;
						...
						if (click(driver, fd.getfundDrawdownEmailingSendBtn(TopOrBottom.TOP, 30), "send button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "clicked on send button", YesNo.No);
							ele = fd.getFundDrawdownSendEmailCongratulationsErrorMsg(30);
							String errorMsg = FundDrawdownPageErrorMessage.congratulationErrorMsg + " "
									+ FundDrawdownPageErrorMessage.generatedEmailedRecipientErrorMsg("1", "Email");
							if (ele != null) {
								String aa1 = ele.getText().trim();
								if (aa1.contains(FundDrawdownPageErrorMessage.congratulationErrorMsg) && aa1.contains(
										FundDrawdownPageErrorMessage.generatedEmailedRecipientErrorMsg("1", "Email"))) {
									log(LogStatus.INFO, "Congratulation Error Message is verified", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Congratulation Error Message is not verified. Expected: "
											+ errorMsg + "\t Actual: " + aa1, YesNo.Yes);
								}
							} else {
								log(LogStatus.ERROR,
										"Congratulations Error Message is not visible so cannot verify it. Error Msg: "
							...
							if (click(driver, fd.getFinishedButtonEmailing(30), "finished button",
									action.SCROLLANDBOOLEAN)) {
								if (fd.fieldValueVerificationOnFundDrawdown(environment, mode, TabName.FundDrawdowns,
										FundDrawdownPageFieldLabelText.DD_No.toString(), SmokeDD1_ID)) {
									log(LogStatus.INFO, "successfully verified DD No so landing page is drawdown page",
											YesNo.No);
								} else {
									log(LogStatus.ERROR, "could not verify landing page after finished button",
											YesNo.Yes);
							...
							log(LogStatus.ERROR, "Not able to click on send button so cannot send email to contact",
									YesNo.Yes);
						}
					} else {
	...
	public void PESmokeTc052_1_VerifySentEmailAndActivityFromDrawdownPage(String environment, String mode) {
			...
			if (cp.clickOnCreatedContact(environment, SmokeC1_FName, SmokeC1_LName)) {
				if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Open_Activities, null)) {
					...
					if (mode.toString().equalsIgnoreCase(Mode.Classic.toString())) {
						if (cp.verifyCreatedActivityHistory(environment, mode,
								ContactPageErrorMessage.emailSubject(SmokeCC1_ID, SmokeCOMM8_ID))) {
		...
		boolean flag = false;
		for (int i = 0; i < 10; i++) {
			if (EmailLib.mailReceived(gmailUserName, gmailPassword, crmUser1EmailID, SmokeC1_EmailID,
					subjectWithoutEmail, "")) {
		...
		// check user bcc
		for (int i = 0; i < 10; i++) {
			if (EmailLib.mailReceived(gmailUserName2, gmailPassword, crmUser1EmailID, SmokeC1_EmailID,
					subjectWithoutEmail, "")) {
				log(LogStatus.INFO, "successfully verified email present", YesNo.No);
				flag = true;
				break;
	...
	public void PESmokeTc052_2_VerifySentEmailAndActivityFromDrawdownPage(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.FundDrawdowns)) {
			if (fd.clickOnCreatedDrawdown(environment, mode, SmokeDD1_ID)) {
				...
				SmokeCC1_ID = ExcelUtils.readData(smokeFilePath, "CapitalCall", excelLabel.Commitment_ID, SmokeCOMM1_ID,
						excelLabel.CapitalCalllID);
				System.err.println("SmokeCC1_ID : " + SmokeCC1_ID);
				saa = fd.verifyEmailingContentGrid(driver, PageName.FundDrawdown, SmokeC1_FName + " " + SmokeC1_LName,
						SmokeINS1, SmokeC1_EmailID, SmokeCC1_ID, SmokeCOMM8_ID, Smoke_LP1, Smoke_P1, true);
				sa.combineAssertions(saa);
					...
					if (j == 1 || j == 2) {
						...
						if (click(driver, fd.getWrenchIcon(30), "wrench icon", action.SCROLLANDBOOLEAN)) {
							...
							if (j == 1) {
								...
								if (selectVisibleTextFromDropDown(driver,
										market.getColumnToDisplayViewDropDownList(PageName.FundDrawdown, 30),
										"view drop down list", ViewFieldsName.Contact_Fields)) {
									log(LogStatus.INFO,
											"Select value from view drop down list : " + ViewFieldsName.Contact_Fields,
											YesNo.Yes);
								...
								if (click(driver, fd.getCancelButtonOnColumnsToDisplay(10), "Cancel Button",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Clicked on Cancel Button", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not Able to Click on Cancel Button", YesNo.Yes);
								...
								if (click(driver, fd.getCrossIconOnColumnsToDisplay(10), "Cross Icon",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Clicked on Cross Icon", YesNo.No);
								} else {
						...
						String[] ss = { value };
						if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.FundDrawdown, ss, null, null,
								true).isEmpty()) {
							log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
							ThreadSleep(2000);
							if (compareMultipleList(driver, value,
									market.getSelectProspectsHeaderTextList(PageName.FundDrawdown)).isEmpty()) {
								log(LogStatus.PASS, "Selected Prospects Header Text is verified ", YesNo.No);
							} else {
	...
	public void PESmokeTc053_2_VerifySentEmailAndActivityFromCapitalCallPage(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.CapitalCalls)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeCC2_ID)) {
				if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					if (bp.clickOnShowMoreActionDownArrow(environment, PageName.CapitalCall,
							ShowMoreActionDropDownList.Send_Capital_Call_Notices, 30)) {
					...
					if (ccp.getSendCapitalCallNotices_Classic(30) != null) {
						log(LogStatus.PASS, "send capital call notices button is successfully present", YesNo.No);
						if (click(driver, ccp.getSendCapitalCallNotices_Classic(30), "send capital call notices button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "successfully click on send capital call button", YesNo.No);
						} else {
							log(LogStatus.ERROR, "could not click on send button", YesNo.Yes);
				...
				if (flag) {
					log(LogStatus.ERROR,
							SmokeC1_FName + " " + SmokeC1_LName + " is found in emailing grid but it should not be",
							YesNo.Yes);
					sa.assertTrue(false,
							SmokeC1_FName + " " + SmokeC1_LName + " is found in emailing grid but it should not be");
				} else {
					log(LogStatus.INFO, "could not find " + SmokeC1_FName + " " + SmokeC1_LName + " as expected",
							YesNo.No);
				}

				if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.CapitalCall,
						SmokeC2_FName + " " + SmokeC2_LName, SmokeINS1, 30)) {
					log(LogStatus.INFO, "successfully clicked on checkbox of " + SmokeC2_FName + " " + SmokeC2_LName,
							YesNo.No);
					if (click(driver, fd.getStep1NextButtonEmailing(30), "step 1 next button",
							action.SCROLLANDBOOLEAN)) {
						if (fd.selectEmailTemplateFromFundDrawdown(folder, template)) {
							if (click(driver, fd.getStep2NextButtonEmailing(30), "step 2 next button",
									action.SCROLLANDBOOLEAN)) {
								List<WebElement> labellist = fd.getFundDrawdownEmailingProcessingOptionsLableTextList();
								...
								for (int i = 0; i < expctRsult.length; i++) {
									for (int j = 0; j < labellist.size(); j++) {
										if (labellist.get(j).getText().trim().contains(expctRsult[i])) {
											if (j != 2) {
												if (checkBoxList.get(j).getAttribute("checked") == null) {
													log(LogStatus.INFO, expctRsult[i] + "check box is not selected",
															YesNo.No);
												} else {
													sa.assertTrue(false, expctRsult[i] + "check box is selected");
													log(LogStatus.ERROR, expctRsult[i] + "check box is selected",
															YesNo.Yes);
												}
											} else {
												System.err.println(
														"checked:   " + checkBoxList.get(j).getAttribute("checked"));
												if (checkBoxList.get(j).getAttribute("checked").contains("true")) {
													log(LogStatus.INFO, expctRsult[i] + "check box is selected",
															YesNo.No);
												} else {
													sa.assertTrue(false, expctRsult[i] + "check box is not selected");
													log(LogStatus.ERROR, expctRsult[i] + "check box is not selected",
								...
								if (click(driver, fd.getfundDrawdownEmailingSendBtn(TopOrBottom.BOTTOM, 30),
										"send button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "clicked on send button", YesNo.No);
									WebElement ele = fd.getFundDrawdownSendEmailCongratulationsErrorMsg(30);
									String errorMsg = FundDrawdownPageErrorMessage.congratulationErrorMsg + " "
											+ FundDrawdownPageErrorMessage.generatedEmailedRecipientErrorMsg("1",
													"Email");
									if (ele != null) {
										String aa1 = ele.getText().trim();
										if (aa1.contains(FundDrawdownPageErrorMessage.congratulationErrorMsg)
												&& aa1.contains(FundDrawdownPageErrorMessage
														.generatedEmailedRecipientErrorMsg("1", "Email"))) {
											log(LogStatus.INFO, "Congratulation Error Message is verified", YesNo.No);
										} else {
											log(LogStatus.ERROR,
									...
									if (click(driver, fd.getFinishedButtonEmailing(30), "finished button",
										...
										if (mode.equalsIgnoreCase(Mode.Lightning.toString()))
											switchToDefaultContent(driver);
										if (ccp.fieldValueVerificationOnCapitalCalls(environment, mode,
												TabName.CapitalCalls, CapitalCallPageFieldLabelText.CC_No.toString(),
												SmokeCC2_ID, EditViewMode.View)) {
											log(LogStatus.INFO,
													"after clicking finished button, successfully verified capital call detail page is open",
													YesNo.No);
										} else {
											log(LogStatus.ERROR,
													"after clicking finished button, landing page could not be verified",
													YesNo.Yes);
											sa.assertTrue(false,
													"after clicking finished button, landing page could not be verified");
										}
	...
	public void PESmokeTc054_VerifyMailReceivedAndActivity(String environment, String mode) {
		...
		boolean flag = false;
		for (int i = 0; i < 10; i++) {
			if (EmailLib.mailReceived(gmailUserName, gmailPassword, crmUser1EmailID, SmokeC2_EmailID, subject,
					emailBody)) {
				log(LogStatus.INFO, "successfully verified email present", YesNo.No);
				flag = true;
				break;
	...
	public void PESmokeTc055_1_CreateDistributionAndVerify(String environment, String mode) {
		...
		String capitalReturn = ExcelUtils.readData(smokeFilePath, "FundDistribution", excelLabel.Variable_Name, "FD1",
				excelLabel.CapitalReturn);
		String totalCapitalCalled = ExcelUtils.readData(smokeFilePath, "FundDrawdown", excelLabel.Variable_Name, "DD1",
				excelLabel.CallAmount);
		String dividends = ExcelUtils.readData(smokeFilePath, "FundDistribution", excelLabel.Variable_Name, "FD1",
				excelLabel.Dividends);
		String realizedGain = ExcelUtils.readData(smokeFilePath, "FundDistribution", excelLabel.Variable_Name, "FD1",
				excelLabel.RealizedGain);
		String OtherProceeds = ExcelUtils.readData(smokeFilePath, "FundDistribution", excelLabel.Variable_Name, "FD1",
				excelLabel.OtherProceeds);
		...
		if (lp.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fp.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				...
				for (int i = 1; i <= 2; i++) {
					ThreadSleep(2000);
					if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
							ShowMoreActionDropDownList.Create_Distribution, 30)) {
				...
				if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
					...
					for (int j = 0; j < linkClick.length; j++) {
						...
						sendKeys(driver, fdi.getCapitalReturn(30), capitalReturn, "capitalReturn",
								action.SCROLLANDBOOLEAN);
						sendKeys(driver, fdi.getDividends(30), dividends, "dividends", action.SCROLLANDBOOLEAN);
						sendKeys(driver, fdi.getRealizedGain(30), realizedGain, "realized gain",
								action.SCROLLANDBOOLEAN);
						sendKeys(driver, fdi.getTxtOtherProceeds(30), OtherProceeds, "other proceeds",
								action.SCROLLANDBOOLEAN);
						sendKeys(driver, fdi.getDistributionDate(30), FifteenDayAfterDate, "distribution date",
								action.BOOLEAN);
						if (click(driver, fdi.getSetupInvestorDist(30), "setup investor distribution sbutton",
								action.SCROLLANDBOOLEAN)) {
							...
							if (ele != null) {
								...
								if (click(driver, ele, linkClick[j], action.SCROLLANDBOOLEAN)) {
									...
									if (Mode.Lightning.toString().equalsIgnoreCase(mode)) {
										...
										xpath = "//div[contains(@class,\"slds-page-header\")]//*[contains(text(),\""
												+ linkClick[j] + "\")]";
									} else {
										xpath = "//h2[contains(text(),'" + linkClick[j] + "')]";
									}
		...
		if (lp.clickOnTab(environment, mode, TabName.FundsTab)) {
			if (fp.clickOnCreatedFund(environment, mode, Smoke_Fund1)) {
				...
				if (bp.clickOnShowMoreActionDownArrow(environment, PageName.FundsPage,
					...
					if (fd.getValueInCreateDrawdownsOrDistributionsPage(fields.split("<break>")[1]) != null) {
						if (fd.getValueInCreateDrawdownsOrDistributionsPage(fields.split("<break>")[1])
								.contains("0.0")) {
							log(LogStatus.INFO, "successfully verified total capital called value", YesNo.No);
						} else {
							log(LogStatus.ERROR, "could not verify total capital called value", YesNo.Yes);
					...
					if (getValueFromElementUsingJavaScript(driver, fdi.getCapitalReturn(30), "capital amount value")
							.contains("0.0")) {
					...
					if (getValueFromElementUsingJavaScript(driver, fdi.getDividends(30), "Dividends value")
							.contains("0.0")) {
					...
					if (getValueFromElementUsingJavaScript(driver, fdi.getRealizedGain(30), "RealizedGain value")
							.contains("0.0")) {
					...
					if (getValueFromElementUsingJavaScript(driver, fdi.getTxtOtherProceeds(30), "OtherProceeds value")
							.contains("0.0")) {
					...
					if (!isSelected(driver, fdi.getCanCheckRecalled(30), "can capital be recalled checkbox"))
						log(LogStatus.INFO, "successfully found can capital be recalled checkbox as unchecked",
								YesNo.No);
					else {
						log(LogStatus.ERROR, "checkbox for capital to be recalled is checked but it should not be",
								YesNo.Yes);
					...
					click(driver, fdi.getCanCheckRecalled(30), "can capital be recalled checkbox",
							action.SCROLLANDBOOLEAN);

					sendKeys(driver, fdi.getDistributionDate(30), FifteenDayAfterDate, "distribution date",
							action.BOOLEAN);
					// verify values present in capitalReturn, dividends, realizedGain and
					// OtherProceeds
					String a = getValueFromElementUsingJavaScript(driver, fdi.getCapitalReturn(30),
							"value textbox CapitalReturn");
					if (a.trim().contains(capitalReturn)) {
						log(LogStatus.INFO, "successfully verified CapitalReturn textbox value", YesNo.No);
					} else {
						log(LogStatus.ERROR,
								"could not verify CapitalReturn textbox value" + a + " and " + capitalReturn,
								YesNo.Yes);
						sa.assertTrue(false, "could not verify CapitalReturn textbox value");
					}
					...
					if (a.trim().contains(dividends)) {
						...
						log(LogStatus.ERROR, "could not verify Dividends textbox value" + a + " and " + dividends,
								YesNo.Yes);
						sa.assertTrue(false, "could not verify Dividends textbox value");
					}
					...
					if (a.trim().contains(realizedGain)) {
						...
						log(LogStatus.ERROR, "could not verify RealizedGain textbox value" + a + " and " + realizedGain,
								YesNo.Yes);
						sa.assertTrue(false, "could not verify RealizedGain textbox value");
					}
					a = getValueFromElementUsingJavaScript(driver, fdi.getTxtOtherProceeds(30),
							"value textbox OtherProceeds");
					if (a.trim().contains(OtherProceeds)) {
						log(LogStatus.INFO, "successfully verified OtherProceeds textbox value", YesNo.No);
					} else {
						log(LogStatus.ERROR,
								"could not verify OtherProceeds textbox value" + a + " and " + OtherProceeds,
								YesNo.Yes);
						sa.assertTrue(false, "could not verify OtherProceeds textbox value");
					}
					String totalDistributions1 = bp.convertNumberAccordingToFormatWithCurrencySymbol(totalDistributions,
							"0,000.00");

					if (getText(driver, fdi.getTotalDistributions(30), "total distribution value", action.BOOLEAN)
							.trim().contains(totalDistributions1)) {
						log(LogStatus.INFO, "call amount verified successfully", YesNo.No);
					} else {
						log(LogStatus.ERROR,
								"call amount value could not be verified" + getText(driver,
										fdi.getTotalDistributions(30), "total distribution value", action.BOOLEAN)
										+ " and " + capitalReturn,
								YesNo.Yes);
						sa.assertTrue(false, "call amount value could not be verified");
					}

					click(driver, fdi.getSetupInvestorDist(30), "setup investor distribution sbutton",
							action.SCROLLANDBOOLEAN);

					String[][] investorDistGrid = { { Smoke_LP1, SmokeCOMM1_ID,
							fdi.getCapitalReturn(SmokeCOMM1_CommitmentAmount, totalCommitments, capitalReturn),
							fdi.getDividends(SmokeCOMM1_CommitmentAmount, totalCommitments, dividends),
							fdi.getRealizedGain(SmokeCOMM1_CommitmentAmount, totalCommitments, realizedGain),
							fdi.getOtherProceeds(SmokeCOMM1_CommitmentAmount, totalCommitments, OtherProceeds),
							fdi.getTotalInvestorDistributions(
									fdi.getCapitalReturn(SmokeCOMM1_CommitmentAmount, totalCommitments, capitalReturn),
									fdi.getDividends(SmokeCOMM1_CommitmentAmount, totalCommitments, dividends),
									fdi.getRealizedGain(SmokeCOMM1_CommitmentAmount, totalCommitments, realizedGain),
									fdi.getOtherProceeds(SmokeCOMM1_CommitmentAmount, totalCommitments,
											OtherProceeds)) },
							{ Smoke_LP2, SmokeCOMM2_ID,
									fdi.getCapitalReturn(SmokeCOMM2_CommitmentAmount, totalCommitments, capitalReturn),
									fdi.getDividends(SmokeCOMM2_CommitmentAmount, totalCommitments, dividends),
									fdi.getRealizedGain(SmokeCOMM2_CommitmentAmount, totalCommitments, realizedGain),
									fdi.getOtherProceeds(SmokeCOMM2_CommitmentAmount, totalCommitments, OtherProceeds),
									fdi.getTotalInvestorDistributions(
											fdi.getCapitalReturn(SmokeCOMM2_CommitmentAmount, totalCommitments,
													capitalReturn),
											fdi.getDividends(SmokeCOMM2_CommitmentAmount, totalCommitments, dividends),
											fdi.getRealizedGain(SmokeCOMM2_CommitmentAmount, totalCommitments,
													realizedGain),
											fdi.getOtherProceeds(SmokeCOMM2_CommitmentAmount, totalCommitments,
													OtherProceeds)) },
							{ Smoke_LP1, SmokeCOMM3_ID,
									fdi.getCapitalReturn(SmokeCOMM3_CommitmentAmount, totalCommitments, capitalReturn),
									fdi.getDividends(SmokeCOMM3_CommitmentAmount, totalCommitments, dividends),
									fdi.getRealizedGain(SmokeCOMM3_CommitmentAmount, totalCommitments, realizedGain),
									fdi.getOtherProceeds(SmokeCOMM3_CommitmentAmount, totalCommitments, OtherProceeds),
									fdi.getTotalInvestorDistributions(
											fdi.getCapitalReturn(SmokeCOMM3_CommitmentAmount, totalCommitments,
													capitalReturn),
											fdi.getDividends(SmokeCOMM3_CommitmentAmount, totalCommitments, dividends),
											fdi.getRealizedGain(SmokeCOMM3_CommitmentAmount, totalCommitments,
													realizedGain),
											fdi.getOtherProceeds(SmokeCOMM3_CommitmentAmount, totalCommitments,
													OtherProceeds)) },
							{ Smoke_LP3, SmokeCOMM4_ID,
									fdi.getCapitalReturn(SmokeCOMM4_CommitmentAmount, totalCommitments, capitalReturn),
									fdi.getDividends(SmokeCOMM4_CommitmentAmount, totalCommitments, dividends),
									fdi.getRealizedGain(SmokeCOMM4_CommitmentAmount, totalCommitments, realizedGain),
									fdi.getOtherProceeds(SmokeCOMM4_CommitmentAmount, totalCommitments, OtherProceeds),
									fdi.getTotalInvestorDistributions(
											fdi.getCapitalReturn(SmokeCOMM4_CommitmentAmount, totalCommitments,
													capitalReturn),
											fdi.getDividends(SmokeCOMM4_CommitmentAmount, totalCommitments, dividends),
											fdi.getRealizedGain(SmokeCOMM4_CommitmentAmount, totalCommitments,
													realizedGain),
											fdi.getOtherProceeds(SmokeCOMM4_CommitmentAmount, totalCommitments,
													OtherProceeds)) },
							{ Smoke_LP4, SmokeCOMM5_ID,
									fdi.getCapitalReturn(SmokeCOMM5_CommitmentAmount, totalCommitments, capitalReturn),
									fdi.getDividends(SmokeCOMM5_CommitmentAmount, totalCommitments, dividends),
									fdi.getRealizedGain(SmokeCOMM5_CommitmentAmount, totalCommitments, realizedGain),
									fdi.getOtherProceeds(SmokeCOMM5_CommitmentAmount, totalCommitments, OtherProceeds),
									fdi.getTotalInvestorDistributions(
											fdi.getCapitalReturn(SmokeCOMM5_CommitmentAmount, totalCommitments,
													capitalReturn),
											fdi.getDividends(SmokeCOMM5_CommitmentAmount, totalCommitments, dividends),
											fdi.getRealizedGain(SmokeCOMM5_CommitmentAmount, totalCommitments,
													realizedGain),
											fdi.getOtherProceeds(SmokeCOMM5_CommitmentAmount, totalCommitments,
													OtherProceeds)) } };
					...
					List<WebElement> commitmentAmounts = fd.getCommitmentAmountListFromInvestorDistribution(environment,
							mode);
					...
					if (!commitments.isEmpty()) {
						...
						if (commitments.size() == commitmentAmounts.size()) {
							...
							for (int i = 0; i < commitments.size(); i++) {
								...
								commitName = commitments.get(i).getText().trim();
								ExcelUtils.writeData(smokeFilePath, commitName, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.Commitment_ID);
								System.err.println(commitName);
								amount = commitmentAmounts.get(i).getText().trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.Commitment_Amount);
								System.err.println(amount);
								amount = capitalReturns.get(i).getAttribute("value").trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.CapitalReturn);
								System.err.println(amount);
								amount = dividend.get(i).getAttribute("value").trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0.000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.Dividends);
								System.err.println(amount);
								amount = realisedGains.get(i).getAttribute("value").trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0.000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.RealizedGain);
								System.err.println(amount);
								amount = otherProceeds.get(i).getAttribute("value").trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0.000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.OtherProceeds);
								System.err.println(amount);
								amount = totalDistribution.get(i).getText().trim();
								amount = fd.convertNumberAccordingToFormatWithoutCurrencySymbol(amount, "0.000");
								ExcelUtils.writeData(smokeFilePath, amount, "InvestorDistribution",
										excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.TotalDistributions);
								System.err.println(amount);
							}
					...
					if (click(driver, fdi.getGenerateInvDist(30), "generate inv dist", action.SCROLLANDBOOLEAN)) {
						...
						if (bp.openAppFromAppLauchner(ObjectName.Fund_Distributions.toString(), 20)) {
							...
							FDID = fp.getFirstValueFromRelatedList(environment, mode,
									RelatedList.FundDistribution.toString(), Smoke_Fund1).getText().trim();
						...
						if (FDID != null) {
							if (clickUsingJavaScript(driver,
									fp.getFirstValueFromRelatedList(environment, mode,
											RelatedList.FundDistribution.toString(), Smoke_Fund1),
									"FundDistribution id", action.SCROLLANDBOOLEAN)) {

								ExcelUtils.writeData(smokeFilePath, FDID, "FundDistribution", excelLabel.Variable_Name,
										"FD1", excelLabel.FundDistributionID);
								for (int i = 0; i < 5; i++) {
									ExcelUtils.writeData(smokeFilePath, FDID, "InvestorDistribution",
											excelLabel.Variable_Name, "ID" + (i + 1), excelLabel.FundDistributionID);
								...
								if (bp.scrollToRelatedListViewAll_Lightning(environment, mode,
										RelatedList.InvestorDistributions, true)) {
									log(LogStatus.INFO, "succesfully scrolled to InvestorDistributions related listi",
											YesNo.No);
								} else {
									log(LogStatus.ERROR, "coould not scroll to InvestorDistributions related list",
											YesNo.Yes);
									sa.assertTrue(false, "coould not scroll to InvestorDistributions related list");
								}

								if (bp.clickOnViewAllRelatedList(environment, mode,
										RelatedList.InvestorDistributions)) {

								} else {
									log(LogStatus.ERROR, "could not click on view all of related list of "
											+ RelatedList.InvestorDistributions.toString(), YesNo.Yes);
									sa.assertTrue(false, "could not click on view all of related list of "
											+ RelatedList.InvestorDistributions.toString());
								}
								List<String> s = fp.returnAllValuesInRelatedList(environment, mode,
										RelatedList.InvestorDistributions.toString());
								if (s.size() > 0) {
									for (int i = 0; i < 5; i++) {
										...
										ExcelUtils.writeData(smokeFilePath, s.get(i), "InvestorDistribution",
												excelLabel.Variable_Name, "ID" + (i + 1),
												excelLabel.InvestorDistributionID);
									}
								} else {
									log(LogStatus.ERROR, "Either Id's not present or locator not correct", YesNo.Yes);
									sa.assertTrue(false, "Either Id's not present or locator not correct");
								}
							} else {
							...
							log(LogStatus.ERROR, "fund distribution link is not present so cannot conitnue " + FDID,
									YesNo.Yes);
	...
	public void PESmokeTc056_VerifyCreatedInvestorDistribution(String environment, String mode) {
		...
		SmokeID1Data[0] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.InvestorDistributionID);
		SmokeID1Data[1] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.FundDistributionID);
		SmokeID1Data[2] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.Commitment_ID);
		SmokeID1Data[3] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.CapitalReturn);
		SmokeID1Data[4] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.Dividends);
		SmokeID1Data[5] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.RealizedGain);
		SmokeID1Data[6] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.OtherProceeds);
		SmokeID1Data[7] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID1",
				excelLabel.TotalDistributions);

		SmokeID2Data[0] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.InvestorDistributionID);
		SmokeID2Data[1] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.FundDistributionID);
		SmokeID2Data[2] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.Commitment_ID);
		SmokeID2Data[3] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.CapitalReturn);
		SmokeID2Data[4] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.Dividends);
		SmokeID2Data[5] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.RealizedGain);
		SmokeID2Data[6] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.OtherProceeds);
		SmokeID2Data[7] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID2",
				excelLabel.TotalDistributions);

		SmokeID3Data[0] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.InvestorDistributionID);
		SmokeID3Data[1] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.FundDistributionID);
		SmokeID3Data[2] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.Commitment_ID);
		SmokeID3Data[3] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.CapitalReturn);
		SmokeID3Data[4] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.Dividends);
		SmokeID3Data[5] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.RealizedGain);
		SmokeID3Data[6] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.OtherProceeds);
		SmokeID3Data[7] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID3",
				excelLabel.TotalDistributions);

		SmokeID4Data[0] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.InvestorDistributionID);
		SmokeID4Data[1] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.FundDistributionID);
		SmokeID4Data[2] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.Commitment_ID);
		SmokeID4Data[3] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.CapitalReturn);
		SmokeID4Data[4] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.Dividends);
		SmokeID4Data[5] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.RealizedGain);
		SmokeID4Data[6] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.OtherProceeds);
		SmokeID4Data[7] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID4",
				excelLabel.TotalDistributions);

		SmokeID5Data[0] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.InvestorDistributionID);
		SmokeID5Data[1] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.FundDistributionID);
		SmokeID5Data[2] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.Commitment_ID);
		SmokeID5Data[3] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.CapitalReturn);
		SmokeID5Data[4] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.Dividends);
		SmokeID5Data[5] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.RealizedGain);
		SmokeID5Data[6] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.OtherProceeds);
		SmokeID5Data[7] = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Variable_Name, "ID5",
				excelLabel.TotalDistributions);
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeID1ID)) {
				for (int i = 0; i < 8; i++) {
					if (id.fieldValueVerificationOnInvestorDistribution(environment, mode,
							TabName.InvestorDistributions, labels[i], SmokeID1Data[i])) {
				...
				if (mode.equalsIgnoreCase(Mode.Classic.toString())) {
					if (id.getCapitalRecallableValue(mode, 30).getAttribute("title").contains("Not Checked")) {
						log(LogStatus.ERROR, "checkbox for capital recallabel checkbox is unchecked but it should be",
								YesNo.Yes);
					...
					try {
						if (id.getCapitalRecallableValue(mode, 30).getAttribute("class").contains("unchecked")) {
							log(LogStatus.ERROR,
									"checkbox for capital recallabel checkbox is unchecked but it should be",
									YesNo.Yes);
							sa.assertTrue(false,
									"checkbox for capital recallabel checkbox is unchecked but it should be");
						} else {
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeID2ID)) {
				...
				for (int i = 0; i < 8; i++) {
					if (id.fieldValueVerificationOnInvestorDistribution(environment, mode,
							TabName.InvestorDistributions, labels[i], SmokeID2Data[i])) {
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeID3ID)) {
				...
				for (int i = 0; i < 8; i++) {
					if (id.fieldValueVerificationOnInvestorDistribution(environment, mode,
							TabName.InvestorDistributions, labels[i], SmokeID3Data[i])) {
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeID4ID)) {
				...
				for (int i = 0; i < 8; i++) {
					if (id.fieldValueVerificationOnInvestorDistribution(environment, mode,
							TabName.InvestorDistributions, labels[i], SmokeID4Data[i])) {
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (ccp.clickOnCreatedCapitalCall(environment, mode, SmokeID5ID)) {
				...
				for (int i = 0; i < 8; i++) {
					if (id.fieldValueVerificationOnInvestorDistribution(environment, mode,
							TabName.InvestorDistributions, labels[i], SmokeID5Data[i])) {
	...
	public void PESmokeTc057_VerifyCommitmentPageAfterDistributions(String environment, String mode) {
		...
		String labelName = CommitmentPageFieldLabelText.Commitment_ID + "," + CommitmentPageFieldLabelText.Partner_Type
				...
				+ CommitmentPageFieldLabelText.Total_Commitment_Due + ","
				+ CommitmentPageFieldLabelText.Commitment_Called + "," + CommitmentPageFieldLabelText.Called_Due + ","
				+ CommitmentPageFieldLabelText.Total_Distributions + ","
				+ CommitmentPageFieldLabelText.Capital_Returned_Recallable + ","
		...
		String labelValue = SmokeCOMM8_ID + "," + SmokeCOMM8_partnerType + "," + Smoke_LP1 + ","
				...
				+ SmokeCOMM8_TotalAmountReceived + "," + SmokeCOMM8_TotalUncalledAmount + ","
				+ SmokeCOMM8_TotalCommitmentDue + "," + SmokeCOMM8_CommitmentCalled + "," + SmokeCOMM8_CalledDue + ","
				+ SmokeCOMM8_TotalDist + "," + SmokeCOMM8_CapitalReturnedRecallable + ","
				...
				if (fd.clickOnCreatedDrawdown(environment, mode, SmokeFD1ID)) {
					...
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						if (bp.clickOnShowMoreDropdownOnly(environment, mode, PageName.FundDistribution)) {
							log(LogStatus.PASS, "successfully clicked on dropdown", YesNo.No);
							WebElement ele = bp.actionDropdownElement(environment, PageName.FundDistribution,
									ShowMoreActionDropDownList.Send_Distribution_Notices, 30);
							if (ele != null) {
								log(LogStatus.PASS, "successfully found send capital call notices button", YesNo.No);
								if (click(driver,
										bp.actionDropdownElement(environment, PageName.FundDistribution,
												ShowMoreActionDropDownList.Send_Distribution_Notices, 30),
										ShowMoreActionDropDownList.Send_Distribution_Notices.toString(),
										action.BOOLEAN)) {
									log(LogStatus.PASS, "successfully click on send button", YesNo.No);
								} else {
									log(LogStatus.ERROR, "cannot click on send button", YesNo.Yes);
						...
						if (fdd.getSendDistributionNotices_Classic(30) != null) {
							log(LogStatus.PASS, "send capital call notices button is successfully present", YesNo.No);
							if (click(driver, fdd.getSendDistributionNotices_Classic(30),
									"send capital call notices button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "successfully click on send capital call button", YesNo.No);
							} else {
								log(LogStatus.ERROR, "could not click on send button", YesNo.Yes);
					...
					// List<String> result
					// =market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(PageName.Send_Distribution_Notices,ContactAndAccountName,
					// searchContactInEmailProspectGrid.No);
					if (j == 0) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
								SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, 30)) {
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							for (int i = 0; i <= 3; i++) {
								if (i == 0) {
									if (click(driver, market.getStep1NextBtn(PageName.Send_Distribution_Notices,
											TopOrBottom.TOP, 10), "Top Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on top next button", YesNo.Yes);
								...
								if (i == 1) {
									if (click(driver,
											market.getStep2PreviousBtn(PageName.Send_Distribution_Notices,
													TopOrBottom.TOP, 10),
											"Top previous button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top previous button", YesNo.No);
									} else {
								...
								if (i == 2) {
									if (click(driver,
											market.getStep1NextBtn(PageName.Send_Distribution_Notices,
													TopOrBottom.BOTTOM, 10),
											"bottom Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
									} else {
					...
					if (j == 1) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1CancelBtn(PageName.Send_Distribution_Notices, TopOrBottom.TOP, 10),
									"Top cancel button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on top cancel button step 1", YesNo.No);
							} else {
								log(LogStatus.ERROR, "Not able to click on top cancel button step 1", YesNo.Yes);
					...
					if (j == 3) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1NextBtn(PageName.Send_Distribution_Notices, TopOrBottom.BOTTOM, 10),
									"bottom Next button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
								ThreadSleep(2000);
								if (click(driver, market.getStep2CancelBtn(PageName.Send_Distribution_Notices,
										TopOrBottom.TOP, 10), "top cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on top cancel button step 2", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not able to click on top cancel button step 2", YesNo.Yes);
									sa.assertTrue(false, "Not able to click on top cancel button step 2");
					...
					if (j == 4) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
							...
							if (click(driver,
								...
								ThreadSleep(2000);
								if (click(
										driver, market.getStep2CancelBtn(PageName.Send_Distribution_Notices,
												TopOrBottom.BOTTOM, 10),
										"bottom cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on bottom cancel button step 2", YesNo.No);
								} else {
					...
					if (j == 5) {
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
							...
							log(LogStatus.ERROR, "select contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.No);
							if (click(driver,
									market.getStep1NextBtn(PageName.Send_Distribution_Notices, TopOrBottom.TOP, 10),
									"Top Next button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on top next button step 1", YesNo.No);
								ThreadSleep(2000);
								for (int i = 0; i <= 3; i++) {
									if (i == 0) {
										if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
											log(LogStatus.ERROR, "Select Capital Call Notice template", YesNo.No);
											if (click(driver,
													market.getStep2NextBtn(PageName.Send_Distribution_Notices,
															TopOrBottom.TOP, 10),
													"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
											} else {
									...
									if (i == 1) {
										if (click(driver,
												market.getStep3PreviousBtn(PageName.Send_Distribution_Notices,
														TopOrBottom.TOP, 10),
												"Top previous button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top previous button step 3", YesNo.No);
										} else {
						...
						if (fd.ScrollAndClickOnContactNameCheckBoxInFundDrawdown(PageName.Send_Distribution_Notices,
							...
							if (click(driver,
								...
								if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
									log(LogStatus.ERROR, "Select Capital Call Notice template", YesNo.No);
									if (click(
											driver, market.getStep2NextBtn(PageName.Send_Distribution_Notices,
													TopOrBottom.TOP, 10),
											"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
										ThreadSleep(2000);
										if (click(driver,
												market.getStep3CancelBtn(PageName.Send_Distribution_Notices,
														TopOrBottom.TOP, 10),
												"top cancel button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top cancel button step 3", YesNo.No);
										} else {
					...
					log(LogStatus.ERROR, "Not able to click on Draw Down " + SmokeFD1ID
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName, YesNo.Yes);
					sa.assertTrue(false, "Not able to click on Draw Down " + SmokeFD1ID
							+ " so cannot send email to contact " + SmokeC1_FName + " " + SmokeC1_LName);
				}
			} else {
				log(LogStatus.ERROR, "Not able to click on fund Draw Down tab so cannot send email to contact "
		...
		if (bp.clickOnTab(environment, mode, TabName.FundDistributions)) {
			if (fd.clickOnCreatedDrawdown(environment, mode, SmokeFD1ID)) {
				if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
					if (bp.clickOnShowMoreDropdownOnly(environment, mode, PageName.FundDistribution)) {
						log(LogStatus.PASS, "successfully clicked on dropdown", YesNo.No);
						ele = bp.actionDropdownElement(environment, PageName.FundDistribution,
								ShowMoreActionDropDownList.Send_Distribution_Notices, 30);
						if (ele != null) {
							...
							if (click(driver,
									bp.actionDropdownElement(environment, PageName.FundDistribution,
											ShowMoreActionDropDownList.Send_Distribution_Notices, 30),
									ShowMoreActionDropDownList.Send_Distribution_Notices_Button.toString(),
									action.BOOLEAN)) {
								log(LogStatus.PASS, "successfully click on send button", YesNo.No);
							} else {
								log(LogStatus.FAIL, "cannot click on send button", YesNo.Yes);
					...
					if (fdi.getSendDistributionNotices_Classic(30) != null) {
						...
						if (click(driver, fdi.getSendDistributionNotices_Classic(30),
								"send capital call notices button", action.SCROLLANDBOOLEAN)) {
				...
				// verify mailing grid
				SmokeID1ID = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Commitment_ID,
						SmokeCOMM8_ID, excelLabel.InvestorDistributionID);
				saa = fdi.verifyEmailingContentGrid(driver, PageName.FundDistribution,
						SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, SmokeC1_EmailID, SmokeID1ID, SmokeCOMM8_ID,
						Smoke_LP1, Smoke_P1, false);
				sa.combineAssertions(saa);
				SmokeID2ID = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Commitment_ID,
						SmokeCOMM2_ID, excelLabel.InvestorDistributionID);
				saa = fdi.verifyEmailingContentGrid(driver, PageName.FundDistribution,
						SmokeC2_FName + " " + SmokeC2_LName, SmokeINS1, SmokeC2_EmailID, SmokeID2ID, SmokeCOMM2_ID,
						Smoke_LP2, Smoke_P1, false);
				sa.combineAssertions(saa);
				List<WebElement> li = fd.listOfContactNames(30);
				boolean flag = false;
				...
				if (click(driver, fd.getWrenchIcon(30), "wrench icon", action.SCROLLANDBOOLEAN)) {
					...
					if (ele != null) {
						...
						if (msg.contains("Selected Fields (7/20)")) {
							...
							log(LogStatus.ERROR, "Message Not verified : Actual : " + msg + " \t Expected : "
									+ "Selected Fields (7/20)", YesNo.Yes);
							sa.assertTrue(false, "Message Not verified : Actual : " + msg + " \t Expected : "
									+ "Selected Fields (7/20)");
						}
					} else {
					...
					if (click(driver, fd.getApplyColumnsToDisplay2(30), "Apply button", action.BOOLEAN)) {
							...
							if (ele != null) {
								...
								if (click(driver, ele, linkClick[j], action.BOOLEAN)) {
									...
									if (parentId != null) {
										...
										ele = FindElement(driver, xpath, "on new window : " + linkClick[j],
												action.BOOLEAN, 15);
						...
						if (sendKeys(driver, fd.getSearchTextboxEmailingGrid(30), SmokeC1_LName, "search box",
							...
							if (click(driver, fd.getSearchIconEmailingGrid(30), "search icon", action.BOOLEAN)) {
								((JavascriptExecutor) driver).executeScript("arguments[0].scrollTo( 0 ,0)",
										fd.getScrollBoxStep1FundDrawdownEmailingGrid(30));
								SmokeID1ID = ExcelUtils.readData(smokeFilePath, "InvestorDistribution",
										excelLabel.Commitment_ID, SmokeCOMM8_ID, excelLabel.InvestorDistributionID);
								saa = fd.verifyEmailingContentGrid(driver, PageName.FundDrawdown,
										SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, SmokeC1_EmailID,
										SmokeC1_FName + " " + SmokeC1_LName, SmokeCOMM8_ID, Smoke_LP1, Smoke_P1, false);
								sa.combineAssertions(saa);

								if (fd.getRecordsCount(30).getText().trim().contains("Records: " + n)) {
									log(LogStatus.INFO, "successfully verified number of records : " + n, YesNo.No);
								} else {
									log(LogStatus.ERROR, "could not verify correct number of records : " + n,
											YesNo.Yes);
				...
				if (flag) {
					if (click(driver, fd.getStep1HeaderCheckbox(30), "header checkbox", action.SCROLLANDBOOLEAN)) {
						...
						if (click(driver, fd.getStep1NextButtonEmailing(30), "next button", action.SCROLLANDBOOLEAN)) {
							...
							if (selectVisibleTextFromDropDown(driver, fd.getFundDrawdownFolderDropDownList(20),
									"folder dropdown", expectedResult.split(",")[2])) {
								log(LogStatus.INFO, " Able to Select " + expectedResult.split(",")[2], YesNo.No);

								if (fd.clickOnEmailTemplatePreviewLink(template)) {
									log(LogStatus.INFO, "successfully verified email preview link for " + template,
											YesNo.No);
								} else {
							...
							if (fd.selectEmailTemplateFromFundDrawdown(expectedResult.split(",")[2], template)) {
								log(LogStatus.INFO, "successfully selected template : " + expectedResult.split(",")[2],
										YesNo.No);
								if (click(driver, fd.getStep2NextButtonEmailing(30), "step 2 next button",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Clicked on step 2 next button", YesNo.No);
										...
										if (labellist.get(j).getText().trim().contains(expctRsult[j])) {
											...
											if (j == 2) {
												if (("true").equalsIgnoreCase(checkBoxAttr)) {
													log(LogStatus.INFO, expctRsult[j] + "check box is selected",
															YesNo.No);
												} else {
													sa.assertTrue(false, expctRsult[j] + "check box is not selected");
													log(LogStatus.ERROR, expctRsult[j] + "check box is not selected",
															YesNo.Yes);
												}
											} else {

												if (checkBoxAttr == null) {
													log(LogStatus.INFO, expctRsult[j] + "check box is not selected",
															YesNo.No);
												} else {
													sa.assertTrue(false, expctRsult[j] + "check box is selected");
													log(LogStatus.ERROR, expctRsult[j] + "check box is selected",
															YesNo.Yes);
												}
											}
									...
									if (click(driver, ele, "Use My Signature ", action.BOOLEAN)) {
										...
										log(LogStatus.ERROR, "Not ABle to Click on Use My Signature CheckBox",
												YesNo.Yes);
									}
									...
									if (click(driver, fd.getfundDrawdownEmailingSendBtn(TopOrBottom.BOTTOM, 30),
										...
										if (ele != null) {
											...
											if (aa1.contains(FundDrawdownPageErrorMessage.congratulationErrorMsg)
													&& aa1.contains(FundDrawdownPageErrorMessage
															.generatedEmailedRecipientErrorMsg("1", "Email"))) {
												log(LogStatus.INFO, "Congratulation Error Message is verified",
														YesNo.No);
											} else {
												log(LogStatus.ERROR,
														"Congratulation Error Message is not verified. Expected: "
								...
								log(LogStatus.ERROR,
										"could not select email template : " + expectedResult.split(",")[2], YesNo.Yes);
								sa.assertTrue(false,
										"could not select email template : " + expectedResult.split(",")[2]);
							}
						} else {
							log(LogStatus.ERROR, "step 1 next button is not clickable", YesNo.Yes);
							sa.assertTrue(false, "step 1 next button is not clickable");
	...
	public void PESmokeTc059_verifyReceivedEmailAndCreatedActivityForFundDistribution(String environment, String mode) {
		...
		try {
			...
			// TODO Auto-generated catch block
			sa.assertTrue(false,
					EmailTemplate1_Subject + " Email Template is not verified from email : " + gmailUserName);
			log(LogStatus.FAIL,
					EmailTemplate1_Subject + " Email Template is not verified from email : " + gmailUserName, YesNo.No);
			e.printStackTrace();
		}
		if (text != null) {
		...
		if (bp.clickOnTab(environment, mode, TabName.FundDistributions)) {
			if (fd.clickOnCreatedDrawdown(environment, mode, SmokeFD1ID)) {
				...
				SmokeID2ID = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Commitment_ID,
						SmokeCOMM2_ID, excelLabel.InvestorDistributionID);
				SoftAssert saa = fdi.verifyEmailingContentGrid(driver, PageName.FundDistribution,
						SmokeC1_FName + " " + SmokeC1_LName, SmokeINS1, SmokeC1_EmailID, SmokeID1ID, SmokeCOMM8_ID,
						Smoke_LP1, Smoke_P1, true);
				sa.combineAssertions(saa);
					...
					if (bp.clickOnShowMoreDropdownOnly(environment, mode, PageName.FundDistribution)) {
						...
						if (ele != null) {
							...
							if (click(driver,
									bp.actionDropdownElement(environment, PageName.FundDistribution,
											ShowMoreActionDropDownList.Send_Distribution_Notices, 30),
									ShowMoreActionDropDownList.Send_Distribution_Notices.toString(), action.BOOLEAN)) {
								log(LogStatus.PASS, "successfully click on send button", YesNo.No);
							} else {
								log(LogStatus.FAIL, "cannot click on send button", YesNo.Yes);
								sa.assertTrue(false, "could not click on send button");
					...
					if (j == 1 || j == 2) {
						...
						if (click(driver, fd.getWrenchIcon(30), "wrench icon", action.SCROLLANDBOOLEAN)) {
							...
							if (j == 1) {
								...
								if (selectVisibleTextFromDropDown(driver, market
										.getColumnToDisplayViewDropDownList(PageName.Send_Distribution_Notices, 30),
										"view drop down list", ViewFieldsName.Contact_Fields)) {
									log(LogStatus.INFO,
											"Select value from view drop down list : " + ViewFieldsName.Contact_Fields,
											YesNo.Yes);
								...
								if (click(driver, fdd.getCancelButtonOnColumnsToDisplay(10), "Cancel Button",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Clicked on Cancel Button", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not Able to Click on Cancel Button", YesNo.Yes);
								...
								if (click(driver, fdd.getCrossIconOnColumnsToDisplay(10), "Cross Icon",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Clicked on Cross Icon", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not Able to Click on Cross Icon", YesNo.Yes);
						...
						String[] ss = { value };
						if (market.addAndRemoveCloumnInSelectProspectGrid(mode, PageName.Send_Distribution_Notices, ss,
								null, null, true).isEmpty()) {
							log(LogStatus.PASS, "column added form column to display popup", YesNo.No);
							ThreadSleep(2000);
							if (compareMultipleList(driver, value,
									market.getSelectProspectsHeaderTextList(PageName.Send_Distribution_Notices))
									.isEmpty()) {
								log(LogStatus.PASS, "Selected Prospects Header Text is verified ", YesNo.No);
							} else {
							...
							ThreadSleep(2000);
							if (market.columnToDisplayRevertToDefaultsSettings(PageName.Send_Distribution_Notices,
									mode)) {
								log(LogStatus.PASS, "column to display settings is revert to default successfully",
										YesNo.No);
							} else {
	...
	public void PESmokeTc060_2_VerifySendDistributionNoticesFromInvestorDistribution(String environment, String mode) {
		...
		SmokeID2ID = ExcelUtils.readData(smokeFilePath, "InvestorDistribution", excelLabel.Commitment_ID, SmokeCOMM2_ID,
				excelLabel.InvestorDistributionID);
		SoftAssert saa = new SoftAssert();
		int n = 1;
		WebElement ele;
		...
		if (bp.clickOnTab(environment, mode, TabName.InvestorDistributions)) {
			if (fd.clickOnCreatedDrawdown(environment, mode, SmokeID2ID)) {
				...
				if (bp.clickOnShowMoreActionDownArrow(environment, PageName.InvestorDistribution,
						ShowMoreActionDropDownList.Send_Distribution_Notices, 30)) {
					...
					if (ele != null) {
						...
						msg = ele.getText().trim();
						String s = "Emailing Investor Distribution Notice(s) of Investor Distribution ID " + SmokeID2ID
								+ " for Fund - " + Smoke_Fund1;

						if (msg.contains(s)) {
							log(LogStatus.INFO, "Message verified " + msg, YesNo.Yes);
						} else {
							log(LogStatus.ERROR, "Message Not verified : Actual : " + msg + " \t Expected : " + s,
									YesNo.Yes);
					...
					if (click(driver, fd.getStep1NextButtonEmailing(30), "next button", action.SCROLLANDBOOLEAN)) {
						...
						if (ele != null) {
							...
							if (HomePageErrorMessage.Step2_SelectAnEmailTemplate.equals(msg)) {
								...
								sa.assertTrue(false, "Step2 Page Verified Not Verified Actual : " + msg
										+ " \t Expected :" + HomePageErrorMessage.Step2_SelectAnEmailTemplate);
								log(LogStatus.FAIL, "Step2 Page Verified Not Verified Actual : " + msg
										+ " \t Expected :" + HomePageErrorMessage.Step2_SelectAnEmailTemplate,
										YesNo.Yes);
							}
						} else {
						...
						if (fd.selectEmailTemplateFromFundDrawdown(EmailTemplate1_FolderName,
								EmailTemplate1_TemplateName)) {
							log(LogStatus.INFO, "successfully selected template : " + EmailTemplate1_TemplateName,
									YesNo.No);
							if (click(driver, fd.getStep2NextButtonEmailing(30), "step 2 next button",
									action.BOOLEAN)) {
								log(LogStatus.INFO, "Clicked on step 2 next button", YesNo.No);
								...
								if (ele != null) {
									...
									if (HomePageErrorMessage.step3_ReviewAndConfirm.equals(msg)) {
										...
										sa.assertTrue(false, "Step3 Page Verified Not Verified Actual : " + msg
												+ " \t Expected :" + HomePageErrorMessage.step3_ReviewAndConfirm);
										log(LogStatus.FAIL, "Step3 Page Verified Not Verified Actual : " + msg
												+ " \t Expected :" + HomePageErrorMessage.step3_ReviewAndConfirm,
												YesNo.Yes);
									}
								} else {
							...
							log(LogStatus.ERROR, "could not select email template : " + EmailTemplate1_TemplateName,
									YesNo.Yes);
	...
	@Parameters({ "environment", "mode" })
	@Test
	public void PESmokeTc061_verifyReceivedEmailAndCreatedActivityForInvestorDistribution(String environment,
			String mode) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		ContactsPageBusinessLayer cp = new ContactsPageBusinessLayer(driver);
		SoftAssert tsa;
	...
	public void PESmokeTc062_VerifyContactTransferButtonAtContactPageAndOfficeLocationRelatedListAtAccountPage(
		...
		if (bp.clickOnTab(environment, mode, TabName.ContactTab)) {
			if (cp.clickOnCreatedContact(environment, SmokeC5_FName, SmokeC5_LName)) {
				...
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.ContactPage, ShowMoreActionDropDownList.Edit,
						20)) {
					log(LogStatus.INFO, "Clicked on Edit Button", YesNo.No);
		...
		if (ip.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			...
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINS2)) {
				...
				if (cp.clickOnRelatedList(environment, mode, RecordType.IndividualInvestor,
						RelatedList.Office_Locations, RelatedTab.Details.toString())) {
		...
		if (bp.clickOnTab(environment, mode, TabName.NavatarSetup)) {
			...
			if (ctt.clickOnNavatarSetupSideMenusTab(environment, NavatarSetupSideMenuTab.ContactTransfer)) {
				...
				if (click(driver, ctt.getEditButtonforNavatarSetUpSideMenuTab(environment,
					...
					if (click(driver,
							ctt.getCancelButtonforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.ContactTransfer, 20),
							"cancel button", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.INFO, "click on cancel button", YesNo.No);
					} else {
						log(LogStatus.ERROR, "Not able to click on cancel button so cannot close co investment pop up",
								YesNo.Yes);
				...
				if (click(driver, ctt.getEditButtonforNavatarSetUpSideMenuTab(environment,
					...
					if (!isSelected(driver,
							ctt.getEnableCheckBoxforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.ContactTransfer, EditViewMode.Edit,
									ClickOrCheckEnableDisableCheckBox.Click, 10),
							"Enabled CheckBox")) {
					...
					if (click(driver,
							ctt.getSaveButtonforNavatarSetUpSideMenuTab(environment,
									NavatarSetupSideMenuTab.ContactTransfer, 10, TopOrBottom.TOP),
	...
	public void PESmokeTc064_VerifyOfficeLocationSetting(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.NavatarSetup)) {
			...
			if (ctt.clickOnNavatarSetupSideMenusTab(environment, NavatarSetupSideMenuTab.OfficeLocations)) {
				...
				if (!isSelected(driver,
						ctt.getEnableCheckBoxforNavatarSetUpSideMenuTab(environment, mode,
								NavatarSetupSideMenuTab.OfficeLocations, EditViewMode.View,
								ClickOrCheckEnableDisableCheckBox.Click, 10),
						"Enabled CheckBox")) {
				...
				if (click(driver, ctt.getEditButtonforNavatarSetUpSideMenuTab(environment,
					...
					if (click(driver,
							ctt.getCancelButtonforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.OfficeLocations, 20),
							"cancel button", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.INFO, "click on cancel button", YesNo.No);
					} else {
						log(LogStatus.ERROR, "Not able to click on cancel button so cannot close co investment pop up",
				...
				if (click(driver, ctt.getEditButtonforNavatarSetUpSideMenuTab(environment,
					...
					if (!isSelected(driver,
							ctt.getEnableCheckBoxforNavatarSetUpSideMenuTab(environment, mode,
									NavatarSetupSideMenuTab.OfficeLocations, EditViewMode.Edit,
									ClickOrCheckEnableDisableCheckBox.Click, 10),
							"Enabled CheckBox")) {
						log(LogStatus.INFO, "Enable Office Locations is Unchecked", YesNo.No);
						...
						if (click(driver,
							...
							if (click(driver,
									ctt.getSaveButtonforNavatarSetUpSideMenuTab(environment,
											NavatarSetupSideMenuTab.OfficeLocations, 10, TopOrBottom.TOP),
									"Save Button", action.BOOLEAN)) {
								ThreadSleep(10000);
	...
	public void PESmokeTc066_CreateOfficeLocationsForInstitutionContactTransferAcc1(String environment, String mode) {
		...
		String[] label = { excelLabel.Office_Location_Name.toString(), excelLabel.Street.toString(),
				excelLabel.City.toString(), "State/Province", excelLabel.Country.toString(),
				excelLabel.Primary.toString() };

		log(LogStatus.INFO, "Going to Verify Office Location 1 & 2", YesNo.No);

		String[][] locations = {
				{ Smoke_OFFLoc1Name, Smoke_OFFLoc1Street, Smoke_OFFLoc1City, Smoke_OFFLoc1StateProvince,
						Smoke_OFFLoc1Country, Smoke_OFFLoc1Primary },
				{ Smoke_OFFLoc2Name, Smoke_OFFLoc2Street, Smoke_OFFLoc2City, Smoke_OFFLoc2StateProvince,
						Smoke_OFFLoc2Country, Smoke_OFFLoc2Primary } };
		...
		for (String[] location : locations) {
			if (ip.clickOnTab(environment, mode, TabName.OfficeLocations)) {
				...
				if (ip.clickOnCreatedOfficeLocation(environment, mode, location[0])) {
					...
					if (ip.clickOnRelatedList(environment, mode, RecordType.IndividualInvestor,
							RelatedList.Office_Locations, RelatedTab.Details.toString())) {

						log(LogStatus.PASS, "Clicked on Details TAB : " + RelatedList.Office_Locations, YesNo.No);

						for (int i = 0; i < location.length; i++) {
							if (ip.fieldValueVerificationOnOfficeLocationPage(environment, mode, label[i],
									location[i])) {
	...
	public void PESmokeTc067_VerifyBillingAddressOfInstitutionContactTransferAcc1AfterCreatingPrimaryOfficeLocation(
		...
		if (ip.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			...
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINS2)) {
				...
				if (ip.clickOnShowMoreActionDownArrow("PE", PageName.InstitutionsPage, ShowMoreActionDropDownList.Edit,
						20)) {
	...
	public void PESmokeTc070_UpdatePrimaryOfOfficeLocation1AtContactTransferAcc1DetailPageAndVerifyitsimpact(
		...
		if (ip.clickOnTab(environment, mode, TabName.OfficeLocations)) {
			...
			if (ip.clickOnCreatedOfficeLocation(environment, mode, Smoke_OFFLoc1Name)) {
				...
				if (ip.clickOnRelatedList(environment, mode, RecordType.IndividualInvestor,
					...
					ThreadSleep(2000);
					if (ip.clickOnShowMoreActionDownArrow("PE", PageName.ContactPage, ShowMoreActionDropDownList.Edit,
							10)) {
						...
						log(LogStatus.SKIP, "Not Able to Click on edit button on new window  : " + Smoke_OFFLoc1Name,
								YesNo.Yes);
					}
		...
		String[][] locations = {
				{ Smoke_OFFLoc1Name, Smoke_OFFLoc1Street, Smoke_OFFLoc1City, Smoke_OFFLoc1StateProvince,
						Smoke_OFFLoc1Country, Smoke_OFFLoc1UpdatedPrimary },
				{ Smoke_OFFLoc2Name, Smoke_OFFLoc2Street, Smoke_OFFLoc2City, Smoke_OFFLoc2StateProvince,
						Smoke_OFFLoc2Country, Smoke_OFFLoc2UpdatedPrimary } };
	...
	public void PESmokeTc071_UpdatePrimaryOfficeLocationValueAndVerifyItsImpact(String environment, String mode) {
		...
		if (flag) {
			...
			String[][] locations = { { Smoke_OFFLoc1UpdName, Smoke_OFFLoc1UpdStreet, Smoke_OFFLoc1UpdCity,
					Smoke_OFFLoc1UpdStateProvince, Smoke_OFFLoc1UpdCountry, Smoke_OFFLoc1UpdatedPrimary } };
				...
				if (ip.clickOnTab(environment, mode, TabName.OfficeLocations)) {
					...
					if (ip.clickOnCreatedOfficeLocation(environment, mode, location[0])) {
						...
						if (ip.clickOnRelatedList(environment, mode, RecordType.IndividualInvestor,
							...
							sa.assertTrue(false, "Not Able to Clicked on link : " + RelatedList.Office_Locations);
							log(LogStatus.FAIL, "Not Able to Clicked on link : " + RelatedList.Office_Locations,
									YesNo.Yes);
						}
			...
			if (ip.clickOnTab(environment, mode, TabName.ContactTab)) {
				log(LogStatus.INFO, "Clicked on Contact Tab", YesNo.No);
				if (cp.clickOnCreatedContact(environment, SmokeC5_FName, SmokeC5_LName)) {
					log(LogStatus.INFO, "Clicked on Created Contact : " + SmokeC5_FName + " " + SmokeC5_LName,
							YesNo.No);
	...
	public void PESmokeTc072_CreateActivitiesWithContact(String environment, String mode) {
		...
		String[][] basic = { { ActivityRelatedLabel.Subject.toString(), Smoke_NewTask1Subject },
				{ excelLabel.Related_To.toString(), contactName } };
		String task = Smoke_NewTask1Subject;

		if (home.createActivityTimeline("", true, NewInteractions_DefaultValues.Task.toString(), basic,
				new String[][] { { "Due Date Only", todaysDate } }, null, null, false, null, null, null, null, null,
				null)) {
		...
		String[][] basicCall = { { excelLabel.Subject.toString(), task },
				{ excelLabel.Related_To.toString(), contactName }, { excelLabel.Related_To.toString(), Smoke_Fund1 } };
		String[][] advanceCall = { { "Due Date Only", startDate } };

		if (home.createActivityTimeline("", true, NewInteractions_DefaultValues.Call.toString(), basicCall, advanceCall,
				null, null, false, null, null, null, null, null, null)) {
			log(LogStatus.INFO, "Able to create call : " + task, YesNo.No);

		} else {
			sa.assertTrue(false, "Not Able to create call : " + task);
			log(LogStatus.SKIP, "Not Able Able to create call : " + task, YesNo.Yes);
		}
	...
	public void PESmokeTc073_VerifyContactTransferAB(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.ContactTab)) {
			if (cp.clickOnCreatedContact(environment, SmokeC5_FName, SmokeC5_LName)) {
				...
				// Azhar Start
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.InstitutionsPage,
						ShowMoreActionDropDownList.Contact_Transfer, 10)) {
					...
					if (click(driver, cp.getContactTransferCancelButton(environment, mode, 10),
							"contact Transfer Cancel Button", action.BOOLEAN)) {
						log(LogStatus.INFO, "Clicked on Cancel Button", YesNo.No);
					} else {
						sa.assertTrue(false, "Not Able to Click on Cancel Button");
				...
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.InstitutionsPage,
					...
					if (cp.verifyContactTransferUIonContactPage(environment, mode, SmokeC5_FName + " " + SmokeC5_LName,
							SmokeINS2, 10)) {
					...
					// Azhar Start
					if (cp.enteringValueforLegalNameOnContactTransferPage(environment, mode, SmokeINDINV2,
							AddressAction.CrossIcon, 10)) {
					...
					if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Communincations,
							RelatedTab.Communications.toString())) {
	...
	public void PESmokeTc075_VerifyActivitiesAtNewInstitutionContactTransferAcc2(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINDINV2)) {
				...
				if (ip.verifyActivityHistoryRelatedList(environment, mode, TabName.ContactTab, Smoke_CallLog1Subject,
						Smoke_Fund1, null)) {
	...
	public void PESmokeTc076_VerifyContactTransferBC(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.ContactTab)) {
			if (cp.clickOnCreatedContact(environment, SmokeC5_FName, SmokeC5_LName)) {
				...
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.ContactPage,
						ShowMoreActionDropDownList.Contact_Transfer, 10)) {
					log(LogStatus.INFO, "Clicked on Contact Transfer", YesNo.No);
					...
					if (cp.enteringValueforLegalNameOnContactTransferPage(environment, mode, SmokeINS3,
							AddressAction.Clear, 10)) {
						log(LogStatus.PASS, "Able to Transfer Contact", YesNo.No);
					} else {
						sa.assertTrue(false, "Not Able to Transfer Contact");
						log(LogStatus.FAIL, "Not Able to Transfer Contact", YesNo.Yes);
					...
					switchToDefaultContent(driver);
					refresh(driver);
					String[][] contactLabelWithValue = {
							{ excelLabel.Name.toString(), SmokeC5_FName + " " + SmokeC5_LName },
					...
					for (String[] labelWithValue : labelsWithValues) {
						...
						if (!cp.fieldValueVerificationOnContactPage(environment, TabName.InstituitonsTab,
								labelWithValue[0], labelWithValue[1])) {
							log(LogStatus.PASS,
									labelWithValue[0] + " : with value : " + labelWithValue[1] + "  not present",
									YesNo.No);
						} else {
							sa.assertTrue(false,
					...
					if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Affiliations,
							RelatedTab.Affiliations.toString())) {
						log(LogStatus.INFO, "Click on Affiliations", YesNo.Yes);
...
//								}
				...
				if (ip.verifyNoDataAtActivityHistorySection(environment, mode, TabName.InstituitonsTab, 10)) {
					...
					log(LogStatus.FAIL, "Activity History Section Not Verified For No Records/No Activities Msg",
							YesNo.Yes);
				}
...
//							}
		...
		if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINS3)) {
				...
				if (ip.clickOnRelatedList(environment, mode, RecordType.Institution, RelatedList.Open_Activities,
					...
					if (ip.verifyOpenActivityRelatedList(environment, mode, TabName.InstituitonsTab,
							Smoke_NewTask1Subject, SmokeINS2, SmokeC5_FName + " " + SmokeC5_LName)) {
						log(LogStatus.INFO, "Open Activity Grid  Verified For " + Smoke_NewTask1Subject, YesNo.No);
					} else {
						sa.assertTrue(false, "Open Activity Grid Not Verified For " + Smoke_NewTask1Subject);
						log(LogStatus.FAIL, "Open Activity Grid Not Verified For " + Smoke_NewTask1Subject, YesNo.Yes);
					...
					if (ip.verifyActivityHistoryRelatedList(environment, mode, TabName.InstituitonsTab,
							Smoke_CallLog1Subject, Smoke_Fund1, SmokeC5_FName + " " + SmokeC5_LName)) {
						...
						log(LogStatus.FAIL, "Activitiy History Grid Not Verified For " + Smoke_CallLog1Subject,
								YesNo.Yes);
					}
	...
	public void PESmokeTc081_CreateActivitiesWithContact(String environment, String mode) {
		...
		String[][] basic = { { ActivityRelatedLabel.Subject.toString(), Smoke_NewTask2Subject },
				{ excelLabel.Related_To.toString(), contactName } };

		if (lp.createActivityTimeline("", true, NewInteractions_DefaultValues.Task.toString(), basic,
				new String[][] { { "Due Date Only", todaysDate } }, null, null, false, null, null, null, null, null,
				null)) {
			log(LogStatus.INFO, "Able to create task : " + task, YesNo.No);
		...
		String[][] basicCall = { { excelLabel.Subject.toString(), task },
				{ excelLabel.Related_To.toString(), contactName }, { excelLabel.Related_To.toString(), Smoke_Fund2 } };
		String[][] advanceCall = { { "Due Date Only", startDate } };

		if (lp.createActivityTimeline("", true, NewInteractions_DefaultValues.Call.toString(), basicCall, advanceCall,
				null, null, false, null, null, null, null, null, null)) {
			log(LogStatus.INFO, "Able to create call : " + task, YesNo.No);
		...
		if (bp.clickOnTab(environment, mode, TabName.ContactTab)) {
			if (cp.clickOnCreatedContact(environment, SmokeC6_FName, SmokeC6_LName)) {
				...
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.ContactPage,
					...
					if (cp.verifyContactTransferUIonContactPage(environment, mode, SmokeC6_FName + " " + SmokeC6_LName,
							SmokeINS4, 10)) {
						log(LogStatus.PASS, "Contact Transfer UI Verify", YesNo.Yes);
					} else {
						sa.assertTrue(false, "Contact Transfer UI Not Verify");
					...
					if (cp.enteringValueforLegalNameOnContactTransferPage(environment, mode, SmokeINDINV3,
							AddressAction.Retain, 10)) {
						log(LogStatus.PASS, "Able to Transfer Contact", YesNo.No);
					} else {
						sa.assertTrue(false, "Not Able to Transfer Contact");
					...
					if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Affiliations,
						...
						if (cp.verifyOpenActivityRelatedList(environment, mode, TabName.ContactTab,
								Smoke_NewTask2Subject, null, null)) {
							...
							log(LogStatus.FAIL, "Open Activity Grid Not Verified For " + Smoke_NewTask2Subject,
									YesNo.Yes);
						}

						if (cp.verifyActivityHistoryRelatedList(environment, mode, TabName.ContactTab,
								Smoke_CallLog2Subject, Smoke_Fund2, null)) {
							log(LogStatus.INFO, "Activity History Grid Verified For " + Smoke_CallLog2Subject,
									YesNo.No);
						} else {
							sa.assertTrue(false, "Activity History Grid Not Verified For " + Smoke_CallLog2Subject);
							log(LogStatus.FAIL, "Activity History Grid Not Verified For " + Smoke_CallLog2Subject,
									YesNo.Yes);
						}
		...
		if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINS4)) {
				...
				if (ip.verifyNoDataAtActivityHistorySection(environment, mode, TabName.InstituitonsTab, 10)) {
					...
					log(LogStatus.FAIL, "Activity History Grid Not Verified For No Records/No Past Activity Msg",
							YesNo.Yes);
				}
				...
				for (String[] activityRecord : activitiesRecords) {
					...
					if (ip.verifyActivitiesRelatedList(environment, mode, TabName.InstituitonsTab, activityRecord[0],
							activityRecord[1], activityRecord[2])) {
	...
	public void PESmokeTc084_VerifyActivitiesAtNewInstitutionContactTransferAcc5(String environment, String mode) {
		...
		if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINDINV3)) {
				...
				if (ip.verifyOpenActivityRelatedList(environment, mode, TabName.InstituitonsTab, Smoke_NewTask2Subject,
						null, SmokeC6_FName + " " + SmokeC6_LName)) {
				...
				if (ip.verifyActivityHistoryRelatedList(environment, mode, TabName.InstituitonsTab,
						Smoke_CallLog2Subject, Smoke_Fund2, SmokeC6_FName + " " + SmokeC6_LName)) {
		...
		if (bp.clickOnTab(environment, mode, TabName.ContactTab)) {
			if (cp.clickOnCreatedContact(environment, SmokeC6_FName, SmokeC6_LName)) {
				...
				if (cp.clickOnShowMoreActionDownArrow("PE", PageName.ContactPage,
					...
					if (cp.enteringValueforLegalNameOnContactTransferPage(environment, mode, SmokeINS5,
							AddressAction.Clear, 10)) {
						log(LogStatus.PASS, "Able to Transfer Contact", YesNo.No);
					} else {
						sa.assertTrue(false, "Not Able to Transfer Contact");
					...
					String[][] labelWithBlankValue = { { excelLabel.Fax.toString(), "" },
							{ excelLabel.Phone.toString(), "" }, { excelLabel.Mailing_Address.toString(), "" },
					...
					if (cp.clickOnRelatedList(environment, mode, RecordType.Contact, RelatedList.Affiliations,
						...
						if (cp.verifyActivityHistoryRelatedList(environment, mode, TabName.ContactTab,
								Smoke_CallLog2Subject, Smoke_Fund2, null)) {
							log(LogStatus.PASS, "Activity History Grid Verified For " + Smoke_CallLog2Subject,
									YesNo.No);
						} else {
							sa.assertTrue(false, "Activity History Grid Not Verified For " + Smoke_CallLog2Subject);
							log(LogStatus.FAIL, "Activity History Grid Not Verified For " + Smoke_CallLog2Subject,
		...
		if (bp.clickOnTab(environment, mode, TabName.InstituitonsTab)) {
			if (ip.clickOnCreatedInstitution(environment, mode, SmokeINS4)) {
				...
				if (click(driver, ele2, RelatedTab.Communications.toString(), action.SCROLLANDBOOLEAN)) {
					...
					if (ip.verifyNoDataAtActivityHistorySection(environment, mode, TabName.InstituitonsTab, 10)) {
						log(LogStatus.PASS, "Activity History Grid Verified For No Records/No Past Activity Msg",
								YesNo.No);
					} else {
						sa.assertTrue(false, "Activity History Grid Not Verified For No Records/No Past Activity Msg");
						log(LogStatus.FAIL, "Activity History Grid Not Verified For No Records/No Past Activity Msg",
					...
					if (ip.clickOnViewAllRelatedList(environment, mode, RelatedList.Activities)) {
						...
						String[][] activitiesRecords = {
								{ Smoke_NewTask2Subject, SmokeC6_FName + " " + SmokeC6_LName, null },
								{ Smoke_CallLog2Subject, SmokeC6_FName + " " + SmokeC6_LName, Smoke_Fund2 } };

						for (String[] activityRecord : activitiesRecords) {

							if (ip.verifyActivitiesRelatedList(environment, mode, TabName.InstituitonsTab,
									activityRecord[0], activityRecord[1], activityRecord[2])) {
								log(LogStatus.PASS, "Activities Grid Verified For " + activityRecord[0], YesNo.Yes);
							} else {
								sa.assertTrue(false, "Activities Grid Not Verified For " + activityRecord[0]);
								log(LogStatus.FAIL, "Activities Grid Not Verified For " + activityRecord[0], YesNo.Yes);
	...
	public void PESmokeTc089_1_VerifyBulkEmailfunctionality(String environment, String mode) {
		...
		if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
			...
			if (msg.contains(HomePageErrorMessage.step1_BuildYourOwn)) {
				...
				sa.assertTrue(false, "Message Not Verified Actual : " + msg + " \t Expected :"
						+ HomePageErrorMessage.step1_BuildYourOwn);
				log(LogStatus.FAIL, "Message Not Verified Actual : " + msg + " \t Expected :"
						+ HomePageErrorMessage.step1_BuildYourOwn, YesNo.Yes);
			}

			if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
					"R2" + SmokeReportName)) {
				...
				if (HomePageErrorMessage.bulkEmailErrorPopUpMsg.equals(msg)) {
					log(LogStatus.PASS,
							"Error PopUp Message Verified Actual : " + HomePageErrorMessage.bulkEmailErrorPopUpMsg,
							YesNo.No);
				} else {
					sa.assertTrue(false, "Error PopUp Message Not Verified Actual : " + msg + " \t Expected :"
							+ HomePageErrorMessage.bulkEmailErrorPopUpMsg);
					log(LogStatus.FAIL, "Error PopUp Message Not Verified Actual : " + msg + " \t Expected :"
							+ HomePageErrorMessage.bulkEmailErrorPopUpMsg, YesNo.Yes);
				}
			...
			if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, "Public Reports",
				...
				if (HomePageErrorMessage.bulkEmailErrorPopUpMsg1.equals(msg)) {
					log(LogStatus.PASS,
							"Error PopUp Message Verified Actual : " + HomePageErrorMessage.bulkEmailErrorPopUpMsg1,
							YesNo.No);
				} else {
					sa.assertTrue(false, "Error PopUp Message Not Verified Actual : " + msg + " \t Expected :"
							+ HomePageErrorMessage.bulkEmailErrorPopUpMsg1);
					log(LogStatus.FAIL, "Error PopUp Message Not Verified Actual : " + msg + " \t Expected :"
							+ HomePageErrorMessage.bulkEmailErrorPopUpMsg1, YesNo.Yes);
				}
			...
			//
			if (sendKeys(driver, hp.getSearchForAReportsTextBox(environment, mode, 10), SmokeReportName,
					"Report Search Text Box", action.BOOLEAN)) {
				...
				if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName, SmokeReportName)) {
					...
					log(LogStatus.FAIL, "Not Able to Click On " + SmokeReportFolderName + " : " + SmokeReportName,
							YesNo.Yes);
				}
	...
	public void PESmokeTc089_2_verifyCancelNextPreviousOnBulkEmail(String environment, String mode) {
		...
		for (int j = 0; j <= 7; j++) {
			...
			if (home.ClickOnItemOnNavatarEdge(navigationMenuName, bulkActionNavigationLink, action.BOOLEAN, 20)) {
				...
				// List<String> result
				// =market.selectProspectsContactAndVerifyReviewProspectListInEmailProspect(PageName.BulkEmail,ContactAndAccountName,
				// searchContactInEmailProspectGrid.No);
				if (j == 0) {
					if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
							SmokeReportName)) {
						...
						result = hp.selectContactAndVerifyInBulkEmail(environment, mode, SmokeC3_FName, SmokeC3_LName,
								SmokeC3_LName, searchContactInEmailProspectGrid.Yes, 10);

						if (result.isEmpty()) {
							log(LogStatus.PASS, "Able to Search/Check Contact : " + SmokeC3_FName + " " + SmokeC3_LName,
									YesNo.Yes);

							for (int i = 0; i <= 3; i++) {
								ThreadSleep(3000);
								if (i == 0) {
									if (click(driver, market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
											"Top Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on top next button", YesNo.Yes);
								...
								if (i == 1) {
									if (click(driver,
											market.getStep2PreviousBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
											"Top previous button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top previous button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on top previous button", YesNo.Yes);
								...
								if (i == 2) {
									if (click(driver,
											market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
											"bottom Next button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on BOTTOM next button", YesNo.Yes);
								...
								if (i == 3) {
									if (click(driver,
											market.getStep2PreviousBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
											"bottom previous button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on BOTTOM previous button", YesNo.No);
									} else {
										log(LogStatus.ERROR, "Not able to click on BOTTOM previous button", YesNo.Yes);
							...
							sa.assertTrue(false,
									"Not Able to Search/Check Contact : " + SmokeC3_FName + " " + SmokeC3_LName);
							log(LogStatus.FAIL,
									"Not Able to Search/Check Contact : " + SmokeC3_FName + " " + SmokeC3_LName,
									YesNo.Yes);
						}
				...
				if (j == 1) {
					...
					if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
						...
						if (result.isEmpty()) {
							...
							if (click(driver, market.getStep1CancelBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
									"Top cancel button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on top cancel button step 1", YesNo.No);
							} else {
								log(LogStatus.ERROR, "Not able to click on top cancel button step 1", YesNo.Yes);
						...
						sa.assertTrue(false, "Not Able to Click On " + SmokeReportFolderName + " : " + SmokeReportName);
						log(LogStatus.SKIP, "Not Able to Click On " + SmokeReportFolderName + " : " + SmokeReportName,
								YesNo.Yes);
					}
				...
				if (j == 3) {
					...
					if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
						...
						if (result.isEmpty()) {
							log(LogStatus.PASS, "Able to Search/Check Contact : " + SmokeC3_FName + " " + SmokeC3_LName,
									YesNo.Yes);
							if (click(driver, market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
									"bottom Next button", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
								ThreadSleep(2000);
								if (click(driver, market.getStep2CancelBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
										"top cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on top cancel button step 2", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not able to click on top cancel button step 2", YesNo.Yes);
					...
					if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
						...
						if (result.isEmpty()) {
							...
							if (click(driver, market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
								...
								log(LogStatus.ERROR, "clicked on BOTTOM next button", YesNo.No);
								ThreadSleep(2000);
								if (click(driver, market.getStep2CancelBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
										"bottom cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.ERROR, "clicked on bottom cancel button step 2", YesNo.No);
								} else {
									log(LogStatus.ERROR, "Not able to click on bottom cancel button step 2", YesNo.Yes);
								...
								for (int i = 0; i <= 3; i++) {
									...
									if (i == 0) {
										if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
											log(LogStatus.ERROR, "Select Capital Call Notice template", YesNo.No);
											if (click(driver,
													market.getStep2NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
													"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
												log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
											} else {
												log(LogStatus.ERROR, "Not able to click on top next button step 2",
									...
									if (i == 1) {
										if (click(driver,
												market.getStep3PreviousBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
												"Top previous button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top previous button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on top previous button step 3",
									...
									if (i == 3) {
										if (click(driver,
												market.getStep3PreviousBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
												"bottom previous button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on bottom previous button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on bottom previous button step 3",
					...
					if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName,
						...
						if (result.isEmpty()) {
							...
							if (click(driver, market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
								...
								if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
									log(LogStatus.ERROR, "Select Capital Call Notice template", YesNo.No);
									if (click(driver, market.getStep2NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
											"Top Next button step 2 ", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.ERROR, "clicked on top next button step 2", YesNo.No);
										ThreadSleep(2000);
										if (click(driver,
												market.getStep3CancelBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
												"top cancel button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on top cancel button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on top cancel button step 3",
							...
							if (click(driver, market.getStep1NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
								...
								if (market.selectEmailTemplateFromEmailProspect(null, "Capital Call Notice")) {
									...
									if (click(driver, market.getStep2NextBtn(PageName.BulkEmail, TopOrBottom.TOP, 10),
										...
										ThreadSleep(2000);
										if (click(driver,
												market.getStep3CancelBtn(PageName.BulkEmail, TopOrBottom.BOTTOM, 10),
												"buttom cancel button step 3", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "clicked on buttom cancel button step 3", YesNo.No);
										} else {
											log(LogStatus.ERROR, "Not able to click on buttom cancel button step 3",
			...
			if (hp.clickOnTemplateForReportOnBulkEmail(environment, mode, SmokeReportFolderName, SmokeReportName)) {
				...
				if (result.isEmpty()) {
					...
					if (click(driver, market.getEmailProspectStep1NextBtn(20), "step 1 next button",
						...
						if (market.selectEmailTemplateFromEmailProspect(EmailTemplate1_FolderName,
								EmailTemplate1_TemplateName)) {
							...
							if (ele != null) {
								if (click(driver, ele, EmailTemplate1_TemplateName + " preview link",
										action.SCROLLANDBOOLEAN)) {
									...
									if (parentWindow != null) {
										...
										ele = isDisplayed(driver, FindElement(driver, xpath, "", action.BOOLEAN, 20),
												"visibility", 20, "PE Test Custom Email Template template name");
										if (ele != null) {
											log(LogStatus.INFO,
													EmailTemplate1_TemplateName + " is open in preview mode", YesNo.No);
										} else {
											sa.assertTrue(false,
													EmailTemplate1_TemplateName + " is not open in preview mode");
											log(LogStatus.ERROR,
													EmailTemplate1_TemplateName + " is not open in preview mode",
													YesNo.Yes);
										}
										...
										sa.assertTrue(false, "No new window is open after click on"
												+ EmailTemplate1_TemplateName + " preview link");
										log(LogStatus.ERROR, "No new window is open after click on"
												+ EmailTemplate1_TemplateName + " preview link", YesNo.Yes);
									}
									ThreadSleep(5000);
									...
									if (click(driver, market.getEmailProspectStep2NextBtn(30), "step 2 next button",
										...
										if (ele != null) {
											msg = ele.getText().trim();
											if (MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg("1")
													.equals(msg)) {
												log(LogStatus.INFO,
														MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg(
																"1") + " error message is verified",
														YesNo.No);
											} else {
												sa.assertTrue(false,
														"error message is not verified Expected: "
													...
													if (j != 2) {
														if (checkBoxList.get(j).getAttribute("checked") == null) {
															...
															sa.assertTrue(false,
																	expctRsult[i] + "check box is selected");
															log(LogStatus.ERROR,
																	expctRsult[i] + "check box is selected", YesNo.Yes);
														}
													} else {
														System.err.println("checked:   "
																+ checkBoxList.get(j).getAttribute("checked"));
										...
										if (click(driver, market.getEmailProspectSendBtn(TopOrBottom.TOP, 30),
												"send button", action.SCROLLANDBOOLEAN)) {
											...
											if (ele != null) {
												...
												if (HomePageErrorMessage.step4_YourEmail.equals(msg)) {
													...
													sa.assertTrue(false,
															"Step4 Page Verified Not Verified Actual : " + msg
																	+ " \t Expected :"
																	+ HomePageErrorMessage.step4_YourEmail);
													log(LogStatus.FAIL,
															"Step4 Page Verified Not Verified Actual : " + msg
																	+ " \t Expected :"
																	+ HomePageErrorMessage.step4_YourEmail,
											...
											if (ele != null) {
												...
												if (msg.contains(
														MarketingInitiativesPageErrorMsg.congratulationErrorMsg)
														&& msg.contains(MarketingInitiativesPageErrorMsg
																.generatedEmailedRecipientErrorMsg("1", "Bulk"))) {
													log(LogStatus.INFO, "Congratulation Error Message is verified",
															YesNo.No);
												} else {
											...
											if (click(driver, market.getEmailProspectFinishBtn(30), "finish button",
													action.BOOLEAN)) {
												log(LogStatus.INFO, "Clicked on finish button", YesNo.No);
											} else {
												sa.assertTrue(false, "Not able to click on finish button");
									...
									sa.assertTrue(false, "Not able to click on " + EmailTemplate1_TemplateName
											+ " link so cannot check preview email template");
									log(LogStatus.ERROR, "Not able to click on " + EmailTemplate1_TemplateName
											+ " link so cannot check preview email template", YesNo.Yes);
								}
							} else {
								sa.assertTrue(false, EmailTemplate1_TemplateName
										+ " preview link is not visible so cannot click on it");
								log(LogStatus.ERROR, EmailTemplate1_TemplateName
										+ " preview link is not visible so cannot click on it", YesNo.Yes);
							}
					...
					log(LogStatus.FAIL, "Not Able to Search/Check Contact : " + SmokeC3_FName + " " + SmokeC3_LName,
							YesNo.Yes);
				}
