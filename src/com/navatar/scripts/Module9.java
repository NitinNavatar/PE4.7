...
//		driver.switchTo().window(parentWindow);
									...
									if (lp.clickOnTab(projectName, TabName.HomeTab)) {
										...
										if (home.sdgGridExpandedByDefaultIfNotThenExpand(TitleOfSDG)) {
											...
											if (FundFistSDGList.size() != 0) {
												...
												if (FundFistSDGList.contains(fieldToSelect.toUpperCase())) {
	...
	public void M9Tc053_RemoveContactFromRecyclebinAndUncheckAllRowButtonVerifySDGDataOnAppPage(String projectName) {
		...
		String[][] listViewSheetData = { { M9LV_5_Member, M9LV_5_TabName, M9LV_5_ListViewName, M9LV_5_ListAccessibility,
				M9LV_5_Filter, M9LV_5_Field, M9LV_5_Operators, M9LV_5_FilterValue } };
				...
				if (lp.addListView(projectName, row, 10)) {
					...
					if (clickUsingJavaScript(driver, ele, "Check box against : " + concatFullName, action.BOOLEAN)) {
						...
						if (clickUsingJavaScript(driver, ele, "Restore Button : " + concatFullName, action.BOOLEAN)) {
							...
							log(LogStatus.ERROR, "Not Able to Click on Restore Button for " + concatFullName,
									YesNo.Yes);
		...
		for (int i = 0; i < dataProviderName.size(); i++) {
			...
			if (BP.openAppFromAppLauchner(appPage, 50)) {
				if (EP.editPageAndAddSDG(appPage, tableNames.get(i), dataProviderName.get(i))) {
					...
					log(LogStatus.PASS, "App page has been edited and " + dataProviderName.get(i) + " has been added",
							YesNo.No);
						...
						if (list != null) {
							for (int i = 0; i < list.size(); i++) {
								if (list.get(i).equals("Centrient Pharmaceuticals -2018")) {
									...
									log(LogStatus.ERROR,
											list.get(i) + " is not matched with the Centrient Pharmaceuticals -2018",
											YesNo.No);
									status++;
								}
							}
	...
	public void M9Tc068_AddFilterSequenceForFundAndVerifyRecord(String projectName) {
		...
		String fundSDGName = table[0];
		String pageSize = "100";
		int fundRecordCountBeforeFilter, fundRecordCountAfterFilter, fundraisingRecordCountAfterFilter,
				fundraisingRecordCountBeforeFilter;
		lp.CRMLogin(superAdminUserName, adminPassword, appName);
			...
			if (SB.sequenceFilter(projectName, "Fundraising Filter Grid", "Fund Name", "1")) {
				...
				if (BP.openAppFromAppLauchner(appPage, 50)) {
					...
					if (AppBuilder.selectFilter("Fund", filterOptionValue1)) {
						...
						CommonLib.ThreadSleep(8000);
						if (!SB.verifyRecordExistOrNotOnSDG(fundraisingSDGName, "Fundraising SDG")) {
							if (SB.verifyRecordAfterApplyingGlobalFilter(fundraisingSDGName, filterOptionValue1,
									"Fund Name", "Fundraising SDG")) {
								log(LogStatus.PASS, "Sequence filter has been Applied Records are matched", YesNo.No);
								sa.assertTrue(true, "Sequence filter has been Applied Records are matched");
							} else {
						...
						if (!SB.verifyRecordExistOrNotOnSDG(fundSDGName, "Fund SDG")) {
							if (SB.verifyRecordAfterApplyingGlobalFilter(fundSDGName, filterOptionValue1, "Fund",
									"Fund SDG")) {
								log(LogStatus.PASS, "Sequence filter has been Applied Records are matched", YesNo.No);
								sa.assertTrue(true, "Sequence filter has been Applied Records are matched");
							} else {
				...
				if (BP.openAppFromAppLauchner(appPage, 50)) {
					...
					if (AppBuilder.selectFilter("Fund", "All")) {
						...
						if (fundRecordCountBeforeFilter == fundRecordCountAfterFilter
							...
							log(LogStatus.ERROR, "Records are not matched After Selecting All from the Fund",
									YesNo.Yes);
		...
		if (status == 0) {
			...
			if (BP.openAppFromAppLauchner(appPage, 50)) {
				...
				if (AppBuilder.selectFilter("Firm", "Applied Systems, Inc.")) {
					...
					if (fundraisingRowCountAfterFilter == fundraisingRowCountBeforeFilter) {
						log(LogStatus.INFO, "Fundraising grid is not filter based on the Applied Systems, Inc",
								YesNo.No);
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			if (SB.CheckedOrUncheckedCheckox(sdgName, Condition.SelectCheckbox)) {
				...
				if (count == 10) {
					...
					log(LogStatus.ERROR, "The count of selected checkbox is " + count + " in " + sdgName + " Grid",
							YesNo.Yes);
			...
			if (SB.CheckedOrUncheckedCheckox(sdgName, Condition.UnSelectCheckbox)) {
				...
				if (count == 0) {
					...
					log(LogStatus.FAIL, "The count of selected checkbox is " + count + " in " + sdgName + " Grid",
							YesNo.Yes);
					sa.assertTrue(false, "The count of selected checkbox is " + count + " in " + sdgName + " Grid");
				}
			} else {
	...
	public void M9Tc080_VerifyFieldsWhichEditableOnSingleRecordAndMultipleRecord(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			/*
			 * CommonLib.switchToDefaultContent(driver); ThreadSleep(2000); driver.close();
			 * driver.switchTo().window(parentWindow);
			 * 
			 ...
			 * "Location Preferences", "Asia", "Multipicklist", val)) { log(LogStatus.INFO,
			 ...
			 * (SB.updateSDGRecordAndVerifySaveCancelButton(sdgName, "Account 8",
			 * "Location Preferences", null, "Multipicklist", val1)) { log(LogStatus.INFO,
			 ...
			 * (SB.updateSDGRecordAndVerifySaveCancelButton(sdgName, "Account 1 Updated",
			 * "Location Preferences", "null", "Multipicklist", val)) { log(LogStatus.INFO,
	...
	public void M9Tc081_VerifyMultipleFieldsWhichEditableOnSameTimeWithSingleOrMultipleRecored(String projectName) {
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			log(LogStatus.INFO, appPage + " has been open from the App launcher", YesNo.No);
			CommonLib.refresh(driver);
			if (SB.upadateLegalNameWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName, "Account 1 Updated",
					"Account Name", "", "Website", "www.yahoo.com", "Revenue", "1000000")) {
				log(LogStatus.PASS,
			...
			CommonLib.refresh(driver);
			if (SB.upadatePhoneWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName, "Account 1 Updated",
					"Phone", "6532145698", "Website", "www.yahoo.com", "Revenue", "1000000")) {
				log(LogStatus.PASS,
						"\"1 records has error. Kindly resolve them and try again.\", error message is visible on record update of Legal Name, Website, Revenue field. Save and Cancel Button are also visible",
						YesNo.No);
				sa.assertTrue(true,
			...
			if (SB.clickTwoCheckboxupadatePhoneWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName,
					"Account 1 Updated", "Account 2", "Phone", "6532145698", "Website", "www.yahoo.com", "Revenue",
					"1000000")) {
				log(LogStatus.PASS,
				...
				ArrayList<String> data = SB.clickCancelBtnAndVerifyRecordNotMatched(sdgName, "Account 1 Updated",
						fieldName, value);
				if (data.isEmpty()) {
					log(LogStatus.PASS, "Page has been refreash and data is not updated in the Account 1 Updated Field",
							YesNo.No);
					sa.assertTrue(true,
			...
			if (SB.clickTwoCheckboxupadatePhoneWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName,
					"Account 1 Updated", "Account 5", "Phone", "6532145698", "Website", "www.yahoo.com", "Revenue",
					"1000000")) {
				log(LogStatus.PASS,
				...
				if (SB.verifyErrorMessageOnTriangleIconForPermission(sdgName, "Phone")) {
					...
					if (data.isEmpty()) {
						log(LogStatus.PASS,
								"Page has been refresh and data is not updated in the Account 1 Updated Field",
								YesNo.No);
						sa.assertTrue(true,
								"Page has been refresh and data is not updated in the Account 1 Updated Field");
					} else {
					...
					if (data1.isEmpty()) {
						log(LogStatus.PASS, "Page has been refresh and data is matched", YesNo.No);
						sa.assertTrue(true, "Page has been refresh and data is matched");
					} else {
	...
	public void M9Tc083_UpdatePhoneWebSiteRevenueAndRefreashSDGAndVerifyRecord(String projectName) {
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			CommonLib.refresh(driver);
			if (SB.upadatePhoneWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName, "Account 1 Updated",
					"Phone", "9876543321", "Website", "www.yahoo.com", "Revenue", "1000000")) {
				log(LogStatus.PASS,
						"\"1 records has error. Kindly resolve them and try again.\", error message is visible on record update of Legal Name, Website, Revenue field. Save and Cancel Button are also visible",
						YesNo.No);
				sa.assertTrue(true,
				...
				if (SB.verifyErrorMessageOnTriangleIconForPermission(sdgName, "Phone")) {
					...
					if (SB.reloadSDG(sdgName)) {
						...
						ArrayList<String> data = SB.VerifyRecordNotMatchedOnSDG(sdgName, "Account 1 Updated", fieldName,
								value);
						if (data.isEmpty()) {
							log(LogStatus.PASS,
									"Page has been refreash and data is not updated in the Account 1 Updated Field",
	...
	public void M9Tc084_VerifyMultipleFieldWillNotEditSameTimeOnDifferentPage(String projectName) {
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			if (SB.verifyMultipleFieldWillNotEditOnDifferentPage(sdgName, "Account 2", "Account 3", "AIH Limited",
					"AIG Group")) {
			...
			if (SB.clickTwoCheckboxupadatePhoneWebsiteRevenueAndVerifyErrorMessageSaveCancelButton(sdgName,
					"Account 1 Updated", "Account 8", "Phone", "6532145698", "Website", "www.yahoo.com", "Revenue",
					"1000000")) {
				log(LogStatus.PASS,
						"\"1 record has error. Kindly resolve them and try again.\", error message is visible on record update of Legal Name, Website, Revenue field. Save and Cancel Button are also visible",
						YesNo.No);
	...
	public void M9Tc085_VerifyAllfieldsWillEditAtAameTimeOnSamePage(String projectName) {
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			if (SB.verifyAllFieldEditAtSameTimeInWebsiteRevenue(sdgName, "Account 1 Updated", "Website",
					"www.yahoo.com", "Revenue", "1000000")) {
				log(LogStatus.PASS,
				...
				for (String record : recordName) {
					...
					if (BP.clickOnTab(projectName, TabName.InstituitonsTab)) {
						...
						if (BP.clickOnAlreadyCreated(environment, mode, TabName.InstituitonsTab, record, 50)) {
	...
	public void M9Tc086_CreateCustompicklistfieldAndSetCustomSourcefieldAsControllingField(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			String[][] labelAndValues = { { M9FC_12_FieldType, M9FC_12_FieldLabel, excelLabel.Length.toString(),
					M9FC_12_FieldValues, M9FC_12_ObjectName } };
	...
	public void M9Tc089_verifyValidation(String projectName) {
		...
		if (BP.openAppFromAppLauchner(appPage, 50)) {
			...
			log(LogStatus.INFO, appPage + " has been open from the App launcher", YesNo.No);
			CommonLib.refresh(driver);
			if (SB.VerifyValidationUpdateSDGRecordButValueshouldnotUpdate(sdgName, "Account 1 Updated", "Revenue",
					"Test of = Account 5")) {
			...
			if (SB.updateSDGRecordAndVerifySaveCancelButton(sdgName, "Account 1 Updated", "Revenue", "52369874", "Text",
					null)) {
			...
			if (SB.upadateSingleRecordVerifyErrorMessageSaveCancelButton(sdgName, "Account 1 Updated", "Revenue",
					"1234567890987654321")) {
				log(LogStatus.PASS,
				...
				if (SB.verifyErrorMessageOnTriangleIconForOutsideRange(sdgName, "Account 1 Updated")) {
					log(LogStatus.PASS, "outside range message has been verified on the hover of trianlge icon",
							YesNo.No);
					sa.assertTrue(true, "outside range message has been verified on the hover of trianlge icon");

				} else {
					log(LogStatus.FAIL, "outside range message is not visible on the hover of trianlge icon",
							YesNo.Yes);
