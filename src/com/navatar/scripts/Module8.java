...
public class Module8 extends BaseLib {
	...
	public void M8Tc003_verifyHomePage(String projectName) {
		...
		WebElement[] ele2 = { home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Open_SDG_Record, 5),
				home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Open_SDG_Record, 5),
				home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Open_SDG_Record, 5) };
		String[] tagNames = { "MyHome_ActiveDeals_Baseline", "IR - Investors - Fundraising Pipeline - Home",
				"Smart_Caller_MY_Home" };
		...
		for (int i = 0; i < ele2.length; i++) {
			if (ele2[i] != null) {
				if (click(driver, ele2[i], labelName[i] + " open sdg record ", action.SCROLLANDBOOLEAN)) {
					...
					if (parentid != null) {
						ThreadSleep(5000);
						WebElement ele4 = FindElement(driver, "//h1//*[text()='" + tagNames[i] + "']",
								labelName[i] + " tag name xpath", action.BOOLEAN, 10);
						if (ele4 != null) {
						...
						ele4 = FindElement(
								driver, "//*[text()='Default Sort']/../following-sibling::div//*[text()='"
										+ defaultSorting[i] + "']",
								labelName[i] + " tag name xpath", action.BOOLEAN, 10);
						if (ele4 != null) {
						...
						log(LogStatus.FAIL, "Not able to switch on open sdg record window of " + labelName[i],
								YesNo.Yes);
		...
		if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Toggle_Filters, 10), "filter icon",
			...
			if (home.SearchDealFilterDataOnHomePage(SDGGridName.Deals, "Deal", M8DealName1, Operator.Equals,
				...
				ThreadSleep(3000);
				if (clickUsingJavaScript(driver, home.sdgGridHeadersDealsNameList().get(0), "deal data",
						action.SCROLLANDBOOLEAN)) {
					...
					if (parentid != null) {
						ThreadSleep(5000);
						WebElement ele4 = FindElement(driver, "//h1//*[text()='" + M8DealName1 + "']",
								M8DealName1 + " tag name xpath", action.BOOLEAN, 10);
						if (ele4 != null) {
						...
						log(LogStatus.FAIL, "new window is not open after click on deal name " + M8DealName1,
								YesNo.Yes);
			...
			log(LogStatus.FAIL, "Not able to click on filter icon so cannot search deal name : " + M8DealName1,
					YesNo.Yes);
		...
		if (click(driver, home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Toggle_Filters, 10),
			...
			if (home.SearchDealFilterDataOnHomePage(SDGGridName.Fundraising, "Fundraising", M8FRName1, Operator.Equals,
				...
				ThreadSleep(3000);
				if (clickUsingJavaScript(driver, home.sdgGridHeadersFundRaisingsFundraisingNameList().get(0),
						"FundRaising data", action.SCROLLANDBOOLEAN)) {
					log(LogStatus.PASS, "clicked on FundRaising name " + M8FRName1, YesNo.No);
					String parentid = null;
					parentid = switchOnWindow(driver);
					if (parentid != null) {
						ThreadSleep(5000);
						WebElement ele4 = FindElement(driver, "//h1//*[text()='" + M8FRName1 + "']",
								M8FRName1 + " tag name xpath", action.BOOLEAN, 10);
						if (ele4 != null) {
						...
						log(LogStatus.FAIL, "new window is not open after click on FundRaising name " + M8FRName1,
								YesNo.Yes);
			...
			log(LogStatus.FAIL, "Not able to click on filter icon so cannot search FundRaising name : " + M8FRName1,
					YesNo.Yes);
		...
		if (click(driver, home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Toggle_Filters, 10),
				"filter icon", action.SCROLLANDBOOLEAN)) {
			log(LogStatus.PASS,
					"click on filter icon so cannot search My Call List name : " + M8CON1FName + " " + M8CON1LName,
					YesNo.Yes);
			if (home.SearchDealFilterDataOnHomePage(SDGGridName.My_Call_List, "Name", M8CON1FName + " " + M8CON1LName,
					Operator.Equals, YesNo.Yes)) {
				log(LogStatus.PASS, "Search My Call List Name in filter " + M8CON1FName + " " + M8CON1LName, YesNo.No);
				ThreadSleep(3000);
				if (clickUsingJavaScript(driver, home.sdgGridHeadersMyCallListNameList().get(0), "my call list data",
						action.SCROLLANDBOOLEAN)) {
					...
					if (parentid != null) {
						ThreadSleep(5000);
						WebElement ele4 = FindElement(driver,
								"//h1//*[text()='" + M8CON1FName + " " + M8CON1LName + "']",
								M8CON1FName + " " + M8CON1LName + " tag name xpath", action.BOOLEAN, 10);
						if (ele4 != null) {
							log(LogStatus.PASS, M8CON1FName + " " + M8CON1LName + " My Call List page is open",
									YesNo.No);
						} else {
							log(LogStatus.PASS, M8CON1FName + " " + M8CON1LName + " My Call List page is open",
									YesNo.No);
						...
						log(LogStatus.FAIL, "new window is not open after click on My Call List name " + M8CON1FName
								+ " " + M8CON1LName, YesNo.Yes);
						sa.assertTrue(false, "new window is not open after click on My Call List name " + M8CON1FName
								+ " " + M8CON1LName);
					}
				} else {
					log(LogStatus.PASS, "Not able to click on My Call List name " + M8CON1FName + " " + M8CON1LName,
							YesNo.No);
					sa.assertTrue(false, "Not able to click on My Call List name " + M8CON1FName + " " + M8CON1LName);
				}
			} else {
				log(LogStatus.FAIL, "Not able to Search My Call List Name in filter " + M8CON1FName + " " + M8CON1LName,
						YesNo.No);
				sa.assertTrue(false,
						"Not able to Search My Call List Name in filter " + M8CON1FName + " " + M8CON1LName);
			}
		} else {
			log(LogStatus.FAIL, "Not able to click on filter icon so cannot search My Call List name : " + M8CON1FName
					+ " " + M8CON1LName, YesNo.Yes);
			sa.assertTrue(false, "Not able to click on filter icon so cannot search My Call List name : " + M8CON1FName
					+ " " + M8CON1LName);
		}
		lp.CRMlogout();
		sa.assertAll();
	...
	public void M8Tc004_verifyLogNotesButton(String projectName) {
		...
		if (click(driver, home.sdgGridHeadersMyCallListNameCallLogList().get(0), "call log button",
				action.SCROLLANDBOOLEAN)) {
	...
	public void M8Tc005_verifyRecordsCount(String projectName) {
		...
		if (selectVisibleTextFromDropDown(driver,
				home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(SDGGridName.Deals, ActionType.Page, 10),
				"deal sdg grid page drop down", "2")) {
		...
		if (selectVisibleTextFromDropDown(driver,
				home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(SDGGridName.Fundraising, ActionType.Page, 10),
				"Fundraising sdg grid page drop down", "2")) {
		...
		if (selectVisibleTextFromDropDown(driver, home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(
				SDGGridName.My_Call_List, ActionType.Page, 10), "My_Call_List sdg grid page drop down", "2")) {
		...
		if (selectVisibleTextFromDropDown(driver,
				home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(SDGGridName.Deals, ActionType.PageSize, 10),
				"deal sdg grid page drop down", "20")) {
		...
		if (selectVisibleTextFromDropDown(driver, home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(
				SDGGridName.Fundraising, ActionType.PageSize, 10), "Fundraising sdg grid page drop down", "20")) {
		...
		if (selectVisibleTextFromDropDown(driver, home.sdgGridPageAndPageSizeofDealFundraisingAndMyCallList(
				SDGGridName.My_Call_List, ActionType.PageSize, 10), "My_Call_List sdg grid page drop down", "20")) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			if (click(driver, home.sdgGridSideIcons(sdgGridName[i], SDGGridSideIcons.Toggle_Filters, 10), "filter icon",
					action.SCROLLANDBOOLEAN)) {
				log(LogStatus.PASS,
						"click on filter icon so cannot search My Call List name : " + M8CON1FName + " " + M8CON1LName,
						YesNo.Yes);
				...
				if (home.SearchDealFilterDataOnHomePage(sdgGridName[i], searchHeaderName[i], DataList[i], operator[i],
					...
					WebElement ele4 = FindElement(driver, xpath, DataList[i] + " tool tip xpath on " + sdgGridName[i],
							action.BOOLEAN, 10);
					if (ele4 != null) {
						log(LogStatus.PASS, DataList[i] + " tool tip is displaying on " + sdgGridName[i], YesNo.No);
					} else {
						log(LogStatus.PASS, DataList[i] + " tool tip is not displaying on " + sdgGridName[i],
								YesNo.Yes);
				...
				log(LogStatus.FAIL,
						"Not able to click on filter icon so cannot search " + sdgGridName[i] + " : " + DataList[i],
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on filter icon so cannot search " + sdgGridName[i] + " : " + DataList[i]);
			}
	...
	public void M8Tc006_1_verifyDefaultTheme(String projectName) {
		...
		ThreadSleep(5000);
		WebElement[][] ele2 = {
				{ home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
						home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Open_SDG_Record, 5),
						home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Toggle_Filters, 5),
						home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Reload, 5) },
				{ home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Manage_fields, 5),
						home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Open_SDG_Record, 5),
						home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Toggle_Filters, 5),
						home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Reload, 5) },
				{ home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Manage_fields, 5),
						home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Open_SDG_Record, 5),
						home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Toggle_Filters, 5),
						home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Reload, 5) } };
		...
		for (WebElement[] webElements : ele2) {
			for (int i = 0; i < webElements.length; i++) {
				if (webElements[i] != null) {
					log(LogStatus.PASS,
							filterIconName[i] + " is displaying and tool tip is available for " + labelName[j],
							YesNo.No);
				} else {
					log(LogStatus.PASS,
							filterIconName[i] + " is displaying and tool tip is not  available for " + labelName[j],
							YesNo.Yes);
					sa.assertTrue(false,
							filterIconName[i] + " is displaying and tool tip is not available for " + labelName[j]);
				}
		...
		if (edit.clickOnEditPageLink()) {
			...
			for (int i = 0; i < sdgNames.length; i++) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					click(driver, home.getSelectThemeinputBoxButton(10), "clear button", action.SCROLLANDBOOLEAN);
					ThreadSleep(2000);
	...
	public void M8Tc007_verifywrenchIconForStandardTheme(String projectName) {
		...
		for (int i = 0; i < ele2.length; i++) {
			String list = "";
			if (ele2[i] != null) {
				log(LogStatus.PASS, "Manage Field icon is displaying and tool tip is available for " + labelName[i],
						YesNo.No);
				ThreadSleep(3000);
				if (click(driver, ele2[i], "manage field icon", action.SCROLLANDBOOLEAN)) {
					...
					if (click(driver,
							home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Cancel, 10),
				...
				log(LogStatus.PASS,
						"Manage field icon is not displaying so cannot check manage field icon for " + labelName[i],
						YesNo.Yes);
				sa.assertTrue(false,
						"Manage field icon is not displaying and cannot check manage field icon for " + labelName[i]);
			}
		}
		lp.CRMlogout();
		sa.assertAll();
	...
	public void M8Tc008_verifySDGFiltersForStandardTheme(String projectName) {
		...
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		List<WebElement> lst = new ArrayList<WebElement>();
		List<String> dealsDefaultList = new ArrayList<String>();
		List<String> fundraisingDefaultList = new ArrayList<String>();
		List<String> mycalllistDefaultList = new ArrayList<String>();
				...
				if (home.SearchDealFilterDataOnHomePage(sdgGridName[i], dropDownName[i], searchDataName[i],operators[i], searchData[i])) {
					...
					if (i == 0) {
						...
						if (!lst.isEmpty()) {
							...
							log(LogStatus.FAIL,
									"Firm filter data list is not found so cannot check Firm list after applied filter in My_Call_List SDG",
									YesNo.Yes);
							sa.assertTrue(false,
									"firm filter data list is not found so cannot check Firm list after applied filter in My_Call_List SDG");
						}
					}
					click(driver, home.sdgGridSideIcons(sdgGridName[i], SDGGridSideIcons.Toggle_Filters, 10),
							"filter icon", action.SCROLLANDBOOLEAN);
				} else {
					log(LogStatus.FAIL, "Not able to Search " + sdgGridName[i] + " in filter " + labelName[i],
							YesNo.No);
					sa.assertTrue(false, "Not able to Search " + sdgGridName[i] + " Name in filter " + labelName[i]);
				}
			} else {
				log(LogStatus.FAIL,
						"Not able to click on filter icon so cannot search " + sdgGridName[i] + " : " + labelName[i],
						YesNo.Yes);
				sa.assertTrue(false,
						"Not able to click on filter icon so cannot search " + sdgGridName[i] + " : " + labelName[i]);
			}
		...
		for(int k=0;k<lst.size();k++){
			...
			actual.add(lst.get(k).getText().trim());
		}
		

		if (!lst.isEmpty()) {
			 if(compareList(dealsDefaultList, actual)) {
			...
			log(LogStatus.FAIL, "Stage data list is not found so cannot check stage list after refresh in deal SDG",
					YesNo.Yes);
			sa.assertTrue(false,
					"Stage data list is not found so cannot check stage list after refresh filter in deal SDG");
		}
		...
		for(int k=0;k<lst.size();k++){
			...
			actual2.add(lst.get(k).getText().trim());
		}
		

		if (!lst.isEmpty()) {
			 if(compareList(fundraisingDefaultList, actual2)) {
			...
			log(LogStatus.FAIL,
					"2nd closing data list is not found so cannot check 2nd closing list after refresh in Fundraising SDG",
					YesNo.Yes);
			sa.assertTrue(false,
					"2nd closing data data list is not found so cannot check 2nd closing list after refresh in Fundraising SDG");
		}
		lst = new ArrayList<WebElement>();
		...
		for(int k=0;k<lst.size();k++){
			...
			actual3.add(lst.get(k).getText().trim());
		}
		

		if (!lst.isEmpty()) {
			 if(compareList(mycalllistDefaultList, actual3)) {
			...
			log(LogStatus.FAIL,
					"Firm filter data list is not found so cannot check Firm list after refresh in My_Call_List SDG",
					YesNo.Yes);
			sa.assertTrue(false,
					"firm filter data list is not found so cannot check Firm list after refresh in My_Call_List SDG");
		}
	...
	public void M8Tc010_verifyReloadIconForStandardTheme(String projectName) {
		...
		ThreadSleep(5000);
		String[] labelName = { "Deal", "FundRaising", "My Call List" };
		WebElement[] ele2 = { home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Reload, 5),
				home.sdgGridSideIcons(SDGGridName.Fundraising, SDGGridSideIcons.Reload, 5),
				home.sdgGridSideIcons(SDGGridName.My_Call_List, SDGGridSideIcons.Reload, 5) };
		...
		for (int i = 0; i < ele2.length; i++) {
			if (ele2[i] != null) {
				...
				if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(), 10)) {
					...
					if (home.clickOnCheckboxOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(),
							10)) {
						ThreadSleep(5000);

						log(LogStatus.INFO, "successfully clicked on checkbox button", YesNo.No);

						if (clickUsingJavaScript(driver, home.getsdgSaveButton(projectName, 10), "save",
								action.SCROLLANDBOOLEAN)) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					for (int i1 = 0; i1 < themelistwebelement.size(); i1++) {
						if (themelistwebelement.get(i1).getText().equalsIgnoreCase("Light")) {
							if (click(driver, themelistwebelement.get(i1), "Light theme xpath",
									action.SCROLLANDBOOLEAN)) {
	...
	public void M8Tc011_2_verifyLightThemeForSDG(String projectName) {
		...
		ThreadSleep(5000);
		WebElement[] ele2 = {
				home.sdgGridSideIconsForLightTheme(SDGGridName.Deals, SDGGridSideIcons.Side_DropDOwnButtonforLightTheme,
						10),
				home.sdgGridSideIconsForLightTheme(SDGGridName.Fundraising,
						SDGGridSideIcons.Side_DropDOwnButtonforLightTheme, 10),
				home.sdgGridSideIconsForLightTheme(SDGGridName.My_Call_List,
						SDGGridSideIcons.Side_DropDOwnButtonforLightTheme, 10) };
		String[] labelName = { "Deal", "FundRaising", "My Call List" };
		...
		for (WebElement webElements : ele2) {
			if (webElements != null) {
				...
				if (click(driver, webElements, "drop down  button of " + labelName[j], action.SCROLLANDBOOLEAN)) {
					...
					int i = 0;
					WebElement[] ele3 = {
							home.sdgGridSideIconsForLightTheme(sdgGridNames[j], SDGGridSideIcons.Manage_fields, 10),
					...
					for (WebElement webElements1 : ele3) {
						if (webElements1 != null) {
							log(LogStatus.PASS, filterIconName[i] + "Drop Down menu is displaying for " + labelName[j],
									YesNo.No);

						} else {
							log(LogStatus.PASS,
									filterIconName[i] + " Drop Down menu is not displaying for " + labelName[j],
									YesNo.Yes);
							sa.assertTrue(false,
									filterIconName[i] + "Drop Down menu is not displaying for " + labelName[j]);
						}
		...
		int j1 = 0;
		WebElement[] ele4 = {
				home.sdgGridSideIconsForLightTheme(SDGGridName.Deals, SDGGridSideIcons.Toggle_Filters, 10),
		...
		for (int j = 0; j < ele1.length; j++) {
			if (click(driver, ele1[j], "drop down  button of " + labelName[j], action.SCROLLANDBOOLEAN)) {
				...
				if (j == 0) {
					...
					ele2 = home.sdgGridSideIconsForLightTheme(SDGGridName.Fundraising, SDGGridSideIcons.Manage_fields,
							5);
				} else {
					ele2 = home.sdgGridSideIconsForLightTheme(SDGGridName.My_Call_List, SDGGridSideIcons.Manage_fields,
							5);
				}
				String list = "";
				if (ele2 != null) {
					log(LogStatus.PASS, "Manage Field icon is displaying and tool tip is available for " + labelName[j],
							YesNo.No);
					ThreadSleep(3000);
					...
					log(LogStatus.PASS,
							"Manage field icon is not displaying so cannot check manage field icon for " + labelName[j],
							YesNo.Yes);
					sa.assertTrue(false, "Manage field icon is not displaying and cannot check manage field icon for "
							+ labelName[j]);
				}
	...
	public void M8Tc013_verifySDGFiltersForLightTheme(String projectName) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			if (click(driver, home.sdgGridSideIconsForLightTheme(sdgGridName[i], SDGGridSideIcons.Toggle_Filters, 10),
					"filter icon", action.SCROLLANDBOOLEAN)) {
				log(LogStatus.PASS,
						"click on filter icon so cannot search My Call List name : " + M8CON1FName + " " + M8CON1LName,
						YesNo.Yes);
				if (home.SearchDealFilterDataOnHomePage(sdgGridName[i], dropDownName[i], searchDataName[i],
						operators[i], searchData[i])) {
					log(LogStatus.PASS, "Search My Call List Name in filter " + labelName[i], YesNo.No);
					ThreadSleep(3000);
					if (i == 0) {
						lst = home.sdgGridHeadersDealsGridDealStageColumn(sdgGridName[i], headerName[i]);
						if (!lst.isEmpty()) {
							...
							log(LogStatus.FAIL,
									"Stage data list is not found so cannot check stage list after applied filter in deal SDG",
									YesNo.Yes);
							sa.assertTrue(false,
									"Stage data list is not found so cannot check stage list after applied filter in deal SDG");
						}
						...
						if (!lst.isEmpty()) {
							...
							log(LogStatus.FAIL,
									"2nd closing data list is not found so cannot check 2nd closing list after applied filter in Fundraising SDG",
									YesNo.Yes);
							sa.assertTrue(false,
									"2nd closing data data list is not found so cannot check 2nd closing list after applied filter in Fundraising SDG");
						}
					} else {
						lst = home.sdgGridHeadersDealsGridDealStageColumn(sdgGridName[i], headerName[i]);
						if (!lst.isEmpty()) {
				...
				if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(), 10)) {
					...
					if (home.clickOnCheckboxOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(),
						...
						if (click(driver, ele2[i], "drop down  button of " + labelName[i], action.SCROLLANDBOOLEAN)) {
							...
							if (i == 0) {
								ele1 = home.sdgGridSideIconsForLightTheme(SDGGridName.Deals, SDGGridSideIcons.Reload,
										5);
							} else if (i == 1) {
								ele1 = home.sdgGridSideIconsForLightTheme(SDGGridName.Fundraising,
										SDGGridSideIcons.Reload, 5);
							} else {
								ele1 = home.sdgGridSideIconsForLightTheme(SDGGridName.My_Call_List,
										SDGGridSideIcons.Reload, 5);
							}
	...
	public void M8Tc021_2_updateDealFundRaisingMyCallListWithMaxChar(String projectName) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			...
			if (i == 0) {
				lst = home.sdgGridHeadersDealsNameList();
				dealName = lst.get(0).getText();
				updatedName = dealName + "#@&*%.()"+bp.generateRandomNumber();
			} else if (i == 1) {
				lst = home.sdgGridHeadersFundRaisingsFundraisingNameList();
				dealName = lst.get(0).getText();
				updatedName = dealName + "#@&*%.()"+bp.generateRandomNumber();
			...
			if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(), 10)) {
				...
				WebElement ele = home.SDGInputTextbox(projectName, textBoxName[i].toString(), 10);
				sendKeys(driver, ele, updatedName, "title textbox", action.BOOLEAN);
				if (home.clickOnCheckboxOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(), 10)) {
					ThreadSleep(5000);
					...
					if (clickUsingJavaScript(driver, home.getsdgSaveButton(projectName, 10), "save",
						...
						log(LogStatus.INFO, "successfully clicked on save button", YesNo.No);
						ThreadSleep(7000);
						if (click(driver, home.sdgGridSideIcons(sdgGridName[i], SDGGridSideIcons.Toggle_Filters, 10),
								"filter icon", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "click on filter icon so cannot search My Call List name : "
									+ M8CON1FName + " " + M8CON1LName, YesNo.No);
							if (home.SearchDealFilterDataOnHomePage(sdgGridName[i], dropDownName[i], updatedName,
									operators[i], searchData[i])) {
								log(LogStatus.PASS, "Search My Call List Name in filter " + labelName[i], YesNo.No);
								ThreadSleep(5000);
								if (i == 0) {
								...
								click(driver,
										home.sdgGridSideIcons(sdgGridName[i], SDGGridSideIcons.Toggle_Filters, 10),
										"filter icon", action.SCROLLANDBOOLEAN);
							} else {
								log(LogStatus.FAIL,
										"Not able to Search " + sdgGridName[i] + " in filter " + labelName[i],
										YesNo.No);
								sa.assertTrue(false,
										"Not able to Search " + sdgGridName[i] + " Name in filter " + labelName[i]);
							}
						} else {
							log(LogStatus.FAIL, "Not able to click on filter icon so cannot search " + sdgGridName[i]
									+ " : " + labelName[i], YesNo.Yes);
							sa.assertTrue(false, "Not able to click on filter icon so cannot search " + sdgGridName[i]
									+ " : " + labelName[i]);
						}
	...
	public void M8Tc022_verifyDefaultInlineEditIconForStandardTheme(String projectName) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			...
			if (i != 2) {
				...
				if (!home.clickOnEditButtonOnSDGGridOnHomePage(projectName, dealName, editPageLabel[i].toString(),
						10)) {
	...
	public void M8Tc023_verifyInlineEditInDealGridForStandardTheme(String projectName) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			...
			if (!lst.isEmpty()) {
				...
				log(LogStatus.FAIL, "Deal SDG grid check box list is not visible so cannot update stage column values",
						YesNo.Yes);
				sa.assertTrue(false,
						"Deal SDG grid check box list is not visible so cannot update stage column values");
			...
			ThreadSleep(1000);
			if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, stageDataList.get(i).getText().toString(),
					editPageLabel[i].toString(), 10)) {
				log(LogStatus.PASS, "clicked on edit icon of " + dealNameList.get(i), YesNo.No);
				ThreadSleep(3000);
				if (clickUsingJavaScript(driver,
						home.sdgGridSideDealStageColumnDropDownListInEditMode(sdgGridName[i], 10), "",
						action.BOOLEAN)) {
					...
					if (click(driver, ele, "Deal received value", action.SCROLLANDBOOLEAN)) {
						...
						ThreadSleep(2000);
						if (click(driver,
								home.sdgGridSideDealStageColumnUpdateSelecteditemsCheckBox(sdgGridName[i], 3, 3, 10),
								"Update3SelecteditemsCheckBox", action.BOOLEAN)) {
							log(LogStatus.PASS, "clicked on Update 3 Selected items CheckBox", YesNo.No);
							ThreadSleep(1000);
							if (click(driver,
									FindElement(driver,
											"//button[@class='slds-button slds-button_brand' or text()='"
													+ Buttons.Apply + "']",
											"", action.BOOLEAN, 5),
								...
								if (!stageDataList.isEmpty()) {
									for (int k = 0; k < stageDataList.size() - 7; k++) {
										...
										if (s.equalsIgnoreCase("Deal Received")) {
											log(LogStatus.PASS, labelName[i] + " SDG Grid Header Name is verified ",
													YesNo.No);
										} else {
											log(LogStatus.FAIL, labelName[i] + " SDG Grid Header Name is not verified ",
													YesNo.Yes);
											sa.assertTrue(false,
													labelName[i] + " SDG Grid Header Name is not verified ");
										}
									}

									click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
											Buttons.Save, 10), "Save button", action.BOOLEAN);

								} else {
									log(LogStatus.FAIL,
											"Stage data list is not found so cannot check stage list after Update value in deal SDG",
											YesNo.Yes);
									sa.assertTrue(false,
											"Stage data list is not found so cannot check stage list after update value in deal SDG");
								}
								...
								log(LogStatus.PASS, "Not able to click on apply button so cannot update stage value",
										YesNo.Yes);
	...
	public void M8Tc024_verifyInlineEditInFundRaisingGridForStandardTheme(String projectName) {
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			...
			if (!lst.isEmpty()) {
				...
				log(LogStatus.FAIL,
						"Fundraising SDG grid check box list is not visible so cannot update stage column values",
						YesNo.Yes);
				sa.assertTrue(false,
						"Fundraising SDG grid check box list is not visible so cannot update stage column values");
			...
			if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, StageData, editPageLabel[i].toString(), 10)) {
				...
				if (clickUsingJavaScript(driver,
					...
					if (click(driver, ele, "Deal received value", action.SCROLLANDBOOLEAN)) {
						...
						ThreadSleep(2000);
						if (click(driver,
								home.sdgGridSideDealStageColumnUpdateSelecteditemsCheckBox(sdgGridName[i], 5, 3, 10),
								"Update5SelecteditemsCheckBox", action.BOOLEAN)) {
							log(LogStatus.PASS, "clicked on Update 3 Selected items CheckBox", YesNo.No);
							ThreadSleep(1000);
							if (click(driver,
									FindElement(driver,
								...
								if (!stageDataList.isEmpty()) {
									for (int k = 0; k < stageDataList.size() - 5; k++) {
										String s = stageDataList.get(k).getText().trim();
										if (s.equalsIgnoreCase("Follow up Diligence")) {
											log(LogStatus.PASS, labelName[i] + " SDG Grid Stage value is verified ",
													YesNo.No);
										} else {
											log(LogStatus.FAIL, labelName[i] + " SDG Grid Stage value is not verified ",
													YesNo.Yes);
											sa.assertTrue(false,
													labelName[i] + " SDG Grid Stage value is not verified ");
										}
									}
									...
									log(LogStatus.FAIL,
											"Stage data list is not found so cannot check stage list after Update value in Fundraising SDG",
											YesNo.Yes);
									sa.assertTrue(false,
											"Stage data list is not found so cannot check stage list after update value in Fundraising SDG");
								}
		...
		for (int i = 0; i < sdgGridName.length; i++) {
			...
			if (!lst.isEmpty()) {
				...
				log(LogStatus.FAIL,
						"Fundraising SDG grid check box list is not visible so cannot update Closing column values",
						YesNo.Yes);
				sa.assertTrue(false,
						"Fundraising SDG grid check box list is not visible so cannot update Closing column values");
			...
			if (home.clickOnEditButtonOnSDGGridOnHomePage(projectName, StageData, editPageLabel[1].toString(), 10)) {
				...
				ThreadSleep(3000);
				if (clickUsingJavaScript(driver,
						home.sdgGridSideFundRaisingClosingColumnDropDownListInEditMode(sdgGridName[i], 10), "",
						action.BOOLEAN)) {
					...
					if (click(driver, ele, "1st Closing value", action.SCROLLANDBOOLEAN)) {
						...
						ThreadSleep(2000);
						if (click(driver,
								home.sdgGridSideDealStageColumnUpdateSelecteditemsCheckBox(sdgGridName[i], 10, 4, 10),
								"Update10SelecteditemsCheckBox", action.BOOLEAN)) {
							...
							if (click(driver,
								...
								if (!stageDataList.isEmpty()) {
									for (int k = 0; k < stageDataList.size(); k++) {
										...
										if (s.equalsIgnoreCase("1st Closing")) {
											log(LogStatus.PASS, labelName[i] + " SDG Grid Closing value is verified ",
													YesNo.No);
										} else {
											log(LogStatus.FAIL,
													labelName[i] + " SDG Grid Closing value is not verified ",
													YesNo.Yes);
											sa.assertTrue(false,
													labelName[i] + " SDG Grid Closing value is not verified ");
										}
									...
									log(LogStatus.FAIL,
											"Closing data list is not found so cannot check Closing list after Update value in Fundraising SDG",
											YesNo.Yes);
									sa.assertTrue(false,
											"Closing data list is not found so cannot check Closing list after update value in Fundraising SDG");
								}
								...
								log(LogStatus.PASS, "Not able to click on apply button so cannot update Closing value",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to click on apply button so cannot update Closing value");
							}
	...
	public void M8Tc025_verifyTaskAndEventSection(String projectName) {
		...
		if (click(driver, home.todayTasksDownArrow(Task.TodayTasks, 10), "today task down arrow",
				action.SCROLLANDBOOLEAN)) {
			log(LogStatus.PASS, "Clicked on today tasks down arrow ", YesNo.No);
			ThreadSleep(1000);
			TodayTaskDownArrowValues[] value = { TodayTaskDownArrowValues.Today, TodayTaskDownArrowValues.MyTasks,
					TodayTaskDownArrowValues.AllOverdue, TodayTaskDownArrowValues.CompletedWithinLast7Days,
					TodayTaskDownArrowValues.DelegatedTasks };
	...
	public void M8Tc026_createTaskAndEvents(String projectName) {
		...
		for (int i = 1; i < 15; i++) {
			String status = ExcelUtils.readData(phase1DataSheetFilePath, "Task1", excelLabel.Variable_Name,
					"M8Task" + (i), excelLabel.Status);
			String subject = ExcelUtils.readData(phase1DataSheetFilePath, "Task1", excelLabel.Variable_Name,
					"M8Task" + (i), excelLabel.Subject);
			String name = ExcelUtils.readData(phase1DataSheetFilePath, "Task1", excelLabel.Variable_Name,
					"M8Task" + (i), excelLabel.Name);
			String accountName = ExcelUtils.readData(phase1DataSheetFilePath, "Task1", excelLabel.Variable_Name,
					"M8Task" + (i), excelLabel.Related_To);
			if (i == 11 || i == 12) {
				date = yesterdaysDate;
			} else if (i == 13 || i == 14) {
			...
			if (click(driver, gp.getGlobalActionIcon(projectName, 15), "Global Action Related item", action.BOOLEAN)) {
				...
				if (clickUsingJavaScript(driver, gp.getActionItem(projectName, GlobalActionItem.New_Task, 15),
					...
					if (sendKeys(driver, ele, subject, "Subject", action.SCROLLANDBOOLEAN)) {
						...
						// Due Date
						if (sendKeys(driver, ip.getdueDateTextBoxInNewTask(projectName, 20), date,
							...
							if (sendKeys(driver, ele, name, "Name Text Label", action.SCROLLANDBOOLEAN)) {
								...
								ele = cp.getContactNameOrRelatedAssociationNameOnTask(projectName, PageName.TaskPage,
										PageLabel.Name.toString(), name, action.SCROLLANDBOOLEAN, 15);
							...
							// Related To
							click(driver,
									ip.getrelatedAssociationsdropdownButton(projectName, PageName.TaskPage,
											PageLabel.Related_To.toString(), action.SCROLLANDBOOLEAN, 10),
									"dropdown button", action.SCROLLANDBOOLEAN);
							if (cp.SelectRelatedAssociationsdropdownButton(projectName, PageName.TaskPage,
									PageLabel.Related_To.toString(), TabName.InstituitonsTab, action.SCROLLANDBOOLEAN,
									20)) {
								log(LogStatus.INFO,
										"Able to Select Drown Down Value : "
												+ cp.getTabName(projectName, TabName.InstituitonsTab) + " For Label "
												+ PageLabel.Related_Associations,
										YesNo.No);
								ThreadSleep(2000);

								ele = cp.getLabelTextBoxForNameOrRelatedAssociationOnTask(projectName,
										PageName.TaskPage, PageLabel.Related_To.toString(), action.SCROLLANDBOOLEAN,
										15);
								if (sendKeys(driver, ele, accountName, "Related To Text Label",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "Enter Value to Related To Text Box : " + accountName,
											YesNo.Yes);
									ThreadSleep(1000);

									ele = cp.getContactNameOrRelatedAssociationNameOnTask(projectName,
											PageName.TaskPage, PageLabel.Related_To.toString(), accountName,
											action.SCROLLANDBOOLEAN, 15);
									if (click(driver, ele,
											"Select TestCustomObject From Label : " + PageLabel.Related_To,
											action.SCROLLANDBOOLEAN)) {
									...
									sa.assertTrue(false,
											"Not Able to Enter Value to Related To Text Box : " + accountName);
									log(LogStatus.SKIP,
											"Not Able to Enter Value to Related To Text Box : " + accountName,
											YesNo.Yes);
								}
								...
								sa.assertTrue(false,
										"Not Able to Select Drown Down Value : "
												+ cp.getTabName(projectName, TabName.InstituitonsTab) + " For Label "
												+ PageLabel.Related_Associations);
								log(LogStatus.SKIP,
										"Not Able to Select Drown Down Value : "
							...
							if (i == 6 || i == 7 || i == 8 || i == 9 || i == 10) {
								if (bp.ClickOnCrossButtonForAlreadySelectedItem(projectName, PageName.TaskPage,
										PageLabel.Assigned_To.toString(), false, owner, action.SCROLLANDBOOLEAN, 15)) {
									log(LogStatus.INFO, "Clicked on Cross Button against : " + owner + " For Label : "
											+ PageLabel.Assigned_To.toString(), YesNo.No);

									ThreadSleep(2000);
									// Assigned To
									String AdminName = AdminUserFirstName + " " + AdminUserLastName;
									flag = bp.selectRelatedAssociationOrContactOrRelatedToDropDownAndClickOnItem(
											projectName, PageName.TaskPage, PageLabel.Assigned_To.toString(),
											TabName.TaskTab, AdminName, action.BOOLEAN, 10);
									if (flag) {
										log(LogStatus.INFO, "Selected " + AdminName
												+ " For  Drown Down Value For Label " + PageLabel.Assigned_To,
												YesNo.No);
										ThreadSleep(1000);
										ExcelUtils.writeData(phase1DataSheetFilePath, AdminName, "Task1",
												excelLabel.Variable_Name, "M8Task" + (i), excelLabel.Assigned_To);

									} else {
										sa.assertTrue(false,
												"could not select admin name in " + PageLabel.Assigned_To.toString());
										log(LogStatus.ERROR,
												"could not select admin name in " + PageLabel.Assigned_To.toString(),
												YesNo.Yes);
									...
									sa.assertTrue(false,
											"cross button could not be clicked on " + PageLabel.Assigned_To.toString());
									log(LogStatus.ERROR,
											"cross button could not be clicked on " + PageLabel.Assigned_To.toString(),
											YesNo.Yes);
								...
								ExcelUtils.writeData(phase1DataSheetFilePath, owner, "Task1", excelLabel.Variable_Name,
										"M8Task" + (i), excelLabel.Assigned_To);
							}
							if (click(driver, home.getCreateTaskStatusDropDown(10), "status drop down list",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.FAIL, "clicked on status drop down", YesNo.No);
								ThreadSleep(1000);
								if (click(driver, home.createTaskDropDownValue(status, 10), "status drop down list",
										action.SCROLLANDBOOLEAN)) {
									...
									log(LogStatus.FAIL,
											"Not able to click on status drop down so cannot select " + status,
											YesNo.Yes);
									sa.assertTrue(false,
											"Not able to click on status drop down so cannot select " + status);
								}
							} else {
								log(LogStatus.FAIL, "Not able to click on status drop down so cannot select " + status,
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to click on status drop down so cannot select " + status);
							}
							if (clickUsingJavaScript(driver, gp.getSaveButtonForEvent(projectName, 20), "save",
								...
								ThreadSleep(1000);
								ExcelUtils.writeData(phase1DataSheetFilePath, date, "Task1", excelLabel.Variable_Name,
										"M8Task" + (i), excelLabel.Due_Date);
							} else {
								log(LogStatus.ERROR,
							...
							log(LogStatus.ERROR, "duedate textbox is not visible so task could not be created",
									YesNo.Yes);
							sa.assertTrue(false, "duedate textbox is not visible so task could not be created");
						}
					} else {
						log(LogStatus.ERROR, subject + " : Subject textbox is not visible so task could not be created",
								YesNo.Yes);
		...
		for (int k = 1; k < 11; k++) {
			...
			if (click(driver, gp.getGlobalActionIcon(projectName, 15), "Global Action Related item", action.BOOLEAN)) {
				...
				if (clickUsingJavaScript(driver, gp.getActionItem(projectName, GlobalActionItem.New_Event, 15),
					...
					meetingEventSubject = ExcelUtils.readData(phase1DataSheetFilePath, "Events",
							excelLabel.Variable_Name, "M8Event" + (k), excelLabel.Subject);

					String Name = ExcelUtils.readData(phase1DataSheetFilePath, "Events", excelLabel.Variable_Name,
							"M8Event" + (k), excelLabel.Name);
					String accountName = ExcelUtils.readData(phase1DataSheetFilePath, "Events",
							excelLabel.Variable_Name, "M8Event" + (k), excelLabel.Related_To);
					String location = ExcelUtils.readData(phase1DataSheetFilePath, "Events", excelLabel.Variable_Name,
							"M8Event" + (k), excelLabel.Location);
					ExcelUtils.writeData(phase1DataSheetFilePath, startDate, "Events", excelLabel.Variable_Name,
							"M8Event" + (k), excelLabel.Start_Date);
					ExcelUtils.writeData(phase1DataSheetFilePath, endDate, "Events", excelLabel.Variable_Name,
							"M8Event" + (k), excelLabel.End_Date);

					String[][] event1 = { { PageLabel.Subject.toString(), meetingEventSubject },
							{ PageLabel.Start_Date.toString(), startDate }, { PageLabel.End_Date.toString(), endDate },
					...
					clickUsingJavaScript(driver,
							lp.getrelatedAssociationsdropdownButton(projectName, PageName.TaskPage,
									PageLabel.Related_To.toString(), action.BOOLEAN, 10),
							"dropdown button", action.BOOLEAN);

					flag2 = gp.selectRelatedAssociationOrContactOrRelatedToDropDownAndClickOnItem(projectName,
							PageName.NewEventPopUp, PageLabel.Related_To.toString(), TabName.InstituitonsTab,
							accountName, action.SCROLLANDBOOLEAN, 10);
					...
					if (flag1) {
						if (flag2) {
							log(LogStatus.INFO, "Selected " + accountName + " For Label " + PageLabel.Related_To,
									YesNo.No);
							...
							if (k == 6 || k == 7 || k == 8 || k == 9) {
								if (bp.ClickOnCrossButtonForAlreadySelectedItem(projectName, PageName.TaskPage,
									...
									if (flag) {
										log(LogStatus.INFO, "Selected " + AdminName
												...
												YesNo.No);
										ThreadSleep(1000);
										ExcelUtils.writeData(phase1DataSheetFilePath, AdminName, "Events",
												excelLabel.Variable_Name, "M8Event" + (k), excelLabel.Assigned_To);
								...
								ExcelUtils.writeData(phase1DataSheetFilePath, owner, "Events", excelLabel.Variable_Name,
										"M8Event" + (k), excelLabel.Assigned_To);
							}

							click(driver, home.getAllDayEventCheckBox(10), "all day event check box",
									action.SCROLLANDBOOLEAN);

							ele = gp.getLabelTextBoxForGobalAction(projectName, GlobalActionItem.New_Event,
									PageLabel.Location.toString(), 10);
							...
							if (click(driver, gp.getSaveButtonForEvent(projectName, 10), "Save Button",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "Click on Save Button For Event : " + meetingEventSubject,
										YesNo.No);
								ThreadSleep(500);
							} else {
								sa.assertTrue(false,
										"Not Able to Click on Save Button For Event : " + meetingEventSubject);
								log(LogStatus.SKIP,
										"Not Able to Click on Save Button For Event : " + meetingEventSubject,
										YesNo.Yes);
							}
							...
							BaseLib.sa.assertTrue(false,
									"Not Able to Select " + accountName + " For Label " + PageLabel.Related_To);
							log(LogStatus.ERROR,
									"Not Able to Select " + accountName + " For Label " + PageLabel.Related_To,
									YesNo.Yes);
						...
						BaseLib.sa.assertTrue(false,
								"Event can not created as Some data is not entered : " + meetingEventSubject);
						log(LogStatus.ERROR,
								"Event can not created as Some data is not entered : " + meetingEventSubject,
								YesNo.Yes);
					}
		...
		if (click(driver, home.viewAllAndviewClendarLink(Task.TodayTasks, ViewAllAndViewCalendarLink.viewALl, 10),
				"view all", action.SCROLLANDBOOLEAN)) {
		...
		if (click(driver, home.viewAllAndviewClendarLink(Task.TodayEvents, ViewAllAndViewCalendarLink.ViewCalendar, 10),
				"view calendar", action.SCROLLANDBOOLEAN)) {
		...
		if (clickUsingJavaScript(driver, home.ctreatedTaskAndEventListOnHomePage(Task.TodayTasks).get(0), "",
				action.SCROLLANDBOOLEAN)) {
		...
		if (clickUsingJavaScript(driver, home.ctreatedTaskAndEventListOnHomePage(Task.TodayEvents).get(0), "",
				action.SCROLLANDBOOLEAN)) {
	...
	public void M8Tc027_verifyTodaysSectionUponSelectingDifferentValuesFromDropDown(String projectName) {
		...
		for (int i = 0; i < value.length; i++) {
			...
			if (click(driver, ele, "Select a view of your tasks Arrow Icon for " + defaultValue, action.BOOLEAN)) {
				...
				log(LogStatus.SKIP, "Not able to Click on Select a view of your tasks Arrow Icon for " + defaultValue,
						YesNo.Yes);
			...
			if (expectedList.containsAll(actualList)) {
				...
				log(LogStatus.FAIL,
						defaultValue + " data not verified Actual : " + actualList + " Expected : " + expectedList,
						YesNo.Yes);
				sa.assertTrue(false,
						defaultValue + " data not verified Actual : " + actualList + " Expected : " + expectedList);
			}
	...
	public void M8Tc028_updateAndDeleteTaskAndEvents(String projectName) {
		...
		List<WebElement> lst = new ArrayList<WebElement>();
		ThreadSleep(5000);
		String[] taskAndEvent = { M8OtherSubject, M8EinsteinEvent10Subject };
		String t = "";
		...
		for (int i = 0; i < taskAndEvent.length; i++) {
			...
			if (click(driver, home.createdtaskOnHomePage(t), t, action.BOOLEAN)) {
				...
				if (click(driver, tp.getEditButton(projectName, 30), t, action.BOOLEAN)) {
					...
					try {
						gp.getLabelTextBoxForTask(projectName, PageName.TaskPage, PageLabel.Subject.toString(), 20)
								.clear();
					} catch (Exception e) {
					...
					if (sendKeys(driver,
						...
						if (clickUsingJavaScript(driver, lp.getCustomTabSaveBtn(projectName, 20), "save",
							...
							if (click(driver, home.createdtaskOnHomePage(upadtedTask), upadtedTask, action.BOOLEAN)) {
								log(LogStatus.PASS, "Task/Event Updated " + upadtedTask + " so going to delete it ",
										YesNo.No);
								if (click(driver, cp.getDeleteButton(projectName, 20), "Delete Button",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Able to Click on Delete button : " + upadtedTask, YesNo.No);
									ThreadSleep(2000);
									if (click(driver, cp.getDeleteButtonPopUp(projectName, 20), "Delete Button",
											action.BOOLEAN)) {
										log(LogStatus.INFO, "Able to Click on Delete button on PopUp : " + upadtedTask,
												YesNo.No);
										ThreadSleep(2000);
										lp.clickOnTab(projectName, TabName.HomeTab);
										...
										sa.assertTrue(false,
												"Not Able to Click on Delete button on PopUp : " + upadtedTask);
										log(LogStatus.SKIP,
												"Not Able to Click on Delete button on PopUp : " + upadtedTask,
												YesNo.Yes);
									}
									...
									log(LogStatus.SKIP, "Not Able to Click on Delete button : " + upadtedTask,
											YesNo.Yes);
								}

							} else {
								log(LogStatus.FAIL,
										"Task/Event has not been Updated " + upadtedTask + " so cannot delete it ",
										YesNo.Yes);
								sa.assertTrue(false,
										"Task/Event has not been Updated " + upadtedTask + " so cannot delete it ");
							}
				...
				log(LogStatus.FAIL, "Not able to Clicked on " + t + " so cannot update it to " + upadtedTask,
						YesNo.Yes);
	...
	public void M8Tc029_verifyCreateButtonInStandardTheme(String projectName) {
		...
		for (int i = 0; i < sdgName.length; i++) {
			...
			if (click(driver, home.sdgGridSideIcons(sdgName[i], SDGGridSideIcons.Open_SDG_Record, 10),
					"Open_SDG_Record", action.SCROLLANDBOOLEAN)) {
				log(LogStatus.PASS, "click on Open_SDG_Record icon so cannot search My Call List name : " + M8CON1FName
						+ " " + M8CON1LName, YesNo.Yes);
				ThreadSleep(5000);
				String parentid = switchOnWindow(driver);
				if (parentid != null) {
					ThreadSleep(5000);
					if (click(driver, lp.getRelatedTab(projectName, RelatedTab.Related.toString(), 30), "Related",
							action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "clicked on related tab", YesNo.No);
						ThreadSleep(1000);
						if (click(driver, home.openSDGRecordOrSettingsPageNewBtnInActions("Actions", "New", 10),
								"new button", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "clicked on new button", YesNo.No);
							ThreadSleep(1000);

							if (sendKeys(driver, home.openSDGRecordOrSettingsPageActionPopUptextBoxXpath(
									excelLabel.Name.toString(), 10), buttonName[i], "name", action.SCROLLANDBOOLEAN)) {
							...
							if (sendKeys(driver, home.openSDGRecordOrSettingsPageActionPopUptextBoxXpath(
									excelLabel.Event.toString(), 10), event[i], "event", action.SCROLLANDBOOLEAN)) {
							...
							if (sendKeys(driver,
									home.openSDGRecordOrSettingsPageActionPopUptextBoxXpath(
											excelLabel.Action_Order.toString(), 10),
									ActionOrder[i], "ActionOrder", action.SCROLLANDBOOLEAN)) {
							...
							if (click(driver,
									home.openSDGRecordOrSettingsPageActionPopUptextBoxXpath(
											excelLabel.Action_Type.toString(), 10),
									"action type", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "clicked on action type drop down", YesNo.No);
								if (click(driver, home.ActionTypeDropDownList(ActionType[i], 10), "",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked ActionType name " + ActionType[i], YesNo.No);
								} else {
									log(LogStatus.PASS, "Not able to clicked on  ActionType name " + ActionType[i],
											YesNo.No);
									sa.assertTrue(false, "Not able to clicked on ActionType name " + ActionType[i]);
								}
							} else {
								log(LogStatus.PASS,
										"Not able to clicked on action type drop down so cannot select" + ActionType[i],
										YesNo.No);
								sa.assertTrue(false, "Not able to clicked on action type drop down so cannot select"
										+ ActionType[i]);
							}
							if (sendKeys(driver,
									home.openSDGRecordOrSettingsPageActionPopUptextBoxXpath(
											excelLabel.Event_Payload.toString(), 10),
									payLoad[i], "Event_Payload", action.SCROLLANDBOOLEAN)) {
				...
				log(LogStatus.FAIL, "Not able to click on Open_SDG_Record icon so cannot check create button ",
						YesNo.Yes);
			...
			if (click(driver, home.sdgGridSideIcons(sdgName[i], SDGGridSideIcons.Open_SDG_Record, 10),
				...
				if (parentid != null) {
					...
					if (click(driver, lp.getRelatedTab(projectName, RelatedTab.Related.toString(), 30), "Related",
						...
						log(LogStatus.PASS, "clicked on related tab", YesNo.No);
						ThreadSleep(1000);
						if (click(driver, home.createdButtonEditAndDeleteBtn(buttonName[i], "Edit", 30), "edit button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "clicked on new button", YesNo.No);
							ThreadSleep(1000);
							...
							if (click(driver,
									...
									"action type", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "clicked on action type drop down", YesNo.No);
								if (click(driver, home.ActionTypeDropDownList("List", 10), "",
										action.SCROLLANDBOOLEAN)) {
								...
								log(LogStatus.PASS,
										"Not able to clicked on action type drop down so cannot select List",
										YesNo.Yes);
								sa.assertTrue(false,
										"Not able to clicked on action type drop down so cannot select List");
							}
							...
							log(LogStatus.PASS, "not able to clicked on edit button so cannot change action type ",
									YesNo.Yes);
				...
				ThreadSleep(5000);
				if (click(driver,
						home.sdgGridSideIconsForLightTheme(sdgName[i],
								SDGGridSideIcons.Side_DropDOwnButtonforStandardTheme, 10),
						"drop down  button sdg grid", action.SCROLLANDBOOLEAN)) {
				...
				log(LogStatus.FAIL, "Not able to click on Open_SDG_Record icon so cannot change action type ",
						YesNo.Yes);
					...
					if (click(driver, lp.getRelatedTab(projectName, RelatedTab.Related.toString(), 30), "Related",
						...
						if (click(driver, home.createdButtonEditAndDeleteBtn(buttonName[i], "Edit", 30), "edit button",
							...
							if (click(driver,
									...
									"action type", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "clicked on action type drop down", YesNo.No);
								if (click(driver, home.ActionTypeDropDownList("Row Button", 10), "",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked ActionType name Row Button", YesNo.No);
								} else {
									log(LogStatus.PASS, "Not able to clicked on  ActionType name Row Button",
											YesNo.Yes);
		...
		if (edit.clickOnEditPageLink()) {
			...
			for (int i = 0; i < sdgNames.length; i++) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					ThreadSleep(5000);
					switchToDefaultContent(driver);
					click(driver, home.getSelectThemeinputBoxButton(10), "input box", action.SCROLLANDBOOLEAN);
					ThreadSleep(2000);
					List<WebElement> themelistwebelement = home.sdgGridSelectThemeList();
					WebElement selectedTheme = null;
					for (int i1 = 0; i1 < themelistwebelement.size(); i1++) {

						if (themelistwebelement.get(i1).getText().equalsIgnoreCase("Light")) {

							selectedTheme = themelistwebelement.get(i1);
							break;
						}

					}

					if (clickUsingJavaScript(driver, selectedTheme, "Light theme xpath", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "Select Ligth Theme for " + sdgGrid[i], YesNo.No);
			...
			if (click(driver, home.getCustomTabSaveBtn(projectName, 10), "save button", action.BOOLEAN)) {
				...
				if (clickUsingJavaScript(driver, edit.getBackButton(10), "back button", action.BOOLEAN)) {
					...
					ThreadSleep(5000);
					WebElement[] ele2 = {
							home.sdgGridSideIconsForLightTheme(SDGGridName.Deals,
									SDGGridSideIcons.Side_DropDOwnButtonforLightTheme, 10),
							home.sdgGridSideIconsForLightTheme(SDGGridName.Fundraising,
									SDGGridSideIcons.Side_DropDOwnButtonforLightTheme, 10),
							home.sdgGridSideIconsForLightTheme(SDGGridName.My_Call_List,
					...
					String[] labelName = { "Deal", "FundRaising", "My Call List" };
					String[] filterIconName = { "Manage_fields", "Setup", "Reload" };
					SDGGridName[] sdgGridNames = { SDGGridName.Deals, SDGGridName.Fundraising,
							SDGGridName.My_Call_List };
					int j = 0;
					for (WebElement webElements : ele2) {
						if (webElements != null) {
							log(LogStatus.PASS, "Drop Down button is displaying for " + labelName[j], YesNo.No);
							if (click(driver, webElements, "drop down  button of " + labelName[j],
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "Clicked on Drop Down button for " + labelName[j], YesNo.No);
								ThreadSleep(2000);
								int i = 0;
								WebElement[] ele3 = {
										home.sdgGridSideIconsForLightTheme(sdgGridNames[j],
												SDGGridSideIcons.Manage_fields, 10),
										home.sdgGridSideIconsForLightTheme(sdgGridNames[j], SDGGridSideIcons.Setup, 10),
										home.sdgGridSideIconsForLightTheme(sdgGridNames[j], SDGGridSideIcons.Reload,
												10) };
								for (WebElement webElements1 : ele3) {
									if (webElements1 != null) {
										log(LogStatus.PASS,
												filterIconName[i] + "Drop Down menu is displaying for " + labelName[j],
												YesNo.No);

									} else {
										log(LogStatus.PASS, filterIconName[i] + " Drop Down menu is not displaying for "
												+ labelName[j], YesNo.Yes);
										sa.assertTrue(false, filterIconName[i] + "Drop Down menu is not displaying for "
												+ labelName[j]);
									}
									i++;
								}
							} else {
								log(LogStatus.FAIL, "Not able to Click on Drop Down button for " + labelName[j],
										YesNo.Yes);
								sa.assertTrue(false, "Not able to Click on Drop Down button for " + labelName[j]);
							}
					...
					WebElement[] ele4 = {
							home.sdgGridSideIconsForLightTheme(SDGGridName.Deals, SDGGridSideIcons.Toggle_Filters, 10),
							home.sdgGridSideIconsForLightTheme(SDGGridName.Fundraising, SDGGridSideIcons.Toggle_Filters,
									10),
							home.sdgGridSideIconsForLightTheme(SDGGridName.My_Call_List,
									SDGGridSideIcons.Toggle_Filters, 10) };
					for (WebElement webElements : ele4) {
						if (webElements != null) {
							log(LogStatus.PASS, "toggle filter button is displaying for " + labelName[j1], YesNo.No);
						} else {
							log(LogStatus.FAIL, "toggle filter button is not displaying for " + labelName[j1],
									YesNo.Yes);
							sa.assertTrue(false, "toggle filter is not displaying for " + labelName[j1]);
						}
						j1++;
					...
					ThreadSleep(5000);
					SDGGridName[] sdgName = { SDGGridName.Deals, SDGGridName.Fundraising, SDGGridName.My_Call_List };
					String[] buttonName = { M8CreateDealButtonName, M8CreateFundRaisingButtonName,
							M8CreateContactButtonName };
					for (int i = 0; i < sdgName.length; i++) {
						if (home.sdgGridHeadersCreateButtonList(sdgName[i], buttonName[i]).size() >= 10) {
							log(LogStatus.PASS, buttonName[i] + " is displaying on " + sdgName[i], YesNo.No);
		...
		if (edit.clickOnEditPageLink()) {
			...
			for (int i = 0; i < sdgNames.length; i++) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					if (clickUsingJavaScript(driver, selectedTheme, "Dark theme xpath", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "Select Dark Theme for " + sdgGrid[i], YesNo.No);
		...
		if (edit.clickOnEditPageLink()) {
			...
			for (int i = 0; i < sdgNames.length; i++) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					ThreadSleep(5000);
					switchToDefaultContent(driver);
					click(driver, home.getSelectThemeinputBoxButton(10), "Select theme Input box",
							action.SCROLLANDBOOLEAN);
					ThreadSleep(2000);
					List<WebElement> themelistwebelement = home.sdgGridSelectThemeList();
					WebElement selectedTheme = null;
					...
					if (clickUsingJavaScript(driver, selectedTheme, "Standard theme xpath", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "Select Standard Theme for " + sdgGrid[i], YesNo.No);

					} else {
						log(LogStatus.PASS, "Not able to select Standard Theme for " + sdgGrid[i], YesNo.Yes);
						sa.assertTrue(false, "Not able to select Standard Theme for " + sdgGrid[i]);
					}
	...
	public void M8Tc035_verifyUIofWrenchPopUp(String projectName) {
		...
		for (String[] userPass : userAndPassword) {
			...
			if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				...
				if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Cancel, 10),
						"cancel button", action.SCROLLANDBOOLEAN)) {
			...
			if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				...
				if (userPass[0] == superAdminUserName) {
					if (selectVisibleTextFromDropDown(driver,
							...
							"sdgGridSelectFieldToDisplayFieldFinderDropDown", "Deal Description")) {
						log(LogStatus.PASS, "select Deal Description from field finder", YesNo.No);
						ThreadSleep(2000);
						if (click(driver,
								home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Add, 10),
								"Add button", action.SCROLLANDBOOLEAN)) {
							log(LogStatus.PASS, "clicked on Add button for " + SDGGridName.Deals, YesNo.No);
							ThreadSleep(1000);

							if (click(driver,
									home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Save, 10),
	...
	public void M8Tc037_verifySaveCancelButtonFunctionality(String projectName) {
		...
		for (String[] userPass : userAndPassword) {
			...
			if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				...
				List<WebElement> defaultVisibleList = null;
				if (userPass[0] == superAdminUserName) {
					for (int i = 0; i < 6; i++) {
						if (i == 0) {

							defaultVisibleList = home.sdgGridHeadersLabelNameList(SDGGridName.Deals);
							if (selectVisibleTextFromDropDown(driver,
									home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
									"drop down", "Pipeline Comments")) {
								log(LogStatus.PASS, "select Pipeline Comments(Notes) text from visible field",
										YesNo.No);
								ThreadSleep(2000);
								

								if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										Buttons.close, 10), "Add button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked on close button for " + SDGGridName.Deals, YesNo.No);
									ThreadSleep(5000);

									lst = home.sdgGridHeadersLabelNameList(SDGGridName.Deals);
									if (compareList(defaultVisibleList, lst)) {
										log(LogStatus.PASS, SDGGridName.Deals + " SDG Grid Header Name is verified ",
												YesNo.No);
									} else {
										log(LogStatus.FAIL,
									...
									log(LogStatus.PASS, "Not able to click on close button for " + SDGGridName.Deals,
											YesNo.No);
									sa.assertTrue(false, "Not able to click on close button for " + SDGGridName.Deals);
								}

							} else {
								log(LogStatus.PASS, "Cannot select Notes  from visible field ", YesNo.No);
								sa.assertTrue(false, "Cannot select Notes  from visible field ");
							}
						...
						if (i == 1) {
							...
							if (selectVisibleTextFromDropDown(driver,
								...
								if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										Buttons.Cancel, 10), "Cancel button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked on Cancel button for " + SDGGridName.Deals, YesNo.No);
									ThreadSleep(5000);
									...
									log(LogStatus.PASS, "Not able to click on Cancel button for " + SDGGridName.Deals,
											YesNo.No);
									sa.assertTrue(false, "Not able to click on Cancel button for " + SDGGridName.Deals);
								}

							} else {
								log(LogStatus.PASS, "Cannot select Notes from field finder", YesNo.No);
								sa.assertTrue(false, "Cannot select Notes from field finder");
							}
							}else{
							...
							if (selectVisibleTextFromDropDown(driver,
								...
								if(click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
									...
									if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										...
										String headerName="Pipeline Comments";
									...
									log(LogStatus.PASS, "Not able to click on Add button for " + SDGGridName.Deals,
											YesNo.No);
									sa.assertTrue(false, "Not able to click on Add button for " + SDGGridName.Deals);
								}
						...
						if (i == 3) {
							...
							if (click(driver,
								...
								defaultVisibleList = home.sdgGridHeadersLabelNameList(SDGGridName.Deals);
								for (int j = 0; j < defaultVisibleList.size(); j++) {

								 textList.add(defaultVisibleList.get(j).getText().trim());
								}

							if (click(driver, home.sdgGridVisibleFieldTextInManageFieldPopUp("Source Firm", 10),
									"Source Firm text", action.SCROLLANDBOOLEAN)) {
								log(LogStatus.PASS, "Click on  Source Firm text from visible field", YesNo.No);
								ThreadSleep(2000);

								if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										Buttons.Move_Up, 10), "Move Up button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked on Move Up button for " + SDGGridName.Deals, YesNo.No);
									ThreadSleep(1000);

									if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
											Buttons.close, 10), "close button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.PASS, "clicked on close button for " + SDGGridName.Deals,
												YesNo.No);
										ThreadSleep(5000);
										...
										if (compareList(textList, actualResult)) {
											log(LogStatus.PASS,
										...
										log(LogStatus.PASS,
												"Not able to click on close button for " + SDGGridName.Deals, YesNo.No);
										sa.assertTrue(false,
												"Not able to click on close button for " + SDGGridName.Deals);
									}

								} else {
									log(LogStatus.PASS, "Not able to click on move up button for " + SDGGridName.Deals,
											YesNo.No);
									sa.assertTrue(false,
											"Not able to click on move up button for " + SDGGridName.Deals);
								}
						...
						if (i == 4) {
							...
							if (click(driver, home.sdgGridVisibleFieldTextInManageFieldPopUp("Source Firm", 10),
								...
								if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										Buttons.Move_Down, 10), "Save button", action.SCROLLANDBOOLEAN)) {
									log(LogStatus.PASS, "clicked on move down button for " + SDGGridName.Deals,
											YesNo.No);
									ThreadSleep(1000);

									if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
											Buttons.Cancel, 10), "close button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.PASS, "clicked on cancel button for " + SDGGridName.Deals,
												YesNo.No);
										ThreadSleep(5000);
										...
										log(LogStatus.PASS,
												"Not able to click on cancel button for " + SDGGridName.Deals,
												YesNo.No);
										sa.assertTrue(false,
												"Not able to click on cancel button for " + SDGGridName.Deals);
									}
									...
									log(LogStatus.PASS,
											"Not able to click on move down button for " + SDGGridName.Deals, YesNo.No);
									sa.assertTrue(false,
											"Not able to click on move down button for " + SDGGridName.Deals);
								}
						...
						if (i == 5) {
							...
							if (click(driver,
								...
								if (click(driver, home.sdgGridVisibleFieldTextInManageFieldPopUp("Source Firm", 10),
									...
									if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
											Buttons.Move_Up, 10), "Save button", action.SCROLLANDBOOLEAN)) {
									...
									log(LogStatus.PASS, "Cannot select Source Firm from visible field finder",
											YesNo.No);
									sa.assertTrue(false, "Cannot select Source Firm from visible field finder");
								}
								...
								log(LogStatus.PASS, "Not able to click on manage field icon of " + SDGGridName.Deals,
										YesNo.No);
								sa.assertTrue(false, "Not able to click on manage field icon of " + SDGGridName.Deals);
							}
						}
				...
				if (userPass[0] == superAdminUserName) {
					for (int i = 0; i < 3; i++) {
						if (i == 0) {
							if (selectVisibleTextFromDropDown(driver,
									home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
									"drop down", "Deal Keywords")) {
								log(LogStatus.PASS, "select Deal Keywords text from  field finder", YesNo.No);
								ThreadSleep(2000);
								click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
										Buttons.Add, 10), "Add button", action.SCROLLANDBOOLEAN);
						...
						if (i == 1) {
							...
							if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
								...
								if (selectVisibleTextFromDropDown(driver,
									home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
									"drop down", "Company Name-->Institution")) {
								log(LogStatus.PASS, "select Company Name-->Institution text from field finder",
										YesNo.No);
								ThreadSleep(2000);
								if (selectVisibleTextFromDropDown(driver,
										home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
										"drop down", "Bank Name")) {
									log(LogStatus.PASS, "select Bank Name text from field finder", YesNo.No);
									ThreadSleep(2000);
									if (click(driver,
											home.sdgGridFieldFinderRemoveSelectedFieldRemoveButtonInManageFieldPopUp(
													"Bank Name", 10),
											"Bank Name remove button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.PASS, "clicked on remove button for " + SDGGridName.Deals,
												YesNo.No);
										ThreadSleep(1000);
										...
										log(LogStatus.PASS,
												"Not able to click on remove button for " + SDGGridName.Deals,
												YesNo.No);
										sa.assertTrue(false,
												"Not able to click on remove button for " + SDGGridName.Deals);
									}
						...
						if (i == 2) {
							...
							if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
								...
								if (selectVisibleTextFromDropDown(driver,
									home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
									"drop down", "Company Name-->Institution")) {
								log(LogStatus.PASS, "select Company Name-->Institution text from finder field", YesNo.No);
								ThreadSleep(2000);
								if (selectVisibleTextFromDropDown(driver,
										home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
										"drop down", "Bank Name")) {
									log(LogStatus.PASS, "select Bank Name text related from finder field", YesNo.No);
									ThreadSleep(2000);
					...
					String headerName = "Bank Name";
					if (compareMultipleList(driver, headerName, home.sdgGridHeadersLabelNameList(SDGGridName.Deals))
							.isEmpty()) {
						log(LogStatus.PASS, SDGGridName.Deals + " SDG Grid Header Name is verified for user", YesNo.No);
					} else {
						log(LogStatus.FAIL, SDGGridName.Deals + " SDG Grid Header Name is not verified for user",
								YesNo.Yes);
	...
	@Parameters({ "projectName" })
	@Test
	public void M8Tc039_1_createFieldsForCustomObject(String projectName) {
		SetupPageBusinessLayer setup = new SetupPageBusinessLayer(driver);
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		...
		if (home.clickOnSetUpLink()) {
			...
			String[][] labelAndValues = { { M8_Object1FieldName, M8_Object1, null, null },
					{ M8_Object2FieldName, M8_Object2, null, null }, { M8_Object3FieldName, M8_Object3, null, null },
					{ M8_Object4FieldName, M8_Object4, null, null } };
			for (String[] objects : labelAndValues) {

				String[][] valuesandLabel = { { objects[2], objects[3] } };

				if (setup.addCustomFieldforFormula(environment, mode, object.Deal,
						...
						if (i == 0) {
							if (selectVisibleTextFromDropDown(driver,
								...
								if (selectVisibleTextFromDropDown(driver,
									...
									if (selectVisibleTextFromDropDown(driver,
											home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
											"drop down", M8_Object3)) {
										log(LogStatus.PASS, "select " + M8_Object3 + " text from visible field",
												YesNo.No);
										ThreadSleep(2000);
										click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(
												Buttons.Add, 10), "Add button", action.SCROLLANDBOOLEAN);
										...
										log(LogStatus.PASS, "Cannot select " + M8_Object3 + " from field finder",
												YesNo.No);
	...
	public void M8Tc040_1_removeEditPermissionOfPEStandardUser(String projectName) {
		...
		for (String userName : userNames) {
			...
			if (home.clickOnSetUpLink()) {
				String parentID = switchOnWindow(driver);
				if (parentID != null) {
					log(LogStatus.INFO, "Able to switch on new window, so going to remove " + permission + " for " + a
							+ " " + b + " " + c + " " + d, YesNo.No);
					ThreadSleep(500);
					if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
						...
						ThreadSleep(2000);
						if (setup.permissionChangeForUserONObject(driver, userName, new String[][] { { a, permission },
								{ b, permission }, { c, permission }, { d, permission } }, 20)) {
							log(LogStatus.PASS, permission + " permission change for " + userName + " on object " + a
									+ " " + b + " " + c + " " + d, YesNo.No);
						} else {
							sa.assertTrue(false, permission + " permission not change for " + userName + " on object "
									+ a + " " + b + " " + c + " " + d);
							log(LogStatus.FAIL, permission + " permission not change for " + userName + " on object "
									+ a + " " + b + " " + c + " " + d, YesNo.Yes);
						}
					} else {
					...
					log(LogStatus.FAIL, "could not find new window to switch, so cannot to remove " + permission
							+ " for " + a + " " + b + " " + c + " " + d, YesNo.Yes);
					sa.assertTrue(false, "could not find new window to switch, to remove " + permission + " for " + a
							+ " " + b + " " + c + " " + d);
				}
	...
	public void M8Tc040_2_checkErrorMsgAfterRemoveEditPermissionOfPEStandardUser(String projectName) {
		...
		if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
			...
			for (int i = 0; i < 1; i++) {
				if (i == 0) {
					if (selectVisibleTextFromDropDown(driver,
							home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10), "drop down",
							"Deal Type")) {
						log(LogStatus.PASS, "select Deal Type text from visible field", YesNo.No);
						ThreadSleep(2000);
						click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Add, 10),
								"Add button", action.SCROLLANDBOOLEAN);
						...
						if (click(driver,
							...
							if (ele != null) {
								if (ele.getText().trim()
										.equalsIgnoreCase(HomePageErrorMessage.selectFieldPopUpErrorMessage)) {
									log(LogStatus.PASS, "Error Message is verified "
											+ HomePageErrorMessage.selectFieldPopUpErrorMessage, YesNo.No);
								} else {
									log(LogStatus.PASS, "Error Message is not verified "
											+ HomePageErrorMessage.selectFieldPopUpErrorMessage, YesNo.Yes);
									sa.assertTrue(false, "Error Message is not verified "
											+ HomePageErrorMessage.selectFieldPopUpErrorMessage);
								}
							} else {
							...
							click(driver,
									home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.close, 10),
									"close button", action.SCROLLANDBOOLEAN);
						} else {
	...
	public void M8Tc041_1_AddEditPermissionOfPEStandardUser(String projectName) {
		...
		String permission = "Manage All";
		for (String userName : userNames) {
			switchToDefaultContent(driver);
			if (home.clickOnSetUpLink()) {
				String parentID = switchOnWindow(driver);
				if (parentID != null) {
					log(LogStatus.INFO, "Able to switch on new window, so going to add " + permission + " for " + a
							+ " " + b + " " + c + " " + d, YesNo.No);
					ThreadSleep(500);
					if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
						log(LogStatus.INFO, "click on Object : " + object.Profiles, YesNo.No);
					...
					log(LogStatus.FAIL, "could not find new window to switch, so cannot to add " + permission + " for "
							+ a + " " + b + " " + c + " " + d, YesNo.Yes);
					sa.assertTrue(false, "could not find new window to switch, to add " + permission + " for " + a + " "
							+ b + " " + c + " " + d);
				}
					...
					if (selectVisibleTextFromDropDown(driver,
						...
						if (click(driver,
							...
							if (compareMultipleList(driver, headerName, lst).isEmpty()) {
								...
								log(LogStatus.FAIL, SDGGridName.Deals + " SDG Grid Header Name is not verified ",
										YesNo.Yes);
								sa.assertTrue(false, SDGGridName.Deals + " SDG Grid Header Name is not verified ");
							}
						} else {
	...
	public void M8Tc042_1_createFieldsForCustomObjectWithMaxChar(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			log(LogStatus.ERROR, "Not able to click on setup link so cannot create Fields Objects for custom object",
					YesNo.Yes);
	...
	public void M8Tc042_2_verifycreatedFieldsForCustomObjectWithMaxChar(String projectName) {
		...
		for (String[] userPass : userAndPassword) {
			...
			if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				...
				log(LogStatus.PASS, "clicked on manage field icon of " + SDGGridName.Deals, YesNo.No);
				List<WebElement> lst = home.sdgGridSelectVisibleFieldsListInManageFieldPopUp();
				String fieldName = "Deal sample 123456:\"?><!@#$%^&*()1234567-->Institution";
					...
					for (int i = 0; i < 1; i++) {
						if (i == 0) {
							...
							if (selectVisibleTextFromDropDown(driver,
									home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10),
									"drop down", fieldName)) {
								log(LogStatus.PASS, fieldName + " text from visible field", YesNo.No);
								ThreadSleep(2000);
			...
			if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				...
				if (selectVisibleTextFromDropDown(driver,
					...
					if (selectVisibleTextFromDropDown(driver,
						...
						if (ele != null) {
							if (ele.getText().trim()
									.equalsIgnoreCase(HomePageErrorMessage.FieldPopUpToastErrorMessage(fieldName))) {
								log(LogStatus.PASS, "Error Message is verified "
										+ HomePageErrorMessage.selectFieldPopUpErrorMessage, YesNo.No);
							} else {
								log(LogStatus.PASS, "Error Message is not verified "
	...
	public void M8Tc044_1_renameCreatedStandardObjectAndCheckImpact(String projectName) {
			...
			ThreadSleep(3000);
				if (setup.searchStandardOrCustomObject(environment, mode,object.Override)){
					...
					if(selectVisibleTextFromDropDown(driver, setup.getOverrideSetupComponentDropdown(10), "Override setup component dropdown", "Custom Field")){
						...
						if(selectVisibleTextFromDropDown(driver, setup.getOverrideObjectDropdown(10), "Override object dropdown", "Deal")){
							log(LogStatus.INFO, "Select Deal text in object dropdown in override setup page", YesNo.No);
							ThreadSleep(5000);
							if(setup.updateFieldLabelInOverridePage(driver, "Source Firm", "New Source Firm", action.SCROLLANDBOOLEAN)){
								log(LogStatus.INFO, "Field label: Source firm successfully update to New source firm", YesNo.No);
								
							}else{
								log(LogStatus.ERROR, "Not able to update Field label: Source firm successfully update to New source firm", YesNo.Yes);
								sa.assertTrue(false, "Not able to update Field label: Source firm successfully update to New source firm");	
							}
						}else{
							log(LogStatus.ERROR, "Not able to select text: deal in  object dropdown in override page", YesNo.Yes);
							sa.assertTrue(false, "Not able to select text: deal in  object dropdown in override page");
						}
					}else{
		...
		if (home.clickOnSetUpLink()) {
			...
			ThreadSleep(3000);
			if(click(driver, setup.getObjectManager_Lighting(10), "object manager tab", action.BOOLEAN)){
			
				if (setup.updateCreatedCustomFieldforFormula(environment, mode, object.Deal,
					ObjectFeatureName.FieldAndRelationShip, "Deal Sample 1", "Deal Sample 1 New")) {
				log(LogStatus.PASS, "Update Field Object name for created object for : Deal sample 1 New", YesNo.No);			
				} else {
				log(LogStatus.PASS, "Deal Sample 1 Field Object name is not udated : Deal sample 1 New", YesNo.Yes);
				sa.assertTrue(false, "Deal Sample 1  Field Object name is not udated for : Deal sample 1 New");
			
				}	
			}else{
				log(LogStatus.PASS, "Not able to click on object manager tab", YesNo.Yes);
				sa.assertTrue(false, "Not able to click on object manager tab");
	...
	public void M8Tc044_2_verifyRenameObjectInDealSDGGrid(String projectName) {
		...
		String headerName = "Deal Sample 1 New";
		ThreadSleep(5000);
	...
	@Parameters({ "projectName" })
	@Test
	public void M8Tc045_1_removeObjectPermissionFromObject(String projectName) {
		SetupPageBusinessLayer setup = new SetupPageBusinessLayer(driver);
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		...
		if (home.clickOnSetUpLink()) {
			...
			if (setup.giveAndRemoveObjectPermissionFromObjectManager(object.Deal,
					ObjectFeatureName.FieldAndRelationShip, PermissionType.removePermission, "New Source Firm","Source Firm",
					"PE Standard User")) {
			...
			log(LogStatus.ERROR,
					"Not able to click on setup link so cannot remove permission on New Source Firm from Deal Object",
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on setup link so cannot remove permission on New Source Firm from Deal Object");
		}
	...
	@Parameters({ "projectName" })
	@Test
	public void M8Tc045_3_giveObjectPermissionFromObject(String projectName) {
		SetupPageBusinessLayer setup = new SetupPageBusinessLayer(driver);
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		ContactsPageBusinessLayer con = new ContactsPageBusinessLayer(driver);
		...
		if (home.clickOnSetUpLink()) {
			...
			log(LogStatus.ERROR,
					"Not able to click on setup link so cannot add permission on New Source Firm from Deal Object",
					YesNo.Yes);
			sa.assertTrue(false,
					"Not able to click on setup link so cannot add permission on New Source Firm from Deal Object");
		}
	...
	@Parameters({ "projectName" })
	@Test
	public void M8Tc046_1_createCloneUser(String projectName) {
		SetupPageBusinessLayer setup = new SetupPageBusinessLayer(driver);
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		ContactsPageBusinessLayer con = new ContactsPageBusinessLayer(driver);
			...
			if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
				...
				if (clickUsingJavaScript(driver, ele, "PE Standard user link", action.BOOLEAN)) {
					...
					if (click(driver, setup.getCloneButton(30), "clone button", action.SCROLLANDBOOLEAN)) {
						...
						ThreadSleep(5000);
						switchToFrame(driver, 60, setup.getSetUpPageIframe(60));
						if (sendKeys(driver, setup.getProfileNameTextBox(30), "Cloned PE User",
	...
	@Parameters({ "projectName" })
	@Test
	public void M8Tc046_2_CreateCRMUserWithClonedStandardUserProfile(String projectName) {
		SetupPageBusinessLayer setup = new SetupPageBusinessLayer(driver);
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		String parentWindow = null;
	...
	public void M8Tc046_3_verifySDGHeaderUsingClonedUser(String projectName) {
		...
		if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
			...
			log(LogStatus.PASS, "clicked on manage field icon of " + SDGGridName.Deals, YesNo.No);
			if (selectVisibleTextFromDropDown(driver,
					home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10), "drop down", 1)) {
				log(LogStatus.PASS, "select text from visible field", YesNo.No);
				ThreadSleep(2000);
				click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Add, 10),
						"Add button", action.SCROLLANDBOOLEAN);

				if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Save, 10),
						"save button", action.SCROLLANDBOOLEAN)) {
					log(LogStatus.PASS, "clicked on save button for " + SDGGridName.Deals, YesNo.No);
					ThreadSleep(1000);
					WebElement ele = home.getSelectFieldPopUpErrorMsg(10);
					if (ele != null) {
						if (ele.getText().trim().equalsIgnoreCase(HomePageErrorMessage.selectFieldPopUpErrorMessage)) {
							log(LogStatus.PASS,
									"Error Message is verified " + HomePageErrorMessage.selectFieldPopUpErrorMessage,
									YesNo.No);
						} else {
							log(LogStatus.PASS, "Error Message is not verified "
									+ HomePageErrorMessage.selectFieldPopUpErrorMessage, YesNo.Yes);
					...
					click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.close, 10),
							"close button", action.SCROLLANDBOOLEAN);
				} else {
					log(LogStatus.PASS, "Not able to click on save button for " + SDGGridName.Deals, YesNo.No);
					sa.assertTrue(false, "Not able to click on save button for " + SDGGridName.Deals);
		...
		if (edit.clickOnEditPageLink()) {
			...
			for (int i = 0; i < sdgNames.length; i++) {
				...
				if (click(driver, ele, "sdg grid " + sdgNames[i], action.BOOLEAN)) {
					...
					if (clickUsingJavaScript(driver, selectedTheme, "Light theme xpath", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "Select Light Theme for " + sdgGrid[i], YesNo.No);

					} else {
						log(LogStatus.PASS, "Not able to select Light Theme for " + sdgGrid[i], YesNo.Yes);
						sa.assertTrue(false, "Not able to select Light Theme for " + sdgGrid[i]);
					}
	...
	public void M8Tc048_add20FieldsInWrenchIconPopUp(String projectName) {
		...
		ArrayList<String> fieldFinderList = new ArrayList<String>();
		List<WebElement> relatedFieldList = null;
		lp.CRMLogin(superAdminUserName, adminPassword);
		ThreadSleep(5000);
		if (click(driver, home.sdgGridSideIcons(SDGGridName.Deals, SDGGridSideIcons.Manage_fields, 5),
				"manage field icon", action.SCROLLANDBOOLEAN)) {
			...
			ThreadSleep(2000);
			List<WebElement> visibleList = home.sdgGridSelectVisibleFieldsListInManageFieldPopUp();
			List<WebElement> fieldList = home.sdgGridSelectFieldFinderListInManageFieldPopUp();
			for (int i = 0; i < visibleList.size(); i++) {

				actualList.add(visibleList.get(i).getText().trim());

			}
			for (int i = 0; i < fieldList.size(); i++) {

				fieldFinderList.add(fieldList.get(i).getText().trim());

			}
			fieldFinderList.removeAll(actualList);
			log(LogStatus.INFO, "Field finder list : " + fieldFinderList, YesNo.No);
			int count = 20;
			int alreadyedSelected = alreadyedSelectedFields.size();
			int loopCount = count - alreadyedSelected;
			for (int i = 0; i < loopCount; i++) {
				if (selectVisibleTextFromDropDown(driver,
						home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10), "drop down",
						fieldFinderList.get(i))) {
					log(LogStatus.PASS, "select text from finder field : " + fieldFinderList.get(i), YesNo.No);
					ThreadSleep(3000);
					if (!home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Add, 10).isEnabled()) {
						relatedFieldList = home.sdgGridSelectFieldFinderListInManageFieldPopUp();
						selectVisibleTextFromDropDown(driver,
								home.sdgGridSelectFieldToDisplayFieldFinderDropDownInManageFieldPopUp(10), "drop down",
								relatedFieldList.get(i).getText().trim());
						log(LogStatus.PASS, "select text from finder related field ", YesNo.No);
						ThreadSleep(2000);
					}

					if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Add, 10),
							"Add button", action.SCROLLANDBOOLEAN)) {
						log(LogStatus.PASS, "clicked on add button for visible field : " + (i + 1), YesNo.No);
					} else {
						log(LogStatus.PASS, "not able to clicked on add button for visible field : " + (i + 1),
								YesNo.No);
						sa.assertTrue(false, "not able to clicked on add button for visible field : " + (i + 1));
					}
				} else {
					log(LogStatus.PASS, "Cannot select value" + fieldFinderList.get(i) + " from field finder",
							YesNo.No);
					sa.assertTrue(false, "Cannot select value" + fieldFinderList.get(i) + " from field finder");
				}
			}
			if (click(driver, home.sdgGridSelectFieldToDisplaySaveCancelBtnInManageFieldPopUp(Buttons.Save, 10),
					"save button", action.SCROLLANDBOOLEAN)) {
				...
				// remove size 2 form header list one for checkbox & for action
				// button of the row
				if ((lst.size() - 2) == (loopCount + alreadyedSelected)) {
					log(LogStatus.PASS,
							SDGGridName.Deals + " All " + (loopCount + alreadyedSelected) + " SDG header is added",
							YesNo.No);
				} else {
					log(LogStatus.FAIL,
							SDGGridName.Deals + " All " + (loopCount + alreadyedSelected) + " SDG header is not added",
							YesNo.Yes);
					sa.assertTrue(false,
							SDGGridName.Deals + " All " + (loopCount + alreadyedSelected) + " SDG header is not added");
				}
