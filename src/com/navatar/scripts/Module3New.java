...
//	Scanner scn = new Scanner(System.in);
	...
	public String NavigationMenuTestData_PEExcel = System.getProperty("user.dir")
			+ "\\UploadFiles\\Module 3\\UploadCSV\\Navigation Menu Test Data - Parent - Child - All.csv";
...
//	Scanner scn = new Scanner(System.in);
	...
	// All Navigation Item should be deleted Except - Bulk Email- Bulk Fundraising -
	// Bulk Commitments - Call - Meeting - Task - New Deal- New Institution- New
	// Contact
	// Fund CReation Popupoen without any rdio button i.e no record type should be
	// selected
	...
	public void M3Tc001_2_CreatePreconditionData(String projectName) {
		...
		if (lp.clickOnTab(projectName, TabName.Object2Tab)) {
			...
			ExcelUtils.writeData(phase1DataSheetFilePath, M3Contact1EmailID, "Contacts", excelLabel.Variable_Name,
					"M3CON1", excelLabel.Contact_EmailId);

			if (cp.createContact(projectName, M3Contact1FName, M3Contact1LName, M3Ins1, M3Contact1EmailID,
					M3Contact1RecordType, null, null, CreationPage.ContactPage, null, null)) {
		...
		if (lp.clickOnTab(projectName, TabName.Object1Tab)) {
			...
			if (ip.createInstitution(projectName, environment, mode, M3Ins9, M3Ins9RecordType,
					InstitutionPageFieldLabelText.Parent_Institution.toString(), M3Ins9Parent)) {
				log(LogStatus.INFO, "successfully Created Account/Entity : " + value + " of record type : " + type,
						YesNo.No);
			} else {
				sa.assertTrue(false, "Not Able to Create Account/Entity : " + value + " of record type : " + type);
		...
		if (bp.clickOnTab(projectName, TabName.PartnershipsTab)) {
			...
			sa.assertTrue(false,
					"Not able to click on partnership tab so cannot create partnership: " + M3PartnerShip1);
		}
	...
	public void M3Tc002_VerifyTheNavigationMenuItems(String projectName) {
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify list : " + filesName,
					YesNo.Yes);
			sa.assertTrue(false,
	...
	@Parameters({ "projectName" })
	@Test
	public void M3Tc003_VerifyImpactOnGlobalActionByDoingAllAvailableLinksToGetDisableFromNavatarSetup(
			String projectName) {
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			if (ele == null) {
				log(LogStatus.INFO, navigationLabel[1] + " is not present on " + navigationMenuName + " after uncheck "
						+ setupSideMenuTab, YesNo.No);
			} else {
				log(LogStatus.ERROR, navigationLabel[1] + " should not present on " + navigationMenuName
						+ " after uncheck " + setupSideMenuTab, YesNo.Yes);
				sa.assertTrue(false, navigationLabel[1] + " should not present on " + navigationMenuName
						+ " after uncheck " + setupSideMenuTab);
			}
			...
			log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot uncheck absenece of "
					+ navigationLabel[1], YesNo.Yes);
			sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot uncheck absenece of "
					+ navigationLabel[1]);
		}
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			WebElement ele = npbl.getNavigationLabel(projectName, navigationLabel[1], action.BOOLEAN, 10);
			if (ele != null) {
				log(LogStatus.INFO, navigationLabel[1] + " is  present on " + navigationMenuName + " after check "
						+ setupSideMenuTab, YesNo.No);

			} else {
				log(LogStatus.ERROR, navigationLabel[1] + " should be present on " + navigationMenuName
						+ " after check " + setupSideMenuTab, YesNo.Yes);
				sa.assertTrue(false, navigationLabel[1] + " should be present on " + navigationMenuName
						+ " after check " + setupSideMenuTab);
			...
			log(LogStatus.ERROR,
					"Not Able to Click on " + navigationMenuName + " so cannot check presence of " + navigationLabel[1],
					YesNo.Yes);
			sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot check presence of "
					+ navigationLabel[1]);
		}
	...
	public void M3Tc004_VerifyTheLinkInNavigationMenuWhenEnablefromNavatarSetup(String projectName) {
		...
		NavatarSetupSideMenuTab[] navatarSetupSideMenuTab = { NavatarSetupSideMenuTab.DealCreation,
				NavatarSetupSideMenuTab.IndividualInvestorCreation };
		NavatarSetupSideMenuTab setupSideMenuTab = null;
		...
		String[] urlLabels = { "/apex/navpeII__Deal_Creation?retURL=/lightning/page/home",
				"/apex/navpeII__IndividualInvestor?retURL=/lightning/page/home" };
		...
		for (int i = 0; i < navatarSetupSideMenuTab.length; i++) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				...
				if (ele == null) {
					log(LogStatus.INFO, navigationLabels[i] + " is not present on " + navigationMenuName
							+ " even after enabling " + setupSideMenuTab, YesNo.No);
				} else {
					log(LogStatus.ERROR, navigationLabels[i] + " should not present on " + navigationMenuName
							+ " even after enabling " + setupSideMenuTab, YesNo.Yes);
					sa.assertTrue(false, navigationLabels[i] + " should not present on " + navigationMenuName
							+ "even after enabling " + setupSideMenuTab);

				}

			} else {
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot check absenece of "
						+ navigationLabels[i], YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot check absenece of "
						+ navigationLabels[i]);
			}
...
//		scn.next();
			...
			if (npbl.createNavigationItem(projectName, labelWithValue, 20)) {
				...
				if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
					...
					if (ele != null) {
						...
						log(LogStatus.ERROR,
								navigationLabels[i] + " should be present on " + navigationMenuName + " after creation",
								YesNo.Yes);
						sa.assertTrue(false, navigationLabels[i] + " should be present on " + navigationMenuName
								+ " after creation");
					...
					sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot check presence of "
							+ navigationLabels[i] + " after creation");
				}
		...
		for (int i = 0; i < navigationLabels.length; i++) {
			...
			if (hp.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					...
					if (sendKeys(driver, setup.getQucikSearchInSetupPage(120), "Visualforce Pages", "quick search",
							action.BOOLEAN)) {
						ThreadSleep(3000);
						if (click(driver, setup.getvisualForcePagesLink(120), "visual force link", action.BOOLEAN)) {
							...
							if (click(driver, ele, "preview link for bulk email", action.SCROLLANDBOOLEAN)) {
								...
								if (!flag) {
									log(LogStatus.ERROR, "could not switch to third window , so cannot get " + vfPage[i]
											+ " url link", YesNo.Yes);
									sa.assertTrue(false, "could not switch to third window , so cannot get " + vfPage[i]
											+ " url link");
								}
								ThreadSleep(3000);
		...
		for (int i = 0; i < navatarSetupSideMenuTab.length; i++) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				...
				WebElement ele = npbl.getNavigationLabel(projectName, navigationLabels[i], action.BOOLEAN, 10);
				if (ele != null) {
					log(LogStatus.INFO, navigationLabels[i] + " is present on " + navigationMenuName
							+ " even after disabling " + setupSideMenuTab + " but after creation from Navigation Tab",
							YesNo.No);
				} else {
					log(LogStatus.ERROR, navigationLabels[i] + " should be present on " + navigationMenuName
							+ " even after disabling " + setupSideMenuTab + " but after creation from Navigation Tab",
							YesNo.Yes);
					sa.assertTrue(false, navigationLabels[i] + " should be present on " + navigationMenuName
							+ "even after disabling " + setupSideMenuTab + " but after creation from Navigation Tab");
		...
		for (i = 0; i < BulkActionNavigationLinks.length; i++) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				log(LogStatus.INFO, "Able to Click on " + navigationMenuName + " Going to click on : "
						+ bulkActionNavigationLink + " for creation ", YesNo.No);
				...
				if (ele != null && click(driver, ele, bulkActionNavigationLink, action.BOOLEAN)) {
					...
					log(LogStatus.ERROR, "Not Able to Click on " + bulkActionNavigationLink
							+ " so cannot create data related to this ", YesNo.Yes);
					sa.assertTrue(false, "Not Able to Click on " + bulkActionNavigationLink
							+ " so cannot create data related to this ");
				...
				if (flag) {
					if (i == 0) {
						...
						if (hp.selectFundNameOrCompanyNameOnCreateFundraisings(environment, mode,
								PopUpName.selectFundPopUp, M3Fund1, null)) {
							...
							if (click(driver, home.getSelectFundNamePopUpContinueBtn(), "continue button",
								...
								if (click(driver, home.getSearchBasedOnAccountsAndContactsTab(30),
									...
									ThreadSleep(3000);
									if (hp.applyFilterOnSearchBasedOnAccountsandContacts(
											...
											if (click(driver, hp.getAddToFundraisingListBtn(30),
												...
												if (click(driver,
													...
													if (click(driver, home.getCreateFundraisingConfirmationOkBtn(30),
														...
														switchToDefaultContent(driver);
														if (home.clickOnTab(environment, mode,
																TabName.FundraisingsTab)) {
															log(LogStatus.INFO, "clicked on create fundraising button",
																	YesNo.No);
															if (hp.clickOnAlreadyCreatedItem(projectName, fr, false,
																	120)) {
																log(LogStatus.INFO,
																		"succescfuly found fundraising" + fr, YesNo.No);
															} else {
																log(LogStatus.ERROR,
																		"Not able to found fundraising" + fr,
																		YesNo.Yes);
																sa.assertTrue(false,
																		"Not able to found fundraising" + fr);
															}
															...
															log(LogStatus.ERROR,
																	"Not able to click on fundraising tab so not check created "
																			+ fr,
																	YesNo.Yes);
															sa.assertTrue(false,
																	"Not able to click on fundraising tab so not check created "
						...
						if (hp.selectFundraisingNameOrCommitmentType(environment, mode, M3FRName1, null, null, null,
								CommitmentType.fundraisingName)) {
							if (hp.commitmentInfoAndAdditionalInfo(environment, mode, commitmentInformation, null, null,
									null)) {
								log(LogStatus.INFO,
						...
						if (hp.createNewDealPipeLine(environment, mode, M3Ins10, "Due Diligence", null)) {
							...
							if (hp.clickOnCreateDealButtonAndVerifyingLandingPage(environment, mode, M3Ins10)) {
								...
								sa.assertTrue(false,
										"Not Able to Click on Create Deal Button or Landing Page Not Verified");
								log(LogStatus.SKIP,
										"Not Able to Click on Create Deal Button or Landing Page Not Verified",
										YesNo.Yes);
							}
							...
							log(LogStatus.SKIP, "Not Able to Add values for mandatory Field for Deal Creation popup",
									YesNo.Yes);
						}
					} else {
						...
						if (hp.createIndiviualInvestor(environment, mode, labelNamesAndValue, null, TopOrBottom.TOP)) {
							...
							// ExcelUtils.writeData(smokeFilePath, emailId, "Contacts",
							// excelLabel.Variable_Name,"SmokeC7", excelLabel.Contact_EmailId);
				...
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot click on : "
						+ bulkActionNavigationLink + " for creation ", YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot click on : "
						+ bulkActionNavigationLink + " for creation ");
			}
...
//		}
	...
	public void M3Tc007_VerifyTheCreateNewNavigationLink(String projectName) {
		...
		String[] createNewNavigationLinks = { CreateNew_DefaultValues.New_Deal.toString(),
				...
				CreateNew_DefaultValues.New_Institution.toString(), CreateNew_DefaultValues.New_Institution.toString(),
				CreateNew_DefaultValues.New_Institution.toString(), CreateNew_DefaultValues.New_Institution.toString(),
				CreateNew_DefaultValues.New_Institution.toString(), CreateNew_DefaultValues.New_Institution.toString(),
				CreateNew_DefaultValues.New_Institution.toString(), CreateNew_DefaultValues.New_Institution.toString(),
				CreateNew_DefaultValues.New_Institution.toString() };
		...
		for (i = 0; i < createNewNavigationLinks.length; i++) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				log(LogStatus.INFO, "Able to Click on " + navigationMenuName + " Going to click on : "
						+ createNewNavigationLink + " for creation ", YesNo.No);
				...
				if (ele != null && click(driver, ele, createNewNavigationLink, action.BOOLEAN)) {
					...
					log(LogStatus.ERROR, "Not Able to Click on " + createNewNavigationLink
							+ " so cannot create data related to this ", YesNo.Yes);
					sa.assertTrue(false, "Not Able to Click on " + createNewNavigationLink
							+ " so cannot create data related to this ");
				...
				if (flag) {
					...
					if (i == 0) {
						...
						if (ip.createInstitutionPopUp(projectName, environment, mode, value, type,
								InstitutionPageFieldLabelText.Parent_Institution.toString(), parent)) {
							log(LogStatus.INFO,
									"successfully Created Account/Entity : " + value + " of record type : " + type,
									YesNo.No);
						} else {
							sa.assertTrue(false,
									"Not Able to Create Account/Entity : " + value + " of record type : " + type);
							log(LogStatus.SKIP,
									"Not Able to Create Account/Entity : " + value + " of record type : " + type,
									YesNo.Yes);
						}
				...
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot click on : "
						+ createNewNavigationLink + " for creation ", YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot click on : "
						+ createNewNavigationLink + " for creation ");
			}
			refresh(driver);
			ThreadSleep(5000);
		}
	...
	public void M3Tc008_VerifyTheFunctionlityOfMeetingTypeFieldInTaskPopup(String projectName) {
		...
		String[] newInteractionsNavigationLinks = { NewInteractions_DefaultValues.Meeting.toString(),
				NewInteractions_DefaultValues.Meeting.toString() };
		int i = 0;
		boolean flag = false;
				...
				if (flag) {
					...
					if (click(driver, ele, "Cross/Cancel Buton on Meting pop up ", action.BOOLEAN)) {
						...
						if (homeUrl.equalsIgnoreCase(actualUrl)) {
							log(LogStatus.INFO,
									"After Click on Cross/Cancel Buton on Meeting pop up page is redirected to previous page/url",
									YesNo.Yes);
						} else {
							log(LogStatus.ERROR,
									"After Click on Cross/Cancel Buton on Meeting pop up page is not redirected to previous page/url \n Actual : "
											+ actualUrl + " \n Expeted : " + homeUrl,
									YesNo.Yes);
							sa.assertTrue(false,
									"After Click on Cross/Cancel Buton on Meeting pop up page is not redirected to previous page/url \n Actual : "
	...
	public void M3Tc009_VerifyNavigationLinkAfterClickingBackOnBrowserButton(String projectName) {
		...
		/* navigationMenuName = NavigationMenuItems.Bulk_Actions.toString(); */
		navigationMenuName = NavigationMenuItems.Create.toString();
		String[] newInteractionsNavigationLinks = { BulkActions_DefaultValues.Bulk_Email.toString(),
				BulkActions_DefaultValues.Bulk_Fundraising.toString(),
				BulkActions_DefaultValues.Bulk_Commitments.toString() };
		i = 0;
	...
	public void M3Tc010_VerifyNavigationLabelAlignmentWithImageAndLabelName(String projectName) {
		...
		for (int i = 0; i < navigationMenuNames.length; i++) {
			...
			if (npbl.clickOnNavatarEdgeLink(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				log(LogStatus.INFO, "Able to Click on " + navigationMenuName, YesNo.No);
				if (npbl.clickOnNavatarEdgeLink(projectName, navigationMenuName, action.BOOLEAN, 30)) {
					log(LogStatus.INFO, "Second Time Click on " + navigationMenuName + " so it should be minimzed",
							YesNo.No);
					ThreadSleep(3000);
					ele = npbl.getNavatarQuickLinkMinimize_Lighting(projectName, 5);
					if (ele == null) {
						log(LogStatus.INFO, "After Click on " + navigationMenuName + " Second Time it is minimized",
								YesNo.No);
					} else {
						log(LogStatus.ERROR,
								"After Click on " + navigationMenuName + " Second Time it is not minimized", YesNo.Yes);
						sa.assertTrue(false,
								"After Click on " + navigationMenuName + " Second Time it is not minimized");
					}
				} else {
					log(LogStatus.ERROR, "Second Time Click on " + navigationMenuName + " so it is not minimzed",
							YesNo.Yes);
					sa.assertTrue(false, "Second Time Click on " + navigationMenuName + " so it is not minimzed");
				}
			} else {
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify image and icon",
						YesNo.Yes);
	...
	public void M3Tc011_1_UploadCsvToCreateTheNavigationData_Action(String projectName) {
		...
		if (flag) {
			...
			if (dataload.dataImportWizard(ObjectName.Navigation, ObjectType.Custom,
					"\\UploadFiles\\Module 3\\UploadCSV\\Navigation Menu Test Data - Child.csv",
					DataImportType.AddNewRecords, "9")) {
	...
	public void M3Tc011_2_UploadCsvToCreateTheNavigationData_Impact(String projectName) {
		...
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		System.err.println(
				NavigationMenuTestData_PEExcel + " >>>>>>>>>>>>><<<<<<<<<<<< " + NavigationMenuTestData_PESheet);
		...
		if (!csvRecords.isEmpty()) {
			...
			Map<String, Integer> navigationParentLabelWithSortedOrder = sortByValue(true,
					navigationParentLabelWithOrder);
			Map<String, String> navigationParentLabelWithChildAndOrder = navigationParentLabelWithChildAndOrder(
					csvRecords);
			Map<String, String> navigationParentLabelWithChildSorted = navigationParentLabelWithChildSorted(
					navigationParentLabelWithChildAndOrder);

			// Verification on navigation menu
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				log(LogStatus.INFO, "Able to Click on " + navigationMenuName, YesNo.No);
				npbl.verifyingNavigationMenuLink(projectName, navigationParentLabelWithSortedOrder,
						navigationParentLabelWithChildSorted, action.BOOLEAN, 0);
				;
			} else {
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify order",
						YesNo.Yes);
	...
	public void M3Tc012_RenameNavigationLabelAndVerifyImpactOnNavigationMenu(String projectName) {
		...
		String updatedName = "";
		WebElement ele;
		String navigationLabel = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Navigation_Label_Name);
		String updatedNavigationLabel = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath",
				excelLabel.TestCases_Name, currentlyExecutingTC, excelLabel.Update_Navigation_Label_Name);
		String navLb;
		...
		if (npbl.clickOnTab(projectName, navigationTab)) {
			...
			if (npbl.clickOnAlreadyCreatedItem(projectName, navLb, true, 15)) {
				...
				if (click(driver, ele, ShowMoreActionDropDownList.Edit.toString(), action.BOOLEAN)) {
					...
					ele = npbl.getNavigationField(projectName, CSVLabel.Navigation_Label.toString(), action.BOOLEAN,
							20);
					...
					if (sendKeys(driver, ele, updatedName, CSVLabel.Navigation_Label.toString(), action.BOOLEAN)) {
						...
						ThreadSleep(2000);
						if (click(driver, npbl.getCustomTabSaveBtn(projectName, 10), "save button",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.ERROR, "Click on save Button : " + navLb, YesNo.No);
							ThreadSleep(2000);
							if (ExcelUtils.writeDataOnCSVFile(NavigationMenuTestData_PEExcel, updatedName,
									CSVLabel.Navigation_Label.toString(), navLb,
									CSVLabel.Navigation_Label.toString())) {
								log(LogStatus.INFO, updatedName + " value has been written under "
										+ CSVLabel.Navigation_Label.toString() + " for " + navLb, YesNo.No);
							} else {
								log(LogStatus.ERROR, updatedName + " value has not been written under "
										+ CSVLabel.Navigation_Label.toString() + " for " + navLb, YesNo.Yes);
								sa.assertTrue(false, updatedName + " value has not been written under "
										+ CSVLabel.Navigation_Label.toString() + " for " + navLb);
							}
				...
				log(LogStatus.ERROR, "Item not found: " + navLb + " on Tab : " + navigationTab
						+ " so cannot update navigation label for " + navLb, YesNo.Yes);
				sa.assertTrue(false, "Item not found: " + navLb + " on Tab : " + navigationTab
						+ " so cannot update navigation label for " + navLb);
			}
		} else {
			log(LogStatus.ERROR,
					"Not Able to Click on Tab : " + navigationTab + " so cannot update navigation label for " + navLb,
					YesNo.Yes);
			sa.assertTrue(false,
					"Not Able to Click on Tab : " + navigationTab + " so cannot update navigation label for " + navLb);
		}
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify update label Name : "
					+ updatedName, YesNo.Yes);
			sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot verify update label Name : "
					+ updatedName);
		}
	...
	public void M3Tc013_ChangeTheOrderOfTheLabelAndVerifyImpactOnNavigationMenu(String projectName) {
		...
		navigationMenuName = NavigationMenuItems.Create.toString();
		String updatedOrder = "";
		String[] navigationLabels = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Navigation_Label_Name).split(breakSP);
		String[] updatedOrderLabel = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Updated_Order).split(breakSP);
		...
		for (int j = 0; j < navigationLabels.length; j++) {
			...
			if (npbl.enterValueOnEditPopUpForNavigationTab(projectName, navigationLabel,
					new String[][] { { orderLabel, updatedOrder } }, 20)) {
				log(LogStatus.INFO, "Able to update " + orderLabel + " with value " + updatedOrder + " field for "
						+ navigationLabels, YesNo.No);
				if (ExcelUtils.writeDataOnCSVFile(NavigationMenuTestData_PEExcel, updatedOrder,
						CSVLabel.Navigation_Label.toString(), navigationLabel, CSVLabel.Order.toString())) {
					log(LogStatus.INFO, updatedOrder + " value has been written under " + CSVLabel.Order.toString()
							+ " for " + navigationLabel, YesNo.No);
				} else {
					log(LogStatus.ERROR, updatedOrder + " value has not been written under " + CSVLabel.Order.toString()
							+ " for " + navigationLabel, YesNo.Yes);
					sa.assertTrue(false, updatedOrder + " value has not been written under " + CSVLabel.Order.toString()
							+ " for " + navigationLabel);
				}

			} else {
				log(LogStatus.ERROR, "Not Able to update " + orderLabel + "with value " + updatedOrder + " field for "
						+ navigationLabels, YesNo.Yes);
				sa.assertTrue(false, "Not Able to update " + orderLabel + "with value " + updatedOrder + " field for "
						+ navigationLabels);
			}
		}
		...
		if (!csvRecords.isEmpty()) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				log(LogStatus.INFO, "Able to Click on " + navigationMenuName, YesNo.No);
				npbl.verifyingNavigationMenuLink(projectName, navigationParentLabelWithSortedOrder,
						navigationParentLabelWithChildSorted, action.BOOLEAN, 1);
				;
			} else {
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify order",
						YesNo.Yes);
	...
	public void M3Tc016_AddNewNavigationItemsAndVerifyItOnNavigationMenu(String projectName) {
		...
		if (flag) {
			flag = false;
			String[][] labelWithValue = { { navigationLabel, navigationLabelValue }, { orderLabel, orderLabelValue },
					{ urlLabel, urlValue }, { navigationTypeLabel, navigationTypeValue } };
			...
			log(LogStatus.ERROR, "Not Able to retrieve url from DashBoard so cannot create navigation label : "
					+ navigationLabelValue, YesNo.Yes);
			sa.assertTrue(false, "Not Able to retrieve url from DashBoard so cannot create navigation label : "
		...
		if (flag) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				...
				if (!csvRecords.isEmpty()) {
					...
					log(LogStatus.FAIL, "Unable to Fetch Records from CSV File : " + NavigationMenuTestData_PEExcel,
							YesNo.Yes);
					sa.assertTrue(false, "Unable to Fetch Records from CSV File : " + NavigationMenuTestData_PEExcel);
				}
				...
				if (click(driver, ele, navigationLabelValue, action.BOOLEAN)) {
					...
					if (lp.getSelectedTab(projectName, dashBoardTab)) {
						log(LogStatus.INFO,
								"After Click on " + navigationLabelValue + " it is redirected to " + dashBoardTab,
								YesNo.Yes);

					} else {
						log(LogStatus.ERROR,
								"After Click on " + navigationLabelValue + "it should be redirected to " + dashBoardTab,
								YesNo.Yes);
						sa.assertTrue(false, "After Click on " + navigationLabelValue + "it should be redirected to "
								+ dashBoardTab);
					}
				} else {
				...
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot check label : "
						+ navigationLabelValue, YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot check label : "
						+ navigationLabelValue);
			}

		} else {
			log(LogStatus.ERROR, "Not Able to Create " + navigationLabelValue + "so type : " + navigationTypeValue
					+ " so cannot check order on " + navigationTypeValue, YesNo.Yes);
			sa.assertTrue(false, "Not Able to Create " + navigationLabelValue + "so type : " + navigationTypeValue
					+ " so cannot check order on " + navigationTypeValue);
		}
		...
		for (String userName : userNames) {
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					...
					log(LogStatus.FAIL, "could not find new window to switch, so cannot select " + recordType,
							YesNo.Yes);
				...
				if (ele != null) {
					...
					if (actualHeader.equals(expecedHeader)) {
						...
						log(LogStatus.ERROR,
								...
								YesNo.Yes);
						sa.assertTrue(false, "Header Text not verified Actual : " + actualHeader + " \t Expected : "
								+ expecedHeader);
					}
	...
	public void M3Tc020_VerifyQuickCreateObjectWhenRecordHasMultipleRecordTypes(String projectName) {
		...
		String actionObjectLabelValue = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath",
				excelLabel.TestCases_Name, currentlyExecutingTC, excelLabel.Action_Object);
		;
		String navigationLabel = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Navigation_Label_Name);
		...
		String recordTypeList = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				dependentTC, excelLabel.Record_Type);
		String recordTypeArray[] = recordTypeList.split(breakSP);
		boolean flag = false;
		...
		if (npbl.enterValueOnEditPopUpForNavigationTab(projectName, navigationLabel, labelWithValue, 20)) {
			log(LogStatus.INFO, actionObjectLabelValue + " value has been updated & saved under " + actionObjectLabel
					+ " for " + navigationLabel, YesNo.No);
			flag = true;

		} else {
			log(LogStatus.ERROR, "Not Able to enter value on" + actionObjectLabel + " for " + navigationLabel,
					YesNo.Yes);
		...
		if (flag) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
					...
					if (ele != null) {
							...
							if (ele != null) {
								...
								if (i == 0) {
									if (isSelected(driver, ele, recordTypeArray[i] + " radio button")) {
										log(LogStatus.INFO, recordTypeArray[i] + " radio button is selected by default",
												YesNo.No);
									} else {
										log(LogStatus.ERROR,
												recordTypeArray[i] + " radio button is not selected by default",
												YesNo.Yes);
										sa.assertTrue(false,
												recordTypeArray[i] + " radio button is not selected by default");
									}
								...
								ele = npbl.getrecordTypeWithDescription(recordTypeArray[i],
										recordTypeArray[i] + recordTypeDescription, 10);
								if (ele != null) {
									log(LogStatus.INFO, "Record Type & description verified : " + recordTypeArray[i]
											+ recordTypeDescription, YesNo.No);

								} else {
									log(LogStatus.ERROR, "Record Type & description not verified : "
											+ recordTypeArray[i] + recordTypeDescription, YesNo.Yes);
									sa.assertTrue(false, "Record Type & description not verified : "
											+ recordTypeArray[i] + recordTypeDescription);
								}
						...
						if (ele != null) {
							...
							if (ele.getText().trim().equals(expecedHeader)) {
								...
								log(LogStatus.ERROR, "Header Text not verified Actual : " + actualHeader
										+ " \t Expected : " + expecedHeader, YesNo.Yes);
								sa.assertTrue(false, "Header Text not verified Actual : " + actualHeader
										+ " \t Expected : " + expecedHeader);
							}

						} else {
							log(LogStatus.ERROR, "No PopUp is open so cannoy verify Heading " + expecedHeader,
									YesNo.Yes);
				...
				log(LogStatus.ERROR,
						"Not Able to Click on " + navigationMenuName + " so cannot click on label : " + navigationLabel,
						YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName + " so cannot click on label : "
						+ navigationLabel);
				...
				if (click(driver, lp.getNewButton(projectName, 90), "new button", action.BOOLEAN)) {
						...
						if (ele != null) {
							...
							if (i == 0) {
								if (isSelected(driver, ele, recordTypeArray[i] + " radio button")) {
									...
									log(LogStatus.ERROR,
											recordTypeArray[i] + " radio button is not selected by default", YesNo.Yes);
									sa.assertTrue(false,
											recordTypeArray[i] + " radio button is not selected by default");
								}
							}
							...
							if (ele != null) {
								...
								log(LogStatus.ERROR, "Record Type & description not verified : " + recordTypeArray[i]
										+ recordTypeDescription, YesNo.Yes);
								sa.assertTrue(false, "Record Type & description not verified : " + recordTypeArray[i]
					...
					if (ele != null) {
						...
						if (actualHeader.equals(expecedHeader)) {
							...
							log(LogStatus.ERROR, "Header Text not verified Actual : " + actualHeader + " \t Expected : "
									+ expecedHeader, YesNo.Yes);
							sa.assertTrue(false, "Header Text not verified Actual : " + actualHeader + " \t Expected : "
									+ expecedHeader);
						}
			...
			log(LogStatus.ERROR, "Not Able to enter value on" + actionObjectLabel + " for " + navigationLabel
					+ " so cannot check on Navigation Menu", YesNo.Yes);
			sa.assertTrue(false, "Not Able to enter value on" + actionObjectLabel + " for " + navigationLabel
					+ " so cannot check on Navigation Menu");
		}
	...
	public void M3Tc021_1_UpdateTheRecordTypeAndDescriptionImpactOnNavigationMenu(String projectName) {
		...
		String navigationLabel = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				dependentTC, excelLabel.Navigation_Label_Name);
	...
	public void M3Tc021_2_UpdateTheRecordTypeAndDescriptionImpactOnNavigationMenu(String projectName) {
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			if (click(driver, ele, navigationLabel, action.BOOLEAN)) {
				...
				if (ele != null) {
					log(LogStatus.INFO, "Pop Up open after clicking on " + navigationLabel, YesNo.No);
					ele = npbl.getrecordTypeWithDescription(upDated + recordTypeArray[0],
							upDated + recordTypeArray[0] + recordTypeDescription, 10);
					if (ele != null) {
						log(LogStatus.INFO, "Record Type & description Updated & verified", YesNo.No);

					} else {
						log(LogStatus.ERROR, "Record Type & description not Updated & not verified " + upDated
								+ recordTypeArray[0] + " " + upDated + recordTypeArray[0] + recordTypeDescription,
								YesNo.Yes);
						sa.assertTrue(false, "Record Type & description not Updated & not verified " + upDated
								+ recordTypeArray[0] + " " + upDated + recordTypeArray[0] + recordTypeDescription);
					}
				} else {
					log(LogStatus.ERROR, "No Pop Up is open after clicking on " + navigationLabel, YesNo.Yes);
	...
	public void M3Tc022_1_CreateACustomObject(String projectName) {
		...
		if (flag) {
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					...
					if (sp.addObjectToTab(environment, mode, projectName, object.Tabs, tabCustomObj, "Bell",
							parentID)) {
		...
		for (String userName : userNames) {
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					log(LogStatus.INFO,
							"Able to switch on new window, so going to set" + permission + " for " + onObject,
							YesNo.No);
					ThreadSleep(500);
					if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
						log(LogStatus.INFO, "click on Object : " + object.Profiles, YesNo.No);
						...
						if (setup.permissionChangeForUserONObject(driver, userName,
								new String[][] { { onObject, permission }, { onObject, "Delete" } }, 20)) {
							log(LogStatus.PASS,
									permission + " permission change for " + userName + " on object " + onObject,
									YesNo.No);
						} else {
							sa.assertTrue(false,
					...
					log(LogStatus.FAIL,
							"could not find new window to switch, so cannot to set" + permission + " for " + onObject,
							YesNo.Yes);
					sa.assertTrue(false,
							"could not find new window to switch, to set" + permission + " for " + onObject);
				}
	...
	public void M3Tc022_2_AddCustomObectInNavigationMenu(String projectName) {
		...
		String[][] labelWithValue = { { navigationLabel, navigationLabelValue }, { orderLabel, orderLabelValue },
				{ actionObjectLabel, actionObjecValue }, { navigationTypeLabel, navigationTypeValue } };
		WebElement ele;
		if (npbl.createNavigationItem(projectName, labelWithValue, 20)) {
			log(LogStatus.INFO, "created " + navigationLabelValue, YesNo.No);
			flag = true;
		...
		if (flag) {
			...
			if (npbl.clickOnTab(projectName, navigationTab)) {
				...
				if (npbl.CheckAlreadyCreatedItem(projectName, navigationLabelValue, true, 15)) {
					...
					log(LogStatus.ERROR, "Item not found: " + navigationLabelValue + " on Tab : " + navigationTab,
							YesNo.Yes);
			...
			log(LogStatus.ERROR,
					"Not Able to create " + navigationLabelValue + " so cannot check on " + navigationMenuName,
					YesNo.Yes);
			sa.assertTrue(false,
					"Not Able to create " + navigationLabelValue + " so cannot check on " + navigationMenuName);
		}
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					if (sp.searchStandardOrCustomObject("", Mode.Lightning.toString(), object.Test_Custom_Object)) {
						if (sp.clickOnObjectFeature("", Mode.Lightning.toString(), object.Test_Custom_Object,
								ObjectFeatureName.recordTypes)) {
							if (i == 0) {
								flag = sp.createRecordTypeForObject(projectName, fundRecordType, isMakeAvailable, null,
										isMakeDefault, null, 10);
							} else {
								isMakeDefault = false;
								flag = sp.createRecordTypeForObject(projectName, ffrecordType, isMakeAvailable, null,
										isMakeDefault, null, 10);
							}
							if (flag) {
								log(LogStatus.ERROR, "Created Record Type : " + recordTypeArray[i], YesNo.No);
	...
	public void M3Tc024_VerifyQuickCreateObjectForCustomObjectWhenRecordHasMultipleRecordTypes(String projectName) {
		...
		if (flag) {
					...
					for (int i = 0; i < recordTypeArray.length; i++) {
						...
						if (ele != null) {
							...
							if (ele != null) {
								...
								log(LogStatus.ERROR, "No PopUp is open so cannot verify Heading " + expecedHeader,
										YesNo.Yes);
								sa.assertTrue(false, "No PopUp is open so cannot verify Heading " + expecedHeader);
							}
			...
			log(LogStatus.ERROR, "Not Able to enter value on" + actionObjectLabel + " for " + navigationLabel
					+ " so cannot check on navigation menu and tab", YesNo.Yes);
			sa.assertTrue(false, "Not Able to enter value on" + actionObjectLabel + " for " + navigationLabel
					+ " so cannot check on navigation menu and tab");
		}
		switchToDefaultContent(driver);
		lp.CRMlogout();
		sa.assertAll();
					...
					if (sp.searchStandardOrCustomObject("", Mode.Lightning.toString(), object.Test_Custom_Object)) {
						if (sp.clickOnObjectFeature("", Mode.Lightning.toString(), object.Test_Custom_Object,
							...
							if (sp.clickOnAlreadyCreatedLayout(recordTypeArray[i])) {
								...
								if (click(driver, sp.getEditButton(environment, "Classic", 10), "edit",
										action.SCROLLANDBOOLEAN)) {
									...
									if (ele != null) {
										...
										if (i == 1) {
											if (isSelected(driver, ele,
													recordTypeArray[i] + " " + recordTypeLabel.Active)) {
												ThreadSleep(2000);
												if (click(driver, ele,
														recordTypeArray[i] + " " + recordTypeLabel.Active,
														action.BOOLEAN)) {
													log(LogStatus.INFO, recordTypeArray[i] + " "
															+ recordTypeLabel.Active + " is clicked", YesNo.No);
													ThreadSleep(2000);
													if (click(driver, sp.getCreateUserSaveBtn_Lighting(30),
													...
													log(LogStatus.ERROR, recordTypeArray[i] + " "
															+ recordTypeLabel.Active + " is not clickable", YesNo.Yes);
													sa.assertTrue(false, recordTypeArray[i] + " "
															+ recordTypeLabel.Active + " is not clickable");
												}
											}
										} else {
											ele = sp.getRecordTypeLabel(projectName,
													recordTypeLabel.Record_Type_Name.toString(), 20);
										...
										log(LogStatus.ERROR, recordTypeArray + " ele not found on edit page",
												YesNo.Yes);
			...
			if (flag) {
				...
				if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
					...
					if (click(driver, ele, labelValue, action.BOOLEAN)) {
						log(LogStatus.INFO, "Click on " + labelValue + " going to verify record type options",
								YesNo.No);
						ThreadSleep(5000);
						String actualHeader = "";
						ele = npbl.getPopUpHeader(projectName, 10);
						...
						log(LogStatus.ERROR, "Not Able to Click on " + labelValue + " so cannot verify pop up window",
								YesNo.Yes);
					...
					log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName
							+ " so cannot verify pop up window for label : " + labelValue, YesNo.Yes);
					sa.assertTrue(false, "Not Able to Click on " + navigationMenuName
							+ " so cannot verify pop up window for label : " + labelValue);
				}
	...
	public void M3Tc026_1_DeActivateARecordTypeForInstitution_Action(String projectName) {
		...
		lp.CRMLogin(superAdminUserName, adminPassword);
		WebElement ele;
		String recordType = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Record_Type);
		if (home.clickOnSetUpLink()) {
			String parentID = switchOnWindow(driver);
			SetupPageBusinessLayer sp = new SetupPageBusinessLayer(driver);
			if (parentID != null) {
				if (sp.searchStandardOrCustomObject("", Mode.Lightning.toString(), object.Firm)) {
					if (sp.clickOnObjectFeature("", Mode.Lightning.toString(), object.Firm,
							...
							if (click(driver, sp.getEditButton(environment, "Classic", 10), "edit",
								...
								if (ele != null) {
									...
									if (isSelected(driver, ele, recordType + " " + recordTypeLabel.Active)) {
										ThreadSleep(2000);
										if (click(driver, ele, recordType + " " + recordTypeLabel.Active,
												action.BOOLEAN)) {
											log(LogStatus.INFO,
													recordType + " " + recordTypeLabel.Active + " is clicked",
													YesNo.No);
											ThreadSleep(2000);
											...
											log(LogStatus.ERROR,
													recordType + " " + recordTypeLabel.Active + " is not clickable",
													YesNo.Yes);
											sa.assertTrue(false,
													recordType + " " + recordTypeLabel.Active + " is not clickable");
										}
										...
										log(LogStatus.ERROR,
												recordType + " " + recordTypeLabel.Active + " is already inactive",
												YesNo.Yes);
										// sa.assertTrue(false, recordType+" "+recordTypeLabel.Active+" is already
										// inactive");
									}
								} else {
						...
						log(LogStatus.ERROR, "object feature " + ObjectFeatureName.recordTypes + " is not clickable",
								YesNo.Yes);
	...
	public void M3Tc026_2_DeActivateARecordTypeForInstitution_Impact(String projectName) {
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			if (click(driver, ele, navigationLabel, action.BOOLEAN)) {
				...
				if (sendKeys(driver, ip.getLegalName(projectName, 20), institutionName, "leagl name text box",
						action.SCROLLANDBOOLEAN)) {
					...
					if (click(driver, lp.getCustomTabSaveBtn(projectName, 20), "save", action.SCROLLANDBOOLEAN)) {
						...
						if (ele != null) {
							...
							if (actualHeader.contains(expecedHeader)) {
								log(LogStatus.INFO,
										"Error Message verified after click on save button : " + expecedHeader,
										YesNo.Yes);
							} else {
								log(LogStatus.ERROR, "Error Message not verified after click on save button Actual : "
										+ actualHeader + " \t Expected : " + expecedHeader, YesNo.Yes);
								sa.assertTrue(false, "Error Message not verified after click on save button Actual : "
										+ actualHeader + " \t Expected : " + expecedHeader);
							}
					...
					sa.assertTrue(false,
							"Not able to pass data in legal name text box so cannot click on save button ");
					log(LogStatus.SKIP, "Not able to pass data in legal name text box so cannot click on save button ",
							YesNo.Yes);
				}
				ele = npbl.getCrossButtonForNavigationLabelPopuP(projectName, "", action.BOOLEAN, 30);
				click(driver, ele, navigationLabel + " pop up cross button", action.BOOLEAN);
	...
	public void M3Tc027_VerifyAndCreateNavigationItemWhichAreRedirectedtoListView(String projectName) {
		...
		String navigationLabel = CSVLabel.Navigation_Label.toString();
		String navigationLabelValue = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath",
				excelLabel.TestCases_Name, currentlyExecutingTC, excelLabel.Navigation_Label_Name);
		String orderLabel = CSVLabel.Order.toString();
		...
		if (flag) {
			...
			log(LogStatus.ERROR, "Not Able to retrieve url from Navigation so cannot create navigation label : "
					+ navigationLabelValue, YesNo.Yes);
			sa.assertTrue(false, "Not Able to retrieve url from Navigation so cannot create navigation label : "
					+ navigationLabelValue);
		}
		...
		if (flag) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 60)) {
				...
				if (click(driver, ele, navigationLabelValue, action.BOOLEAN)) {
					...
					if (flag) {
						...
						log(LogStatus.ERROR,
								allListView + " is not selected after clicking on after clicking on " + navigationTab,
								YesNo.Yes);
						sa.assertTrue(false,
								allListView + " is not selected after clicking on after clicking on " + navigationTab);
					}
			...
			log(LogStatus.ERROR, "Not Able to Create " + navigationLabelValue + "so type : " + navigationTypeValue
					+ " so cannot check on " + navigationTypeValue, YesNo.Yes);
			sa.assertTrue(false, "Not Able to Create " + navigationLabelValue + "so type : " + navigationTypeValue
					+ " so cannot check on " + navigationTypeValue);
		}
	...
	public void M3Tc028_CreateRecordthroughnavigationMenuandVerifyitontabsListview(String projectName) {
		...
		String navigationLabelValues = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath",
				excelLabel.TestCases_Name, currentlyExecutingTC, excelLabel.Navigation_Label_Name) + "<break>"
				+ tabCustomObj;
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				...
				if (click(driver, ele, navigationLabelValue, action.BOOLEAN)) {
					...
					if (i == 0) {
						...
						if (cp.createContactPopUp(projectName, M3Contact3FName, M3Contact3LName, M3Ins1,
								M3Contact3EmailID, M3Contact3RecordType, null, null, CreationPage.ContactPage, null)) {
							log(LogStatus.INFO,
									"successfully Created Contact : " + M3Contact3FName + " " + M3Contact3LName,
									YesNo.No);
							ThreadSleep(2000);
							String labelValue = "";
							boolean isSelectAll = true;
							if (npbl.clickOnTab(projectName, tabObj2)) {
								...
								if (npbl.CheckAlreadyCreatedItem(projectName, labelValue, isSelectAll, 15)) {
									...
									log(LogStatus.ERROR, "Item not found: " + labelValue + " on Tab : " + tabObj2,
											YesNo.Yes);
							...
							sa.assertTrue(false,
									"Not Able to Create Contact : " + M3Contact3FName + " " + M3Contact3LName);
							log(LogStatus.SKIP,
									"Not Able to Create Contact: " + M3Contact3FName + " " + M3Contact3LName,
									YesNo.Yes);
						}
						...
						if (cop.createRecord(projectName, M3TestCustomObj1RecordType, null, M3TestCustomObj1Name,
								true)) {
							log(LogStatus.INFO, "successfully Created custom record : " + M3TestCustomObj1Name,
									YesNo.No);
							ThreadSleep(2000);
							String labelValue = "";
							boolean isSelectAll = true;
							if (npbl.clickOnTab(projectName, tabCustomObj)) {
								...
								if (npbl.CheckAlreadyCreatedItem(projectName, labelValue, isSelectAll, 15)) {
									log(LogStatus.INFO, "Item found: " + labelValue + " on Tab : " + tabCustomObj,
											YesNo.No);
								} else {

									log(LogStatus.ERROR, "Item not found: " + labelValue + " on Tab : " + tabCustomObj,
											YesNo.Yes);
							...
							log(LogStatus.SKIP, "Not Able to Create custom record  : " + M3TestCustomObj1Name,
									YesNo.Yes);
						}
				...
				log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName
						+ " so cannot create record for label : " + navigationLabelValue, YesNo.Yes);
				sa.assertTrue(false, "Not Able to Click on " + navigationMenuName
						+ " so cannot create record for label : " + navigationLabelValue);
			}
			ele = npbl.getCrossButtonForNavigationLabelPopuP(projectName, "", action.BOOLEAN, 2);
	...
	public void M3Tc029_VerifyWhereChildNodeIsMovedFromOneParentToAnotherInNavigationMenu(String projectName) {
		...
		String[] AllParents = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Parent).split(breakSP);
		String actionObject = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Action_Object);
		String child = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				currentlyExecutingTC, excelLabel.Navigation_Label_Name);
		...
		String[][] labels = { { CSVLabel.Navigation_Label.toString(), child },
				{ CSVLabel.Order.toString(), String.valueOf(order) },
				{ CSVLabel.Action_Object.toString(), actionObject }, { CSVLabel.Parent.toString(), AllParents[0] },
				{ CSVLabel.Navigation_Type.toString(), navigationMenuName } };
		if (npbl.createNavigationItem(projectName, labels, 20)) {
		...
		if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
			...
			log(LogStatus.ERROR, "Not Able to Click on " + navigationMenuName + " so cannot verify " + parentWithChild,
					YesNo.Yes);
		...
		if (npbl.enterValueOnEditPopUpForNavigationTab(projectName, child, labelWithValue, 20)) {
			log(LogStatus.INFO, parentValue + " value has been updated & saved under " + parentLabel + " for " + child,
					YesNo.No);
		} else {
			log(LogStatus.ERROR,
					parentValue + " value has not been updated & saved under " + parentLabel + " for " + child,
					YesNo.Yes);
			sa.assertTrue(false,
					parentValue + " value has not been updated & saved under " + parentLabel + " for " + child);
	...
	public void M3Tc030_VerifyTheImpactOnNavigationMenuWhenDeletingParentNodeu(String projectName) {
		...
		String[] AllParents = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				dependentTc, excelLabel.Parent).split(breakSP);
		String child = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name, dependentTc,
				excelLabel.Navigation_Label_Name);
		...
		if (flag) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
				...
				if (ele == null) {
					...
					log(LogStatus.ERROR, child + " should not be under " + navLb + " after deletion of " + navLb,
							YesNo.Yes);
		...
		if (flag) {
			...
			if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 60)) {
				...
				if (ele != null) {
					...
					log(LogStatus.ERROR, child + " should be under " + name + " after restortion of " + name,
							YesNo.Yes);
	...
	public void M3Tc032_VerifyWhetherCustomNavigationwillbeaddedintheNavigationMenu(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			if (parentID != null) {
				if (sp.searchStandardOrCustomObject(environment, mode, object.Navigation)) {
					...
					if (sp.clickOnObjectFeature(environment, mode, object.Navigation,
							ObjectFeatureName.FieldAndRelationShip)) {
						log(LogStatus.INFO,
								"object feature " + ObjectFeatureName.FieldAndRelationShip + " is clickable", YesNo.No);
						if (sendKeys(driver, sp.getsearchTextboxFieldsAndRelationships(10),
								PageLabel.Navigation_Type.toString() + Keys.ENTER, "Navigation Type", action.BOOLEAN)) {
							...
							if (sp.clickOnAlreadyCreatedLayout(PageLabel.Navigation_Type.toString())) {
								...
								if (click(driver, sp.getValuesNewButton(20), "Values New Button", action.BOOLEAN)) {
									...
									ThreadSleep(5000);
									if (sendKeys(driver, dp.getTextArea(20), customNavigationMenu, customNavigationMenu,
											action.BOOLEAN)) {
										log(LogStatus.INFO, "enter value on textarea " + customNavigationMenu,
												YesNo.No);
										if (click(driver, npbl.getCustomTabSaveBtn(projectName, 10), "save button",
												action.SCROLLANDBOOLEAN)) {
											log(LogStatus.ERROR, "Click on save Button : " + customNavigationMenu,
													YesNo.No);
											ThreadSleep(10000);
										} else {
											log(LogStatus.ERROR,
													"Not Able to Click on save Button : " + customNavigationMenu,
													YesNo.Yes);
											sa.assertTrue(false,
													"Not Able to Click on save Button : " + customNavigationMenu);
										}
										...
										sa.assertTrue(false,
												"Not Able to enter value to textarea " + customNavigationMenu);
										log(LogStatus.SKIP,
												"Not Able to enter value to textarea " + customNavigationMenu,
												YesNo.Yes);
									}
								...
								log(LogStatus.ERROR, PageLabel.Navigation_Type.toString() + " is not clickable",
										YesNo.Yes);
								sa.assertTrue(false,
										PageLabel.Navigation_Type.toString() + "status field is not clickable");
							}
						} else {
							log(LogStatus.ERROR, PageLabel.Navigation_Type.toString() + " search is not visible",
									YesNo.Yes);
							sa.assertTrue(false, PageLabel.Navigation_Type.toString() + " search is not visible");
						}
					} else {
						log(LogStatus.ERROR,
								"object feature " + ObjectFeatureName.FieldAndRelationShip + " is not clickable",
								YesNo.Yes);
						sa.assertTrue(false,
								"object feature " + ObjectFeatureName.FieldAndRelationShip + " is not clickable");
					}
			...
			if (parentID != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.App_Manager)) {
					...
					if (setup.clickOnEditForApp(driver, appName, AppDeveloperName, AppDescription, 60)) {
						log(LogStatus.INFO, "able to click on edit button against " + appName + " : " + AppDeveloperName
								+ " " + AppDescription, YesNo.No);
						ThreadSleep(1000);
						if (setup.clickOnAppSettingList(driver, AppSetting.Utility_Items, 30)) {
							...
							if (click(driver, setup.getAddUtilityItem(30), "Add Utility Item", action.BOOLEAN)) {
								...
								ThreadSleep(500);
								if (setup.searchAndClickOnAddUtilityItem(environment, mode,
										object.navatarEdgeMenu.toString())) {
									log(LogStatus.INFO,
											"Able to search and click on " + object.navatarEdgeMenu.toString(),
											YesNo.No);
									ThreadSleep(500);
									if (sendKeys(driver,
											setup.commonInputElement(projectName, PageLabel.Label.toString(),
													action.BOOLEAN, 30),
											customNavigationMenu,
											PageLabel.Label.toString() + " textbox value : " + customNavigationMenu,
											action.BOOLEAN)) {
										ThreadSleep(500);
										log(LogStatus.INFO, "send value to " + PageLabel.Label.toString()
												+ " textbox value : " + customNavigationMenu, YesNo.No);
										if (sendKeys(driver,
												setup.commonInputElement(projectName,
														PageLabel.Navigation_Type.toString(), action.BOOLEAN, 10),
												customNavigationMenu,
												PageLabel.Label.toString() + " textbox value : " + customNavigationMenu,
												action.BOOLEAN)) {
											ThreadSleep(500);
											log(LogStatus.INFO, "send value to " + PageLabel.Navigation_Type.toString()
													+ " textbox value : " + customNavigationMenu, YesNo.No);

											for (int i = 0; i < 3; i++) {
												if (click(driver, setup.getMoveThisUtilityDownTheList(60),
														"Move this utility down the list.", action.BOOLEAN)) {
													log(LogStatus.INFO,
															"Click on Move this utility down the list Button",
															YesNo.No);
													ThreadSleep(1000);
												} else {
													sa.assertTrue(false,
															"Not Able to Click on Move this utility down the list Button");
													log(LogStatus.FAIL,
															"Not Able to Click on Move this utility down the list Button",
															YesNo.Yes);
												}
											...
											if (click(driver, setup.getCustomTabSaveBtn(projectName, 30),
											...
											sa.assertTrue(false,
													"Not Able to send value to " + PageLabel.Navigation_Type.toString()
															+ " textbox value : " + customNavigationMenu);
											log(LogStatus.FAIL,
													"Not Able to send value to " + PageLabel.Navigation_Type.toString()
															+ " textbox value : " + customNavigationMenu,
													YesNo.Yes);
										...
										sa.assertTrue(false, "Not Able to send value to " + PageLabel.Label.toString()
												+ " textbox value : " + customNavigationMenu);
										log(LogStatus.FAIL, "Not Able to send value to " + PageLabel.Label.toString()
												+ " textbox value : " + customNavigationMenu, YesNo.Yes);
									}
								} else {
									log(LogStatus.ERROR,
											"Not Able to search and click on " + object.navatarEdgeMenu.toString(),
											YesNo.Yes);
									sa.assertTrue(false,
											"Able Able to search and click on " + object.navatarEdgeMenu.toString());
								}
						...
						log(LogStatus.ERROR, "Not able to click on edit button against " + appName + " : "
								+ AppDeveloperName + " " + AppDescription, YesNo.Yes);
						sa.assertTrue(false, "Not able to click on edit button against " + appName + " : "
								+ AppDeveloperName + " " + AppDescription);
					}

				} else {
					log(LogStatus.ERROR, "Not able to search/click on Apps so cannot add " + customNavigationMenu,
							YesNo.Yes);
				...
				log(LogStatus.FAIL, "could not find new window to switch, so cannot add " + customNavigationMenu,
						YesNo.Yes);
		...
		if (ExpectedNavigationMenuitem.equals(ActualNavigationMenuitem)) {
			...
			log(LogStatus.FAIL, "Navigation Menu not verified Actual : " + ActualNavigationMenuitem + " Expected : "
					+ ExpectedNavigationMenuitem, YesNo.Yes);
			sa.assertTrue(false, "Navigation Menu not verified Actual : " + ActualNavigationMenuitem + " Expected : "
					+ ExpectedNavigationMenuitem);
	...
	@Parameters({ "projectName" })
	@Test
	public void M3Tc033_VerifyTheFunctionalityByAddingCustomNavigationLinkInTheNewlyAddedCustomNavigation(
			String projectName) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		NavigationPageBusineesLayer npbl = new NavigationPageBusineesLayer(driver);
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		...
		String navigationLabel = CSVLabel.Navigation_Label.toString();
		String navigationLabelValue = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath",
				excelLabel.TestCases_Name, dependentTC, excelLabel.Navigation_Label_Name);
		...
		String actionObjectLabel = CSVLabel.Action_Object.toString();
		String actionObjecValue = ExcelUtils.readData(phase1DataSheetFilePath, "FilePath", excelLabel.TestCases_Name,
				dependentTC, excelLabel.Action_Object);
		...
		if (flag) {
			...
			log(LogStatus.ERROR, "Not Able to Create " + navigationLabelValue + "of type : " + navigationTypeValue
					+ " so cannot check on " + navigationTypeValue, YesNo.Yes);
			sa.assertTrue(false, "Not Able to Create " + navigationLabelValue + "of type : " + navigationTypeValue
					+ " so cannot check on " + navigationTypeValue);
		}
	...
	@Parameters({ "projectName" })
	@Test
	public void M3Tc034_1_VerifyTheFunctionalityByrenamingAndDeletingandRemovingUtilityItemCustomLabelintheCustomnavigationMenu(
			String projectName) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		NavigationPageBusineesLayer npbl = new NavigationPageBusineesLayer(driver);
		lp.CRMLogin(crmUser1EmailID, adminPassword);
		...
		if (npbl.clickOnTab(projectName, navigationTab)) {
			...
			if (npbl.clickOnAlreadyCreatedItem(projectName, navLb, true, 15)) {
				...
				if (click(driver, ele, ShowMoreActionDropDownList.Delete.toString(), action.BOOLEAN)) {
					...
					if (click(driver, npbl.getDeleteButtonPopUp(projectName, 10), "delete", action.BOOLEAN)) {
						log(LogStatus.INFO,
								"Able to Click on delete button on delete popup : " + navLb + " For : " + navLb,
								YesNo.No);
						ThreadSleep(5000);
						if (npbl.clickOnNavatarEdgeLinkHomePage(projectName, navigationMenuName, action.BOOLEAN, 30)) {
							log(LogStatus.INFO, "Able to Click on " + navigationMenuName, YesNo.No);
							...
							if (ele == null) {
								...
								log(LogStatus.ERROR, navigationLabel + " label should not present after delete",
										YesNo.Yes);
								sa.assertTrue(false, navigationLabel + " label should not present after delete");
							...
							log(LogStatus.ERROR,
									"Not Able to Click on " + navigationMenuName + " so cannot verify order",
									YesNo.Yes);
							sa.assertTrue(false,
									"Not Able to Click on " + navigationMenuName + " so cannot verify order");
						}
	...
	@Parameters({ "projectName" })
	@Test
	public void M3Tc034_2_VerifyTheFunctionalityByrenamingAndDeletingandRemovingUtilityItemCustomLabelintheCustomnavigationMenu(
			String projectName) {
		LoginPageBusinessLayer lp = new LoginPageBusinessLayer(driver);
		NavigationPageBusineesLayer npbl = new NavigationPageBusineesLayer(driver);
		HomePageBusineesLayer home = new HomePageBusineesLayer(driver);
		...
		if (home.clickOnSetUpLink()) {
			String parentID = switchOnWindow(driver);
			if (parentID != null) {
				log(LogStatus.INFO, "Able to switch on new window, so going to remove " + customNavigationMenu,
						YesNo.No);
				ThreadSleep(500);
				if (setup.searchStandardOrCustomObject(environment, mode, object.App_Manager)) {
					log(LogStatus.INFO, "click on Object : " + object.App_Manager, YesNo.No);
					...
					if (setup.clickOnEditForApp(driver, appName, AppDeveloperName, AppDescription, 90)) {
						...
						if (setup.clickOnAppSettingList(driver, AppSetting.Utility_Items, 90)) {
							...
							if (setup.ClickAndRemoveUtilityItem(environment, mode, customNavigationMenu)) {
								...
								if (click(driver, setup.getCustomTabSaveBtn(projectName, 30), " Save Button",
										action.BOOLEAN)) {
									log(LogStatus.INFO, "Click on Save Button", YesNo.No);
									ThreadSleep(20000);
								} else {
					...
					log(LogStatus.ERROR, "Not able to search/click on Apps so cannot remove " + customNavigationMenu,
							YesNo.Yes);
					sa.assertTrue(false,
							"Not able to search/click on Apps manager so cannot remove " + customNavigationMenu);
				}
				driver.close();
				driver.switchTo().window(parentID);
			} else {
				log(LogStatus.FAIL, "could not find new window to switch, so cannot remove " + customNavigationMenu,
						YesNo.Yes);
