...
public class Module6 extends BaseLib {
	...
	public void M6tc001_CreatePrecondition(String projectName) {
		...
		for (int i = 0; i < 20; i++) {
			if (lp.clickOnTab(projectName, TabName.Object1Tab)) {
				...
				value = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name,
						"M6INS" + (i + 1), excelLabel.Institutions_Name);
				type = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name,
						"M6INS" + (i + 1), excelLabel.Record_Type);
				if (i == 16) {

					if (ip.createInstitution(projectName, environment, mode, value, type,
							InstitutionPageFieldLabelText.Parent_Institution.toString(), M6Ins1)) {
						log(LogStatus.INFO,
					...
					if (ip.createInstitution(projectName, environment, mode, value, type,
							InstitutionPageFieldLabelText.Parent_Institution.toString(), M6Ins13)) {
						log(LogStatus.INFO,
								"successfully Created Account/Entity : " + value + " of record type : " + type,
								YesNo.No);
					} else {
		...
		for (int i = 0; i < 19; i++) {
			if (lp.clickOnTab(projectName, TabName.Object4Tab)) {
				...
				pipe = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name,
						"M6Deal" + (i + 1), excelLabel.Deal_Name);
				company = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name,
						"M6Deal" + (i + 1), excelLabel.Company_Name);
				stage = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name,
						"M6Deal" + (i + 1), excelLabel.Stage);
				refresh(driver);
				ThreadSleep(3000);
					...
					if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
						...
						if (i == 0) {
							...
							if (dp.getconvertToPortfolioMessage(M6Ins2, 10) != null) {
								log(LogStatus.INFO, "successfully verified convert to portfolio text message",
										YesNo.No);
							} else {
								sa.assertTrue(false, "could not verify convert to portfolio text message before next");
								log(LogStatus.SKIP, "could not verify convert to portfolio text message before next",
										YesNo.Yes);
							}
							if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
								...
								if (dp.getconvertToPortfolioMessageAfterNext(10) != null) {
									if (dp.getconvertToPortfolioMessageAfterNext(10).getText().contains(M6Ins2)
											&& dp.getconvertToPortfolioMessageAfterNext(10).getText()
													.contains(dp.convertToPortfolioAfterNext(M6Ins2))) {
										log(LogStatus.INFO,
												"successfully verified after next convert to portfolio text message",
												YesNo.No);
									} else {
										sa.assertTrue(false,
												"could not verify after next convert to portfolio text message");
										log(LogStatus.SKIP,
												"could not verify after next convert to portfolio text message",
												YesNo.Yes);
									}
									...
									sa.assertTrue(false,
											"could not verify after next convert to portfolio text message");
									log(LogStatus.SKIP, "could not verify after next convert to portfolio text message",
											YesNo.Yes);
								}
								if (click(driver, dp.getconvertToPortfolioCrossButton(10), "cross", action.BOOLEAN)) {
							...
							if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
								...
								if (click(driver, dp.getfinishButton(10), "finish", action.BOOLEAN)) {
									log(LogStatus.INFO,
											"successfully verified finish button after convert to portfolio", YesNo.No);
									ExcelUtils.writeData(phase1DataSheetFilePath, Stage.Closed.toString(), "Deal",
											excelLabel.Variable_Name, "M6Deal2", excelLabel.Updated_Stage);
								} else {
									sa.assertTrue(false,
											"could not verify convert to portfolio as finish button not clicked");
									log(LogStatus.SKIP,
											"could not verify convert to portfolio as finish button not clicked",
											YesNo.Yes);
								}
					...
					if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
						if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
							if (dp.getconvertToPortfolioMessageRepeat(10) != null) {
								if (dp.getconvertToPortfolioMessageRepeat(10).getText()
										.contains(dp.convertToPortfolioRepeat(M6Ins2))) {
			...
			if (ip.clickOnAlreadyCreatedItem(projectName, M6Ins2, 10)) {
				if (ip.clickOnShowMoreActionDownArrow(projectName, PageName.Object1Page,
					...
					if (click(driver, ip.getRadioButtonforRecordType(RecordType.Company.toString(), 10),
							"company record type", action.BOOLEAN)) {
						if (click(driver, ip.getContinueOrNextButton(projectName, 10), "Continue Button",
								action.BOOLEAN)) {
							appLog.info("Clicked on Continue or Nxt Button");
							if (click(driver, ip.getNavigationTabSaveBtn(projectName, 10), "save button",
									action.SCROLLANDBOOLEAN)) {
								appLog.info("clicked on save button");
								ThreadSleep(3000);
								if (ip.fieldValueVerificationOnInstitutionPage(environment, mode, TabName.Object1Tab,
										labelName1[i], labelValues1[i])) {
	...
	public void M6tc005_VerifyConvertingAfterChangingDealStage(String projectName) {
		...
		String labelName1[] = { excelLabel.Average_Deal_Quality_Score.toString(),
				excelLabel.Total_Deals_Shown.toString() };
	...
	public void M6tc006_VerifyConvertingAfterChangingCompany(String projectName) {
		...
		if (ip.clickOnTab(projectName, TabName.Object4Tab)) {
			if (ip.clickOnAlreadyCreatedItem(projectName, M6Deal2, 10)) {
				if (dp.clickOnShowMoreActionDownArrow(projectName, PageName.Object4Page,
						ShowMoreActionDropDownList.Edit, 10)) {
						...
						if (sendKeys(driver, fp.getCompanyName(projectName, 60), M6Ins3, "Company Name",
							...
							if (click(driver, FindElement(driver,
									"//input[contains(@placeholder,'Search')]/../following-sibling::*//*[@title='"
											+ M6Ins3 + "']",
									"Company Name List", action.BOOLEAN, 30), M6Ins3 + "   :   Company Name",
									action.BOOLEAN)) {
								log(LogStatus.INFO, "succssfully selected company", YesNo.Yes);
								if (click(driver, fp.getCustomTabSaveBtn(projectName, 30), "Save Button",
										action.SCROLLANDBOOLEAN)) {
								...
								if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button",
										action.BOOLEAN)) {
									if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
										String text = dp.getconvertToPortfolioMessageAfterNext(10).getText();
										if (text.contains(dp.convertToPortfolioAfterNext(M6Ins3))) {
											log(LogStatus.INFO,
													"successfully verified convert to portfolio message congratulations",
													YesNo.Yes);

										} else {
											sa.assertTrue(false,
													"could not verify convert to portfolio message Expected: "
															+ dp.convertToPortfolioAfterNext(M6Ins3) + " actual: "
															+ text);
											log(LogStatus.SKIP,
													"could not verify convert to portfolio message Expected: "
															+ dp.convertToPortfolioAfterNext(M6Ins3) + " actual: "
										...
										if (dp.getconvertToPortfolioCrossButton(10) != null) {
											log(LogStatus.INFO, "successfully verified presence of cross icon",
													YesNo.Yes);
										...
										if (click(driver, dp.getfinishButton(10), "finish", action.BOOLEAN)) {
											log(LogStatus.INFO,
													"successfully verified finish button after convert to portfolio",
													YesNo.No);
										} else {
											sa.assertTrue(false,
													"could not verify convert to portfolio as finish button not clicked");
										...
										if (fp.FieldValueVerificationOnFundPage(projectName, labelName1[i],
												labelValues1[i])) {
										...
										if (dp.checkValueOfPathComponentValueOfStage(Stage.Closed.toString(), 10)) {
											log(LogStatus.INFO, "stage on path component successfully verified",
													YesNo.Yes);

										} else {
											sa.assertTrue(false, "stage on path component could not be verified");
											log(LogStatus.SKIP, "stage on path component could not be verified",
													YesNo.Yes);
										}
									} else {
										sa.assertTrue(false, "next button is not clickable");
	...
	public void M6tc007_1_VerifyImpactOnIntermediarySourceFirm(String projectName) {
		...
		if (ip.clickOnTab(projectName, TabName.Object4Tab)) {
			log(LogStatus.INFO, "Click on Tab : " + TabName.Object4Tab, YesNo.No);
			pipe = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal20",
					excelLabel.Deal_Name);
			company = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal20",
					excelLabel.Company_Name);
			sf = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal20",
					excelLabel.Source_Firm);
			stage = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal20",
					excelLabel.Stage);
				...
				if (click(driver, fp.sourceFirmLink(10), "source firm link", action.SCROLLANDBOOLEAN)) {
					...
					if (bp.clicktabOnPage("Details")) {
						...
						if (ip.getLastModifiedTime(projectName, 10) != null) {
							scrollDownThroughWebelement(driver, ip.getLastModifiedTime(projectName, 10),
									"last modified time");
							String text = ip.getLastModifiedTime(projectName, 10).getText().trim();
							text = text.split(",")[1];
							if (ip.verifyBeforeTimeOrNot(projectName, text)) {
			...
			if (ip.clickOnAlreadyCreatedItem(projectName, M6Deal11, 10)) {
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (dp.getconvertToPortfolioMessage(M6Ins11, 10) != null) {
						log(LogStatus.INFO,
								"successfully verified max character inst name on before next convert to portfolio message congratulations",
								YesNo.Yes);

					} else {
						sa.assertTrue(false,
								"could not verify max char inst name before next convert to portfolio message");
						log(LogStatus.SKIP,
								"could not verify max char inst name before next convert to portfolio message",
								YesNo.Yes);
					}
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins11))) {
							log(LogStatus.INFO,
									"successfully verified max character inst name on convert to portfolio message congratulations",
									YesNo.Yes);

						} else {
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins11) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					...
					if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
						...
						ThreadSleep(2000);
						if (setup.permissionChangeForUserONObject(driver, PermissionType.removePermission, userName,
								new String[][] { { onObject, permission } }, 20)) {
							log(LogStatus.PASS,
			...
			if (home.clickOnSetUpLink()) {
				...
				if (parentID != null) {
					...
					if (setup.searchStandardOrCustomObject(environment, mode, object.Profiles)) {
						log(LogStatus.INFO, "click on Object : " + object.Profiles, YesNo.No);
						ThreadSleep(2000);
						if (setup.permissionChangeForUserONObject(driver, PermissionType.givePermission, userName,
								new String[][] { { onObject, permission }, { onObject1, permission } }, 20)) {
							log(LogStatus.PASS,
									permission + " permission change for " + userName + " on object " + onObject,
									YesNo.No);
						} else {
						...
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10), rt + Keys.ENTER,
							...
							if (setup.clickOnAlreadyCreatedLayout(rt)) {
								...
								if (click(driver, setup.getEditButton(environment, Mode.Classic.toString(), 10), "edit",
									...
									switchToFrame(driver, 10, setup.getFrame(PageName.RecordTypePortfolioCompany, 10));
									ThreadSleep(2000);
									setup.getRecordTypeLabel(projectName, "Record Type Label", 10)
											.sendKeys(InstRecordType.Portfolio.toString());
									ThreadSleep(2000);
									switchToAlertAndAcceptOrDecline(driver, 30, action.ACCEPT);
									if (clickUsingJavaScript(driver, setup.getSaveButtonInCustomFields(10), "save",
											action.SCROLLANDBOOLEAN)) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins6))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins6) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins6) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.Firm)) {
					if (setup.clickOnObjectFeature(environment, mode, object.Firm, ObjectFeatureName.recordTypes)) {
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10),
								InstRecordType.Portfolio.toString() + Keys.ENTER, "status", action.BOOLEAN)) {
					...
					if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
						...
						if (dp.getconvertToPortfolioMessageRecordTypeInvalid(10) != null) {
							...
							if (text.contains(dp.invalidRecordType)) {
								log(LogStatus.INFO, "successfully verified invalid record type portfolio message",
										YesNo.Yes);

							} else {
								sa.assertTrue(false,
										"could not verify invalid record type portfolio message\nExpected: " + text
						...
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10), rt + Keys.ENTER,
							...
							if (setup.clickOnAlreadyCreatedLayout(rt)) {
								...
								if (click(driver, setup.getEditButton(environment, Mode.Classic.toString(), 10), "edit",
									...
									switchToFrame(driver, 10, setup.getFrame(PageName.RecordTypePortfolioCompany, 10));
									ThreadSleep(2000);
									if (sendKeys(driver, setup.getRecordTypeLabel(projectName, "Record Type Name", 10),
											updateLabel, "label", action.BOOLEAN)) {
										ThreadSleep(2000);
...
//									sa.assertTrue(false, "edit button not clickable");
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.Deal)) {
					if (setup.clickOnObjectFeature(environment, mode, object.Deal,
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins9))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins15) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins15) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
							log(LogStatus.INFO, "successfully verified cross button after convert to portfolio",
									YesNo.No);
						} else {
							sa.assertTrue(false, "could not verify convert to portfolio as cross button not clicked");
							log(LogStatus.SKIP, "could not verify convert to portfolio as cross button not clicked",
									YesNo.Yes);
						}
						if (click(driver, dp.getfinishButton(10), "finish", action.BOOLEAN)) {
							...
							if (setup.clickOnAlreadyCreatedLayout(excelLabel.Stage.toString())) {
								...
								if (click(driver, ele, "activate closed", action.SCROLLANDBOOLEAN)) {
									...
									if (clickUsingJavaScript(driver, ele, "closed", action.BOOLEAN)) {
										...
										ThreadSleep(3000);
										switchToFrame(driver, 10, setup.getFrame(PageName.PipelineCustomPage, 10));
										sendKeys(driver, setup.getFieldLabelTextBox1(10),
												Stage.Closed_Updated.toString(), "label", action.BOOLEAN);
										...
										if (clickUsingJavaScript(driver, fp.getCustomTabSaveBtn(10), "save",
											...
											if (click(driver, ele, "deactivate  closed", action.SCROLLANDBOOLEAN)) {
												ThreadSleep(5000);
												if (!isAlertPresent(driver)) {
													click(driver, ele, "deactivate closed", action.SCROLLANDBOOLEAN);
												}
												...
												if (ele != null) {
													...
													if (clickUsingActionClass(driver, ele)) {
														...
														switchToFrame(driver, 10,
																setup.getFrame(PageName.PipelineCustomPage, 10));

														ele = dp.findActivateLink(projectName,
																Stage.Closed_Updated.toString());
														if (ele != null) {
															log(LogStatus.INFO, "successfully deactivated closed value",
																	YesNo.No);
														} else {
															sa.assertTrue(false, "not able to deactivate closed value");
															log(LogStatus.SKIP, "not able to deactivate closed value",
																	YesNo.Yes);
														}
													} else {
														sa.assertTrue(false, "not able to click on deactivate link");
														log(LogStatus.SKIP, "not able to click on deactivate link",
																YesNo.Yes);
													}
												} else {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins20))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins20) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins20) + "\nactual: " + text, YesNo.Yes);
						}
						if (click(driver, dp.getconvertToPortfolioCrossButton(10), "finish", action.BOOLEAN)) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins1))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins1) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins1) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
					...
					if (setup.clickOnObjectFeature(environment, mode, object.Deal,
						...
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10),
							...
							if (setup.clickOnAlreadyCreatedLayout(excelLabel.Stage.toString())) {
								...
								ele = setup.clickOnEditInFrontOfFieldValues(projectName,
										Stage.Closed_Updated.toString(), false);
								if (click(driver, ele, "edit", action.BOOLEAN)) {
									switchToDefaultContent(driver);
									ThreadSleep(3000);
									switchToFrame(driver, 10, setup.getFrame(PageName.PipelineCustomPage, 10));
									ThreadSleep(3000);
									sendKeys(driver, setup.getFieldLabelTextBox1(10), Stage.Closed.toString(), "label",
											action.BOOLEAN);
	...
	public void M6tc017_2_CheckConvertToPortfolioFuncWhenRecordTypeAdvisor(String projectName) {
		...
		String labelValues1[] = { InstRecordType.Portfolio_Company.toString(),
				InstRecordType.Portfolio_Company.toString() };
		String labelName[] = { excelLabel.Highest_Stage_Reached.toString(), excelLabel.Stage.toString(),
				excelLabel.Deal_Quality_Score.toString() };
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins12))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins12) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins12) + "\nactual: " + text, YesNo.Yes);
						}
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins13))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins13) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins13) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins14))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins14) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins14) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins16))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins16) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins16) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins17))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins17) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins17) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
				...
				if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins18))) {
							...
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins18) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins18) + "\nactual: " + text, YesNo.Yes);
						}
						if (dp.getconvertToPortfolioCrossButton(10) != null) {
			...
			if (parentID != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.Deal)) {
					if (setup.clickOnObjectFeature(environment, mode, object.Deal,
						...
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10), updateLabel, "stage",
								action.BOOLEAN)) {
							ThreadSleep(4000);
							if (setup.clickOnAlreadyCreatedLayout(
									PageLabel.Convert_to_Portfolio.toString().replace("_", " "))) {
								switchToDefaultContent(driver);
								...
								if (click(driver, ele, "edit", action.BOOLEAN)) {
									...
									switchToFrame(driver, 10, setup.getFrame(PageName.ConvertToPortfolioFrame, 10));
									ThreadSleep(4000);
									if (sendKeys(driver, setup.getRecordTypeLabel(projectName, "Label", 10),
											updateLabel, "Label", action.BOOLEAN)) {
										if (clickUsingJavaScript(driver, setup.getSaveButtonInCustomFields(10), "save",
												action.SCROLLANDBOOLEAN)) {
											ThreadSleep(3000);
		...
		if (ip.clickOnTab(projectName, TabName.Object4Tab)) {
			if (ip.clickOnAlreadyCreatedItem(projectName, M6Deal2, 10)) {
				if (click(driver, dp.getconvertButton(10), "convert to portfolio button", action.BOOLEAN)) {
					...
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						ThreadSleep(4000);
						if (dp.getconvertToPortfolioMessageRepeat("Convert", 10) != null) {
				...
				if (click(driver, dp.getconvertButton(10), "convert to portfolio button", action.BOOLEAN)) {
					if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
						...
						if (text.contains(dp.convertToPortfolioAfterNext(M6Ins10))) {
							log(LogStatus.INFO,
									"successfully verified inst name on convert to portfolio message congratulations",
									YesNo.Yes);

						} else {
							sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins10) + "\nactual: " + text);
							log(LogStatus.SKIP, "could not verify convert to portfolio message Expected: "
									+ dp.convertToPortfolioAfterNext(M6Ins10) + "\nactual: " + text, YesNo.Yes);
						}
						if (click(driver, dp.getconvertToPortfolioCrossButton("Convert", 10), "finish",
								action.BOOLEAN)) {
							log(LogStatus.INFO, "clicked on cross button", YesNo.No);
						} else {
							sa.assertTrue(false, "could not click on cross button");
	...
	public void M6tc025_1_RenameConvertAsConvertToPortfolio(String projectName) {
		...
		String recTypeArray[] = { "Pitch", "Deal" };
		WebElement ele = null;
		if (home.clickOnSetUpLink()) {
			String parentID = switchOnWindow(driver);
			if (parentID != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.Deal)) {
					if (setup.clickOnObjectFeature(environment, mode, object.Deal,
							ObjectFeatureName.ButtonLinksAndActions)) {
						if (sendKeys(driver, setup.getsearchTextboxFieldsAndRelationships(10), oldLabel, "search",
								action.BOOLEAN)) {
							if (setup.clickOnAlreadyCreatedLayout(oldLabel)) {
								// if
								// (setup.clickOnAlreadyCreatedLayout(PageLabel.Convert_to_Portfolio.toString().replace("_",
								// " "))) {
								switchToDefaultContent(driver);
								ThreadSleep(3000);
								...
								if (click(driver, ele, "edit", action.BOOLEAN)) {
									...
									switchToFrame(driver, 20, setup.getSetUpPageIframe(60));
									if (sendKeys(driver, setup.getRecordTypeLabel(projectName, "Label", 10),
											PageLabel.Convert_to_Portfolio.toString().replace("_", " "), "label",
											action.BOOLEAN)) {
										if (clickUsingJavaScript(driver, setup.getSaveButtonInCustomFields(10), "save",
												action.SCROLLANDBOOLEAN)) {
											ThreadSleep(3000);
				...
				if (setup.clickOnObjectFeature("", Mode.Lightning.toString(), object.Deal,
						ObjectFeatureName.recordTypes)) {
					ThreadSleep(5000);
					if (setup.createRecordTypeForObject(projectName, pitch, false, profileforSelection, false, null,
							10)) {
				...
				if (setup.clickOnObjectFeature("", Mode.Lightning.toString(), object.Deal,
						ObjectFeatureName.recordTypes)) {
					ThreadSleep(5000);
					if (setup.createRecordTypeForObject(projectName, deal, false, profileforSelection, false, null,
							10)) {
			...
			if (parentID != null) {
				...
				if (setup.searchStandardOrCustomObject(environment, mode, object.Deal)) {
					if (setup.clickOnObjectFeature(environment, mode, object.Deal, ObjectFeatureName.pageLayouts)) {
						if (setup.createPageLayout(projectName,
								new String[][] { { PageLabel.Page_Layout_Name.toString(), layouts[0] } }, existing,
								null, null, 10)) {
					...
					if (setup.clickOnObjectFeature(environment, mode, object.Deal, ObjectFeatureName.pageLayouts)) {
						if (setup.createPageLayout(projectName,
								new String[][] { { PageLabel.Page_Layout_Name.toString(), layouts[1] } }, existing,
								null, null, 10)) {
					...
					if (setup.clickOnObjectFeature(environment, mode, object.Deal, ObjectFeatureName.pageLayouts)) {
						if (click(driver, setup.getpageLayoutAssignment(projectName, 10), "layout assignment",
								action.BOOLEAN)) {
							switchToDefaultContent(driver);
							ThreadSleep(3000);
							switchToFrame(driver, 20, setup.getSetUpPageIframe(60));

							if (click(driver, setup.geteditAssignment(projectName, 10), "edit assignment",
									action.BOOLEAN)) {
								switchToDefaultContent(driver);
								ThreadSleep(3000);
								switchToFrame(driver, 20, setup.getSetUpPageIframe(60));
								...
								for (String re : rt) {
									...
									if (click(driver, ele, "layout name for record type", action.BOOLEAN)) {
										...
										ThreadSleep(3000);
										if (selectVisibleTextFromDropDown(driver, ele, "page layout selector",
												layouts[i])) {

										} else {
											log(LogStatus.FAIL, "could not select " + layouts[i] + " on dropdown",
													YesNo.Yes);
						...
						if (click(driver, ele, "Edit Button", action.SCROLLANDBOOLEAN)) {
							...
							if (selectVisibleTextFromDropDown(driver, setup.getavailableRecordType(60),
								...
								if (click(driver, setup.getAddBtn(60), "Custom Tab Add Button",
									...
									// sa.assertTrue(false,"Not able to click on add button so cannot add custom
									// tabs");
									appLog.error("Not able to click on add button so cannot add custom tabs");
								}
							} else {
							...
							if (selectVisibleTextFromDropDown(driver, setup.getSelectedRecordType(60),
									"selected Tab List", rt[0])) {
								log(LogStatus.INFO, "successfully verified " + rt[0], YesNo.No);

							} else {
								log(LogStatus.ERROR, "not able to verify " + rt[0] + " in selected record type",
										YesNo.Yes);
								sa.assertTrue(false, "not able to verify " + rt[0] + " in selected record type");

							}
							if (selectVisibleTextFromDropDown(driver, setup.getSelectedRecordType(60),
									"selected Tab List", rt[1])) {
								log(LogStatus.INFO, "successfully verified " + rt[1], YesNo.No);

							} else {
								log(LogStatus.ERROR, "not able to verify " + rt[1] + " in selected record type",
										YesNo.Yes);
	...
	public void M6tc025_4_RenameConvertAsConvertToPortfolio(String projectName) {
		...
		for (int i = 21; i < 23; i++) {
			if (lp.clickOnTab(projectName, TabName.Object1Tab)) {
				...
				value = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name, "M6INS" + i,
						excelLabel.Institutions_Name);
				type = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name, "M6INS" + i,
		...
		for (int i = 21; i < 23; i++) {
			if (lp.clickOnTab(projectName, TabName.Object4Tab)) {
				log(LogStatus.INFO, "Click on Tab : " + TabName.Object4Tab, YesNo.No);
				pipe = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal" + i,
						excelLabel.Deal_Name);
				company = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal" + i,
						excelLabel.Company_Name);
				stage = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal" + i,
						excelLabel.Stage);
				rt = ExcelUtils.readData(phase1DataSheetFilePath, "Deal", excelLabel.Variable_Name, "M6Deal" + i,
						excelLabel.Record_Type);
				refresh(driver);
				ThreadSleep(3000);
					...
					if (click(driver, dp.getconvertToPortfolio(10), "convert to portfolio button", action.BOOLEAN)) {
						if (click(driver, dp.getnextButton(10), "next button", action.BOOLEAN)) {
							...
							if (text.contains(dp.convertToPortfolioAfterNext(ins[j]))) {
								...
								sa.assertTrue(false, "could not verify convert to portfolio message Expected: "
										+ dp.convertToPortfolioAfterNext(ins[j]) + "\nactual: " + text);
								log(LogStatus.SKIP,
										"could not verify convert to portfolio message Expected: "
												+ dp.convertToPortfolioAfterNext(ins[j]) + "\nactual: " + text,
										YesNo.Yes);
	...
	public void M6tc026_VerifyPathComponent(String projectName) {
		...
		String labelName1[] = { excelLabel.Highest_Stage_Reached.toString(), excelLabel.Stage.toString(),
				excelLabel.Last_Stage_Change_Date.toString() };
	...
	public void M6tc027_LastStageChangedFundriaisingPrecondition(String projectName) {
		...
		if (lp.clickOnTab(projectName, TabName.Object1Tab)) {
			...
			value = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name, "M6LSCFINS1",
					excelLabel.Institutions_Name);
			type = ExcelUtils.readData(phase1DataSheetFilePath, "Entities", excelLabel.Variable_Name, "M6LSCFINS1",
					excelLabel.Record_Type);
			if (ip.createEntityOrAccount(projectName, mode, value, type, null, null, 20)) {
				log(LogStatus.INFO, "successfully Created Account/Entity : " + value + " of record type : " + type,
						YesNo.No);
		...
		if (lp.clickOnTab(projectName, TabName.Object3Tab)) {
			name = ExcelUtils.readData(phase1DataSheetFilePath, "Fund", excelLabel.Variable_Name, "M6LSCFFUND1",
					excelLabel.Fund_Name);
			type = ExcelUtils.readData(phase1DataSheetFilePath, "Fund", excelLabel.Variable_Name, "M6LSCFFUND1",
					excelLabel.Fund_Type);
			cat = ExcelUtils.readData(phase1DataSheetFilePath, "Fund", excelLabel.Variable_Name, "M6LSCFFUND1",
		...
		if (lp.clickOnTab(projectName, TabName.FundraisingsTab)) {
			...
			if (frp.createFundRaising(environment, mode, M6LSCFundraising1, M6LSCFund1, M6LSCIns1, null, null, null,
					null)) {
	...
	public void M6tc028_1_AddFieldsOnFundraisingPage(String projectName) {
		...
		if (home.clickOnSetUpLink()) {
			...
			if (parentID != null) {
				...
				List<String> abc = setup.DragNDrop("", mode, object.Fundraising, ObjectFeatureName.pageLayouts,
	...
	public void M6tc031_DeleteAndRestoreFundraising(String projectName) {
		...
		if (ip.clickOnTab(projectName, TabName.FundraisingsTab)) {
			if (ip.clickOnAlreadyCreatedItem(projectName, M6LSCFundraising1, 10)) {
				ThreadSleep(3000);
				if (ip.clickOnShowMoreActionDownArrow(projectName, PageName.Object5Page,
						ShowMoreActionDropDownList.Delete, 10)) {
					ThreadSleep(3000);
					ele = frp.getDeleteButtonOnDeletePopUp(projectName, 10);
					...
					if (sp.clickOnObjectFeature(environment, mode, object.Fundraising,
						...
						if (sendKeys(driver, sp.getsearchTextboxFieldsAndRelationships(10),
							...
							if (sp.clickOnAlreadyCreatedLayout(excelLabel.Stage.toString())) {
								...
								stage = stage.replace("_", " ");
								ThreadSleep(3000);
								ele = frp.getEditBtn(projectName, stage, action.SCROLLANDBOOLEAN, 10);
								if (ele != null) {
								...
								if (click(driver, ele, "edit", action.SCROLLANDBOOLEAN)) {
									...
									if (sendKeys(driver, sp.getRecordTypeLabel(projectName, "Label", 10), newstage,
											"label", action.BOOLEAN)) {
										if (click(driver, sp.getSaveButtonInCustomFields(10), "save",
												action.SCROLLANDBOOLEAN)) {
											ThreadSleep(3000);
											log(LogStatus.INFO, "save button is clicked", YesNo.No);
	...
	public void M6tc034_VerifyNewLocale(String projectName) {
		...
		for (String t : timezone) {
			...
			if (lp.clickOnTab(projectName, TabName.FundraisingsTab)) {
				fundr = ExcelUtils.readData(phase1DataSheetFilePath, "Fundraisings", excelLabel.Variable_Name,
						"M6LSCFUNDR" + j, excelLabel.FundRaising_Name);
				stage = ExcelUtils.readData(phase1DataSheetFilePath, "Fundraisings", excelLabel.Variable_Name,
						"M6LSCFUNDR" + j, excelLabel.Stage);
