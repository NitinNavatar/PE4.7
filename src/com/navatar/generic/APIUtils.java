			...
			if (!status.equalsIgnoreCase("Created")) {
				for (int j = 1; j < lastcol; j++) {
					...
					if (label.equals("RecordTypeId")) {
						...
						if (value != null && !value.equalsIgnoreCase("") && !value.isBlank() && !value.isEmpty()) {
							String RT = getObjectActiveRecordTypeId("navmnaI__Deal__c", value);
							data.put(label, RT);
							System.out.println(
				...
				createObejectRecordByAPI("navmnaI__Deal__c", data);
				ExcelUtils.writeDataInExcel(filePath, "Created", sheetName, i, 0);
			} else {
				...
				for (int j = 1; j < lastcol; j++) {
					...
					if (label.equals("RecordTypeId")) {
						if (value != null && !value.equalsIgnoreCase("") && !value.isBlank() && !value.isEmpty()) {
							...
							data.put(label, RT);
							System.out.println(
									"Added Data for Fund Team:" + i + " Label :" + label + " with value :" + value);
							// log(LogStatus.INFO, "Added Data for Deal :"+i +" Label :"+label +" with value
							// :"+value, YesNo.No);
							}
	...
	public void ClipRelationObjectDataUpload(String filePath,String sheetName) {
		...
		for (int i =1; i < row + 1; i++) {
			...
			if (!status.equalsIgnoreCase("Created")) {
				...
				createObejectRecordByAPI("navmnaI__Clip_Relation__c", data);
				ExcelUtils.writeDataInExcel(filePath, "Created", sheetName, i, 0);
			} else {
	...
	public void ThemeObjectDataUpload(String filePath,String sheetName) {
		...
		for (int i = 1; i < row + 1; i++) {
			...
			if (!status.equalsIgnoreCase("Created")) {
				...
				createObejectRecordByAPI("navmnaI__Theme__c", data);
				ExcelUtils.writeDataInExcel(filePath, "Created", sheetName, i, 0);
			} else {
				...
				for (int j = 1; j < lastcol; j++) {
					...
					if (label.equals("RecordTypeId")) {
						...
						if (value != null && !value.equalsIgnoreCase("") && !value.isBlank() && !value.isEmpty()) {
							...
							data.put(label, date);
							System.out.println(
									"Added Data for Event WorkAround:" + i + " Label :" + label + " with value :" + value);
						}
					}  else {
						data.put(label, value);
					}
