 ...
 *
 */
public class AdvisorPageBusinessLayer extends AdvisorPage  {
	
	/**
	 * @param driver
	 */
	public AdvisorPageBusinessLayer(WebDriver driver) {
		super(driver);
		// TODO Auto-generated constructor stub
	}

	public boolean createAdvisorItem(String projectName,String miName ,String[][] labelWithValue,int timeOut) {
		...
		if (clickOnTab(projectName, navigationTab)) {
			...
			if(clickUsingJavaScript(driver, getNewButton(projectName, 10), "new button")) {
				...
				enteringValueForAdvisor(projectName, labelWithValue, action.BOOLEAN, timeOut);
				if (click(driver,  getNavigationTabSaveBtn(projectName, 10), "save button", action.SCROLLANDBOOLEAN)) {
					...
					if (ele!=null) {
						...
						String expectedValue="Advisor A -  was created.";
						if (s[0].equals("Advisor ") && s[1].contains("A - ") &&s[2].equals(" was created.")) {
	 ...
	 */
	public void enteringValueForAdvisor(String projectName,String[][] navigationFieldWithValues,action action,int timeOut) {
		...
		for (String[] navigationFieldAndvalue : navigationFieldWithValues) {
			...
			if (navigationField.equalsIgnoreCase(excelLabel.Role.toString())) {
				ele = getDropdownOnCreationPopUp(projectName, PageName.Advisor, navigationField, action.BOOLEAN, timeOut);
				if (click(driver,ele,navigationField+" with value "+navigationvalue, action.BOOLEAN)) {
					...
					if (SelectDropDownValue(projectName, PageName.Advisor, navigationField, navigationvalue, action, timeOut)) {
				...
				if (sendKeys(driver, ele, navigationvalue, navigationField, action)) {
					...
					if (navigationField.equalsIgnoreCase(excelLabel.Advisor.toString()) || navigationField.equalsIgnoreCase(excelLabel.Client.toString())) {
						ThreadSleep(10000);
