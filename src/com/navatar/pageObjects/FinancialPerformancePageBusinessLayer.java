 ...
 *
 */
public class FinancialPerformancePageBusinessLayer extends FinancialPerformancePage  {
	
	/**
	 * @param driver
	 */
	public FinancialPerformancePageBusinessLayer(WebDriver driver) {
		super(driver);
		// TODO Auto-generated constructor stub
	}

	public boolean createFinancialPerformanceItem(String projectName,String[][] labelWithValue,int timeOut) {
		String navigationTab="Financial Performances";
		boolean flag=false;
		if (clickOnTab(projectName, navigationTab)) {
			...
			if(clickUsingJavaScript(driver, getNewButton(projectName, 10), "new button")) {
				...
				enteringValueForFinancialPerformance(projectName, labelWithValue, action.BOOLEAN, timeOut);
				if (click(driver,  getNavigationTabSaveBtn(projectName, 10), "save button", action.SCROLLANDBOOLEAN)) {
					...
					if (ele!=null) {
						...
						String expectedValue="Financial Performance QFP was created.";
						if (s[0].equals("Financial Performance ") && s[1].contains("QFP-") &&s[2].equals(" was created.")) {
	 ...
	 */
	public void enteringValueForFinancialPerformance(String projectName,String[][] navigationFieldWithValues,action action,int timeOut) {
		...
		for (String[] navigationFieldAndvalue : navigationFieldWithValues) {
			...
			if (navigationField.equalsIgnoreCase(excelLabel.Frequency.toString())) {
				 ele = getDropdownOnCreationPopUp(projectName, PageName.Financial_Performance, navigationField, action.BOOLEAN, timeOut);
				 if (click(driver,ele,navigationField+" with value "+navigationvalue, action.BOOLEAN)) {
					 ...
					 if (SelectDropDownValue(projectName, PageName.Financial_Performance, navigationField, navigationvalue, action, timeOut)) {
				...
				if (sendKeys(driver, ele, navigationvalue, navigationField, action)) {
					...
					if (navigationField.equalsIgnoreCase(excelLabel.Company.toString())) {
						ThreadSleep(10000);
