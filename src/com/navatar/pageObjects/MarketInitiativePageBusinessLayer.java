 ...
 *
 */
public class MarketInitiativePageBusinessLayer extends MarketInitiativePage  {
	
	/**
	 * @param driver
	 */
	public MarketInitiativePageBusinessLayer(WebDriver driver) {
		super(driver);
		// TODO Auto-generated constructor stub
	}

	public boolean createMIItem(String projectName,String miName ,String[][] labelWithValue,int timeOut) {
		String navigationTab="Marketing Initiatives";
		boolean flag=false;
		if (clickOnTab(projectName, navigationTab)) {
			...
			if(clickUsingJavaScript(driver, getNewButton(projectName, 10), "new button")) {
				...
				enteringValueForMI(projectName, labelWithValue, action.BOOLEAN, timeOut);
				if (click(driver,  getNavigationTabSaveBtn(projectName, 10), "save button", action.SCROLLANDBOOLEAN)) {
					...
					if (ele!=null) {
						...
						String expectedValue="Marketing Initiative \""+miName+"\" was created.";
	 ...
	 */
	public void enteringValueForMI(String projectName,String[][] navigationFieldWithValues,action action,int timeOut) {
		...
		for (String[] navigationFieldAndvalue : navigationFieldWithValues) {
			...
			if (navigationField.equalsIgnoreCase(excelLabel.Status.toString())) {
				ele = getDropdownOnCreationPopUp(projectName, PageName.Marketing_Initiative, navigationField, action.BOOLEAN, timeOut);
				if (click(driver,ele,navigationField+" with value "+navigationvalue, action.BOOLEAN)) {
					...
					if (SelectDropDownValue(projectName, PageName.Marketing_Initiative, navigationField, navigationvalue, action, timeOut)) {
