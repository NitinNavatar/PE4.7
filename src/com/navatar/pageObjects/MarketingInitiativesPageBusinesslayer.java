...
public class MarketingInitiativesPageBusinesslayer extends MarketingInitiativesPage
		implements MarketingInitiativesPageErrorMsg {
	...
	public boolean createMarketInitiatives(String environment, String mode, String marketInitiativeName) {
		...
		if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
			...
			if (clickUsingJavaScript(driver, getNewButton(environment, mode, 60), "new button")) {
				...
				appLog.error("Not able to click on new button so cannot create Market Initiatives : "
						+ marketInitiativeName);
				return false;
			}
		} else {
...
//			appLog.info("clicked on new button");
		if (sendKeys(driver, getMarketInitiativeNameTextBox(environment, mode, 60), marketInitiativeName,
				"market initiative name text box", action.SCROLLANDBOOLEAN)) {
...
//						 ele = FindElement(driver, xpath, "Header : "+marketInitiativeName, action.BOOLEAN, 30);
				...
				if (ele != null) {
					...
					appLog.error(
							"Create market initiative label text is not visible so cannot verify created market initiative text: "
									+ marketInitiativeName);
				}

			} else {
				appLog.error("Not able to click on save button so cannot create Market Initiatives : "
						+ marketInitiativeName);
			}
		} else {
			appLog.error("Not able to pass value in Name text box: " + marketInitiativeName
					+ "  so cannot create market Initiative");
		}
//		}else {
...
//		}
	...
	public boolean clickOnCreatedMarketInitiatives(String environment, String mode, String marketInitiativeName) {
		...
		if (mode.equalsIgnoreCase(Mode.Classic.toString())) {
			...
			WebElement MI = FindElement(driver,
					"//div[@class='x-panel-bwrap']//a//span[contains(text(),'" + marketInitiativeName + "')]",
					"marketInitiative Name", action.BOOLEAN, 10);
			if (MI != null) {
				if (click(driver, MI, "marketInitiative Name", action.SCROLLANDBOOLEAN)) {
					appLog.info("Clicked on created marketInitiative Name successfully :" + marketInitiativeName);
					return true;
				...
				while (true) {
					...
					if (click(driver, getNextImageonPage(10), "MI Page Next Button", action.SCROLLANDBOOLEAN)) {

						appLog.info("Clicked on Next Button");
						MI = FindElement(driver, "//div[@class='x-panel-bwrap']//a//span[contains(text(),'"
								+ marketInitiativeName + "')]", "marketInitiative Name", action.BOOLEAN, 10);
						if (MI != null) {
			...
			if (clickOnAlreadyCreated_Lighting(environment, mode, TabName.MarketingInitiatives, marketInitiativeName,
					10)) {
	 ...
	 * @return true/false
	 */
	public boolean SearchforProspects(AddProspectsTab addProspectsTab, String fieldName, String operator,
			String value) {
		WebElement ele = null;
		int i = 1;
		if (sendKeys(driver, getAddProspectFieldAutoCompleteTextBox(addProspectsTab, 30), fieldName, "field text box",
				action.SCROLLANDBOOLEAN)) {
			if (addProspectsTab.toString().equalsIgnoreCase(AddProspectsTab.PastMarketingInitiatives.toString())) {
				i = 2;
			}
			ThreadSleep(2000);
			String xpath = "(//ul[@class='ui-autocomplete ui-front ui-menu ui-widget ui-widget-content'])[" + i
					+ "]//li[text()='" + fieldName + "']";
			ele = isDisplayed(driver,
					FindElement(driver, xpath, "field auto complete text", action.SCROLLANDBOOLEAN, 30), "visibility",
					30, "field auto complete text");

			if (ele != null) {
				if (click(driver, ele, fieldName + " text", action.SCROLLANDBOOLEAN)) {
					appLog.info("clicked on field name " + fieldName + " text box");
					if (selectVisibleTextFromDropDown(driver, getAddProspectOperatorDropDownList(addProspectsTab, 30),
							"operator drop down list", operator)) {
						appLog.info("Select Operator : " + operator);
						if (value != null && !value.isEmpty()) {
							if (sendKeys(driver, getAddProspectValueTextBox(addProspectsTab, 30), value,
									"Value text box", action.SCROLLANDBOOLEAN)) {
								appLog.info("passed value in Value Text Box: " + value);
							} else {
								appLog.error("Not able to pass value in Value text box: " + value
										+ " so cannot apply filter");
								return false;
							}
						}
						if (click(driver, getAddProspectSearchBtn(addProspectsTab, 30), "search button",
								action.SCROLLANDBOOLEAN)) {
			...
			appLog.error("Not able to pass value in field auto complete text box : " + fieldName
					+ " so cannot apply filter");
		}
		return false;
	...
	public boolean selectReportFromReportsTab(String reportFolderName, String reportName) {
		...
		if (click(driver, getSelectAReportLookUpIcon(20), "select a report look up icon", action.SCROLLANDBOOLEAN)) {
			...
			ThreadSleep(2000);
			if (sendKeys(driver, getSelectAReportSearchTextBox(30), reportName, "select a reports search text box",
					action.SCROLLANDBOOLEAN)) {
				...
				if (ele != null) {
					...
					if (click(driver, ele, "Sample Report: # of Contacts report link", action.SCROLLANDBOOLEAN)) {
						...
						if (click(driver, getSelectAReportPopUpOKBtn(30), "select a reports pop up ok btn",
								action.SCROLLANDBOOLEAN)) {
							...
							if (click(driver, getLoadReportsBtn(10), "load reports button", action.SCROLLANDBOOLEAN)) {
								...
								appLog.error(
										"Not able to click on load reports button so cannot cannot load selected report :"
												+ reportName);
							}

						} else {
							appLog.error("Not able to click on select a reports OK button so cannot select report :"
									+ reportName);
						}
					} else {
					...
					appLog.error("searched report is not visible in select a report pop up : " + reportName
							+ " so cannot select it");
				}
			} else {
				appLog.error("Not able to pass value in select a rpeort search text box : " + reportName
						+ " so cannot select report: " + reportName);
			}

		} else {
			appLog.error("Not able to click select a report lookup icon so cannot select report Folder: "
					+ reportFolderName + "  report Name: " + reportName);
		}
		return false;
	}
	 ...
	 * @return true/false
	 */
	public List<String> selectProspectsContactAndVerifyReviewProspectList(String environment, String mode,
			AddProspectsTab addProspectsTab, HashMap<String, String> ContactFullNameAndAccountName,
			boolean isCheckedforLink) {
		...
		if (ele != null) {
			...
			if (beforeRecordDig != 0) {
				...
				while (itr.hasNext()) {
					...
					if (ScrollAndClickOnContactNameCheckBoxInAddProspect(addProspectsTab, contactName, accountName,
							10)) {
									...
									if (ele != null) {
										...
										if (click(driver, ele, linkClick[j], action.BOOLEAN)) {
											...
											if (Mode.Lightning.toString().equalsIgnoreCase(mode)) {
												...
												MarketingInitiativesPageBusinesslayer market = new MarketingInitiativesPageBusinesslayer(
														driver);
											...
											result.add("Not able to click on " + linkClick[j]
													+ " so cannot verify landing page");
										}
									} else {
										appLog.error("Not able to click on " + linkClick[j]
						...
						appLog.error("Not able to click on Contact Name :" + contactName
				...
				if (ele != null) {
					if (click(driver, getAddToProspectListBtn(5), "add prospect list button",
							action.SCROLLANDBOOLEAN)) {
						...
						if (ele != null) {
							...
							if (AfterRecordDig == ExpectedCount) {
								...
								appLog.error("Select from search result record count is not matched. Expected: "
										+ ExpectedCount + " \t Actual Result: " + beforeRecordDig);
								result.add("Select from search result record count is not matched. Expected: "
										+ ExpectedCount + " \t Actual Result: " + beforeRecordDig);
							}
						} else {
							appLog.error(
									"Select from search result record is not visible after add contact in prospect list grid so cannot verify record number of select  from search result.");
							result.add(
									"Select from search result record is not visible after add contact in prospect list grid so cannot verify record number of select  from search result.");
						}
						ThreadSleep(5000);
						...
						if (ele != null) {
							...
							if (reviewListRecordDig != 0) {
								...
								while (itr.hasNext()) {
									String contactName = itr.next();
									String accountName = ContactFullNameAndAccountName.get(contactName);
									xpath = "//span[@id='gridReviewList-view-box']//span[contains(@id,'gridReviewList-row-')]/span[text()='"
											+ contactName + "']/../span[text()='" + accountName + "']";
									ele = isDisplayed(driver, FindElement(driver, xpath, "", action.BOOLEAN, 20),
											"visibility", 20, contactName + " select Contact Name");
									if (ele != null) {
										appLog.info(contactName
												+ " :Contact Name is selected successfully in add review prospect list");
									} else {
										appLog.error(contactName
												+ " :Contact Name is not available in add review prospect list");
										result.add(contactName
												+ " :Contact Name is not available in add review prospect list");
									}
								}
								if (reviewListRecordDig == count) {
									appLog.info("Select from search result record number is matched. Record Count is: "
											+ count);
								} else {
									appLog.error("Select from search result record number is not matched.: " + count
											+ "/t Actual Record Count :" + reviewListRecordDig);
									result.add("Select from search result record number is not matched.: " + count
											+ "/t Actual Record Count :" + reviewListRecordDig);
								}

							} else {
								appLog.error("Review Prospect List record count is " + reviewListRecordDig
										+ " that means there is no selected contact name is available on grid so we cannot verify contacts in review prospect list");
								result.add("Review Prospect List record count is " + reviewListRecordDig
										+ " that means there is no selected contact name is available on grid so we cannot verify contacts in review prospect list");
							}
						} else {
							appLog.error(
									"Review Prospect List count is not visible so we cannot verify contacts in review prospect list");
							result.add(
									"Review Prospect List record count is not visible so we cannot verify contacts in review prospect list");
						}
					} else {
						appLog.error(
								"Not able to click on selected contact Name so cannot add contact in review add prospect list and verify contact Name");
						result.add(
								"Not able to click on selected contact Name so cannot add contact in review add prospect list and verify contact Name");
					}
				} else {
					appLog.error(
							"Add prospects list is not enable so we cannot select any contact from select prospects contact list");
					result.add(
							"Add prospects list is not enable so we cannot select any contact from select prospects contact list");
				}
			} else {
				appLog.error("Select From Search Results record count is " + beforeRecordDig
						+ " that means there is no contact is visible on grid so we cannot add prospects contacts");
				result.add("Select From Search Results record count is " + beforeRecordDig
						+ " that means there is no contact is visible on grid so we cannot add prospects contacts");
			}
		} else {
	 ...
	 * @return true/false
	 */
	public boolean ScrollAndClickOnContactNameCheckBoxInAddProspect(AddProspectsTab addProspectsTab, String contactName,
			String accountName, int timeout) {
		int j = 0;
		WebElement ele = null;
		String XpathelementTOSearch = "";
		if (addProspectsTab.toString().equalsIgnoreCase(AddProspectsTab.AccountAndContacts.toString())
				|| addProspectsTab.toString().equalsIgnoreCase(AddProspectsTab.PastMarketingInitiatives.toString())) {
			...
			for (int i = 0; i < 15; i++) {
				if (currentlyExecutingTC.contains("Smoke")) {
					String Fname = ExcelUtils.readData(BaseLib.smokeFilePath, "Contacts", excelLabel.Variable_Name,
							"SmokeC" + i, excelLabel.Contact_FirstName);
					String Lname = ExcelUtils.readData(BaseLib.smokeFilePath, "Contacts", excelLabel.Variable_Name,
							"SmokeC" + i, excelLabel.Contact_LastName);
					...
					String Fname = ExcelUtils.readData(BaseLib.testCasesFilePath, "Contacts", excelLabel.Variable_Name,
							"SmokeC" + i, excelLabel.Contact_FirstName);
					String Lname = ExcelUtils.readData(BaseLib.testCasesFilePath, "Contacts", excelLabel.Variable_Name,
							"SmokeC" + i, excelLabel.Contact_LastName);
					if ((Fname + " " + Lname).equalsIgnoreCase(contactName)) {
						splitedContactName[0] = Fname;
						splitedContactName[1] = Lname;
			...
			XpathelementTOSearch = "//span[@id='Select_from_Search_ResultsArep-view-box-middle']//a[text()='"
					+ splitedContactName[0] + "']/../../following-sibling::span//a[text()='" + splitedContactName[1]
					+ "']/../../../span[2]/span/span[1]";
		}
		...
		for (int i = 0; i <= widgetTotalScrollingHeight / 25; i++) {
			...
			if (sendKeysAndPressEnter(driver, getSearchForAContactTextBoxOnReportTab(10), accountName, accountName,
					action.BOOLEAN)) {
			...
			if (!driver.findElements(By.xpath(XpathelementTOSearch)).isEmpty()) {
				...
				if (ele != null) {
					if (click(driver, ele, "", action.BOOLEAN)) {
						...
						sendKeysAndPressEnter(driver, getSearchForAContactTextBoxOnReportTab(10), "", "",
								action.BOOLEAN);
					} else {
				...
				((JavascriptExecutor) driver).executeScript("arguments[0].scrollTo(" + j + "," + (j = j + 1000) + ")",
						getSelectProspectsGridScrollBox(addProspectsTab, 10));
				try {
					Thread.sleep(1000);
	 ...
	 * @return true/false
	 */
	public boolean addProspects(String environment, String mode, AddProspectsTab addProspectsTab, String fieldName,
			String operator, String value, HashMap<String, String> ContactFullNameAndAccountName,
			boolean isCheckedforLink) {
		WebElement ele = null;
		ThreadSleep(5000);
		...
		if (addProspectsTab.toString().equalsIgnoreCase(AddProspectsTab.PastMarketingInitiatives.toString())) {
			if (click(driver, getPastMarketingInitiativeTab(60), "past marketing initiative tab",
					action.SCROLLANDBOOLEAN)) {
			...
			if (click(driver, getReportTab(60), "reports tab", action.SCROLLANDBOOLEAN)) {
				...
				appLog.error("Not able to click on report tab so cannot apply filter: " + fieldName + " " + operator
						+ " " + value);
				return false;
			}
		}
		if (SearchforProspects(addProspectsTab, fieldName, operator, value)) {
			...
			ThreadSleep(5000);
			List<String> result = selectProspectsContactAndVerifyReviewProspectList(environment, mode, addProspectsTab,
					ContactFullNameAndAccountName, isCheckedforLink);
			if (result.isEmpty()) {
				if (click(driver, getAddProspectMarketingInitiativeBtn(30), "add propects market initiative button",
						action.SCROLLANDBOOLEAN)) {
	 ...
	 * @return true/false
	 */
	public boolean removeContactFromReviewPorspectList(String environment, String mode, AddProspectsTab addProspectsTab,
			String contactName, String accountName) {
		WebElement ele = null;
		String Xpath = "//span[@id='gridReviewList-view-box']//span[contains(@id,'gridReviewList-row-')]/span[text()='"
				+ contactName + "']/../span[text()='" + accountName + "']/../span[2]/a";
		ele = getReviewProspectListRecord(60);
		if (ele != null) {
			String reviewListRecord = ele.getText().trim();
			int reviewListRecordDig = Integer.parseInt((reviewListRecord.split(":")[1].trim()));
			if (reviewListRecordDig != 0) {
				ele = isDisplayed(driver, FindElement(driver, Xpath, "", action.BOOLEAN, 30), "visibility", 20,
						contactName + " Contact remove Icon");
				if (ele != null) {
					if (click(driver, ele, contactName + " remove icon", action.SCROLLANDBOOLEAN)) {
						...
						if (ele != null) {
							...
							int reviewListRecordDigAfterRemoveContact = Integer
									.parseInt((reviewListRecordAfterRemoveContact.split(":")[1].trim()));
							if (reviewListRecordDigAfterRemoveContact == reviewListRecordDig - 1) {
								appLog.info("review porspect list record count " + (reviewListRecordDig - 1)
										+ " is matched after remove contact " + contactName);
								return true;
							} else {
								appLog.error("review porspect list record count " + (reviewListRecordDig - 1)
										+ " is not matched after remove contact " + contactName + " \t Actual : "
										+ reviewListRecordDigAfterRemoveContact);
							}
						} else {
							appLog.error("Review Prospect List count is not visible after remove contact " + contactName
									+ " so we cannot verify record count");
						}
					} else {
						appLog.error("Not able to click on contact " + contactName
								+ " remove icon so cannot remove it from review prospect list");
					}
				} else {
					appLog.error(contactName
							+ " remove icon is not visible in review porspects list so cannot remove contact :"
							+ contactName);
				}
			} else {
				appLog.error("Review Prospect List record count is " + reviewListRecordDig
						+ " that means there is no selected contact name is available on grid so we cannot remove contacts in review prospect list");
			}
		} else {
			appLog.error(
					"Review Prospect List count is not visible so we cannot remove contacts in review prospect list");
		}
		return false;
	}
	 ...
	 * @return true/false
	 */
	public boolean SearchforEmailProspects(String environment, String mode, PageName pageName, String fieldName,
			String operator, String value) {
		WebElement ele = null;
		String xpath = "";
		if (fieldName != null && operator != null && !fieldName.isEmpty() && !operator.isEmpty()) {
			if (sendKeys(driver, getEmailProspectFieldTextBox(30), fieldName, "field text box",
					action.SCROLLANDBOOLEAN)) {
				if (pageName.toString().equalsIgnoreCase(PageName.emailFundraisingContact.toString())) {
					xpath = "//ul[@class='ui-autocomplete ui-front ui-menu ui-widget ui-widget-content ui-corner-all']//a[text()='"
							+ fieldName + "']";
				} else {
					xpath = "//ul[@class='ui-menu ui-widget ui-widget-content ui-autocomplete ui-front']//div[text()='"
							+ fieldName + "']";
				}
				ThreadSleep(2000);
				ele = isDisplayed(driver,
						FindElement(driver, xpath, "field auto complete text", action.SCROLLANDBOOLEAN, 30),
						"visibility", 30, "field auto complete text");
				if (ele != null) {
					if (click(driver, ele, fieldName + " text", action.SCROLLANDBOOLEAN)) {
						appLog.info("clicked on field name " + fieldName + " text box");
						if (selectVisibleTextFromDropDown(driver, getEmailProspectOperatorDropDownList(30),
								"operator drop down list", operator)) {
							appLog.info("Select Operator : " + operator);
							if (value != null && !value.isEmpty()) {
								if (sendKeys(driver, getEmailProspectValueTextBox(20), value, "Value text box",
										action.SCROLLANDBOOLEAN)) {
									appLog.info("passed value in Value Text Box: " + value);
								} else {
									appLog.error("Not able to pass value in Value text box: " + value
	 ...
	 * @return true/false
	 */
	public boolean ScrollAndClickOnContactNameCheckBoxInEmailProspect(PageName pageName, String contactName,
			String accountName, int timeout) {
		int j = 0;
		WebElement ele = null;
		String XpathelementTOSearch = "";
		if (pageName.toString().equalsIgnoreCase(PageName.emailFundraisingContact.toString())) {
			XpathelementTOSearch = "//span[contains(@class,'aw-grid-row')]//a[text()='" + contactName
					+ "']/../following-sibling::span/a[text()='" + accountName + "']/../../span[2]/span/span[1]";
		} else {
			XpathelementTOSearch = "//span[contains(@class,'aw-hpanel-middle')]//span[contains(@class,'aw-grid-row')]//a[text()='"
					+ contactName + "']/../following-sibling::span[text()='" + accountName
					+ "']/../span[2]/span/span[1]";
		}
		By byelementToSearch = By.xpath(XpathelementTOSearch);
	 ...
	 * @return true/false
	 */
	public List<String> selectProspectsContactAndVerifyReviewProspectListInEmailProspect(PageName pageName,
			HashMap<String, String> ContactFullNameAndAccountName, searchContactInEmailProspectGrid searchContact) {
		List<String> result = new ArrayList<String>();
		WebElement ele = null;
		Set<String> contactFullName = ContactFullNameAndAccountName.keySet();
		Iterator<String> itr = contactFullName.iterator();
		while (itr.hasNext()) {
			...
			if (searchContact.toString().equalsIgnoreCase(searchContactInEmailProspectGrid.Yes.toString())) {
				if (sendKeys(driver, getEmailProspectSearchTextBox(20), contactName, "search text box",
						action.SCROLLANDBOOLEAN)) {
					...
					if (ele != null) {
						...
						if (RecordCount == 1 && RecordCount != 0) {
							...
							appLog.error("Email Prospect Record Count is not Matched. Expected: 1" + "\t Actual :"
									+ RecordCount);
							result.add("Email Prospect Record Count is not Matched. Expected: 1" + "\t Actual :"
									+ RecordCount);
						}
					} else {
					...
					appLog.error("Not able to pass value " + contactName
							+ " in search text box so cannot search contact: " + contactName);
					result.add("Not able to pass value " + contactName
							+ " in search text box so cannot search contact: " + contactName);
				}
			}
	...
	public boolean EmailProspects(String environment, String mode, PageName pageName, String fieldName, String operator,
			String value, HashMap<String, String> ContactFullNameAndAccountName,
			searchContactInEmailProspectGrid searchContact, String folderName, String emailTemplateName,
			String processingOptionsName, boolean isToCheckButton) {
		boolean flag = true;
		WebElement ele = null;
		ThreadSleep(5000);
		...
		if (ele != null) {
			if (click(driver, ele, "email prospects hide pop up image", action.SCROLLANDBOOLEAN)) {
				...
				log(LogStatus.INFO, "Not able to click on email prospects hide pop up so cannot apply filter",
						YesNo.No);
				return false;
			}
		}
		if (SearchforEmailProspects(environment, mode, pageName, fieldName, operator, value)) {
			...
			if (isToCheckButton) {
				...
				// Azhar Start
				if (sendKeys(driver, getEmailProspectSearchTextBox(20), value, "search text box",
						action.SCROLLANDBOOLEAN)) {
					...
					appLog.error("Not able to pass value " + value + " in search text box so cannot search contact: "
							+ value);
					flag = false;
				}
			...
			List<String> result = selectProspectsContactAndVerifyReviewProspectListInEmailProspect(pageName,
					ContactFullNameAndAccountName, searchContact);
			if (result.isEmpty()) {
				...
				if (click(driver, getEmailProspectStep1NextBtn(20), "step 1 next button", action.SCROLLANDBOOLEAN)) {
					...
					if (isToCheckButton) {
						...
						List<WebElement> lst = allOptionsInDropDrop(driver, getEmailProspectFolderDropDownList(20),
						...
						if (click(driver, getEmailProspectStep2PreviousBtn(10), "Step 2 Previous Button",
								action.BOOLEAN)) {
							log(LogStatus.INFO, "clicked on step 2 Previous button", YesNo.No);
							ThreadSleep(2000);

							if (click(driver, getEmailProspectStep1NextBottomBtn(10), "step 1 next Bottom button",
									action.BOOLEAN)) {
								log(LogStatus.INFO, "clicked on step 1 next Bottom button", YesNo.No);
								ThreadSleep(2000);

								if (click(driver, getEmailProspectStep2PreviousBottomBtn(10),
										"step 2 Previous Bottom button", action.BOOLEAN)) {
									log(LogStatus.INFO, "clicked on step 2 Previous Bottom button", YesNo.No);
									ThreadSleep(2000);

									if (click(driver, getEmailProspectStep1NextBottomBtn(10),
											"step 1 next Bottom button", action.BOOLEAN)) {
										log(LogStatus.INFO, "clicked on step 1 next Bottom button", YesNo.No);
										ThreadSleep(2000);

									} else {
										log(LogStatus.ERROR, "Not Able to clicked on step 1 next Bottom button",
												YesNo.Yes);
										flag = false;
									}

								} else {
									log(LogStatus.ERROR, "Not Able to clicked on step 2 Previous Bottom button",
											YesNo.Yes);
									flag = false;
								}
					...
					if (selectEmailTemplateFromEmailProspect(folderName, emailTemplateName)) {
						...
						if (isToCheckButton) {
							...
							if (click(driver, getEmailProspectStep2NextBtn(30), "step 2 next button",
									action.SCROLLANDBOOLEAN)) {
								log(LogStatus.INFO, "Clicked on Steps 2 next button", YesNo.No);

								if (click(driver, getEmailProspectStep3PreviousBtn(30), "step 3 Previous button",
										action.SCROLLANDBOOLEAN)) {
									log(LogStatus.INFO, "Clicked on Steps 3 Previous button", YesNo.No);

									if (click(driver, getEmailProspectStep2NextBottomBtn(30),
											"step 2 next Bottom button", action.SCROLLANDBOOLEAN)) {
										log(LogStatus.INFO, "Clicked on Steps 2 next Bottom button", YesNo.No);

										if (click(driver, getEmailProspectStep3PreviousBottomBtn(30),
												"step 3 Previous Bottom button", action.SCROLLANDBOOLEAN)) {
											log(LogStatus.INFO, "Clicked on Steps 3 Previous Bottom button", YesNo.No);
										} else {
											log(LogStatus.INFO, "Not Able to Clicked on Steps 3 Previous button",
													YesNo.No);
											flag = false;
										}

									} else {
										log(LogStatus.INFO, "Not Able to Clicked on Steps 2 next Bottom button",
												YesNo.No);
										flag = false;
									}
						...
						if (click(driver, getEmailProspectStep2NextBtn(30), "step 2 next button",
							...
							if (ele != null) {
								...
								if (aa.contains(MarketingInitiativesPageErrorMsg.selectRecipientOnStep3ErrorMsg(
										String.valueOf(ContactFullNameAndAccountName.size())))) {
									log(LogStatus.INFO,
									...
									if (processingOptionsName != null) {
										...
										for (int i = 0; i < spltdProcessingOption.length; i++) {
											if (ClickOnProcessingOptionCheckBox(processingOptionsName)) {
												log(LogStatus.INFO,
														"clicked on " + spltdProcessingOption[i] + " check box",
														YesNo.No);

											} else {
												log(LogStatus.ERROR, "Not able to click on " + spltdProcessingOption[i]
														+ "check box", YesNo.Yes);
												flag = false;
											}
										}
									...
									if (flag) {
										if (click(driver, getEmailProspectSendBtn(TopOrBottom.BOTTOM, 30),
											...
											String errorMsg = MarketingInitiativesPageErrorMsg.congratulationErrorMsg
													...
													+ MarketingInitiativesPageErrorMsg
															.generatedEmailedRecipientErrorMsg(
																	String.valueOf(
																			ContactFullNameAndAccountName.size()),
																	"Email");
											if (ele != null) {
												String aa1 = ele.getText().trim();
												if (aa1.contains(
														MarketingInitiativesPageErrorMsg.congratulationErrorMsg)
														&& aa1.contains(MarketingInitiativesPageErrorMsg
																.generatedEmailedRecipientErrorMsg(
																		String.valueOf(
																				ContactFullNameAndAccountName.size()),
																		"Email"))) {
									...
									if (flag) {
										if (click(driver, getEmailProspectFinishBtn(30), "finish button",
												action.BOOLEAN)) {
						...
						log(LogStatus.ERROR,
								"Not able to select email template : " + emailTemplateName + " so cannot send email",
								YesNo.Yes);
					}
				} else {
					log(LogStatus.ERROR, "Not able to click on step 1 next button so cannot send email to contact",
							YesNo.Yes);
				}
			...
			log(LogStatus.ERROR, "Not able to apply filter " + fieldName + " " + operator + " " + value
					+ " so cannot select contact and send mail", YesNo.Yes);
		}
		if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
			switchToDefaultContent(driver);
		}
	...
	public boolean ClickOnProcessingOptionCheckBox(String processingOptionsName) {
		...
		ele = isDisplayed(driver, FindElement(driver, xpath, "", action.BOOLEAN, 10), "visibility", 10,
				processingOptionsName + " check box");
		if (ele != null) {
			...
			log(LogStatus.ERROR, processingOptionsName + "check is not visible so cannot click on check box",
					YesNo.Yes);
		}
		return false;
	}
		...
		if (ele != null) {
			if (click(driver, ele, emailTemplateName + " preview link", action.SCROLLANDBOOLEAN)) {
				...
				if (parentWindow != null) {
					...
					ele = isDisplayed(driver, FindElement(driver, xpath, "", action.BOOLEAN, 20), "visibility", 20,
				...
				log(LogStatus.ERROR, "Not able to click on " + emailTemplateName
						+ " email template link so cannot check preview email template", YesNo.Yes);
			}
		} else {
			log(LogStatus.ERROR,
					emailTemplateName + " email template preview link is not visible so cannot click on it", YesNo.Yes);
		}
	...
	public WebElement getAddRowLink(AddProspectsTab addProspectsTab) {
		...
		return isDisplayed(driver,
				FindElement(driver, "//a[@id='" + Xpath + "']", "Add row button", action.BOOLEAN, 10), "Visibility", 10,
				"Add row button");
	}
	...
	public boolean SearchforProspects(AddProspectsTab addProspectsTab, String fieldName, String operator, String value,
		...
		for (int i = 0; i < splitedFieldName.length; i++) {
			...
			if (i < splitedFieldName.length - 1) {
				...
				ele = isDisplayed(driver,
						FindElement(driver, "//a[@id='" + Xpath + "']", "Add row button", action.BOOLEAN, 10),
						"Visibility", 10, "Add row button");
				if (ele != null) {
					if (click(driver, ele, "add row button", action.BOOLEAN)) {
						appLog.info("clicked on add row link");
					} else {
						appLog.error("Not able to click on add row button so cannot add new row and enter filter data "
								+ splitedFieldName[i] + " " + splitedOperator[i] + " " + Splitedvalue[i]);
						return false;
					}
				} else {
					appLog.error("Not able find all row link so cannot add new row and enter filter data "
							+ splitedFieldName[i] + " " + splitedOperator[i] + " " + Splitedvalue[i]);
					return false;
				}
			}
			...
			ele = isDisplayed(
					driver, FindElement(driver, "//input[@name='" + Xpath + "']",
							"add Prospect Field Auto Complete TextBox", action.BOOLEAN, 10),
					"Visibility", 10, "add Prospect Field Auto Complete TextBox");
			if (sendKeys(driver, ele, splitedFieldName[i], "field text box", action.BOOLEAN)) {
				ele = null;
				ThreadSleep(2000);
				Xpath = "//ul[@class='ui-autocomplete ui-front ui-menu ui-widget ui-widget-content'][contains(@style,'display: block;')]/li[text()='"
						+ splitedFieldName[i] + "']";
				...
				if (ele != null) {
					...
					if (click(driver, ele, splitedFieldName[i] + " text", action.BOOLEAN)) {
						...
						if (addProspectsTab.toString()
								.equalsIgnoreCase(AddProspectsTab.AccountAndContacts.toString())) {
							Xpath = "opt" + (i + 1) + "";
						} else {
							Xpath = "optb" + (i + 1) + "";
						}
						ele = FindElement(driver, "//select[@id='" + Xpath + "']", "Add Prospect Operator DropDownList",
								action.BOOLEAN, 10);
						if (selectVisibleTextFromDropDown(driver, ele, "operator drop down list", splitedOperator[i])) {
							...
							if (Splitedvalue[i] != null && !Splitedvalue[i].isEmpty()) {
								...
								ele = FindElement(driver, "//input[@id='" + Xpath + "']", "add Prospect Value TextBox",
										action.BOOLEAN, 10);
								if (sendKeys(driver, ele, Splitedvalue[i], "Value text box", action.BOOLEAN)) {
									...
									appLog.error("Not able to pass value in Value text box: " + Splitedvalue[i]
											+ " so cannot apply filter");
									return false;
								}
							}
							...
							appLog.error(
									"Not able to select operator: " + splitedOperator[i] + " so cannot apply filter");
							return false;
						}
					} else {
					...
					appLog.error(splitedFieldName[i]
							+ " is not visible in field auto complete text box so cannot apply filter.");
					return false;
				}
			} else {
				appLog.error("Not able to pass value in field auto complete text box : " + splitedFieldName[i]
						+ " so cannot apply filter");
				return false;
			}
		}
		if (addFilterLogic != null) {
			...
			ele = isDisplayed(driver,
					FindElement(driver, "//a[@id='" + Xpath + "']", "Add filter logic link", action.BOOLEAN, 10),
					"Visibility", 10, "Add filter logic link");
			if (ele != null) {
				if (click(driver, ele, "add row button", action.BOOLEAN)) {
					appLog.info("clicked on add row link");
					ThreadSleep(2000);
	...
	public List<String> selectProspectsContactAndVerifyReviewProspectList(String environment, String mode,
			AddProspectsTab addProspectsTab, HashMap<String, String> ContactFullNameAndAccountName,
			boolean isAllContact, boolean isCheckedforLink) {
		List<String> result = new ArrayList<String>();
		WebElement ele = null;
		int count = 0;
		String xpath = null;
			...
			if (beforeRecordDig != 0) {
				...
				if (isAllContact) {
					...
					if (click(driver, ele, "All CheckBox", action.SCROLLANDBOOLEAN)) {
						...
						appLog.error(
								"Not able to click on All check box so cannot add contact in review prospect list");
										...
										if (ele != null) {
											...
											if (click(driver, ele, linkClick[j], action.BOOLEAN)) {
												...
												if (Mode.Lightning.toString().equalsIgnoreCase(mode)) {
													...
													MarketingInitiativesPageBusinesslayer market = new MarketingInitiativesPageBusinesslayer(
															driver);
													switchToFrame(driver, 5,
															market.getMarketInitiativeFrame_Lightning(10));
												}
					...
					if (click(driver, getAddToProspectListBtn(5), "add prospect list button",
						...
						if (ele != null) {
							...
							if (!isAllContact) {
								if (AfterRecordDig == ExpectedCount) {
									appLog.info(
											"Select from search result record count is matched : " + AfterRecordDig);
								} else {
									appLog.error("Select from search result record count is not matched. Expected: "
											+ ExpectedCount + " \t Actual Result: " + beforeRecordDig);
									result.add("Select from search result record count is not matched. Expected: "
	 ...
	 */
	public List<String> addAndRemoveCloumnInSelectProspectGrid(String mode, PageName pageName,
			String[] addColumnFieldsName, String[] removeColumnFieldsName, ViewFieldsName viewFieldsName,
			Boolean searchtext) {
		List<String> result = new ArrayList<String>();
		if (click(driver, getSelectProspectsWrenchIcon(pageName, mode, 60), "wrench icon", action.SCROLLANDBOOLEAN)) {
			ThreadSleep(10000);
			if (viewFieldsName != null) {
				if (selectVisibleTextFromDropDown(driver, getColumnToDisplayViewDropDownList(pageName, 30),
						"view drop down list", viewFieldsName)) {
					appLog.info("Value selected from view drop down list : " + viewFieldsName);

				} else {
					appLog.error(
							"Not able to select value from view drop down list so cannot add or remove column from column to display pop up");
					result.add(
							"Not able to select value from view drop down list so cannot add or remove column from column to display pop up");
				}
			}
					...
					for (int i = 0; i < addColumnFieldsName.length; i++) {
						...
						if (!selectedFieldsWebelement.isEmpty()) {
							for (int k = 0; k < selectedFieldsWebelement.size(); k++) {
								if (addColumnFieldsName[i]
										.equalsIgnoreCase(selectedFieldsWebelement.get(k).getText().trim())) {
									appLog.info(addColumnFieldsName[i]
											+ " : Cloumn is already selected in column to display ");
									result.add(addColumnFieldsName[i]
											+ " : Cloumn is already selected in column to display ");
									break;
								}
							}
						...
						for (int j = 0; j < avalFieldsWebelement.size(); j++) {
							if (searchtext) {
								if (sendKeys(driver, getColumnToDisplaySearchTextBox(pageName, 20),
										addColumnFieldsName[i], "search text box", action.BOOLEAN)) {
...
//									}
									if (click(driver, getSearchIconInDisplayToColumn(pageName, 10), "search icon",
											action.BOOLEAN)) {
										appLog.info("clicked on search icon");
									} else {
										appLog.info("Not able to click on search icon so cannot search : "
												+ addColumnFieldsName[i]);
										result.add("Not able to click on search icon so cannot search : "
												+ addColumnFieldsName[i]);
									}
								} else {
									appLog.info(
											"Not able to pass value in search text box : " + addColumnFieldsName[i]);
									result.add("Not able to pass value in search text box : " + addColumnFieldsName[i]);
								}
							}
							ThreadSleep(2000);
							List<String> avalFieldsWebelementText = getColumnToDisplayAvailableFieldsTextList(pageName).stream().map(x -> x.getText())
									.collect(Collectors.toList());
							if (avalFieldsWebelementText.contains(addColumnFieldsName[i])) {
								if (click(driver,
										getColumnToDisplayAvailableFieldsTextList(pageName)
												.get(avalFieldsWebelementText.indexOf(addColumnFieldsName[i])),
										"available fields text", action.SCROLLANDBOOLEAN)) {
									ThreadSleep(1000);
									if (click(driver, getColumnToDisplayLeftToRightBtn(pageName, 30),
											"left to right button", action.BOOLEAN)) {
										appLog.info(
												"clicked on left to right button and column is  added on selected grid "
														+ addColumnFieldsName[i]);
										if (searchtext) {
											if (click(driver, getColumnToDisplaySearchTextCrossIcon(pageName, 20),
													"search text cross icon", action.BOOLEAN)) {
												appLog.info("clicked on search text box cross icon");

											} else {
												appLog.info(
														"Not able to click on search text box cross icon so cannot remove value from text box  "
																+ addColumnFieldsName[i]);
												result.add(
														"Not able to click on search text box cross icon so cannot remove value from text box  "
																+ addColumnFieldsName[i]);
										...
										appLog.error(
												"Not able to click on left to right move button so cannot add column : "
														+ addColumnFieldsName[i]);
										result.add(
												"Not able to click on left to right move button so cannot add column : "
														+ addColumnFieldsName[i]);
									...
									appLog.error("Not able to select text from available fields : "
											+ addColumnFieldsName[i]);
									result.add("Not able to select text from available fields : "
											+ addColumnFieldsName[i]);
								}
							} else {
								appLog.error("No Column Find named: " + addColumnFieldsName[i]
										+ ", So cannot to add to display");
								result.add("No Column Find named: " + addColumnFieldsName[i]
										+ ", So cannot to add to display");
							}
					...
					appLog.error(
							"Column To Display Available Fields Text list is not found so cannot add columns in select prospect grid");
					result.add(
							"Column To Display Available Fields Text list is not found so cannot add columns in select prospect grid");
				}
			}
				...
				if (!selectedFieldsWebelement.isEmpty()) {
					for (int i = 0; i < removeColumnFieldsName.length; i++) {
						for (int j = 0; j < selectedFieldsWebelement.size(); j++) {
							if (click(driver, selectedFieldsWebelement.get(j), "selected fields text",
									action.SCROLLANDBOOLEAN)) {
								ThreadSleep(1000);
								if (click(driver, getColumnToDisplayRightToLeftBtn(pageName, 30),
										"right to left button", action.BOOLEAN)) {
									appLog.info(
											"clicked on right to left button and column is  remove on selected grid "
													+ removeColumnFieldsName[i]);
									break;
									...
									appLog.error(
											"Not able to click on left to right move button so cannot add column : "
													+ removeColumnFieldsName[i]);
									result.add("Not able to click on left to right move button so cannot add column : "
											+ removeColumnFieldsName[i]);
								}
							} else {
								appLog.error(
										"Not able to select text from selected fields : " + removeColumnFieldsName[i]);
								result.add(
										"Not able to select text from selected fields : " + removeColumnFieldsName[i]);
							}
						}
					...
					appLog.error(
							"Column To Display Selected Fields Text list is not found so cannot remove columns in select prospect grid");
					result.add(
							"Column To Display Selected Fields Text list is not found so cannot remove columns in select prospect grid");
				}
			}
	 ...
	 */
	public boolean SearchforEmailProspects(String environment, String mode, PageName pageName, String fieldName,
			String operator, String value, String addFilterLogic) {
		WebElement ele = null;
		String xpath = "";
		String[] splitedFieldName = fieldName.split(",");
		String[] splitedOperator = operator.split(",");
		...
		for (int i = 0; i < splitedFieldName.length; i++) {
			if (fieldName != null && operator != null && !fieldName.isEmpty() && !operator.isEmpty()) {
				if (i < splitedFieldName.length - 1) {
					ele = isDisplayed(driver,
							FindElement(driver, "//a[@id='AddRow2']", "Add row button", action.BOOLEAN, 10),
							"Visibility", 10, "Add row button");
					if (ele != null) {
						if (click(driver, ele, "add row button", action.BOOLEAN)) {
							appLog.info("clicked on add row link");
							...
							appLog.error(
									"Not able to click on add row button so cannot add new row and enter filter data "
											+ splitedFieldName[i] + " " + splitedOperator[i] + " " + Splitedvalue[i]);
							return false;
						}
					} else {
				...
				if (sendKeys(driver, ele, splitedFieldName[i], "field text box", action.BOOLEAN)) {
					...
					if (pageName.toString().equalsIgnoreCase(PageName.emailFundraisingContact.toString())) {
						...
						xpath = "//ul[@class='ui-menu ui-widget ui-widget-content ui-autocomplete ui-front'][contains(@style,'block;')]//div[text()='"
								+ splitedFieldName[i] + "']";
					}
					...
					if (ele != null) {
						if (click(driver, ele, splitedFieldName[i] + " text", action.BOOLEAN)) {
							appLog.info("clicked on field name " + splitedFieldName[i] + " text box");
							ThreadSleep(2000);
							ele = FindElement(driver, "//select[@id='opt" + (i + 1) + "']", "operator drop down list",
									action.BOOLEAN, 5);
							if (selectVisibleTextFromDropDown(driver, ele, "operator drop down list",
									splitedOperator[i])) {
								appLog.info("Select Operator : " + splitedOperator[i]);
								if (Splitedvalue[i] != null && !Splitedvalue[i].isEmpty()) {
									ele = FindElement(driver, "//input[@id='criteriatextbox" + (i + 1) + "']",
											"Value text box", action.BOOLEAN, 5);
									if (sendKeys(driver, ele, Splitedvalue[i], "Value text box", action.BOOLEAN)) {
										appLog.info("passed value in Value Text Box: " + Splitedvalue[i]);
									} else {
										appLog.error("Not able to pass value in Value text box: " + Splitedvalue[i]
								...
								appLog.error("Not able to select operator: " + splitedOperator[i]
										+ " so cannot apply filter");
								return false;
							}
						} else {
							appLog.error(
									"Not able to click on field " + splitedFieldName[i] + " so cannot apply filter");
							return false;
						}
					} else {
						appLog.error(splitedFieldName[i]
				...
				appLog.error(
						"Field Name and Operator cannot be empty or null so cannot apply filter please pass value in field and operator");
				return false;
			}
		}
		if (addFilterLogic != null) {
			ele = isDisplayed(driver,
					FindElement(driver, "//a[@id='pageid:frm:cmdlink']", "Add filter logic link", action.BOOLEAN, 10),
					"Visibility", 10, "Add filter logic link");
			if (ele != null) {
				if (click(driver, ele, "add row button", action.BOOLEAN)) {
					appLog.info("clicked on add row link");
					ThreadSleep(2000);
					ele = isDisplayed(
							driver, FindElement(driver, "//input[@id='pageid:frm:textfilt']",
