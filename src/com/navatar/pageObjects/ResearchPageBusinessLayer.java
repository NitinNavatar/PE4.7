...
//				sa.assertTrue(false,"In " + gridWiseHeading +" Search Keyword" + searchValue + "does not contain in " + gridText);
	...
	public ArrayList<String> VerifyNameAndCountForResearchLeftPanel(String variableName,action action, int timeout ) {
		...
		else if(variableName.contains("ARA_")) {
			headersAndValues = ExcelUtils.dataReadForCurrentRecord(ResearchDataSheetFilePath,"AdvancedData",excelLabel.Variable_Name, variableName);
		}
		else {
...
//				ThreadSleep(2000);
				try {
					wait = new WebDriverWait(driver, Duration.ofSeconds(60)).until(ExpectedConditions.presenceOfElementLocated(
