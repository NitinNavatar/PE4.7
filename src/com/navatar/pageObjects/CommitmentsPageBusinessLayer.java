...
import com.navatar.generic.EnumConstants.CommitmentPageFieldLabelText;
...
public class CommitmentsPageBusinessLayer extends CommitmentsPage {
	 ...
	 */
	public boolean createCommitment(String projectName, String LimitedPartner, String Partnership,
			String finalCommitmnateDate, String commitmentAmount, String excelPath, String basedOnValue) {
		refresh(driver);
		ThreadSleep(5000);
		if (click(driver, getNewButton(environment, mode, 60), "New Button", action.BOOLEAN)) {
			if (sendKeys(driver, getLimitedPartnerTextbox(environment, mode, 60), LimitedPartner,
					"Limited Partner Text Box", action.BOOLEAN)) {
				...
				if (sendKeys(driver, getPartnershipTextBox(environment, mode, 60), Partnership, "Partnership Text Box",
						action.BOOLEAN)) {
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						ThreadSleep(1000);
						if (click(driver,
					...
					if (finalCommitmnateDate != null) {
						if (sendKeys(driver, getFinalCommimentDate(10), finalCommitmnateDate,
								"commitment date Text Box", action.BOOLEAN)) {
					...
					if (commitmentAmount != null) {
						if (sendKeys(driver, getCommimentAmount(10), commitmentAmount, "commitment amount Text Box",
								action.BOOLEAN)) {
	...
	public WebElement getCommitmentCancelBtn(TopOrBottom topOrBottom, int timeOut) {
		...
		ele = FindElement(driver, xpath, "Step 3 Cancel Btn : " + topOrBottom, action.BOOLEAN, timeOut);
		return ele;
					...
					while (true) {
						...
						if (click(driver, getNextImageonPage(10), "Commitment Page Next Button",
							...
							commitment = FindElement(driver,
									"//div[@class='x-panel-bwrap']//span[text()='" + commitmentID + "']",
									"Commitment ID", action.BOOLEAN, 60);
							if (commitment != null) {
	...
	public boolean FieldValueVerificationOnCommitmentPage(String environment, String mode, String labelName,
			String labelValue) {
		String xpath = "";
		WebElement ele = null;
		if (labelName.toString().equalsIgnoreCase(excelLabel.Commitment_Amount.toString())) {
			labelValue = convertNumberAccordingToFormatWithCurrencySymbol(labelValue, "0,000.0");
				...
				|| labelName.toString()
						.equalsIgnoreCase(CommitmentPageFieldLabelText.Capital_Returned_Recallable.toString())) {
			labelValue = convertNumberAccordingToFormatWithCurrencySymbol(labelValue, "0,000.000");

		} else if (labelName.toString()
				.equalsIgnoreCase(CommitmentPageFieldLabelText.Capital_Returned_NonRecallable.toString()))
			labelValue = convertNumberAccordingToFormatWithCurrencySymbol(labelValue, "0.000");

		else if (labelName.toString().equalsIgnoreCase(CommitmentPageFieldLabelText.Total_Commitment_Due.toString()))
		...
		if (mode.equalsIgnoreCase(Mode.Classic.toString())) {
			...
			xpath = "//span[text()='" + labelName + "']/../following-sibling::div//*[text()='" + labelValue + "']";
			ele = FindElement(driver, xpath, labelName + " label text with  " + labelValue, action.SCROLLANDBOOLEAN,
					10);
			...
			if (ele != null) {
				...
				appLog.error(
						"<<<<<<   " + labelName + " label text with  " + labelValue + " not verified " + "   >>>>>>");
			}
			return false;
