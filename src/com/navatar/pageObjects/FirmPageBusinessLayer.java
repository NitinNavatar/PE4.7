...
public class FirmPageBusinessLayer extends FirmPage {
	...
	public boolean verifyCustomAction(String data) {
		...
		for (int i = 0; i < tabNameFieldNameValue.length; i++) {
			...
			if (CommonLib.click(driver, getTabEroBtn(50), "tab ero button", action.SCROLLANDBOOLEAN)) {
				...
				ele = CommonLib.FindElement(driver, xPath, tabNameAndFieldNameValue[0] + " tab name",
						action.SCROLLANDBOOLEAN, 50);
					...
					if (tabNameAndFieldNameValue[0].equals("New Contact")) {
						...
						for (int j = 0; j < ExpectedCustomAction.size(); j++) {
							if (ExpectedCustomAction.get(j).equals(actualCustomAction.get(j))) {
								log(LogStatus.INFO, "Expected result " + ExpectedCustomAction.get(j)
										+ " has been matched with the Actual Result " + actualCustomAction.get(j),
										YesNo.No);

							} else {
								log(LogStatus.ERROR,
										"Expected result " + ExpectedCustomAction.get(j)
												+ " is not matched with the Actual Result " + actualCustomAction.get(j),
										YesNo.No);
								result.add("Expected result " + ExpectedCustomAction.get(j)
										+ " is not matched with the Actual Result " + actualCustomAction.get(j));
							}
						}
						...
						if (CommonLib.sendKeys(driver, ele, fieldValue[1], fieldValue[0], action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, fieldValue[1] + " has been entered in the " + fieldValue[0] + " field",
									YesNo.No);
							...
							if (CommonLib.click(driver, ele, "Save button", action.BOOLEAN)) {
								...
								if (CommonLib.checkElementVisibility(driver, gettoastMessage(50),
							...
							log(LogStatus.ERROR,
									"Not able to enter the value " + fieldValue[1] + " in " + fieldValue[0] + " field",
									YesNo.No);
						...
						if (CommonLib.sendKeys(driver, ele, fieldValue[1], fieldValue[0], action.SCROLLANDBOOLEAN)) {
							...
							xPath = "//span[text()='" + fieldValue[0]
									+ "']/../following-sibling::div//ul[@class='lookup__list  visible']/li//div[contains(@class,'primaryLabel')]";
							elements = CommonLib.FindElements(driver, xPath, "New Client label");
							if (CommonLib.getSelectedOptionOfDropDown(driver, elements,
									fieldValue[0] + "drop down list", fieldValue[1])) {
								log(LogStatus.INFO, fieldValue[1] + " has been selected from the " + fieldValue[0]
										+ " field drop down list", YesNo.No);
								...
								log(LogStatus.ERROR, fieldValue[1] + " is not selected from the " + fieldValue[0]
										+ " field drop down list", YesNo.No);
	...
	public boolean VerifySortingOnFields(String data) {
		...
		for (int i = 0; i < FieldsName.length; i++) {
			...
			String[] fieldNameAndSortOrderNoSortOrder = gridNameAndfieldNameSortOrderNoSortOrder[1]
					.split("<defaultSortBreak>");
			...
			if (defaultSortOrder.contains("ASC")) {
				xPath = "//a[text()='" + gridName + "']/ancestor::article//thead//span[text()='" + defaultSortFieldName
						+ "']/lightning-icon";
				if (checkAscOrder(xPath, defaultSortFieldName)) {
					...
					log(LogStatus.ERROR, "Default Ascending order is not available on " + defaultSortFieldName,
							YesNo.No);
				}
			}
				...
				if (checkDescOrder(xPath, defaultSortFieldName)) {
					...
					log(LogStatus.ERROR, "Default Descending order is not available on " + defaultSortFieldName,
							YesNo.No);
				}
			}
			...
			/*
			 * for(int j=0; j<fieldName.length; j++) {
			 * xPath="//a[text()='"+gridName+"']/ancestor::article//thead//span[text()='"+
			 * fieldName[j]+"']"; ele=FindElement(driver, xPath, fieldName[j]+" field name",
			 * action.SCROLLANDBOOLEAN, 50);
			 * CommonLib.scrollDownThroughWebelementInCenter(driver, ele,
			 * fieldName[j]+" field name");
			 * xPath="//a[text()='"+gridName+"']/ancestor::article//thead//span[text()='"+
			 * fieldName[j]+"']/lightning-icon";
			 * if(checkAscDescOrder(ele,xPath,fieldName[j])) {
			 * log(LogStatus.INFO,"Arrowup and Arrowdown icon is visible on "+fieldName[j],
			 * YesNo.No);
			 * 
			 * } else {
			 * log(LogStatus.ERROR,"Arrowup and Arrowdown icon is not visible on "+fieldName
			 * [j], YesNo.No); }
			 * 
			 * }
			 */
			...
			if (click(driver, getFirmsContactsTab(30), "Contacts Tab", action.SCROLLANDBOOLEAN)) {
				...
				for (int i = 0; i < contactsFieldName.length; i++) {
					if (contactsFieldName[i].equals("Name") || contactsFieldName[i].equals("Firm")) {
						xPath = "//a[text()='Contacts']/ancestor::article//tbody//tr[1]//td[contains(@data-label,'"
								+ contactsFieldName[i] + "')]//a";
						ele = CommonLib.FindElement(driver, xPath, contactsFieldName[i] + " record",
								action.SCROLLANDBOOLEAN, 40);
						if (CommonLib.clickUsingJavaScript(driver, ele, contactsFieldName[i] + " record",
								action.SCROLLANDBOOLEAN)) {
							...
							ele = CommonLib.FindElement(driver, xPath, contactsFieldName[i] + " heading",
									action.SCROLLANDBOOLEAN, 40);
							if (ele != null) {

								log(LogStatus.INFO, "Redirection is working properly on " + contactsFieldName[i],
										YesNo.No);
								driver.close();
								driver.switchTo().window(parentID);
							} else {
								log(LogStatus.ERROR, contactsFieldName[i] + " record is not redirecting on Contact tab",
										YesNo.No);
						...
						xPath = "//a[text()='Contacts']/ancestor::article//tbody//tr[2]//td[contains(@data-label,'"
								+ contactsFieldName[i] + "')]//a";
						ele = CommonLib.FindElement(driver, xPath, contactsFieldName[i] + " record",
								action.SCROLLANDBOOLEAN, 40);
						if (clickUsingJavaScript(driver, ele, contactsFieldName[i] + " record",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "Clicked on " + contactsFieldName[i] + " record", YesNo.No);
							String parentID = CommonLib.switchOnWindow(driver);
							xPath = "//div[contains(@class,'entityNameTitle')]";
			...
			if (click(driver, getFirmsContactsTab(30), "Get Contacts Tab", action.SCROLLANDBOOLEAN)) {
				...
				for (int i = 0; i < affiliationsField.length; i++) {
					if (affiliationsField[i].equals("Name") || affiliationsField[i].equals("Firm")) {
						xPath = "//a[text()='Contacts']/ancestor::article//tbody//tr[1]//td[contains(@data-label,'"
								+ affiliationsField[i] + "')]//a";
						ele = CommonLib.FindElement(driver, xPath, affiliationsField[i] + " record",
								action.SCROLLANDBOOLEAN, 40);
						if (CommonLib.clickUsingJavaScript(driver, ele, affiliationsField[i] + " record",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "Clicked on " + affiliationsField[i] + " record", YesNo.No);
							String parentID = CommonLib.switchOnWindow(driver);
							xPath = "//div[contains(@class,'entityNameTitle')]";
							ele = CommonLib.FindElement(driver, xPath, affiliationsField[i] + " heading",
									action.SCROLLANDBOOLEAN, 40);
							if (ele != null) {

								log(LogStatus.INFO, "Redirection is working properly on " + affiliationsField[i],
										YesNo.No);
								driver.close();
								driver.switchTo().window(parentID);
							} else {
						...
						ele = CommonLib.FindElement(driver, xPath, affiliationsField[i] + " record",
								action.SCROLLANDBOOLEAN, 40);
						if (clickUsingJavaScript(driver, ele, affiliationsField[i] + " record",
								action.SCROLLANDBOOLEAN)) {
							log(LogStatus.INFO, "Clicked on " + affiliationsField[i] + " record", YesNo.No);
							String parentID = CommonLib.switchOnWindow(driver);
							xPath = "//div[contains(@class,'entityNameTitle')]";
							...
							if (ele != null) {
								...
								log(LogStatus.ERROR, "Redirection is not working properly on " + affiliationsField[i],
										YesNo.No);
	...
	public ArrayList<String> VerifyInlineEditingForContactsAndAffiliationsGrid(String record) {
		...
		for (int i = 0; i < data.length; i++) {
										...
										if (ele != null) {
											if (CommonLib.click(driver, ele, columnName.get(j) + " input",
												...
												if (datePickerHandle(driver, monthInDatePicker(50),
														previousMonthButtonInDatePicker(50), "test", "2022", "July",
														"22")) {
			...
			/*
			 * xPath="//a[text()='"+gridName+
			 * "']/ancestor::article//div[contains(@class,'slds-form_inline')]";
			 * ele=CommonLib.FindElement(driver, xPath, gridName+ " ",
			 * action.SCROLLANDBOOLEAN, 50); CommonLib.doubleClickUsingAction(driver, ele);
			 */
			...
			if (CommonLib.click(driver, ele, "Page border", action.BOOLEAN)) {
				...
				if (CommonLib.click(driver, ele, "save button", action.SCROLLANDBOOLEAN)) {
					...
					if (CommonLib.checkElementVisibility(driver, getsaveConfirmationMsg(50),
							"save confirmation message", 50)) {
