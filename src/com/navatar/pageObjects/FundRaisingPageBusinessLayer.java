...
public class FundRaisingPageBusinessLayer extends FundRaisingPage  {
	...
	//Lightning Mthod....
				...
				if (sendKeys(driver, getFundraisingName(environment,mode,60), fundraisingName, "FundRaising Name", action.BOOLEAN)) {
					...
					if (sendKeys(driver, getFundName(environment,mode,60), fundName, "Fund Name", action.BOOLEAN)) {
						...
						if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
							...
							if (click(driver,
									FindElement(driver,
											"//*[contains(@class,'slds-listbox__option-text')]/*[@title='"+fundName+"']/ancestor::lightning-base-combobox-item",
						...
						if (sendKeys(driver, getLegalName(environment,mode,60), legalName, "Legal Name", action.BOOLEAN)) {
							...
							if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
								...
								if (clickUsingJavaScript(driver,
										FindElement(driver,
												"//*[contains(@class,'slds-listbox__option-text')]/*[@title='"+legalName+"']/ancestor::lightning-base-combobox-item",
		 ...
		 * @return true if able to create FundRaising
		 */
		public boolean createFundRaising(String environment,String mode,String fundraisingName, String fundName, String legalName, String closing, String stage, String investmentLikelyAmount, String statusNote, String CompanyName) {
			refresh(driver);
			ThreadSleep(5000);
			if (click(driver, getNewButton(environment,mode,60), "New Button", action.SCROLLANDBOOLEAN)) {
				ThreadSleep(500);
						...
						if (sendKeys(driver, getLegalName(environment,mode,60), legalName, "Legal Name", action.BOOLEAN)) {
							...
							try {
								if(!"".equals(CompanyName)&&CompanyName!=null){
									if (sendKeys(driver, getCompanyName(environment,mode,60),CompanyName, "Company Name", action.BOOLEAN)) {
										ThreadSleep(500);
										if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
											ThreadSleep(5000);
											if (clickUsingJavaScript(driver,
													FindElement(driver,
															"//*[contains(@class,'slds-listbox__option-text')]/*[@title='"+CompanyName+"']/ancestor::lightning-base-combobox-item",
